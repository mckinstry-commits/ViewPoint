...
print 'CREATE FUNCTION mers.mfnJCParentPhaseSummary'
go
...
---				J.Ziebell   8/18/2016  Column Name Change
---             J.Ziebell   8/31/2016  Remaining Cmmt Cost Col Name Change
-- ========================================================================
...
SELECT
	...
	, S.CurrEstHours AS 'Current Est''d Hours'
	, S.CurrEstCost AS 'Curr Est + Incl CO''s'
	, S.ActualHours AS 'Actual Hours'
	, S.ActualCost AS 'Actual Cost'
	, CASE WHEN S.ActualHours > 0 THEN CAST(ROUND((S.ActualCost / S.ActualHours),2) as numeric(36,2)) ELSE 0 END AS 'Actual CST/HR'
	, S.TotalCmtdCost AS 'Committed Cost' 
	, (S.ProjHours - S.ActualHours) AS 'Remaining Hours'
	, (S.ProjCost - S.ActualCost) AS 'Remaining Cost'
	, CASE WHEN ((S.ProjHours - S.ActualHours) > 0) THEN CAST(ROUND(((S.ProjCost - S.ActualCost) / (S.ProjHours - S.ActualHours)),2) as numeric(36,2)) ELSE 0 END AS 'Remaining CST/HR'
	, S.RemainCmtdCost AS 'Remaining Committed Cost'
	, S.ProjHours AS 'Projected Hours'
	, S.ProjCost AS 'Projected Cost'
	, (S.ProjHours - P.ProjHours)  AS 'Change in Hours'
	, (S.ProjCost - P.ProjCost)  As 'Change in Cost'

...
if exists ( select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME='mspJCPRGPivot' and ROUTINE_SCHEMA='mers' and ROUTINE_TYPE='PROCEDURE' )
begin
	print 'DROP PROCEDURE mers.mspJCPRGPivot'
	DROP PROCEDURE mers.mspJCPRGPivot
end
go

print 'CREATE PROCEDURE mers.mspJCPRGPivot'
go

CREATE PROCEDURE [mers].[mspJCPRGPivot]
(
	@JCCo		bCompany 
,	@Contract	bContract
)
as
-- ========================================================================
-- Object Name: mers.mspJCPRGPivot
...
--				J.Ziebell   08/18/2016 Field Rename
--              J.Ziebell   10/06/2016 Set 5 year limit on output
--              J.Ziebell   10/26/2016 Correct for short Contract Values
--              J.Ziebell   11/07/2016 Include Months Not closed and -1 Margin Floor
--              J.Ziebell   11/17/2016 Remove Non Sysdate Months, fix Margin Formula, fix start date.
--              J.Ziebell   11/28/2016 Back to non closed months
--				J.Ziebell	01/24/2018 Stop Pulling in Cleared out detail values
-- ========================================================================
...
--SET @FirstMonth = DATEADD(MONTH,-1, @CutOffDate)
--SET @FirstMonth = DATEADD(MONTH,1,@LockMth)
...
BEGIN
		...
		If @StartDate < @FirstMonth  --@CutOffDate  --
			BEGIN
				SET @StartDate = @FirstMonth --@CutOffDate  --
			END

			SELECT @StartDate = CONCAT(DATEPART(YYYY,@StartDate),'-',DATEPART(MM,@StartDate),'-01')

		IF DATEDIFF(MONTH,@StartDate,@FinalDate) >=60
			BEGIN
				SET @FinalDate = DATEADD(MONTH,59,@StartDate)
			END	
...
SELECT B.JCCo, B.Contract, B.StartDate, B.EndDate INTO #tempDates 
	FROM (SELECT JCCo, Contract, StartDate, EndDate
			FROM  @DisplayRange
			WHERE EndDate >= @FirstMonth /*@CutOffDate*/) as B 
...
DROP TABLE #tempDates
	...
	END
		...
		BEGIN
			INSERT INTO @WIP_Rev (JCCo, Contract, Department, udPRGNumber, JTDEarnedRev)
						...
						WHERE WIP.JCCo = @JCCo
							AND WIP.Contract = LTRIM(@Contract)
							AND WIP.ThroughMonth = @LockMth    --@FirstMonth --
						GROUP BY  WIP.JCCo
							, WIP.Contract
							, WIP.JCCIDepartment
							, WIP.PRGNumber
	...
	BEGIN
		INSERT INTO @JCCP_Cost (JCCo, Contract, Department, udPRGNumber, ProjectedCost, MaxBatchMonth)
			SELECT 
				  JP.JCCo
				, CI.Contract
				, CI.Department
			...
			WHERE JP.JCCo = @JCCo
				...
				AND ((CP.ProjHours <>0) OR (CP.ProjCost <>0))
			GROUP BY  JP.JCCo
					, CI.Contract
					, CI.udPRGNumber
					, CI.Department
...
SELECT B.JCCo, B.Contract, B.Department, B.DeptDesc, B.Job, B.udPRGDescription, B.StartDate, B.EndDate
						, B.RemainingRev, B.ProjDollars, B.ProjectedCost, B.ProjMargin, B.MaxBatchMonth INTO #tempDates2 
	...
	, CASE WHEN (W.JTDEarnedRev IS NULL) THEN R.ProjDollars
			ELSE (R.ProjDollars - W.JTDEarnedRev) END AS RemainingRev, R.ProjDollars, C.ProjectedCost, 
				CASE WHEN ((R.ProjDollars > C.ProjectedCost) AND (C.ProjectedCost = 0)) THEN .9999
					 WHEN (R.ProjDollars = 0) THEN 0
				     WHEN R.ProjDollars > C.ProjectedCost THEN ((R.ProjDollars-C.ProjectedCost)//*CAST(*/R.ProjDollars/* as float)*/)
					ELSE 0 END as ProjMargin
				, C.MaxBatchMonth
			FROM  @DisplayRange A
			...
			LEFT OUTER JOIN @WIP_Rev W
				ON W.JCCo = A.JCCo
				AND W.Contract = LTRIM(A.Contract)
...
set @query = 'SELECT p.Job AS PRG, p.udPRGDescription AS ''PRG Description'', p.Department as ''JC Dept''
			, p.DeptDesc AS ''JC Dept Description'', p.RemainingRev AS ''Calculated Remaining Revenue''
			, ''' + @CalcCol + ''' AS ''Remaining at Margin'', ''' + @CalcCol + ''' AS ''Margin Change Catch Up''
			, ''' + @CalcCol + ''' AS ''Current Month Catch Up'',' + @cols + ' FROM
             (SELECT d.Job, d.udPRGDescription, d.Department, d.DeptDesc, d.RemainingRev
				, CAST(((ISNULL(PR.Amount,0))/(1-d.ProjMargin)) AS DECIMAL (17,5)) AS Amount
				, convert(CHAR(10), d.EndDate, 120) AS PivotDate
					FROM HQCO HQ
						...
						INNER JOIN JCCI CI
								ON d.JCCo = CI.JCCo
								AND d.Contract = CI.Contract
								AND d.Job = CI.udPRGNumber 
						LEFT OUTER JOIN JCJP JP  
								ON JP.JCCo = CI.JCCo 
								AND JP.Contract = CI.Contract
								AND JP.Item = CI.Item
						LEFT OUTER JOIN JCPR PR 
									...
									AND PR.Mth >= d.MaxBatchMonth
									AND PR.Mth = (SELECT MAX(PR1.Mth) FROM JCPR PR1 

USE [Viewpoint]
GO
/****** Object:  UserDefinedFunction [mers].[mfnJCPRGSummary]    Script Date: 8/16/2017 11:38:51 AM ******/
SET ANSI_NULLS ON
GO
SET QUOTED_IDENTIFIER ON
GO

ALTER function [mers].[mfnJCPRGSummary]
(
	@JCCo				bCompany
,	@Contract			bContract
)
...
--				J.Ziebell   10/14/2016 Display WIP Data
--              J.Ziebell   10/26/2016 Correct for Short Contract Numbers
--              J.Ziebell   11/07/2016 Include Months Not closed and -1 Margin Floor
--              J.Ziebell   07/19/2017 Missing Cost Breaking Margin Dollars, added ISNULL
-- ========================================================================
	...
	WIP_Lock 
		...
		WIP_Cur
			...
			WHERE WIPC.JCCo =  @JCCo
				AND WIPC.Contract =  LTRIM(@Contract)
...
SELECT
		...
		, CASE WHEN (ISNULL(C.OrigEstCost,0)) <= 0 THEN 1 
				WHEN (S.OrigContractAmt) = 0 THEN -1
				WHEN (S.OrigContractAmt) > 0 AND (((S.OrigContractAmt-C.OrigEstCost)/S.OrigContractAmt) > -1) THEN ((S.OrigContractAmt-C.OrigEstCost)/S.OrigContractAmt)
				ELSE -1 END AS 'Original Margin'
		--, (S.ContractAmt-C.ProjectedCost) AS 'Current Margin Dollars'
		, CASE WHEN (ISNULL(C.ProjectedCost,0)) <= 0 THEN 1 
				WHEN (S.ContractAmt) = 0 THEN -1
				WHEN (S.ContractAmt) > 0 AND (((S.ContractAmt-C.ProjectedCost)/S.ContractAmt) > -1) THEN ((S.ContractAmt-C.ProjectedCost)/S.ContractAmt)
				ELSE -1 END AS 'Current Margin'
		, CAST(W.LockMargin AS DECIMAL(18,10)) AS 'Last Month Margin'  --'Last Closed Margin'
		, (S.ProjDollars - ISNULL(C.ProjectedCost,0)) AS 'Projected Margin $'
		, CASE WHEN (ISNULL(C.ProjectedCost,0)) <= 0 THEN 1 
				WHEN (S.ProjDollars) = 0 THEN -1
				WHEN (S.ProjDollars) > 0 AND (((S.ProjDollars-C.ProjectedCost)/S.ProjDollars) > -1) THEN ((S.ProjDollars-C.ProjectedCost)/S.ProjDollars) 

...
if exists ( select 1 from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME='mfnProjectCTSum' and ROUTINE_SCHEMA='dbo' and ROUTINE_TYPE='FUNCTION' )
begin
	print 'DROP FUNCTION dbo.mfnProjectCTSum'
	DROP FUNCTION dbo.mfnProjectCTSum
end
go

print 'CREATE FUNCTION dbo.mfnProjectCTSum'
go
CREATE function [dbo].[mfnProjectCTSum]
(
	@JCCo				bCompany
,	@Contract			bContract
,	@Job				bJob
...
--              Ziebell, J. 01/13/2017 Change to DBO, change Reamining, and new field
--              Ziebell, J. 01/26/2017 Add Space in Column headers
-- ========================================================================
...
,   (sum(jccp.ProjCost) - sum(jccp.ActualCost)) AS 'Projected             Remaining Cost'
,	sum(jccp.ProjCost) AS 'Projected                   Final Cost'
,   (sum(jccp.ProjCost) - sum(jccp.CurrEstCost)) AS 'Projected Over/Under'
FROM	JCJM jcjm 
	INNER JOIN HQCO HQ
...
Grant SELECT ON dbo.mfnProjectCTSum TO [MCKINSTRY\Viewpoint Users]

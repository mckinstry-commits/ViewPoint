USE [Viewpoint]
GO
/****** Object:  StoredProcedure [dbo].[bspPREmplVal]    Script Date: 5/3/2017 10:33:41 AM ******/
SET ANSI_NULLS ON
GO
...
/****** Object:  Stored Procedure dbo.bspPREmplVal    Script Date: 8/28/99 9:33:19 AM ******/
    ...
    *				AR 1/22/2016 - 41452 - Removing deprecated SET ROWCOUNT
   ...
   if @@rowcount = 0
   	...
   	where PRCo=@prco and SortName = @empl
        ...
        begin
           	...
           	SELECT TOP(1) @emplout = Employee, @sortname=SortName, @lastname=LastName,
   						@firstname=FirstName, @middlename=MidName, @active=ActiveYN, @inscode=InsCode,
   						@dept=PRDept, @craft=Craft, @class=Class, @jcco=JCCo, @job=Job, @suffix = Suffix
   			FROM dbo.PREHName

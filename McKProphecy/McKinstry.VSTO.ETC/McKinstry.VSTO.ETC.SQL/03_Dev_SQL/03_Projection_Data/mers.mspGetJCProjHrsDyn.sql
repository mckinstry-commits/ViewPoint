...
if exists ( select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME='mspGetJCProjHrsDyn' and ROUTINE_SCHEMA='dbo' and ROUTINE_TYPE='PROCEDURE' )
begin
	print 'DROP PROCEDURE dbo.mspGetJCProjHrsDyn'
	DROP PROCEDURE dbo.mspGetJCProjHrsDyn
end
go

print 'CREATE PROCEDURE dbo.mspGetJCProjHrsDyn'
go

CREATE PROCEDURE [dbo].[mspGetJCProjHrsDyn]
(
	@JCCo		bCompany 
,	@Job		bJob
,	@Pivot      VARCHAR(5)
...
as
-- ========================================================================
-- Object Name: dbo.mspGetJCProjHrsDyn
...
--				J.Ziebell   10/19/2017 Updated Code For Manweeks Logic
--				J.Ziebell   11/10/2017 Updated End Weeks Logic, code to account for switch from weeks to months
...
IF @Pivot = 'WK'
	BEGIN
		...
		SET @FirstMonth = CONCAT(DATEPART(YYYY,@FirstMonth),'-',DATEPART(MM,@FirstMonth),'-01')

		If @StartDate <= @FirstMonth
			BEGIN
				SET @StartDate = @FirstMonth
			END
		...
		SET @FinalWeekEnd = CONCAT(DATEPART(YYYY,@MaxDate),'-',DATEPART(MM,@MaxDate),'-01')
		
		SET @FinalWeekEnd = DATEADD(DAY,-1,DATEADD(MONTH,1,@FinalWeekEnd))

		--SET @MaxDate = @FinalWeekEnd
		SELECT @DayEnd = DATEPART(dw, @FinalWeekEnd)
		IF @DayEnd = 1
			BEGIN
				 SET @MaxDate = @FinalWeekEnd
			END
		ELSE
			BEGIN
				 SET @MaxDate = DATEADD(Day,(1 - @DayEnd),@FinalWeekEnd)
			END
...
set @query = 'SELECT ''' + @CalcCol + ''' AS Used, ParentPhase AS ''Parent Phase Group'', Phase as ''Phase Code'', PhaseDesc AS ''Phase Desc''
				, Employee AS ''Employee ID'', Description, ''' + @CalcCol + ''' AS ''Projected Remaining Manweeks'', ProjRemHours AS ''Projected Remaining Hours'', RemCost AS ''Projected Remaining Total Cost''
				...
				, PhaseActualRt AS ''Phase Actual Rate'', Rate, MTDCost AS ''MTD Actual Cost'', MTDHours AS ''MTD Actuals'',' + @cols + ' from 
				...
				FROM JCPB PB 
					...
					LEFT OUTER JOIN (#tempDates2 d2 
										INNER JOIN JCPD PD 
											...
											AND PD.TransType<>''D''
										    AND (((''' + @Pivot + ''' = ''MTH'') AND (PD.ToDate between d2.StartDate and d2.EndDate))
												OR ((''' + @Pivot + ''' = ''WK'') AND (PD.FromDate between d2.StartDate and d2.EndDate))
												OR ((PD.ToDate IS NULL) AND (PD.DetMth between d2.StartDate and d2.EndDate))))
						ON d2.JCCo = PB.Co
						AND d2.Job = PB.Job 
...
Grant EXECUTE ON dbo.mspGetJCProjHrsDyn TO [MCKINSTRY\Viewpoint Users]

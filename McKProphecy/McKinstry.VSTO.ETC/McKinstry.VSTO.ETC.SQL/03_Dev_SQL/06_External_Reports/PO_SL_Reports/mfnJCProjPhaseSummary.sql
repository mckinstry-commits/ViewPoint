	 ...
	 PendingChangeOrders
		...
		PO_Cost_Sum AS (SELECT
					...
					GROUP BY POF.JCCo
							...
							, POF.Phase
							, POF.CostType),
		SL_Cost_Sum	AS(SELECT SLF.SLCo AS JCCo
						, SLF.Job
						, SLF.PhaseGroup
						, SLF.Phase
						, SLF.CostType
...
SELECT
	...
	, 'X' AS 'Remaining Hours'
	, 'X' AS 'Remaining Cost'
	, 'X' AS 'Remaining CST/HR' 
	, (ISNULL(PO.RemainCommit,0) + ISNULL(SL.RemainCommit,0)) AS 'Remaining Committed Cost'
	, (PB.CurrEstHours - JTD.JTDHours) AS 'Remaining Est Hours'
	, (JTD.JTDCost + (ISNULL(PO.RemainCommit,0) + ISNULL(PO.RemainCommit,0))) AS 'JTD + Remaining Committed'

...
select Co, Mth, BatchId, BatchSeq, Job, PhaseGroup, Phase, CostType, (CurrProjCost - ActualCmtdCost) as CurrProjCost
from 
	JCPB
where Co=1 
	and Job= @Job --' 10187-002' 
	AND (CurrProjCost - ActualCmtdCost) > 0
	AND OrigEstHours =0
for read only
...
Declare @Duration INT, @counter INT, @ActualCost bDollar, @vAmount bDollar
...
OPEN PBcur
FETCH PBcur INTO @vCo, @vMth, @vBatchId, @vBatchSeq, @vJob, @vPhaseGroup, @vPhase, @vCostType, @ActualCost

while @@FETCH_STATUS=0
begin
	select @rcnt=@rcnt+1

	SELECT @vAmount = @ActualCost/@Duration
	...
	begin
		...
		SELECT @vspreadMth1 = dbo.vfFirstDayOfMonth(@vspreadMonth)
		...
		(@Co,@DetSeq,@Mth,@BatchId,@BatchSeq,@Source,@JCTransType,@TransType,@ResTrans,@Job,@PhaseGroup,@Phase,@CostType,@BudgetCode,@EMCo,@Equipment,@PRCo,@Craft,@Class,@Employee,@Description,@DetMth,@FromDate,@ToDate,@Quantity,@Units,@UM,@UnitHours,@Hours,@Rate,@UnitCost,@Amount,@Notes)',N'@BatchSeq int,@Co tinyint,@DetSeq int,@Mth datetime,@BatchId int,@Source varchar(10),@JCTransType varchar(2),@TransType varchar(1),@ResTrans int,@Job varchar(10),@PhaseGroup tinyint,@Phase varchar(20),@CostType int,@BudgetCode varchar(8000),@EMCo tinyint,@Equipment varchar(8000),@PRCo tinyint,@Craft varchar(4),@Class varchar(5),@Employee int,@Description varchar(12),@DetMth datetime,@FromDate datetime,@ToDate datetime,@Quantity float,@Units float,@UM varchar(3),@UnitHours float,@Hours float,@Rate float,@UnitCost float,@Amount float,@Notes varchar(8000)'
		,@BatchSeq=@vBatchSeq
		,@Co=@vCo
		,@DetSeq=@vDetSeq
		,@Mth=@vMth

...
if exists ( select * from INFORMATION_SCHEMA.ROUTINES where ROUTINE_NAME='mspGetRevenueProjectionBatchPivot' and ROUTINE_SCHEMA='mers' and ROUTINE_TYPE='PROCEDURE' )
begin
	print 'DROP PROCEDURE mers.mspGetRevenueProjectionBatchPivot'
	DROP PROCEDURE mers.mspGetRevenueProjectionBatchPivot
end
go

print 'CREATE PROCEDURE mers.mspGetRevenueProjectionBatchPivot'
go

CREATE PROCEDURE mers.mspGetRevenueProjectionBatchPivot
(
	@JCCo		bCompany 
,	@Contract	bContract
)
...
--if not exists ( select 1 from udJCIRD where Co=@JCCo and Contract=@Contract and Mth=@dispMonth)
--begin
--	insert udJCIRD ( Co, Contract, Item, Mth, FromDate, ToDate )
--	select distinct
...
set @query = 'SELECT JCCo, Contract, Item, BatchId, BatchSeq, Month, ' + @cols + ' from 
             (
                select jcci.JCCo, jcci.Contract, jcci.Item, b.Mth as Month, b.BatchId, b.BatchSeq, ProjDollars,

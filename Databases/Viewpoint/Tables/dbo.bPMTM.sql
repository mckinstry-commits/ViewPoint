...
ALTER TABLE [dbo].[bPMTM] ADD 
CONSTRAINT [PK_bPMTM] PRIMARY KEY CLUSTERED  ([PMCo], [Project], [Transmittal]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMTM] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPMTM] WITH NOCHECK ADD
CONSTRAINT [FK_bPMTM_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job])
ALTER TABLE [dbo].[bPMTM] WITH NOCHECK ADD
CONSTRAINT [FK_bPMTM_bPMPM] FOREIGN KEY ([VendorGroup], [ResponsibleFirm], [ResponsiblePerson]) REFERENCES [dbo].[bPMPM] ([VendorGroup], [FirmNumber], [ContactCode])
GO

...
CONSTRAINT [PK_bPMUX] PRIMARY KEY CLUSTERED  ([Template], [XrefType], [XrefCode]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMUX] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPMUX] WITH NOCHECK ADD
CONSTRAINT [FK_bPMUX_bPMUT] FOREIGN KEY ([Template]) REFERENCES [dbo].[bPMUT] ([Template]) ON DELETE CASCADE
GO

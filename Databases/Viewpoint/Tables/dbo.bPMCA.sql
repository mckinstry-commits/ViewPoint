...
ALTER TABLE [dbo].[bPMCA] ADD 
CONSTRAINT [PK_bPMCA] PRIMARY KEY CLUSTERED  ([PMCo], [Addon]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMCA] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPMCA] WITH NOCHECK ADD
CONSTRAINT [FK_bPMCA_bJCCT_BasisCostType] FOREIGN KEY ([PhaseGroup], [BasisCostType]) REFERENCES [dbo].[bJCCT] ([PhaseGroup], [CostType])
ALTER TABLE [dbo].[bPMCA] WITH NOCHECK ADD
CONSTRAINT [FK_bPMCA_bJCCT_CostType] FOREIGN KEY ([PhaseGroup], [CostType]) REFERENCES [dbo].[bJCCT] ([PhaseGroup], [CostType])
ALTER TABLE [dbo].[bPMCA] WITH NOCHECK ADD
CONSTRAINT [FK_bPMCA_bPMCo] FOREIGN KEY ([PMCo]) REFERENCES [dbo].[bPMCO] ([PMCo]) ON DELETE CASCADE
ALTER TABLE [dbo].[bPMCA] WITH NOCHECK ADD
CONSTRAINT [FK_bPMCA_bJCPM] FOREIGN KEY ([PhaseGroup], [Phase]) REFERENCES [dbo].[bJCPM] ([PhaseGroup], [Phase])
GO

...
CONSTRAINT [PK_bPMSI] PRIMARY KEY CLUSTERED  ([PMCo], [Project], [SubmittalType], [Submittal], [Rev], [Item]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMSI] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPMSI] WITH NOCHECK ADD
CONSTRAINT [FK_bPMSI_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job])
ALTER TABLE [dbo].[bPMSI] WITH NOCHECK ADD
CONSTRAINT [FK_bPMSI_bPMSM] FOREIGN KEY ([PMCo], [Project], [Submittal], [SubmittalType], [Rev]) REFERENCES [dbo].[bPMSM] ([PMCo], [Project], [Submittal], [SubmittalType], [Rev]) ON DELETE CASCADE
ALTER TABLE [dbo].[bPMSI] WITH NOCHECK ADD
CONSTRAINT [FK_bPMSI_bPMSC] FOREIGN KEY ([Status]) REFERENCES [dbo].[bPMSC] ([Status])
GO

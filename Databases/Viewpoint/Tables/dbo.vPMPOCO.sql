...
ALTER TABLE [dbo].[vPMPOCO] WITH NOCHECK ADD
CONSTRAINT [FK_vPMPOCO_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job])
ALTER TABLE [dbo].[vPMPOCO] WITH NOCHECK ADD
CONSTRAINT [FK_vPMPOCO_bPOHD] FOREIGN KEY ([POCo], [PO]) REFERENCES [dbo].[bPOHD] ([POCo], [PO])
ALTER TABLE [dbo].[vPMPOCO] WITH NOCHECK ADD
CONSTRAINT [FK_vPMPOCO_bPMSC] FOREIGN KEY ([Status]) REFERENCES [dbo].[bPMSC] ([Status])
GO
...
-- TK-04902 --
-------------------------------------------------------
-- PREFILL DISTRIBUTION FROM PMPOHeader DISTRIBUTION --
-------------------------------------------------------
INSERT INTO dbo.PMDistribution(Seq, VendorGroup, SentToFirm, SentToContact, [Send], 
							   PrefMethod, CC, DateSent, PMCo, Project, POCo, PO, POCOID, POCONum)
	 SELECT Dist.Seq, Dist.VendorGroup, Dist.SentToFirm, Dist.SentToContact, Dist.[Send], 
	        Dist.PrefMethod, Dist.CC, dbo.vfDateOnly(), Dist.PMCo, Dist.Project, Dist.POCo,
	        Dist.PO, i.KeyID, i.POCONum
       FROM dbo.PMDistribution Dist
       JOIN inserted i ON Dist.POCo=i.POCo AND Dist.Project=i.Project AND Dist.PO=i.PO
	  WHERE Dist.POCo=i.POCo AND Dist.Project=i.Project AND Dist.PO=i.PO
	    AND Dist.PurchaseOrderID IS NOT NULL
...
GO
ALTER TABLE [dbo].[vPMPOCO] ADD CONSTRAINT [CK_vPMPOCO_POCONum] CHECK (([POCONum]>=(0)))
GO
ALTER TABLE [dbo].[vPMPOCO] ADD CONSTRAINT [CK_vPMPOCO_ReadyForAcctg] CHECK (([ReadyForAcctg]='Y' OR [ReadyForAcctg]='N'))
GO

...
BEGIN
	select @validcnt = count(*) from inserted where OldSurchargeRate is not null and OldSurchargeRate<0
	if @validcnt > 0
	begin
...
IF UPDATE(OldMinAmt)
BEGIN
	select @validcnt = count(*) from inserted where OldMinAmt is not null and OldMinAmt<0
	if @validcnt > 0
	begin

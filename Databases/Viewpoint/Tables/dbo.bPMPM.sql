...
ALTER TABLE [dbo].[bPMPM] ADD 
CONSTRAINT [PK_bPMPM] PRIMARY KEY CLUSTERED  ([VendorGroup], [FirmNumber], [ContactCode]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMPM] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPMPM] WITH NOCHECK ADD
CONSTRAINT [FK_bPMPM_bHQCountry] FOREIGN KEY ([MailCountry]) REFERENCES [dbo].[bHQCountry] ([Country])
ALTER TABLE [dbo].[bPMPM] WITH NOCHECK ADD
CONSTRAINT [FK_bPMPM_bHQST] FOREIGN KEY ([MailCountry], [MailState]) REFERENCES [dbo].[bHQST] ([Country], [State])
ALTER TABLE [dbo].[bPMPM] WITH NOCHECK ADD
CONSTRAINT [FK_bPMPM_bPMFM] FOREIGN KEY ([VendorGroup], [FirmNumber]) REFERENCES [dbo].[bPMFM] ([VendorGroup], [FirmNumber])
GO

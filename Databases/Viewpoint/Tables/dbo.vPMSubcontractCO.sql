...
ALTER TABLE [dbo].[vPMSubcontractCO] WITH NOCHECK ADD
CONSTRAINT [FK_vPMSubcontractCO_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job])
ALTER TABLE [dbo].[vPMSubcontractCO] WITH NOCHECK ADD
CONSTRAINT [FK_vPMSubcontractCO_bSLHD] FOREIGN KEY ([SLCo], [SL]) REFERENCES [dbo].[bSLHD] ([SLCo], [SL])
ALTER TABLE [dbo].[vPMSubcontractCO] WITH NOCHECK ADD
CONSTRAINT [FK_vPMSubcontractCO_bPMSC] FOREIGN KEY ([Status]) REFERENCES [dbo].[bPMSC] ([Status])
GO
...
-- TK-05193 --
-------------------------------------------------
-- PREFILL DISTRIBUTION FROM PMSS DISTRIBUTION --
-------------------------------------------------
INSERT INTO dbo.PMDistribution(Seq, VendorGroup, SentToFirm, SentToContact, ResponsiblePerson, [Send], 
							   PrefMethod, CC, DateSent, PMCo, Project, SLCo, SL, SubcontractCOID, SubCO)
	SELECT	TOP 1 1, Dist.VendorGroup, Dist.SendToFirm, Dist.SendToContact, Dist.ResponsiblePerson, 'Y', 
			pm.PrefMethod, 'N', dbo.vfDateOnly(), Dist.PMCo, Dist.Project, Dist.SLCo, Dist.SL, i.KeyID, i.SubCO
      FROM	dbo.PMSS Dist
      JOIN	inserted i ON Dist.PMCo=i.PMCo AND Dist.SLCo=i.SLCo AND Dist.Project=i.Project AND Dist.SL=i.SL
      JOIN	dbo.PMPM pm ON Dist.VendorGroup=pm.VendorGroup AND Dist.SendToFirm=pm.FirmNumber 
			AND Dist.SendToContact=pm.ContactCode
	 WHERE	Dist.PMCo=i.PMCo AND Dist.SLCo=i.SLCo AND Dist.Project=i.Project AND Dist.SL=i.SL
			AND Dist.SendToFirm IS NOT NULL
			AND Dist.SendToContact IS NOT NULL
...
GO
ALTER TABLE [dbo].[vPMSubcontractCO] ADD CONSTRAINT [CK_vPMSubcontractCO_ReadyForAcctg] CHECK (([ReadyForAcctg]='Y' OR [ReadyForAcctg]='N'))
GO
ALTER TABLE [dbo].[vPMSubcontractCO] ADD CONSTRAINT [CK_vPMSubcontractCO_SubCO] CHECK (([SubCO]>=(0)))
GO

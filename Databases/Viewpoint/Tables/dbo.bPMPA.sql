...
ALTER TABLE [dbo].[bPMPA] ADD 
CONSTRAINT [PK_bPMPA] PRIMARY KEY CLUSTERED  ([PMCo], [Project], [AddOn]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMPA] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPMPA] WITH NOCHECK ADD
CONSTRAINT [FK_bPMPA_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job]) ON DELETE CASCADE ON UPDATE CASCADE
ALTER TABLE [dbo].[bPMPA] WITH NOCHECK ADD
CONSTRAINT [FK_bPMPA_bJCCT] FOREIGN KEY ([PhaseGroup], [BasisCostType]) REFERENCES [dbo].[bJCCT] ([PhaseGroup], [CostType])
GO
...
GO
ALTER TABLE [dbo].[bPMPA] ADD CONSTRAINT [CK_bPMPA_RoundAmount] CHECK (([RoundAmount]='Y' OR [RoundAmount]='N'))
GO
ALTER TABLE [dbo].[bPMPA] ADD CONSTRAINT [CK_bPMPA_Standard] CHECK (([Standard]='Y' OR [Standard]='N'))
GO

EXEC sp_bindrule N'[dbo].[brYesNo]', N'[dbo].[bPMPA].[Include]'
GO

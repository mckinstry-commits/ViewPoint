...
ALTER TABLE [dbo].[bPMDR] ADD 
CONSTRAINT [PK_bPMDR] PRIMARY KEY CLUSTERED  ([PMCo], [Project], [DrawingType], [Drawing], [Rev]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMDR] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPMDR] WITH NOCHECK ADD
CONSTRAINT [FK_bPMDR_bPMDG] FOREIGN KEY ([PMCo], [Project], [DrawingType], [Drawing]) REFERENCES [dbo].[bPMDG] ([PMCo], [Project], [DrawingType], [Drawing]) ON DELETE CASCADE
ALTER TABLE [dbo].[bPMDR] WITH NOCHECK ADD
CONSTRAINT [FK_bPMDR_bPMSC] FOREIGN KEY ([Status]) REFERENCES [dbo].[bPMSC] ([Status])
GO

...
ALTER TABLE [dbo].[bPMWD] ADD 
CONSTRAINT [PK_bPMWD] PRIMARY KEY CLUSTERED  ([PMCo], [ImportId], [Sequence]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMWD] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPMWD] WITH NOCHECK ADD
CONSTRAINT [FK_bPMWD_bPMWH] FOREIGN KEY ([PMCo], [ImportId]) REFERENCES [dbo].[bPMWH] ([PMCo], [ImportId]) ON DELETE CASCADE
GO
...
GO
ALTER TABLE [dbo].[bPMWD] ADD CONSTRAINT [CK_bPMWD_ItemUnitFlag] CHECK (([ItemUnitFlag]='N' OR [ItemUnitFlag]='Y' OR [ItemUnitFlag]=NULL))
GO
ALTER TABLE [dbo].[bPMWD] ADD CONSTRAINT [CK_bPMWD_PhaseUnitFlag] CHECK (([PhaseUnitFlag]='N' OR [PhaseUnitFlag]='Y' OR [PhaseUnitFlag]=NULL))
GO

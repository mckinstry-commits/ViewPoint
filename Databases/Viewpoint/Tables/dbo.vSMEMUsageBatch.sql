...
ALTER TABLE [dbo].[vSMEMUsageBatch] WITH NOCHECK ADD
CONSTRAINT [FK_vSMEMUsageBatch_bHQBC] FOREIGN KEY ([SMCo], [Mth], [BatchId]) REFERENCES [dbo].[bHQBC] ([Co], [Mth], [BatchId])
GO
ALTER TABLE [dbo].[vSMEMUsageBatch] ADD CONSTRAINT [CK_vSMEMUsageBatchBatchTransType] CHECK (([BatchTransType]='A' AND [IsReversingEntry]=(0) OR [BatchTransType]='C' OR [BatchTransType]='D' AND [IsReversingEntry]=(1)))
GO
ALTER TABLE [dbo].[vSMEMUsageBatch] ADD CONSTRAINT [CK_vSMEMUsageBatchTrans] CHECK (([IsReversingEntry]=case when [EMTransMth] IS NULL then (0) else (1) end AND [IsReversingEntry]=case when [EMTrans] IS NULL then (0) else (1) end))
GO

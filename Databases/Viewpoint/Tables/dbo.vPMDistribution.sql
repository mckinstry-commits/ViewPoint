...
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [CK_vPMDistribution_CC] CHECK (([CC]='N' OR [CC]='C' OR [CC]='B'))
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [CK_vPMDistribution_PrefMethod] CHECK (([PrefMethod]='M' OR [PrefMethod]='E' OR [PrefMethod]='F'))
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [CK_vPMDistribution_Send] CHECK (([Send]='Y' OR [Send]='N'))
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [FK_vPMDistribution_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job])
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [FK_vPMDistribution_bPMSM] FOREIGN KEY ([SubmittalID]) REFERENCES [dbo].[bPMSM] ([KeyID]) ON DELETE CASCADE
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [FK_vPMDistribution_bPMTL] FOREIGN KEY ([TestLogID]) REFERENCES [dbo].[bPMTL] ([KeyID]) ON DELETE CASCADE
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [FK_vPMDistribution_bPMDG] FOREIGN KEY ([DrawingLogID]) REFERENCES [dbo].[bPMDG] ([KeyID]) ON DELETE CASCADE
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [FK_vPMDistribution_bPMFM] FOREIGN KEY ([VendorGroup], [SentToFirm]) REFERENCES [dbo].[bPMFM] ([VendorGroup], [FirmNumber])
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [FK_vPMDistribution_bPMPM] FOREIGN KEY ([VendorGroup], [SentToFirm], [SentToContact]) REFERENCES [dbo].[bPMPM] ([VendorGroup], [FirmNumber], [ContactCode])
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [FK_vPMDistribution_bPMIL] FOREIGN KEY ([InspectionLogID]) REFERENCES [dbo].[bPMIL] ([KeyID]) ON DELETE CASCADE
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
CONSTRAINT [FK_vPMDistribution_bPMIM] FOREIGN KEY ([IssueID]) REFERENCES [dbo].[bPMIM] ([KeyID]) ON DELETE CASCADE
ALTER TABLE [dbo].[vPMDistribution] WITH NOCHECK ADD
...
---- validate that only one sent to firm contact exists with CC option of 'N'
IF (SELECT COUNT(*) FROM dbo.vPMDistribution v JOIN INSERTED i ON
		i.PurchaseOrderID = v.PurchaseOrderID AND i.CC = 'N' and v.CC = 'N'
		WHERE i.KeyID <> v.KeyID AND i.PurchaseOrderID IS NOT NULL) > 0
	begin
	select @errmsg = 'Only one Sent To Firm Contact is allowed with CC option (None).'
	goto error
	end	

...
ALTER TABLE [dbo].[bPMTC] ADD 
CONSTRAINT [PK_bPMTC] PRIMARY KEY CLUSTERED  ([PMCo], [Project], [Transmittal], [Seq]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMTC] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPMTC] WITH NOCHECK ADD
CONSTRAINT [FK_bPMTC_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job])
ALTER TABLE [dbo].[bPMTC] WITH NOCHECK ADD
CONSTRAINT [FK_bPMTC_bPMTM] FOREIGN KEY ([PMCo], [Project], [Transmittal]) REFERENCES [dbo].[bPMTM] ([PMCo], [Project], [Transmittal]) ON DELETE CASCADE
ALTER TABLE [dbo].[bPMTC] WITH NOCHECK ADD
CONSTRAINT [FK_bPMTC_bPMFM] FOREIGN KEY ([VendorGroup], [SentToFirm]) REFERENCES [dbo].[bPMFM] ([VendorGroup], [FirmNumber])
ALTER TABLE [dbo].[bPMTC] WITH NOCHECK ADD
CONSTRAINT [FK_bPMTC_bPMPM] FOREIGN KEY ([VendorGroup], [SentToFirm], [SentToContact]) REFERENCES [dbo].[bPMPM] ([VendorGroup], [FirmNumber], [ContactCode])
GO

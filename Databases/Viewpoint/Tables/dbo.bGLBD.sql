...
CREATE UNIQUE CLUSTERED INDEX [biGLBD] ON [dbo].[bGLBD] ([GLCo], [GLAcct], [BudgetCode], [Mth]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biGLBD2] ON [dbo].[bGLBD] ([GLCo], [Mth], [BudgetCode], [GLAcct]) INCLUDE ([BudgetAmt]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]

ALTER TABLE [dbo].[bGLBD] WITH NOCHECK ADD
CONSTRAINT [FK_bGLBR_bGLBD_GLCoBudgetCode] FOREIGN KEY ([GLCo], [BudgetCode]) REFERENCES [dbo].[bGLBC] ([GLCo], [BudgetCode])
GO

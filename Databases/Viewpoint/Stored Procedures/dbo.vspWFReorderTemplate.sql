...
*
*****************************************************/
CREATE PROCEDURE [dbo].[vspWFReorderTemplate]
...
BEGIN
	...
	BEGIN TRY
		...
		UPDATE vWFTemplateStepsc SET Task = CAST(n.Names as int)
		FROM vWFTemplateStepsc s, @NewList n, @OldList o
		WHERE s.Task = o.Names AND n.KeyID = o.KeyID AND s.Template = @Template

		UPDATE vWFTemplateTasksc SET Task = CAST(n.Names as int)
		FROM vWFTemplateTasksc t, @NewList n, @OldList o
...
BEGIN
	BEGIN TRY
		...
		UPDATE vWFTemplateStepsc SET Step = CAST(n.Names as int)
		FROM vWFTemplateStepsc s, @NewList n, @OldList o
...
END
GO
GRANT EXECUTE ON  [dbo].[vspWFReorderTemplate] TO [public]
GO

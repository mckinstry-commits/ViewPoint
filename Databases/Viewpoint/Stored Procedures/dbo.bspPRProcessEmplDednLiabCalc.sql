...
@accumbeginmth bMonth, @accumendmth bMonth, @calcamt bDollar OUTPUT, @errmsg varchar(255) OUTPUT
...
DECLARE @rcode int, @rate bUnitCost, --@calcamt bDollar, 
	...
	@liabbasis bDollar,	@accurcalc float, @exemptamt bDollar
...
DECLARE @disposable bDollar, @dedncode bEDLCode, @dednamt bDollar, @numallocs tinyint, @allocgroup tinyint
	...
	IF @method = 'R'
		...
		IF @procname = 'bspPR_AU_SuperWithMin'
			BEGIN
			--get employee's work state
			SELECT @workstate = UnempState FROM dbo.bPREH WITH (NOLOCK) WHERE PRCo = @prco AND Employee = @employee

			-- apply limit to subject amount
			IF (@calcbasis + @accumelig) > @limitamt
				BEGIN
				SELECT @calcbasis = @limitamt - @accumelig
				IF @limitcorrect = 'N' AND @calcbasis < 0.00 SELECT @calcbasis = 0.00
				END

			IF @calcbasis > 0.00
				BEGIN
				EXEC @rcode = @procname @calcbasis, @rate, @workstate, @ppds, @calcamt OUTPUT, @errmsg OUTPUT
				-- please put no code between exec AND @@error check!
	...
	IF @method = 'N'
		...
		-- IF @overcalcs = 'R' SELECT @calcamt = @rate * @calcbasis, @accurcalc = @rate * @calcbasis -- removed for #11339

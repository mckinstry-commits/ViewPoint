...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NBT08]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_NBT08]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @calcamt = 0, @procname = 'bspPR_CA_NBT08'
   ...
   select @rate = .1795, @KP = 4090
   if @A <= 113273 select @rate = .1680, @KP = 2787
   if @A <= 69673 select @rate = .1548, @KP = 1867
   if @A <= 34836 select @rate = .1012, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NBT08] TO [public]
GO

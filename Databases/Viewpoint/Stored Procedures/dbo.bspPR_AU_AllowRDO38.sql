...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_AU_AllowRDO38]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_AU_AllowRDO38]
   /********************************************************
   * CREATED BY: 	EN 3/10/2009 #129888
   * MODIFIED BY:	EN 2/09/2010 #136039 modified from original routine created on 3/10/2009 called 
   *										bspPR_AU_AllowanceRDO - needed to create this alternate 38 hour version
   *
   * USAGE:
   * 	Calculates rate per hour allowance with RDO factor adjustment for a 38 hour week and posts to timecard 
   *	addons table (bPRTA).  This can be used with an RDO fortnight arrangement.
   *	The RDO factor adjustment essentially evens out the subject hours used in rate per hour computation so that
   *	each week the same hours are used.  In other words for an RDO scheme where an RDO day is taken every 4th
   *	week, with this scheme 38 hours are used each week to compute the allowance whereas in the
   *	scheme without the RDO adjustment 40 hours are used the 1st 3 weeks and 32 are used the 4nd week.
   *
   * INPUT PARAMETERS:
   *	@prco	PR Company
   ...
   **********************************************************/
	...
	select @rcode = 0, @hours = 0, @subjecthours = 0, @amt = 0, @procname = 'bspPR_AU_AllowRDO38'
	...
	while @SubjEC is not null
		...
		next_Timecard:
			...
			--Compute allowance modified by RDO factor rate adjustment
			select @subjecthours = @subjecthours * (32.00/40.00)
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_AU_AllowRDO38] TO [public]
GO

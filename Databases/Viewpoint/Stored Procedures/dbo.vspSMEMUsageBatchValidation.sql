...
FetchNext:
	...
	BEGIN
		...
		BEGIN
				EXEC @rcode = dbo.vspSMJobCostDistributionInsert @SMWorkCompletedID=@SMWorkCompletedID, @BatchCo=@SMCo,@BatchMth=@BatchMth,@BatchId = @BatchId, @JCTransType='EM',@errmsg = @msg OUTPUT
				IF @rcode <> 0 
				BEGIN
					SET @ErrorText = @ErrorText + ' - ' + dbo.vfToString(@msg)
					EXEC @rcode = dbo.bspHQBEInsert @co = @SMCo, @mth = @BatchMth, @batchid = @BatchId, @errortext = @ErrorText, @errmsg = @msg OUTPUT

...
/***********************************************************************/
CREATE procedure [dbo].[vspPMDocDistInitForCCO]
/************************************************************************
* Created By:	JG 04/28/2011 TK-04388
* Modified By:	JG 05/05/2011 TK-04388
*				TRL 07/29/2011 TK-07037  Change ContractCO to ID
...
* PM Contract Change Order Document and load email, fax, CC addresses,
* header and query strings.
*
* Called from frmPMContractChangeOrder form.
*
*
*
*
...
* PM Company
* Project
* Document Category	- will be 'CCO'
* User Name
* CCO_KeyID				CCO identifier
...
(@pmco bCompany, @project bProject, @DocCategory varchar(10), @user bVPUserName,
 @CCOID BIGINT = NULL, @template varchar(40) = null,
...
SELECT @description=Description, @contract=Contract,
		@document=CONVERT(VARCHAR(10), ID),@CCO=ID
FROM dbo.PMContractChangeOrder WHERE KeyID=@CCOID
if @@rowcount = 0
	begin
...
if not exists(select Seq from dbo.PMDistribution where ContractCOID = @CCOID
						and Send='Y' and CC='N')
	begin
...
where ContractCOID=@CCOID and Send='Y' and CC='N'
...
exec @retcode = dbo.vspPMDocDistInitCCListBuild @pmco, @project, @DocCategory, null, @document, NULL, NULL,
				----TK-11961
				@CCOID, @ccnames output, @ccaddr output, @bccaddr output, @msg output
...
if ltrim(rtrim(isnull(@template,''))) <> ''
	...
	select @value = @value + ' and a.ID = ' + CONVERT(VARCHAR(10), @CCO)
	...
	if @needitemquery = 'Y'
		...
		select @value = @value + ' and a.ID = ' + CONVERT(VARCHAR(20), @CCO)
...
if isnull(@usestdsubject,'Y') = 'Y'
	begin
	set @subjectline = 'CCO: ' + isnull(@document,'') + ' - ' + isnull(@description,'')
	end
else
	begin
	...
	if isnull(@subjectline,'') = '' set @subjectline = 'CCO: ' + isnull(@document,'') + ' - ' + isnull(@description,'')
	end
...
GO
GRANT EXECUTE ON  [dbo].[vspPMDocDistInitForCCO] TO [public]
GO

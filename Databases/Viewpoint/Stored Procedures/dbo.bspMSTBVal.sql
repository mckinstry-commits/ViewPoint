...
fetch next from bcMSTB into @seq,@transtype,@mstrans,@saledate,@fromloc,@vendorgroup,
   ...
   select @errorstart = 'Seq#' + convert(varchar(6),@seq), @nullnumericflag = 0
...
MSTB_error:	
	...
	IF @SurchargeKeyID IS NOT NULL
		BEGIN
			SELECT @ParentSeq = b1.BatchSeq, @SurchargeCode = b2.SurchargeCode
			  FROM bMSTB b1 WITH (NOLOCK)
			  JOIN bMSTB b2 WITH (NOLOCK) ON b1.KeyID = b2.SurchargeKeyID
			 WHERE b1.Co = @msco AND b1.Mth = @mth AND b1.BatchId = @batchid 
			   AND b2.BatchSeq = @seq 
			 
			SET @errortext = @errortext + ' - Surcharge Code: ' + ISNULL(CAST(@SurchargeCode as varchar(10)), 'N/A')
									    + ' - Parent Seq: ' + ISNULL(CAST(@ParentSeq as varchar(10)), 'N/A')
		END

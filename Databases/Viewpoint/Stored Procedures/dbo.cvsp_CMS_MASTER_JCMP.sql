...
select JCCo=@toco
	...
	, FAX          = case 
						when JCTNME.FAXPHONENO=0 
						then null 
						else convert(nvarchar(max),JCTNME.FAXAREACODE)+convert(nvarchar(max),JCTNME.FAXPHONENO) 
					 end
	, MobilePhone  = case 
						when JCTNME.CELLPHONENUMBER=0 
						then null 
						else convert(nvarchar(max),JCTNME.CELLAREACODE)+convert(nvarchar(max),JCTNME.CELLPHONENUMBER) 
					 end
	, Pager        = case 
						when JCTNME.OTHERPHONENO=0 
						then null 
						else convert(nvarchar(max),JCTNME.OTHERAREACODE)+convert(nvarchar(max),JCTNME.OTHERPHONENO) 
					 end

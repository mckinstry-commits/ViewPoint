...
/***********************************************************************/
CREATE procedure [dbo].[vspPMDocDistInitForDrawingLog]
/************************************************************************
* Created By:	GF 11/08/2009 - issue #135479
* Modified By:	GF 12/04/2009 - issue #136694
...
*				GF 11/12/2010 - issue #142083 use function for fax number
...
* PM Drawing Log Document and load email, fax, CC addresses, header and query strings.
* Called from frmPMDrawingLogs form.
*
*
*
* Input parameters:
* PM Company
* Project
* Document Category	will always be 'DRAWING'
* User Name
* Document Type
* Document
...
(@pmco bCompany, @project bProject, @doccategory varchar(10), @user bVPUserName,
 ...
 @filename varchar(255) = null, @msg varchar(max) output)
as
set nocount on
...
declare @rcode int, @retcode int, @opencursor int, @errmsg varchar(255),
		...
		@ccaddr varchar(max), @groupbylength bigint, @sequence int,
		@faxaddress varchar(100), @bccaddr varchar(max),
		@pmdzkeyid bigint, @pmhikeyid bigint, @sourcekeyid bigint, @needitemquery bYN,
		@usestdcclist varchar(1), @ovrcclist varchar(max),
...
if @@rowcount = 0
	begin
	select @msg = 'Invalid Drawing Log', @rcode = 1
	goto bspexit
	end

---- validate there is other document distribution in vPMDistribution
if not exists(select 1 from dbo.vPMDistribution where DrawingLogID=@sourcekeyid
					AND Send='Y' AND CC='N')
					--PMCo=@pmco and Project=@project
					--and DrawingType=@doctype and Drawing=@document and Send='Y' and CC='N')
	begin
...
---- create cursor on vPMDistribution
declare bcPMDistDrawingLogs cursor LOCAL FAST_FORWARD
for select VendorGroup, SentToFirm, SentToContact, PrefMethod
from dbo.vPMDistribution
WHERE DrawingLogID = @sourcekeyid
and Send='Y' and CC='N'
--where PMCo=@pmco and Project=@project and DrawingType=@doctype
--and Drawing=@document and Send='Y' and CC='N'

---- open bcPMDistDrawingLogs cursor
open bcPMDistDrawingLogs
select @opencursor = 1

---- loop through distribution list
PMDistDrawingLogs_loop:
fetch next from bcPMDistDrawingLogs into @vendorgroup, @senttofirm, @senttocontact, @prefmethod

if @@fetch_status = -1 goto PMDistDrawingLogs_end
if @@fetch_status <> 0 goto PMDistDrawingLogs_loop
...
if ltrim(rtrim(isnull(@template,''))) <> ''
	...
	exec @rcode = dbo.bspHQWDJoinClauseBuild 'DRAWING', 'N', 'Y', 'N', @joinstring output, @msg output
	if @rcode <> 0 goto bspexit
	...
	select @value = @value + ' and a.DrawingType = ' + CHAR(39) + @doctype + CHAR(39)
	select @value = @value + ' and a.Drawing = ' + CHAR(39) + @document + CHAR(39)
	...
	if @needitemquery = 'Y'
		...
		select @value = @value + ' and a.Drawing = ' + CHAR(39) + @document + CHAR(39)
		---- add group by 
		select @value = @value + ' group by a.DrawingType, a.Drawing, ' + @groupby
...
if isnull(@usestdsubject,'Y') = 'Y'
	...
	if isnull(@subjectline,'') = '' set @subjectline = 'Drawing Type: ' + isnull(@doctype,'') + 'Drawing Log: ' + isnull(@document,'') + ' - ' + isnull(@description,'')
	end
...
insert PMHI(SourceTableName, SourceKeyId, CreatedBy, VendorGroup,
			...
			bCCAddresses)
select 'PMDG', @sourcekeyid, @user, @vendorgroup, @senttofirm, @senttocontact,
...
---- deallocate cursor
PMDistDrawingLogs_end:
	if @opencursor = 1
		begin
		close bcPMDistDrawingLogs
		deallocate bcPMDistDrawingLogs
		set @opencursor = 0
		end
...
GO
GRANT EXECUTE ON  [dbo].[vspPMDocDistInitForDrawingLog] TO [public]
GO

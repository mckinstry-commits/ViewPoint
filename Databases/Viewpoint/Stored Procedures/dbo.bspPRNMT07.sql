...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRNMT07    Script Date: 8/28/99 9:33:30 AM ******/
   CREATE proc [dbo].[bspPRNMT07]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @allowance = 3450, @procname = 'bspPRNMT07'
   ...
   if @status = 'S'
   	...
   	if @annualized_wage <= 1900 goto bspexit
   
   
   	if @annualized_wage <= 7400 select @tax_addition = 0, @wage_bracket = 1900, @rate = .017
   	if @annualized_wage > 7400 select @tax_addition = 93.50, @wage_bracket = 7400, @rate = .032
   	if @annualized_wage > 12900 select @tax_addition = 269.50, @wage_bracket = 12900, @rate = .047
   	if @annualized_wage > 17900 select @tax_addition = 504.5, @wage_bracket = 17900, @rate = .053
   end
   
   if @status = 'M'
   	begin
   
   	if @annualized_wage <= 7250 goto bspexit
   
   
   	if @annualized_wage <= 15250 select @tax_addition = 0, @wage_bracket = 7250, @rate = .017
   	if @annualized_wage > 15250 select @tax_addition = 136, @wage_bracket = 15250, @rate = .032
   	if @annualized_wage > 23250 select @tax_addition = 392, @wage_bracket = 23250, @rate = .047
   	if @annualized_wage > 31250 select @tax_addition = 768, @wage_bracket = 31250, @rate = .053
   end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNMT07] TO [public]
GO

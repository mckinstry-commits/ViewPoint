...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_ONT082]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_ONT082]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @rate = 0, @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @V1 = 0, @S = 0, @Y = 0, @LCP = 0, @T2 = 0
   select @calcamt = 0, @procname = 'bspPR_CA_ONT082'
   ...
   select @rate = .1116, @KP = 2565
   if @A <= 72041 select @rate = .0915, @KP = 1117
   if @A <= 36020 select @rate = .0605, @KP = 0
   ...
   select @V1 = 0
   if @T4 > 4162 select @V1 = .2 * (@T4 - 4162)
   if @T4 > 5249 select @V1 = @V1 + .36 * (@T4 - 5249)
   ...
   select @Y = (370 * @addexempts)
   select @S = case when @T4+@V1 < (2*(201+@Y))-(@T4+@V1) then @T4+@V1 else (2*(201+@Y))-(@T4+@V1) end
   if @S < 0 select @S = 0

   -- compute labour-sponsored funds federal tax credit for the year
   select @LCP = case when .15 * @capstock < 1125 then .15 * @capstock else 750 end
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_ONT082] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRORT98    Script Date: 8/28/99 9:33:32 AM ******/
   CREATE   proc [dbo].[bspPRORT98]
   /********************************************************
   ...
   *
   * USAGE:
   * 	Calculates 1998 Oregon Income Tax
   *
   * INPUT PARAMETERS:
   ...
   **********************************************************/
   ...
   select @rcode = 0, @basetax = 0, @dedn = 0, @rate = 0, @procname = 'bspPRORT98'
   ...
   bspcalc: /* calculate Oregon Tax */
   	select @amt = ((@basetax + (@a - @dedn) * @rate) - (90 * @exempts)) / @ppds
   
   bspexit:
   	return @rcode

GO
GRANT EXECUTE ON  [dbo].[bspPRORT98] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_YTT08]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_YTT08]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @rate = 0, @KP = 0, @K1P = 0, @K2P = 0, @K4P = 0, @T4 = 0, @V1 = 0, @LCP = 0, @T2 = 0
   select @calcamt = 0, @procname = 'bspPR_CA_YTT08'
   ...
   select @rate = .1276, @KP = 3960
   if @A <= 123184 select @rate = .1144, @KP = 2334
   if @A <= 75769 select @rate = .0968, @KP = 1000
   if @A <= 37885 select @rate = .0704, @KP = 0
   ...
   select @K4P = round(case when @TCrate*@A < @TCrate*1019 then @TCrate*@A else @TCrate*1019 end,2)
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_YTT08] TO [public]
GO

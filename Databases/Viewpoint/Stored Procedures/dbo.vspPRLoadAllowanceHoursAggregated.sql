...
*				KK 01/30/13 - 8166   Added comments to code
*
...
SELECT 
		...
		MAX(tae.Id)  OVER (PARTITION BY tae.PRCo,
								tae.Threshold,
								tae.ThresholdPeriod,
		...
		SUM(tae.Hours) OVER(PARTITION BY tae.PRCo,
								tae.Threshold,
								tae.ThresholdPeriod,
								tae.AllowanceRuleName,
								tae.AllowanceRulesetName,
...
FROM #TCAllowEmp tae
	...
	OUTER APPLY 
		(	SELECT 
			...
			FROM dbo.vPRAllowanceRules AS vpar
				-- Unflatten the days
				UNPIVOT (DayOfWeekValue FOR DayOfWeek IN (DayOfWeekMonday,DayOfWeekTuesday,DayOfWeekWednesday,DayOfWeekThursday,DayOfWeekFriday,DayOfWeekSaturday,DayOfWeekSunday)) AS dw
	...
	OUTER APPLY (   -- Holiday from Pay Period Control ONLY where "Apply to craft" is checked
...
SELECT	-- Build a group Id to identify consoldiated time cards
		MAX(tae.Id)  OVER (PARTITION BY tae.PRCo,
								tae.Threshold,
								tae.ThresholdPeriod,
								tae.AllowanceRuleName,

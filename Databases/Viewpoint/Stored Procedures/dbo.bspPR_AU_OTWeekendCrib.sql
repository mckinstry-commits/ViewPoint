...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_AU_OTWeekendCrib]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_AU_OTWeekendCrib]
   /********************************************************
   * CREATED BY: 	EN 3/03/2010 #132653
   * MODIFIED BY:  
   *
   * USAGE:
   * 	Computes Meal/Rest/Crib allowance when weekend time posted (subject hours) is over a certain
   *	threshold.  Allowance is computed at 30 minutes of doubletime.
   ...
   **********************************************************/
	...
	declare @rcode int, @totalearns bDollar, @WeekendThreshold bHrs, 
		...
		@emplrate bUnitCost, @VICWeekendThreshold bHrs, @NSWWeekendThreshold bHrs, 
		@WAWeekendThreshold bHrs

	select @rcode = 0, @amt = 0

	--define state overtime crib thresholds
	select @VICWeekendThreshold = 4
	select @NSWWeekendThreshold = 8
	select @WAWeekendThreshold = 4
	...
	if @addonYN = 'Y'
		...
		where h.PRCo = @prco and h.PRGroup = @prgroup and h.PREndDate = @prenddate
			...
			and datepart(weekday,h.PostDate) in (1,7)
		group by h.PostDate
		...
		while @postdate is not null
			...
			select @WeekendThreshold = 99
			--determine thresholds
			if @workstate = 'VIC' select @WeekendThreshold = @VICWeekendThreshold
			if @workstate = 'NSW' select @WeekendThreshold = @NSWWeekendThreshold
			if @workstate = 'WA' select @WeekendThreshold = @WAWeekendThreshold
			...
			if @hoursposted >= @WeekendThreshold
				begin
				...
				--compute allowance as 30 minutes at doubletime
				update @PostDates1 set Allowance = (@emplrate * 2) / 2 where PostDate = @postdate
				end
		...
		while @postdate is not null
			...
			if @hoursposted >= @WeekendThreshold
				begin
				--compute allowance as 30 minutes at doubletime
				update @PostDates2 set Allowance = (@rate * 2) / 2 where PostDate = @postdate
				end
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_AU_OTWeekendCrib] TO [public]
GO

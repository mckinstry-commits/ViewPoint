SET QUOTED_IDENTIFIER ON
GO
SET ANSI_NULLS ON
GO





CREATE proc [dbo].[cvsp_CMS_PRTH_Ben] (@fromco smallint, @toco smallint, 
	@errmsg varchar(1000) output, @rowcount bigint output) 
as

/**
=========================================================================
	Copyright © 2009 Viewpoint Construction Software (VCS) 
	The TSQL code in this procedure may not be reproduced, modified,
	transmitted or executed without written consent from VCS
=========================================================================
	Title:			Timecard Benefits (PRTH)
	Created:		06.15.09
	Created by:		CR
	Function:		Xref for Earnings/Deductions/Liabilities in CMS
	Revisions:		1. 8.10.09 - CR - changed InsState and InsCode, added nolocks,changed PostedDate to use WEEKENDDATE instead of JOURNALDATE.
						2. 8.19.10 - JH - Added TC source as ud field to PRTH 


**/

set @errmsg=''
set @rowcount=0


--Declare Variables for functions
Declare @Job varchar(30)
Set @Job=(Select InputMask from vDDDTc where Datatype='bJob')

--Get Customer Defaults
declare @defaultShift tinyint

select @defaultShift=isnull(b.DefaultNumeric,a.DefaultNumeric) 
from Viewpoint.dbo.budCustomerDefaults a
	full outer join Viewpoint.dbo.budCustomerDefaults b on b.Company=@fromco and a.TableName=b.TableName and a.ColName=b.ColName
where a.Company=0 and a.ColName='Shift' and a.TableName='bPRTH';

--Get HQCO defaults
declare @PhaseGroup tinyint
select @PhaseGroup=PhaseGroup from bHQCO where HQCo=@toco

-- delete existing trans
--no delete necessary, done in earlier SP

-- add new trans
BEGIN TRAN
BEGIN TRY


ALTER Table bPRTH disable trigger all;


insert bPRTH(PRCo, PRGroup,PREndDate,Employee,PaySeq,PostSeq,Type,PostDate, GLCo,
	TaxState,UnempState,InsState, InsCode, PRDept,Cert,Craft,Class,Shift,EarnCode,Hours,
	Rate,Amt, BatchId, udPaidDate, udCMCo, udCMAcct, udCMRef, udTCSource, udSource,udConv
	,udCGCTable,udCGCTableID)

select PRCo=@toco
		, PRGroup=e.PRGroup
		, PREndDate=n.WkEndDate
		, Employee=n.EMPLOYEENUMBER
		, PaySeq=n.PaySeq
		, PostSeq=isnull((select max(PostSeq) 
										from PRTH b 
										where b.PRCo=@toco and b.PREndDate=n.WkEndDate 
													and b.Employee=n.EMPLOYEENUMBER),0) 
							+ row_number() over(partition by n.COMPANYNUMBER,n.WkEndDate, n.EMPLOYEENUMBER 
									order by n.COMPANYNUMBER, n.WkEndDate, n.EMPLOYEENUMBER)
		, Type='J'
		, PostDate=substring(convert(nvarchar(max),n.WEEKENDDATE),5,2) + '/' + substring(convert(nvarchar(max),n.WEEKENDDATE),7,2) + '/' + 
							substring(convert(nvarchar(max),n.WEEKENDDATE),1,4)
		, GLCo=@toco
		, TaxState=e.TaxState
		, UnempState=e.UnempState
		, InsState=e.InsState
		, InsCode=e.InsCode
		, PRDept=e.PRDept
		, Cert=e.CertYN
		, Craft=null
		, Class=null
		, Shift=@defaultShift
		, EarnCode=prearn.EarnCode
		, Hours=0
		, Rate=0
		, Amt=n.ADJAMOUNT
		, BatchId=0
		, udPaidDate=substring(convert(nvarchar(max),n.CHECKDATE),5,2) 
			+ '/' + substring(convert(nvarchar(max),n.CHECKDATE),7,2) + '/' 
			+ substring(convert(nvarchar(max),n.CHECKDATE),1,4)
		, udCMCo=@toco
		, udCMAcct=g.CMAcct
		, udCMRef=CHECKNUMBER
		, udTCSource='BEN1'
		, udSource ='PRTH_Ben'
		, udConv='Y'
		,udCGCTable='PRTMAJ',udCGCTableID=PRTMAJID
		from CV_CMS_SOURCE.dbo.PRTMAJ n
	join bPREH e with(nolock) on e.PRCo=@toco and n.EMPLOYEENUMBER=e.Employee
	join bPRGR g with(nolock) on g.PRCo=@toco and e.PRGroup=g.PRGroup
	join Viewpoint.dbo.budxrefPREarn prearn with(nolock) on prearn.Company=@fromco 
		and convert(varchar(max),DEDNUMBER)=prearn.CMSDedCode and CMSCode='D'
where n.COMPANYNUMBER=@fromco


select @rowcount=@@rowcount;


-- HR Benefits
insert bPRTH(PRCo, PRGroup,PREndDate,Employee,PaySeq,PostSeq,Type,PostDate, GLCo,
	TaxState,UnempState,InsState,InsCode, PRDept,Cert,Craft,Class,Shift,EarnCode,Hours,Rate,Amt, BatchId, 
	udPaidDate, udCMCo, udCMAcct, udCMRef, udTCSource, udSource ,udConv,udCGCTable,udCGCTableID)

select PRCo=@toco
		,PRGroup=e.PRGroup
		,PREndDate=n.WkEndDate
		,Employee=n.EMPLOYEENUMBER
		,PaySeq=n.PaySeq
		,PostSeq=isnull((select max(PostSeq)
						from PRTH b 
						where b.PRCo=@toco and b.PREndDate=n.WkEndDate 
							and b.Employee=n.EMPLOYEENUMBER),0) + 
							row_number() over(partition by n.COMPANYNUMBER,n.WkEndDate, n.EMPLOYEENUMBER 
							order by n.COMPANYNUMBER, n.WkEndDate, n.EMPLOYEENUMBER)
		, Type='J'
		, PostDate=substring(convert(nvarchar(max),WEEKENDDATE),5,2) + '/' + substring(convert(nvarchar(max),WEEKENDDATE),7,2) + '/' + 
						substring(convert(nvarchar(max),WEEKENDDATE),1,4)
		, GLCo=@toco
		, TaxState=e.TaxState
		, UnempState=e.UnempState
		, InsState=e.InsState
		, InsCode=e.InsCode
		, PRDept=e.PRDept
		, Cert=e.CertYN
		, Craft=null
		, Class=null
		, Shift=@defaultShift
		, EarnCode=prearn.EarnCode
		, Hours=0
		, Rate=0
		, Amt=n.DEDUCTIONAMT
		, BatchId=0
		, udPaidDate=substring(convert(nvarchar(max),n.CHECKDATE),5,2) 
			+ '/' + substring(convert(nvarchar(max),n.CHECKDATE),7,2) + '/' 
			+ substring(convert(nvarchar(max),n.CHECKDATE),1,4)
		, udCMCo=@toco
		, udCMAcct=g.CMAcct
		, udCMRef=CHECKNUMBER
		, udTCSource='BEN2'
		, udSource ='PRTH_Ben'
		, udConv='Y'
		,udCGCTable='HRTMBN',udCGCTableID=HRTMBNID
from CV_CMS_SOURCE.dbo.HRTMBN n
	join bPREH e with(nolock) on e.PRCo=@toco and n.EMPLOYEENUMBER=e.Employee
	join bPRGR g with(nolock) on g.PRCo=@toco and e.PRGroup=g.PRGroup
	join Viewpoint.dbo.budxrefPREarn prearn with(nolock) on prearn.Company=@fromco 
		and convert(varchar(max),n.BENEFITNUMBER)=prearn.CMSDedCode and CMSDedCode='H'
where n.COMPANYNUMBER=@fromco


select @rowcount=@rowcount+@@rowcount;


COMMIT TRAN
END TRY

BEGIN CATCH
select @errmsg=isnull(ERROR_PROCEDURE(),'')+' '+convert(varchar(10),isnull(ERROR_LINE(),0))+' '+isnull(ERROR_MESSAGE(),'')
ROLLBACK
END CATCH;

ALTER Table bPRTH enable trigger ALL;

return @@error



GO

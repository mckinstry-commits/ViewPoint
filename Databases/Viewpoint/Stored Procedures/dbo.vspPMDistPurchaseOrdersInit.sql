...
(@PMCo bCompany = null, @Project bJob = null, @POCo bCompany = null, @PO varchar(30) = null,
 ...
 @DateSigned bDate = null, @PurchaseOrderID bigint = null, 
...
--Get EmailOption,
select @EmailOption = isnull(EmailOption,'C') from dbo.bPMPF with (nolock)
...
---- if the firm vendor does not equal the PO vendor then email option cannot be 'N'
IF ISNULL(@vendor,-1) <> ISNULL(@firm_vendor,-2) and @EmailOption = 'N'
	BEGIN
	SET @EmailOption = 'C'
	END

---- check to see if we already have a firm contact where the EmailOption is 'N'.
---- there is only one distribution row allowed with an 'None' option
IF @EmailOption = 'N'
	BEGIN
	IF EXISTS(SELECT PMCo FROM dbo.vPMDistribution WITH (NOLOCK) WHERE PMCo=@PMCo AND Project=@Project
				AND VendorGroup=@VendorGroup AND POCo=@POCo AND PO=@PO AND CC='N')
		BEGIN
		SET @EmailOption = 'C'
		END
	END
...
--Get DateSent
if isnull(@DateSent,'') = '' set @DateSent = GetDate()
...
IF NOT EXISTS(SELECT TOP 1 1 FROM dbo.vPMDistribution WITH (NOLOCK) WHERE PMCo=@PMCo 
	...
	insert into vPMDistribution(PMCo, Project, POCo, PO, Seq, VendorGroup, SentToFirm, SentToContact, 
		PrefMethod, Send, CC, DateSent, PurchaseOrderID)
	values(@PMCo, @Project, @POCo, @PO, @Seq, @VendorGroup, @SentToFirm, @SentToContact, 
		@PrefMethod, 'Y', @EmailOption, @DateSent, @PurchaseOrderID)
	if @@rowcount = 0
		begin

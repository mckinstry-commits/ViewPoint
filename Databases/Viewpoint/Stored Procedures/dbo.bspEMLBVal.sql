...
*			CHS	11/05/08 - #130640
*			GF 01/08/2013 TK-20651 added parameters for bspINLBInDateValu
*
*
...
BEGIN
	...
	if isnull(@fromloc,'')<>''
	begin
		exec @rcode = dbo.bspEMLocVal @co, @fromloc, @msg = @errmsg output
		if @rcode <> 0
		begin
			select @errortext = isnull(@errorstart,'') + ' ' + isnull(@errmsg,'')
			exec @rcode = dbo.bspHQBEInsert @co, @mth, @batchid, @errortext, @errmsg output
	...
	if isnull(@toloc ,'')<>''
	begin
		exec @rcode = dbo.bspEMLocVal @co, @toloc, @msg = @errmsg output
		if @rcode <> 0
		begin
			select @errortext = isnull(@errorstart,'') + ' ' + isnull(@errmsg,'')
			exec @rcode = dbo.bspHQBEInsert @co, @mth, @batchid, @errortext, @errmsg output

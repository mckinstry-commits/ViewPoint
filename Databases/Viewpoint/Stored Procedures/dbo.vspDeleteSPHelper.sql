...
--    a stored procedure.  If it is run with @deleteData = 'Y', all
--    references to the stored procedure will be erased from the database.
-- =============================================
CREATE PROCEDURE [dbo].[vspDeleteSPHelper]
   (@deleteData char(1) = 'N', -- pass in Y if you want the SP data to be deleted
	@SPName varchar(100),        -- name of the form to be deleted (from vDDFH.Form)
...
BEGIN
	...
	values ('Deletion report for stored procedure: ' + @SPName)
	...
	-------------------------------------------------------------------------------
	--
	--    Look for all tables which contain the names of stored procedures
	--
	-------------------------------------------------------------------------------

	-- this finds all tables with columns containing 'Proc' as part of the name
	...
	SELECT tablename=sysobjects.name, syscolumns.name
	   ...
	   and syscolumns.name like '%Proc%'
	   and syscolumns.name not like '%Process%'
	   and syscolumns.name not in ('ProcView','ErrorProcedure')
	   and syscolumns.name <> 'Form'
	   and systypes.name = 'varchar'
	   and syscolumns.length > 10
	ORDER BY sysobjects.name,syscolumns.colid

	-- remove vDDAL because it's huge and it is just a log
	DELETE FROM #spcolumns WHERE tablename = 'vDDAL'
	...
	BEGIN
		...
		SELECT @cmd = 'SELECT @rtnVal = COUNT(*) FROM ' + @TableName
						+ ' WHERE [' + @ColName + '] = ' + CHAR(39) + @SPName + CHAR(39)
		...
		IF @count > 0 BEGIN
			IF @TableName IN (SELECT tablename FROM #formtables) BEGIN
				SELECT @cmd = 'SELECT DISTINCT ''WARNING: SP found in ' + @TableName + '.' + @ColName + ' for Form = '' + Form FROM ' + @TableName + ' WHERE ' + @ColName + ' = ''' + @SPName + ''''
				INSERT INTO #report
				EXEC dbo.sp_executesql @cmd
			END
			ELSE BEGIN
				INSERT INTO #report (notes)
				VALUES ('SP found in ' + @TableName + '.' + @ColName 
			...
			IF @deleteData = 'Y' BEGIN
				BEGIN TRY
					SELECT @cmd = 'UPDATE ' + @TableName + ' SET ' + @ColName
						+ ' = NULL WHERE ' + @ColName + ' = ' + CHAR(39) + @SPName + CHAR(39)
					EXEC dbo.sp_executesql @cmd
				END TRY
				BEGIN CATCH
		...
		ELSE
			...
			IF @ShowAllTablesAndColumns = 'Y' BEGIN
				INSERT INTO #report (notes)
				VALUES ('SP found in ' + @TableName + '.' + @ColName + ' 0 times')
			END
	...
	-------------------------------------------------------------------------------
	--
	--    Look in the descriptions of other stored procedures
	--
	-------------------------------------------------------------------------------
...
REPORTIT:
	...
	if @todelete = 0
		insert into #report (notes) values ('Stored procedure was not found in the database')

	SET NOCOUNT OFF
	select * from #report
	RETURN @rc
GO
GRANT EXECUTE ON  [dbo].[vspDeleteSPHelper] TO [public]
GO

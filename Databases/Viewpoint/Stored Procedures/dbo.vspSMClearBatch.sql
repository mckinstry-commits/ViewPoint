...
CREATE PROCEDURE [dbo].[vspSMClearBatch]
    ...
    *   1               fail
    *****************************************************/
(@SMCo bCompany, @mth bMonth, @BatchId int, @Source varchar(10), @msg varchar(255)=NULL OUTPUT)
AS
SET NOCOUNT ON
...
BEGIN TRY
	...
	END
	UPDATE HQBC Set Status=6 Where Co=@SMCo AND BatchId=@BatchId AND Mth=@mth AND Source='SMLedgerUp'

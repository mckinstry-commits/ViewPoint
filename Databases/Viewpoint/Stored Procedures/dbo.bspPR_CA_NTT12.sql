...
CREATE  PROC [dbo].[bspPR_CA_NTT12]
/********************************************************
...
*				MV 11/29/11 TK-10423 2012 tax updates
*
* USAGE:
...
BEGIN
   ...
   SELECT @ProcName = 'bspPR_CA_NTT12'
	...
	IF @TCP IS NULL SELECT @TCP = 13280

	-- Establish tax rate and constant [Rate, KP] used in T4 Calculation
	IF @A BETWEEN 0 AND 38679
	BEGIN
		SELECT @Rate = .0590, @KP = 0
	END
	ELSE IF @A BETWEEN 38679.01 AND 77360
	BEGIN
		SELECT @Rate = .0860, @KP = 1044
	END
	ELSE IF @A BETWEEN 77360.01 AND 125771
	BEGIN
		SELECT @Rate = .1220, @KP = 3829
	END
	ELSE IF @A > 125771
	BEGIN
		SELECT @Rate = .1405, @KP = 6156
	END
	...
	-- The territorial labour-sponsored funds tax credit (factor LCP) for the Northwest Territories is discontinued
	-- per CA Payroll Deductions Formulas 95th Ed.
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NTT12] TO [public]
GO

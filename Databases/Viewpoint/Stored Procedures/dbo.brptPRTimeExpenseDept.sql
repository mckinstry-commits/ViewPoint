...
CREATE PROCEDURE	[dbo].[brptPRTimeExpenseDept] (
       @PRCo bCompany = 124,
       @PRGroup bGroup = 0,
...
PURPOSE:
Extract Department earnings and associated liabilities for reporting department costs within the
...
PREarnings
		(ExpMth,  Dept, DeptDescription, PRDept, EarnCode, Hours, EarnAmt, ECDesc, STE, LiabType, LiabAmt, LiabTypeDesc, RowNumber) 
AS (
	SELECT	ExpMth,  Dept, MAX(DeptDescription), PRDept, EarnCode, SUM(Hours), SUM(EarnAmt), 
			MAX(ECDesc), SUM(STE), NULL AS LiabType, NULL AS LiabAmt, NULL AS LiabTypeDesc,
			ROW_NUMBER() OVER (PARTITION BY ExpMth, Dept, PRDept ORDER BY ExpMth, Dept, PRDept, EarnCode)
	FROM	PREarningsInitial
	WHERE	ExpMth = @ExpMth AND Dept = @Dept
	GROUP BY ExpMth, Dept, PRDept, EarnCode
),
...
PRLiability
		(ExpMth, Dept, DeptDescription, PRDept, EarnCode, Hours, EarnAmt, ECDesc, STE, LiabType, LiabAmt, LiabTypeDesc, RowNumber) 
AS (
	SELECT	ExpMth,  Dept, MAX(DeptDescription), PRDept, NULL AS EarnCode, NULL AS Hours, NULL AS EarnAmt,
			NULL AS ECDesc, NULL AS STE, LiabType, SUM(LiabAmt), MAX(LiabTypeDesc),
			ROW_NUMBER() OVER (PARTITION BY ExpMth, Dept, PRDept ORDER BY ExpMth, Dept, PRDept, LiabType)
	FROM	PRLiabilityInitial
	WHERE	ExpMth = @ExpMth AND Dept = @Dept
	GROUP BY ExpMth,  Dept, PRDept, LiabType
),

-- Create CTE to join Earnings/Add-ons with Liabilities for employee AND department based ON row number
PRDepartmentCosts
		(ExpMth, Dept, DeptDescription, PRDeptEARN, PRDeptLIAB, EarnCode, Hours, EarnAmt, ECDesc, STE
...
AS (
	...
	FROM	PREarnings 
			FULL OUTER JOIN PRLiability 
				ON PRLiability.ExpMth = PREarnings.ExpMth 
...
SELECT	@PRCo AS PRCo, HQCO.Name AS CoName, PRDepartmentCosts.ExpMth,
...
FROM	PRDepartmentCosts
		LEFT OUTER JOIN	HQCO 
			ON HQCO.HQCo = @PRCo
ORDER BY ExpMth, Dept, RowNumber
...
GO
GRANT EXECUTE ON  [dbo].[brptPRTimeExpenseDept] TO [public]
GO

...
SET ANSI_NULLS ON
GO
CREATE  proc [dbo].[vspJCForecastRevPctVal]
  /***********************************************************
   * CREATED BY:	GP	08/18/2009
   * MODIFIED BY:	
   *				
   * USAGE:
   * Used in JC Forecast Month to validate Rev and Cost Percentages
   *
   * INPUT PARAMETERS
   *   JCCo   
   *   Contract
   ...
   *   1         Failure
   *****************************************************/ 
(@JCCo bCompany = 0, @Contract bContract = null, @ForecastMth bMonth = null,
...
set nocount on
	...
	if @Type = 'Revenue'
	begin
		select @PrevPct = RevenuePct from dbo.JCForecastMonth with (nolock) where JCCo=@JCCo
				and Contract=@Contract and ForecastMonth=@PrevMonth
	...
	if @Type = 'Cost'
	begin
		select @PrevPct = CostPct from dbo.JCForecastMonth with (nolock) where JCCo=@JCCo
				and Contract=@Contract and ForecastMonth=@PrevMonth
		if @PrevPct is not null and @RevPct is not null
		begin
			if @PrevPct > @RevPct
...
GO
GRANT EXECUTE ON  [dbo].[vspJCForecastRevPctVal] TO [public]
GO

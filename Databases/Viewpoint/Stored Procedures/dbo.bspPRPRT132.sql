...
*				MV 12/07/12 TK20093 update effective 1/1/2013
*				MV 12/27/12 TK20390 the REAL tax update effective 1/1/2013
*				EN 12/28/2012 TK-20390 *correction* fixed to not compute tax when annual gross wages < 20000
*
* USAGE:
...
IF @annualized_wage < 20000 -- if annual gross wages < $20,000 then tax is $0.00
BEGIN
	SELECT @amt = 0
END
ELSE
BEGIN
	...
	BEGIN   
		...
		IF @annualized_wage BETWEEN 0.00 AND 9000.00
		BEGIN
			SELECT @tax_subtraction = 0, @rate = .0
		END
		ELSE IF @annualized_wage BETWEEN 9000.01 AND 25000.00
		BEGIN
			SELECT @tax_subtraction = 630, @rate = .07
		END
		ELSE IF @annualized_wage BETWEEN 25000.01 AND 41500.00
		BEGIN
			SELECT @tax_subtraction = 2380, @rate = .14
		END
		ELSE IF @annualized_wage BETWEEN 41500.01 AND 61500.00
		BEGIN
			SELECT @tax_subtraction = 6945, @rate = .25
		END
		ELSE IF @annualized_wage >= 61500.01
		BEGIN
			SELECT @tax_subtraction = 11865, @rate = .33
		END
...
RETURN 0
GO
GRANT EXECUTE ON  [dbo].[bspPRPRT132] TO [public]
GO

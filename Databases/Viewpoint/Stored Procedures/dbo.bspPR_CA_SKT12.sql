...
CREATE  proc [dbo].[bspPR_CA_SKT12]
/********************************************************
...
(@ppds tinyint = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_SKT12'
   ...
   if @TCP is null select @TCP = 14942

   -- establish tax rate and constant
   select @rate = .15, @KP = 3245
   if @A <= 120185 select @rate = .13, @KP = 841
   if @A <= 42065 select @rate = .11, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_SKT12] TO [public]
GO

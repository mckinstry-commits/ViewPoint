...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPRFWT092]    Script Date: 12/13/2007 15:22:31 ******/
   CREATE  proc [dbo].[bspPRFWT092]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @basetax = 0, @dedn = 0, @rate = 0, @amt = 0, @procname = 'bspPRFWT092'
   ...
   if @status = 'M'
   	begin
	if @a < 15750  goto bspexit
	if @a < 24450
		begin
		select @basetax = 0, @dedn = 15750, @rate = .1
		goto bspcalc
		end
	if @a < 75650
		begin
		select @basetax = 870.00, @dedn = 24450, @rate = .15
		goto bspcalc
		end
	if @a < 118130
		begin
		select @basetax = 8550.00, @dedn = 75650, @rate = .25
		goto bspcalc
		end
	if @a < 216600
		begin
		select @basetax = 19170.50, @dedn = 118130, @rate = .28
		goto bspcalc
		end
	if @a < 380700
		begin
		select @basetax = 46741.60, @dedn = 216600, @rate = .33
		goto bspcalc
		end
	select @basetax = 100894.60, @dedn = 380700, @rate = .35
   	goto bspcalc
   	end
   ...
   if @status = 'S'
   	begin
	if @a < 7180 goto bspexit
	if @a < 10400
		begin
		select @basetax = 0, @dedn = 7180, @rate = .1
		goto bspcalc
		end
	if @a < 36200
		begin
		select @basetax = 322.00, @dedn = 10400, @rate = .15
		goto bspcalc
		end
	if @a < 66530
		begin
		select @basetax = 4192.00, @dedn = 36200, @rate = .25
		goto bspcalc
		end
	if @a < 173600
		begin
		select @basetax = 11774.50, @dedn = 66530, @rate = .28
		goto bspcalc
		end
	if @a < 375000
		begin
		select @basetax = 41754.10, @dedn = 173600, @rate = .33
		goto bspcalc
		end
	select @basetax = 108216.10, @dedn = 375000, @rate = .35
   	goto bspcalc
   	end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRFWT092] TO [public]
GO

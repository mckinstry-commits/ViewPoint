...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMOT02    Script Date: 8/28/99 9:33:28 AM ******/
   CREATE      proc [dbo].[bspPRMOT02]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @rate = 0, @counter = 1, @procname = 'bspPRMOT02', @allowances = 0
   ...
   if @status = 'S'
   begin
   	select @dedn = 4700, @fedlimit = 5000, @allowances = 1200 * @exempts
   ...
   if @status = 'M'
   begin
   	select @dedn = 7850, @fedlimit = 10000, @allowances = 1200 * @exempts
   ...
   if @status = 'B'
   begin
   	select @dedn = 3925, @fedlimit = 5000, @allowances = 1200 * @exempts
   ...
   if @status = 'H'
   begin
   	select @dedn = 6900, @fedlimit = 5000
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMOT02] TO [public]
GO

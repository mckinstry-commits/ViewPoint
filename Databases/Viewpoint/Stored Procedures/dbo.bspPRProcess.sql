...
SELECT	@rcode = 0, 
	...
	EXEC @rcode = vspPRGetEmplNoTimecardsForArrears @prco, @prgroup, @employee, @prenddate, @payseq, @errmsg output
...
next_EmployeeSeq:
	...
	IF @@fetch_status <> 0 GOTO next_EmployeeSeq
    	...
    	-- reset arrears history table
    	DELETE vPRArrears 
    	WHERE PRCo = @prco AND PRGroup = @prgroup AND PREndDate = @prenddate AND Employee = @employee AND PaySeq = @payseq --AND ArrearsAmt = 0
    
    	-- If this employee has a record in any of the following tables, 
    	--  skip the delete and go to ADDONS

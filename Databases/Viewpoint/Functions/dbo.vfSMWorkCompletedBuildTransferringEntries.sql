...
(
	...
	(
		...
		FROM WorkCompletedGLEntryCTE
			CROSS APPLY (
				SELECT 1 AS GLTransaction, @NewGLCo AS GLCo, @NewGLAccount AS GLAccount, WorkCompletedGLEntryCTE.Amount
				UNION ALL
				SELECT 2, GLCo, GLAccount, -(WorkCompletedGLEntryCTE.Amount)
				UNION ALL 
				SELECT 3, GLCo, ARGLAcct, WorkCompletedGLEntryCTE.Amount
				WHERE RequiresInterCompany = 1
				UNION ALL 
				SELECT 4, @NewGLCo, APGLAcct, -(WorkCompletedGLEntryCTE.Amount)

...
select      
	...
	, agmt.DateTerminated as SMAgmtDateTerminated   
...
Select    
	...
	, wc.WorkCompleted       
	, wc.Description as WCDesc      
	, wc.SMWorkCompletedID       
	, wc.Scope as WCScope      
	, agmt.Agreement as SMAgmtAgmt  
	, agmt.Revision   
	, agmt.DateActivated as SMAgmtDateActivated  
	...
	CASE when   
			...
			and (  
					(  
					wos.PriceMethod = 'T'   
					and wos.UseAgreementRates = 'Y'  
					)    
				or wos.PriceMethod = 'C'  
				or wc.Coverage is not null 
				)    
			and wc.NoCharge <> 'Y'    
		then wc.PriceTotal   
		else 0 

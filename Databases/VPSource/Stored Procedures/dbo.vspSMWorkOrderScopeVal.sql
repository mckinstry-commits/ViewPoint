...
BEGIN
	...
	SELECT @HasWorkCompleted = 'N', @HasProvisionalWorkCompleted='N', @HasDeletedWorkCompleted='N',@ScopeHasInvoices='N'
	...
	(
		...
		FROM dbo.vSMWorkCompletedDetail
			INNER JOIN dbo.vSMWorkCompleted ON vSMWorkCompletedDetail.SMCo = vSMWorkCompleted.SMCo AND vSMWorkCompletedDetail.WorkOrder = vSMWorkCompleted.WorkOrder AND vSMWorkCompletedDetail.WorkCompleted = vSMWorkCompleted.WorkCompleted
		...
		WHERE vSMWorkCompletedDetail.SMCo = @SMCo AND vSMWorkCompletedDetail.WorkOrder = @WorkOrder AND vSMWorkCompletedDetail.Scope = @Scope AND vSMWorkCompleted.Provisional = 0
	) 
	OR EXISTS 
	(
		SELECT 1
		FROM 
		vSMInvoiceDetail WHERE SMCo = @SMCo AND WorkOrder = @WorkOrder AND Scope = @Scope
	)
	BEGIN
		SET @ScopeHasInvoices = 'Y'
	END

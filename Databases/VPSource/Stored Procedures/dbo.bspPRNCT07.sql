...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRNCT07    Script Date: 8/28/99 9:33:29 AM ******/
  CREATE proc [dbo].[bspPRNCT07]
  /********************************************************
  ...
  **********************************************************/
  ...
  select @rcode = 0, @procname = 'bspPRNCT07'
  ...
  if @status = 'S'
  	...
  	if @annualized_wage > 120000 select @tax_subtraction = 877.5, @rate = .08
  end
  ...
  if @status = 'H'
  	...
  	if @annualized_wage > 160000 select @tax_subtraction = 1170, @rate = .08
  end
  ...
  if @status = 'M' or @status = 'W'
  	...
  	if @annualized_wage > 100000 select @tax_subtraction = 731.25, @rate = .08
  end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNCT07] TO [public]
GO

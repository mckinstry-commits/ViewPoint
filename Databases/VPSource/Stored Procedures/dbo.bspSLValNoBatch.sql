...
/****** Object:  Stored Procedure dbo.bspSLValNoBatch    Script Date: 8/28/99 9:33:42 AM ******/
   CREATE  proc [dbo].[bspSLValNoBatch]
   /***********************************************************
    * CREATED BY	: kf 5/5/97
    * MODIFIED BY	: kf 5/5/97
    *                LM 4/28/99 - when coming from pm allow viewing pending sl's.
    ...
    *****************************************************/   
       ...
       @source char(1), @VendorName char(30) output, @Vendor bVendor output,
   	@job bJob output, @jobdesc bDesc output, @jcco bCompany output, @msg varchar(60) output)
   as
...
GO
GRANT EXECUTE ON  [dbo].[bspSLValNoBatch] TO [public]
GO

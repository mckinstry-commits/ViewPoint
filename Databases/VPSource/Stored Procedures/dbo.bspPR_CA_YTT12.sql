...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_YTT12]    Script Date: 02/27/2008 13:19:16 ******/
CREATE proc [dbo].[bspPR_CA_YTT12]
/********************************************************
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_YTT12'
   ...
   if @TCP is null select @TCP = 10822

   -- establish tax rate and constant
   select @rate = .1276, @KP = 4379
   if @A <= 132406 select @rate = .1144, @KP = 2631
   if @A <= 85414 select @rate = .0968, @KP = 1127
   if @A <= 42707 select @rate = .0704, @KP = 0
   ...
   select @K4P = round(case when @TCrate*@A < @TCrate*1095 then @TCrate*@A else @TCrate*1095 end,2)
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_YTT12] TO [public]
GO

...
/****** Object:  Stored Procedure dbo.bspHQBCExitCheck    Script Date: 8/28/99 9:34:47 AM ******/
	 ...
	 *			GF 03/14/2013 TFS-43067 moved code from delete trigger to here for EMBC/EMGL cleanup
	 *			GF 07/17/2013 TFS-55924 added EM distribution tables cleanup of rest of EM sources
	 *
	 *
     * 
...
--    	EXECUTE sp_executesql @sql, @paramsin, @co, @mth, @batchid, @tablename, @c output
    	...
    	if isnull(@c,0) = 0
			...
			----TFS-55924 TFS-43667
			---- 1. EMAdj 2. EMParts 3. EMDepr 4. EMFuel 5. EMTme 6. EMAlloc 7. EMMiles 8. EMMeter 9. EMRev 10. EMXfer
			IF @source LIKE 'EM%'
			    BEGIN
				DELETE dbo.bEMBC WHERE EMCo = @co and Mth = @mth and BatchId = @batchid
				DELETE dbo.bEMGL WHERE EMCo = @co and Mth = @mth and BatchId = @batchid
				DELETE dbo.bEMJC WHERE EMCo = @co AND Mth = @mth AND BatchId = @batchid
				DELETE dbo.bEMIN WHERE EMCo = @co AND Mth = @mth AND BatchId = @batchid 
				END

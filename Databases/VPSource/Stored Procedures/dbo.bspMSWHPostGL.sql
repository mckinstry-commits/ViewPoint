...
GO
/**********************************************************/
   CREATE procedure [dbo].[bspMSWHPostGL]
   /***********************************************************
    * Created: GG 02/15/01
    * Modified: GF 07/29/2003 - issue #21933 speed improvements
    *
    * Called from the bspMSWHPost procedure to post GL distributions
    * tracked in bMSWG for Hauler Worksheet Invoice batches.
    *
    *
   ...
   declare @rcode int, @apco bCompany, @expjrnl bJrnl, @glexpinterfacelvl tinyint, @glexpsummarydesc varchar(60),
       @glexptransdesc varchar(60), @glexpdetaildesc varchar(60), @glref bGLRef, @openMSWGcursor tinyint,
   ...
   select @rcode = 0, @openMSWGcursor = 0
   ...
   if @glexpinterfacelvl = 1
       ...
       declare bcMSWG cursor LOCAL FAST_FORWARD
       ...
       -- open cursor
       open bcMSWG
       select @openMSWGcursor = 1
   
       gl_summary_posting_loop:
           fetch next from bcMSWG into @glco, @glacct, @amount
   ...
   declare bcMSWG cursor LOCAL FAST_FORWARD 
   for select GLCo, GLAcct, VendorGroup, HaulVendor, APRef, InvDescription, InvDate,
       APTrans,(convert(numeric(12,2), sum(Amount)))
   from bMSWG
   where MSCo = @co and Mth = @mth and BatchId = @batchid
   group by GLCo, GLAcct, VendorGroup, HaulVendor, APRef, InvDescription, InvDate, APTrans
   ...
   gl_transaction_posting_loop:
       fetch next from bcMSWG into @glco, @glacct, @vendorgroup, @vendor, @apref, @invdesc, @invdate, @aptrans, @amount
...
GO
GRANT EXECUTE ON  [dbo].[bspMSWHPostGL] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_FWT09]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_FWT09]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_FWT09'
   ...
   select @rate = .29, @K = 9613
   if @A <= 126264 select @rate = .26, @K = 5825
   if @A <= 77664 select @rate = .22, @K = 2718
   if @A <= 38832 select @rate = .15, @K = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_FWT09] TO [public]
GO

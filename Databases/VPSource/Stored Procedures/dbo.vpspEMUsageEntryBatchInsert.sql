...
-- Description:	Inserts a new EM Usage Entry batch
-- =============================================
CREATE PROCEDURE [dbo].[vpspEMUsageEntryBatchInsert]
	@Key_EMCo AS bCompany, @Key_Mth AS bMonth, @LockedYN AS BIT, @VPUserName AS bVPUserName
AS
BEGIN
	...
	EXEC @batchId = bspHQBCInsert @Key_EMCo, @Key_Mth, 'EMRev', 'EMBF', 'N', 'N', NULL, NULL, @errMsg OUTPUT
	...
	BEGIN
		...
		WHERE Co = @Key_EMCo AND Mth = @Key_Mth AND BatchId = @batchId

		EXEC vpspEMUsageEntryBatchGet @Key_EMCo, @VPUserName, @batchId
	END
	ELSE
	BEGIN
...
GO
GRANT EXECUTE ON  [dbo].[vpspEMUsageEntryBatchInsert] TO [VCSPortal]
GO

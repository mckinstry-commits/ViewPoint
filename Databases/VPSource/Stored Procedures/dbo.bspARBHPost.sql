...
*       ECV 05/14/13 - TFS-50103 Update GLInterfaceLevel when marking SMDetailTransaction as posted.
*		JVH 5/28/13 - TFS-44858	Modified to support SM Flat Price Billing
*		JVH 6/10/13 - TFS-50983 Modified to allow deleting SM adjustment agreement scheduled records
*
* USAGE:
...
declare @rcode int, @opencursor tinyint, @status tinyint, @errorstart varchar(50), 
	...
	@GLInvLev tinyint, @GLInvSummaryDesc varchar(60)
...
SELECT @GLInvLev = GLInvLev, @GLInvSummaryDesc = GLInvSummaryDesc from bARCO with (nolock) where ARCo = @co
	...
	BEGIN TRY
		...
		BEGIN
			...
			FROM dbo.vSMInvoiceARBH
				INNER JOIN dbo.vSMInvoice ON vSMInvoiceARBH.SMCo = vSMInvoice.SMCo AND vSMInvoiceARBH.Invoice = vSMInvoice.Invoice
			WHERE vSMInvoiceARBH.Co = @co AND vSMInvoiceARBH.Mth = @mth AND vSMInvoiceARBH.BatchId = @batchid AND vSMInvoiceARBH.BatchSeq = @seq
		END
	
		--When voiding an SM Invoice the behavoir is to revert the bill back to the state it was in
		--before being voided. The changes to the work completed should not be reverted.
		IF EXISTS
		(
			SELECT 1
			FROM dbo.vSMInvoiceARBH
			WHERE Co = @co AND Mth = @mth AND BatchId = @batchid AND BatchSeq = @seq AND VoidInvoice = 1
		)
		BEGIN
			--Void the invoice so that it can be refrenced by the agreement billing schedule triggers
			UPDATE dbo.vSMInvoice
			SET VoidDate = dbo.vfDateOnly(), VoidedBy = SUSER_NAME()
			FROM dbo.vSMInvoiceARBH
				INNER JOIN dbo.vSMInvoice ON vSMInvoiceARBH.SMCo = vSMInvoice.SMCo AND vSMInvoiceARBH.Invoice = vSMInvoice.Invoice
			WHERE vSMInvoiceARBH.Co = @co AND vSMInvoiceARBH.Mth = @mth AND vSMInvoiceARBH.BatchId = @batchid AND vSMInvoiceARBH.BatchSeq = @seq
			...
			FROM dbo.vSMInvoiceARBH
				INNER JOIN dbo.vSMInvoice ON vSMInvoiceARBH.SMCo = vSMInvoice.SMCo AND vSMInvoiceARBH.Invoice = vSMInvoice.Invoice
				INNER JOIN dbo.vSMAgreementBillingSchedule ON vSMInvoice.SMInvoiceID = vSMAgreementBillingSchedule.SMInvoiceID
			WHERE vSMInvoiceARBH.Co = @co AND vSMInvoiceARBH.Mth = @mth AND vSMInvoiceARBH.BatchId = @batchid AND vSMInvoiceARBH.BatchSeq = @seq AND vSMAgreementBillingSchedule.BillingType = 'A'
			...
			FROM dbo.vSMInvoiceARBH
				INNER JOIN dbo.vSMInvoiceLine ON vSMInvoiceARBH.SMCo = vSMInvoiceLine.SMCo AND vSMInvoiceARBH.Invoice = vSMInvoiceLine.Invoice
			...
			SET IsRemoved = 0, WorkCompleted = NULL, Scope = NULL
			FROM dbo.vSMInvoiceARBH
				INNER JOIN dbo.vSMInvoiceDetail ON vSMInvoiceARBH.SMCo = vSMInvoiceDetail.SMCo AND vSMInvoiceARBH.Invoice = vSMInvoiceDetail.Invoice
			WHERE vSMInvoiceARBH.Co = @co AND vSMInvoiceARBH.Mth = @mth AND vSMInvoiceARBH.BatchId = @batchid AND vSMInvoiceARBH.BatchSeq = @seq
		END
		ELSE
		BEGIN
			--The normal posting behavior for invoice lines is to delete the removed lines and backup the current invoice line
			DELETE InvoicedLine
			FROM dbo.vSMInvoiceARBH
				INNER JOIN dbo.bARBL ON vSMInvoiceARBH.Co = bARBL.Co AND vSMInvoiceARBH.Mth = bARBL.Mth AND vSMInvoiceARBH.BatchId = bARBL.BatchId AND vSMInvoiceARBH.BatchSeq = bARBL.BatchSeq
				INNER JOIN dbo.vSMInvoiceLine ON vSMInvoiceARBH.SMCo = vSMInvoiceLine.SMCo AND vSMInvoiceARBH.Invoice = vSMInvoiceLine.Invoice
				INNER JOIN dbo.vSMInvoiceLine InvoicedLine ON vSMInvoiceLine.SMCo = InvoicedLine.SMCo AND vSMInvoiceLine.InvoiceLine = InvoicedLine.InvoiceLine AND InvoicedLine.Invoiced = 1
			WHERE vSMInvoiceARBH.Co = @co AND vSMInvoiceARBH.Mth = @mth AND vSMInvoiceARBH.BatchId = @batchid AND vSMInvoiceARBH.BatchSeq = @seq AND
				(
					(vSMInvoiceARBH.IsReversing = 0 AND bARBL.ARLine = vSMInvoiceLine.BatchLine) OR
					(vSMInvoiceARBH.IsReversing = 1 AND bARBL.ARLine = InvoicedLine.BatchLine)
				)

			DELETE vSMInvoiceLine
			FROM dbo.vSMInvoiceARBH
				INNER JOIN dbo.vSMInvoiceDetail ON vSMInvoiceARBH.SMCo = vSMInvoiceDetail.SMCo AND vSMInvoiceARBH.Invoice = vSMInvoiceDetail.Invoice
			...
			WHERE vSMInvoiceARBH.Co = @co AND vSMInvoiceARBH.Mth = @mth AND vSMInvoiceARBH.BatchId = @batchid AND vSMInvoiceARBH.BatchSeq = @seq AND vSMInvoiceARBH.IsReversing = 1 AND vSMInvoiceDetail.IsRemoved = 1
			...
			INSERT dbo.vSMInvoiceLine (SMCo, InvoiceLine, Invoiced, NoCharge, [Description], GLCo, GLAccount, Amount, TaxGroup, TaxCode, TaxBasis, TaxAmount, DiscountOffered, TaxDiscount, LastPostedARCo, LastPostedARMth, LastPostedARTrans, LastPostedARLine)
			SELECT vSMInvoiceLine.SMCo, vSMInvoiceLine.InvoiceLine, 1 Invoiced, vSMInvoiceLine.NoCharge, vSMInvoiceLine.[Description], vSMInvoiceLine.GLCo, vSMInvoiceLine.GLAccount, vSMInvoiceLine.Amount, 
				vSMInvoiceLine.TaxGroup, vSMInvoiceLine.TaxCode, vSMInvoiceLine.TaxBasis, vSMInvoiceLine.TaxAmount, vSMInvoiceLine.DiscountOffered, vSMInvoiceLine.TaxDiscount, 
				vSMInvoiceLine.LastPostedARCo, vSMInvoiceLine.LastPostedARMth, vSMInvoiceLine.LastPostedARTrans, vSMInvoiceLine.LastPostedARLine
			FROM dbo.vSMInvoiceARBH
				INNER JOIN dbo.bARBL ON vSMInvoiceARBH.Co = bARBL.Co AND vSMInvoiceARBH.Mth = bARBL.Mth AND vSMInvoiceARBH.BatchId = bARBL.BatchId AND vSMInvoiceARBH.BatchSeq = bARBL.BatchSeq
				INNER JOIN dbo.vSMInvoiceLine ON vSMInvoiceARBH.SMCo = vSMInvoiceLine.SMCo AND vSMInvoiceARBH.Invoice = vSMInvoiceLine.Invoice AND bARBL.ARLine = vSMInvoiceLine.BatchLine
			WHERE vSMInvoiceARBH.Co = @co AND vSMInvoiceARBH.Mth = @mth AND vSMInvoiceARBH.BatchId = @batchid AND vSMInvoiceARBH.BatchSeq = @seq AND vSMInvoiceARBH.IsReversing = 0

			UPDATE vSMInvoiceLine
			SET LastPostedARCo = bARBL.Co, LastPostedARMth = bARBL.Mth, LastPostedARTrans = bARBL.ARTrans, LastPostedARLine  = bARBL.ARLine
			FROM dbo.vSMInvoiceARBH
				INNER JOIN dbo.bARBL ON vSMInvoiceARBH.Co = bARBL.Co AND vSMInvoiceARBH.Mth = bARBL.Mth AND vSMInvoiceARBH.BatchId = bARBL.BatchId AND vSMInvoiceARBH.BatchSeq = bARBL.BatchSeq
				INNER JOIN dbo.vSMInvoiceLine ON vSMInvoiceARBH.SMCo = vSMInvoiceLine.SMCo AND vSMInvoiceARBH.Invoice = vSMInvoiceLine.Invoice AND bARBL.ARLine = vSMInvoiceLine.BatchLine
			WHERE vSMInvoiceARBH.Co = @co AND vSMInvoiceARBH.Mth = @mth AND vSMInvoiceARBH.BatchId = @batchid AND vSMInvoiceARBH.BatchSeq = @seq AND vSMInvoiceARBH.IsReversing = 0
		...
		SET Posted = 1, HQBatchDistributionID = NULL, GLInterfaceLevel = @GLInvLev, [Description] = CASE WHEN @GLInvLev = 1 THEN @GLInvSummaryDesc ELSE REPLACE([Description], 'Trans #', dbo.vfToString(@artrans)) END

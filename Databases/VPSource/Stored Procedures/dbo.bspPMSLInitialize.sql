...
*				GP 08/13/2013 TFS-58551 Do not create a SubCo if creating a new subcontract
*
*
...
---- if new subcontract do not create subco
IF @rectype = 'A'
BEGIN
	--check for existing sl, set subco null if new
	IF EXISTS(SELECT 1 FROM dbo.bPMSL WHERE PMCo = @pmco AND Project = @project AND SL = @formattedsl)
	BEGIN
		--check if sl exists on another aco, set subco null if not
		IF NOT EXISTS(SELECT 1 FROM dbo.bPMSL WHERE PMCo = @pmco AND Project = @project AND SL = @formattedsl AND ISNULL(ACO,'') <> ISNULL(@co,''))
		BEGIN
			SET @subco = NULL
		END
	END

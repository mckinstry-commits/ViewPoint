...
CREATE PROCEDURE [dbo].[vpspPMMaterialsNeededUpdate]
/************************************************************
* CREATED:		02/06/08  CHS
* Modifed BY:	GF 09/18/2011 TK-08528 CHECK FOR MISSING GROUPS
*				GF 11/16/2011 TK-10027
*
*
* USAGE:
*   Updates the PM Materials Needed
*	
* CALLED FROM:
*	ViewpointCS Portal  
*
...
(
	...
	@Original_StringUnitCost varchar(30),
	@Original_ECM bECM,
	@Original_Amount bDollar,
	@Original_StringAmount Varchar(30),
	...
	@Original_RequisitionNum varchar(20),
	@Original_MSCo bCompany,
	@Original_Quote varchar(10),
...
select  @HQMatlGroup = hp.MatlGroup,
		@HQVendorGroup = hp.VendorGroup,
		@HQTaxGroup = hp.TaxGroup
...
IF @VendorGroup IS NULL
	BEGIN
	SET @VendorGroup = @HQVendorGroup
	END
IF @MaterialGroup IS NULL
	BEGIN
	SET @MaterialGroup = @HQMatlGroup
	END
IF @TaxGroup IS NULL
	BEGIN
	SET @TaxGroup = @HQTaxGroup
	END
...
-- re-calculate if any of the four values changes
--if (@Amount <> @Original_Amount) or (@UM <> @Original_UM) or (@Units <> @Original_Units) or (@UnitCost <> @Original_UnitCost)
if (@StringAmount <> @Original_StringAmount) or (@UM <> @Original_UM) or (@Units <> @Original_Units) or (@StringUnitCost <> @Original_StringUnitCost)
begin
	if @UM = 'LS'
		begin
			select @Units = 0, @StringUnitCost = '0'
		end

	else
		begin	
			select @StringAmount = @Units * cast(cast(@StringUnitCost as FLOAT) as numeric(16,5))
		end
end
...
UPDATE dbo.PMMF SET 
		...
		UM = @UM,
		Units = @Units,
		UnitCost = cast(cast(@StringUnitCost as money) as numeric(16,5)),
		ECM = @ECM,
		Amount = cast(cast(@StringAmount as money) as numeric(12,2)),
		ReqDate = @ReqDate,
...
bspmessage:
	...
	return @rcode
GO
GRANT EXECUTE ON  [dbo].[vpspPMMaterialsNeededUpdate] TO [VCSPortal]
GO

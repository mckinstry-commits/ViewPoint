...
from 
	(select JCCo   = @toco
		...
		, Item     =  SPACE(16 - datalength(rtrim(ltrim( 
				row_number() over 
				(Partition  by xj.VPJob --c.COMPANYNUMBER, c.DIVISIONNUMBER, c.JOBNUMBER, c.SUBJOBNUMBER
				order by c.CONTRACTNO) -- c.COMPANYNUMBER, c.DIVISIONNUMBER, /*c.CUSTOMERNUMBER,*/ c.JOBNUMBER, c.SUBJOBNUMBER, c.CONTRACTNO)
	...
	JOIN bJCCM JCCM on JCCM.JCCo=@toco 
		and JCCM.Contract  =  xj.VPJob --dbo.bfMuliPartFormat(rtrim(j.JOBNUMBER), @JobFormat)
	...
	GROUP BY xj.VPJob, c.CONTRACTNO -- dbo.bfMuliPartFormat(rtrim(j.JOBNUMBER), @JobFormat),

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMAT06    Script Date: 8/28/99 9:33:26 AM ******/
  CREATE  proc [dbo].[bspPRMAT06]
  /********************************************************
  ...
  **********************************************************/
  ...
  select @rcode = 0, @rate = .053, @procname = 'bspPRMAT06', @FICArate = .0765
  ...
  if @total_exempts = 1
  	select @annualized_wage = @annualized_wage - 3850
 
  if @total_exempts >1
  	select @annualized_wage = @annualized_wage - (@total_exempts * 1000) - 2850
 
  -- subtract extra deduction if head of household
  if @status = 'H' select @annualized_wage = @annualized_wage - 2100
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMAT06] TO [public]
GO

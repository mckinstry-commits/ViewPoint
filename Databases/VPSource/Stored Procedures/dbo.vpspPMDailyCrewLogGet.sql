...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[vpspPMDailyCrewLogGet]
/************************************************************
* CREATED:		6/01/06	chs
* MODIFIED		6/6/07	chs
* MODIFIED:		6/7/07	CHS
...
d.PRCo, 
d.Crew, c.Description as 'Crew Description',
...
FROM PMDD d with (nolock)
	...
	Left Join PRCR c with (nolock) on d.PRCo=c.PRCo and d.Crew=c.Crew

Where d.PRCo=(select PMCO.PRCo from PMCO where PMCO.PMCo = @JCCo) 
and d.Project=@Job and d.LogDate=@LogDate and d.LogType=@logtype 
and d.DailyLog = @DailyLog
and d.KeyID = IsNull(@KeyID, d.KeyID)

GO
GRANT EXECUTE ON  [dbo].[vpspPMDailyCrewLogGet] TO [VCSPortal]
GO

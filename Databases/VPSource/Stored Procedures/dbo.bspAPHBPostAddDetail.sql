...
/****** Object:  Stored Procedure dbo.bspAPHBPostAddDetail    Script Date: 8/28/99 9:33:59 AM ******/
   ...
   *			 MV/KK 09/02/13 - TFS-60544 - modified the value retgTaxAmt gets wrt retainage only being processed
   *
   * USAGE:
   ...
   *****************************************************/
   ...
   SELECT	@rcode = 0,		@seq = 0,		@taxrate=0,		@pstrate=0,				@gstrate=0,			@retgGstTaxAmt=0,	@payTaxAmt=0,
			@retgTaxAmt=0,	@gstTaxAmt = 0, @pstTaxAmt=0,	@expenseGstYN = 'N',	@retgPSTTaxAmt = 0,	@RetgOnlyYN = 'N'
	...
	--Determine if invoice is retainage only TFS-60544
	IF ((@retainage <> 0) AND ((@grossamt = 0) OR (@grossamt = @retainage)))
		BEGIN
			SELECT @RetgOnlyYN = 'Y' 
		END
    ...
    BEGIN
		...
		-- GST is expensed so calculate GST/retgGST for APTD.GSTtaxAmt
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN -- GST tax basis is not net of retainage - calculate on full gross
						IF @RetgOnlyYN = 'Y'
						BEGIN
							SELECT @retgTaxAmt = @taxamt --TFS 60544
						END
			...
			BEGIN
				...
				BEGIN
					...
					IF @RetgOnlyYN = 'Y'
					BEGIN
						SELECT @retgGstTaxAmt = (@payTaxAmt * @gstrate)/@taxrate 	
						SELECT @retgPSTTaxAmt = @payTaxAmt - @retgGstTaxAmt
					END
			...
			-- retainage only line - zero out tax amounts going to Open seq
			IF @RetgOnlyYN = 'Y' --TFS-60544
			BEGIN
				SELECT @gstTaxAmt = 0, @pstTaxAmt=0, @taxamt = 0 
			END
		END
   ...
   IF @RetgOnlyYN = 'Y' --TFS-60544
   BEGIN
		SELECT @amt = 0 -- if line is retainage only then amount going to open should be 0
   END

...
if not exists (select name from CV_CMS_SOURCE.dbo.syscolumns 
  where id=OBJECT_ID('CV_CMS_SOURCE.dbo.POTMCT')and name='udPOTMCTID')
alter table CV_CMS_SOURCE.dbo.POTMCT add udPOTMCTID int identity(1,1) 

if not exists (select name from CV_CMS_SOURCE.dbo.syscolumns 
  where id=OBJECT_ID('CV_CMS_SOURCE.dbo.POTMDT')and name='udPOTMDTID')
alter table CV_CMS_SOURCE.dbo.POTMDT add udPOTMDTID int identity(1,1) 

if not exists (select name from CV_CMS_SOURCE.dbo.syscolumns 
  where id=OBJECT_ID('CV_CMS_SOURCE.dbo.POTCDT')and name='udPOTCDTID')
...
if not exists (select name from dbo.syscolumns 
  where id=OBJECT_ID('vPOItemLine')and name='udConv')
alter table vPOItemLine add udConv  bYN null

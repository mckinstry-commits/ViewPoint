...
CREATE  proc [dbo].[bspPR_CA_SKT11]
/********************************************************
...
(@ppds tinyint = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_SKT11'
   ...
   if @TCP is null select @TCP = 13535

   -- establish tax rate and constant
   select @rate = .15, @KP = 3157
   if @A <= 116911 select @rate = .13, @KP = 818
   if @A <= 40919 select @rate = .11, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_SKT11] TO [public]
GO

...
GO
  ...
  **********************************************************/
  ...
  SELECT @rcode = 0, @allowance = 1000, @procname = 'bspPRNYC10'
  ...
  BEGIN
  	...
  	IF @annualized_wage > 60000 SELECT @tax_addition = 2101, @wage_bracket = 60000, @rate = .04
   	IF @annualized_wage > 500000 SELECT @tax_addition = 19701, @wage_bracket = 500000, @rate = .0475
  END
  IF @status = 'M'
  BEGIN
  	IF @annualized_wage <= 8000 SELECT @tax_addition = 0, @wage_bracket = 0, @rate = .019
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNYC10] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRPRT00    Script Date: 8/28/99 9:33:32 AM ******/
    CREATE  proc [dbo].[bspPRPRT00]
    /********************************************************
    * CREATED BY: 	bc 6/5/98
    * MODIFIED BY:	bc 7/27/99
    * MODIFIED BY:  EN 1/7/00 - tax table update effective 1/1/2000
    ...
    **********************************************************/
    ...
    select @rcode = 0, @procname = 'bspPRPRT00'
    ...
    if @misc_factor = 1
    begin
    	if @annualized_wage <= 1000 select @rate = .075
    	if @annualized_wage > 1000 select @tax_subtraction = 35, @rate = .11
    	if @annualized_wage > 8500 select @tax_subtraction = 502.5, @rate = .165
    	if @annualized_wage > 15000 select @tax_subtraction = 2452.5, @rate = .295
    	if @annualized_wage > 25000 select @tax_subtraction = 3327.5, @rate = .33
    end
   
    /* select calculation elements for everybody else */
    if @misc_factor <> 1 or @misc_factor is null
    begin
    	if @annualized_wage <= 2000 select @rate = .075
    	if @annualized_wage > 2000 select @tax_subtraction = 70, @rate = .11
    	if @annualized_wage > 17000 select @tax_subtraction = 1005, @rate = .165
    	if @annualized_wage > 30000 select @tax_subtraction = 4905, @rate = .295
    	if @annualized_wage > 50000 select @tax_subtraction = 6655, @rate = .33
    end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRPRT00] TO [public]
GO

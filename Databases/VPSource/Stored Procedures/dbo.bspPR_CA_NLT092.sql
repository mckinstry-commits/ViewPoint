...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NLT092]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_NLT092]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NLT092'
   ...
   if @TCP is null select @TCP = 7778

   -- establish tax rate and constant
   select @rate = .155, @KP = 3261
   if @A <= 62121 select @rate = .128, @KP = 1584
   if @A <= 31061 select @rate = .077, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NLT092] TO [public]
GO

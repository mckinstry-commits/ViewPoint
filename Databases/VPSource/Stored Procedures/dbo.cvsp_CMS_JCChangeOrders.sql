...
select c.JCCo
	...
	, Contract=Job
	, Item= isnull(min(i.Item), MIN(m.MinItem))
...
from JCChangeOrders c 
	...
	left join (select JCCo, Contract, MIN(Item) as MinItem from bJCCI group by JCCo, Contract) m
		on m.JCCo = c.JCCo and m.Contract = c.Job
where c.JCCo=@toco	

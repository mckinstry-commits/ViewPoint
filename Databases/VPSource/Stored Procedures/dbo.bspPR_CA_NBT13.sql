...
CREATE  proc [dbo].[bspPR_CA_NBT13]
/********************************************************
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NBT13'
   ...
   IF		@A BETWEEN 0 AND 38954			SELECT @rate = .091, @KP = 0
   ELSE IF	@A BETWEEN 38954.01 AND 77908	SELECT @rate = .121, @KP = 1169
   ELSE IF  @A BETWEEN 77908.01 AND 126662	SELECT @rate = .124, @KP = 1402
   ELSE IF	@A >= 126662.01					SELECT @rate = .143, @KP = 3809 
...
GRANT EXECUTE ON  [dbo].[bspPR_CA_NBT13] TO [public]
GO

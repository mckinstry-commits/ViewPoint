...
/****** Object:  Stored Procedure dbo.bspAPHBPost    Script Date: 8/28/99 9:36:40 AM ******/
	 ...
	 *				EricV 05/14/13 - TFS-50104 Added the GL Interface Level to the call to the procedure vspAPHBPostSM.
	 *				
     *
     * USAGE:
     *
     ...
     * bspAPHBPostEM, bspAPHBPostIN, and vspAPHBPostSM to update other modules.
     *
     * INPUT PARAMETERS:
    ...
    declare @rcode int, @opencursor tinyint, @openAPLBcursor tinyint, @retholdcode bHoldCode,
		...
		@ocMembership# VARCHAR(60),@OnCostStatus tinyint,@GLExpInterfaceLvl tinyint
		----TK-19327
		,@SLKeyID BIGINT
...
--                   	+ case @oldtaxtype when 1 then @oldtaxamt else 0 end
	...
	SELECT @GLExpInterfaceLvl = GLExpInterfaceLvl FROM bAPCO WITH (NOLOCK) WHERE APCo=@co
	EXEC @rcode = dbo.vspAPHBPostSM @co, @mth, @batchid, @dateposted, @GLExpInterfaceLvl, @errmsg OUTPUT
	IF @rcode <> 0 GOTO bspexit

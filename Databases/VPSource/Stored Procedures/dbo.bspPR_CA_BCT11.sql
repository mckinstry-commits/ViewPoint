...
/****** Object:  StoredProcedure [dbo].[bspPR_CA_BCT11]    Script Date: 02/27/2008 13:19:16 ******/
CREATE  proc [dbo].[bspPR_CA_BCT11]
/********************************************************
...
BEGIN
	...
	SELECT @ProcName = 'bspPR_CA_BCT11'
	...
	IF @TCP IS NULL SELECT @TCP = 11088

	-- establish tax rate and constant
	IF @A BETWEEN 0 AND 36146
	BEGIN
		SELECT @Rate = .0506, @KP = 0
	END
	ELSE IF @A BETWEEN 36146.01 AND 72293
	BEGIN
		SELECT @Rate = .0770, @KP = 954
	END
	ELSE IF @A BETWEEN 72293.01 AND 83001
	BEGIN
		SELECT @Rate = .1050, @KP = 2978
	END
	ELSE IF @A BETWEEN 83001.01 AND 100787
	BEGIN
		SELECT @Rate = .1229, @KP = 4464
	END
	ELSE IF @A > 100787
	BEGIN
		SELECT @Rate = .1470, @KP = 6893
	END
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_BCT11] TO [public]
GO

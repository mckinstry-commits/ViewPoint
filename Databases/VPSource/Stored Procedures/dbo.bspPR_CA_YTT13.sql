...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_YTT13]    Script Date: 02/27/2008 13:19:16 ******/
CREATE proc [dbo].[bspPR_CA_YTT13]
/********************************************************
...
*				CHS	12/11/2012 B-11810 #147557 tax update effective 01/01/13
*
* USAGE:
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_YTT13'
   ...
   if @TCP is null select @TCP = 11038

   -- establish tax rate and constant
                   select @rate = .1276, @KP = 4466
   if @A <= 135054 select @rate = .1144, @KP = 2683
   if @A <=  87123 select @rate = .0968, @KP = 1150
   if @A <=  43561 select @rate = .0704, @KP = 0
   ...
   select @K4P = round(case when @TCrate*@A < @TCrate*1117 then @TCrate*@A else @TCrate*1117 end,2)
...
GRANT EXECUTE ON  [dbo].[bspPR_CA_YTT13] TO [public]
GO

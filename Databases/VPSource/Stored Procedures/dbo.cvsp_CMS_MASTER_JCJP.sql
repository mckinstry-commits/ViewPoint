...
select JCCo        = @toco
	...
	, Contract     = xj.VPJob --min(dbo.bfMuliPartFormat(ltrim(rtrim(j.JOBNUMBER)),@JobFormat))
	, Item         = @defaultItem
	, ActiveYN     = 'Y'
...
group by xj.VPJob --dbo.bfMuliPartFormat(ltrim(rtrim(j.JOBNUMBER)) + ltrim(rtrim(j.SUBJOBNUMBER)),@JobFormat)
	--,j.JCDISTRIBTUION
	,x.newPhase;

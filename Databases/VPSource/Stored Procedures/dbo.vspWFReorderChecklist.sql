...
* Create date:  1/10/2008
* Description:	This procedure extracts, re-orders, and re-inserts tasks/steps in a checklist
*
*	Inputs:
*	@Company		Company checklist is located in
*	@Checklist		Checklist to do reordering on
...
*
*****************************************************/
CREATE PROCEDURE [dbo].[vspWFReorderChecklist]
...
BEGIN
	...
	BEGIN TRY
		...
		FROM WFChecklistSteps s, @NewList n, @OldList o
		WHERE s.Task = o.Names AND n.KeyID = o.KeyID AND s.Company = @Company AND s.Checklist = @Checklist

		UPDATE WFChecklistTasks SET Task = CAST(n.Names as int)
		FROM WFChecklistTasks t, @NewList n, @OldList o
		WHERE t.Task = o.Names AND n.KeyID = o.KeyID AND t.Company = @Company AND t.Checklist = @Checklist
...
BEGIN
	BEGIN TRY
		...
		FROM WFChecklistSteps s, @NewList n, @OldList o
		WHERE s.Step = o.Names AND n.KeyID = o.KeyID AND s.Company = @Company AND s.Checklist = @Checklist AND s.Task = @Task
...
GO
GRANT EXECUTE ON  [dbo].[vspWFReorderChecklist] TO [public]
GO

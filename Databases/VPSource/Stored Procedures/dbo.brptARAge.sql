...
/****** Object:  Stored Procedure dbo.brptARAge    Script Date: 8/28/99 9:32:27 AM ******/    
      ...
      CREATE proc [dbo].[brptARAge] (@Company bCompany=1, @Month bMonth='1/1/2049', @AgeDate bDate='1/1/2049',    
   ...
   set nocount on    
      create table #CN1    
       (CustGroup tinyint null,    
        ...
        NonInvCN int null)    
      Insert into #CN1    
       (CustGroup,Customer,NonInvCN)    
      Select ARCM.CustGroup,ARCM.Customer,Count(ARCN.Customer)    
      From ARCM ARCM with(nolock)    
      Inner Join ARCN with(nolock) On ARCM.CustGroup = ARCN.CustGroup and ARCM.Customer = ARCN.Customer    
      Where ARCN.Invoice is Null or ARCN.Invoice = ''    
      Group By ARCM.CustGroup,ARCM.Customer    
      ...
      create table #CN2    
       (CustGroup tinyint null,    
        ...
        InvCN int null)    
      Insert into #CN2    
       (CustGroup,Customer,Invoice,InvCN)    
      Select ARCN.CustGroup,ARCN.Customer,ARCN.Invoice,Count(ARCN.Customer)    
    ...
    Where ARCN.Invoice Is Not Null   --and ARTH.Contract Is Not Null       
    Group By ARCN.CustGroup,ARCN.Customer,ARCN.Invoice    
...
-- end V1-B-10810    
    ...
    then SMARTH.DescriptionOfWork    
      ...
      ARTH.AppliedTrans, InvoiceARTransType=ARTH1.ARTransType,    
    ...
    then SMARTH1.DescriptionOfWork    
              ...
              ARCM.Name,ARCM.SortName, ARCM.Phone, ARCM.Contact, ARCM.StmntPrint, ARCM.ContactExt,    
        ...
        LastCheckDate = (select Max(a.TransDate) from ARTH a with(nolock) where a.ARTransType = 'P' and a.Customer = ARTH.Customer and     
      ...
      CheckDescription=ARTL.Description,    
       ...
       #CN2.InvCN/*,    
          ...
          FROM    
              ...
              JOIN ARCO with (NOLOCK) ON ARTL.ARCo=ARCO.ARCo --Added DML 09/10/2013
              JOIN #Open OpenC    
     ...
     join #RT RT on RT.ARCo = ARTL.ARCo and RT.RecType = ARTL.RecType    
       Left Join #CN1 On ARTH.CustGroup = #CN1.CustGroup and ARTH.Customer = #CN1.Customer    
       Left Join #CN2 On ARTH.CustGroup = #CN2.CustGroup and ARTH.Customer = #CN2.Customer and ARTH.Invoice = #CN2.Invoice    
        --125713    
...
GRANT EXECUTE ON  [dbo].[brptARAge] TO [public]
GO

...
BEGIN
	IF @taxincome BETWEEN   4100.01 AND  39000 SELECT @basetax =     0.00, @limit =   4100, @rate = @rate1
  	IF @taxincome BETWEEN  39000.01 AND  81000 SELECT @basetax =   526.99, @limit =  39000, @rate = @rate2
  	IF @taxincome BETWEEN  81000.01 AND 185000 SELECT @basetax =  1711.39, @limit =  81000, @rate = @rate3
  	IF @taxincome BETWEEN 185000.01 AND 400000 SELECT @basetax =  4966.59, @limit = 185000, @rate = @rate4
  	IF @taxincome >						400000 SELECT @basetax = 12771.09, @limit = 400000, @rate = @rate5
END
IF @status = 'M' --Married
BEGIN
	IF @taxincome BETWEEN  10000.01 AND  69000 SELECT @basetax =     0.00, @limit =  10000, @rate = @rate1
  	IF @taxincome BETWEEN  69000.01 AND 130000 SELECT @basetax =   890.90, @limit =  69000, @rate = @rate2
  	IF @taxincome BETWEEN 130000.01 AND 231000 SELECT @basetax =  2611.10, @limit = 130000, @rate = @rate3
  	IF @taxincome BETWEEN 231000.01 AND 405000 SELECT @basetax =  5772.40, @limit = 231000, @rate = @rate4
  	IF @taxincome >						405000 SELECT @basetax = 12088.60, @limit = 405000, @rate = @rate5
END
...
GRANT EXECUTE ON  [dbo].[bspPRNDT13] TO [public]
GO

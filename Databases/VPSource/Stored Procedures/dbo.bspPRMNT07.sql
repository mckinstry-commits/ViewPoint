...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMNT07    Script Date: 8/28/99 9:33:28 AM ******/
    CREATE proc [dbo].[bspPRMNT07]
    /********************************************************
    ...
    **********************************************************/
    ...
    select @rcode = 0, @dedn = 3400, @procname = 'bspPRMNT07'
    ...
    if @status = 'S'
    	begin
    		...
    		if @annualized_wage > 23260 select @tax_addition = 1140.09, @wage_bracket = 23260, @rate = .0705
    		if @annualized_wage > 71940 select @tax_addition = 4572.03, @wage_bracket = 71940, @rate = .0785
    end
    
    /* married wage table and tax */
    if @status = 'M'
    	begin
    		if @annualized_wage <= 7300 goto bspexit
    		if @annualized_wage > 7300 select @wage_bracket = 7300, @rate = .0535
    		if @annualized_wage > 38450 select @tax_addition = 1666.53, @wage_bracket = 38450, @rate = .0705
    		if @annualized_wage > 131050 select @tax_addition = 8194.83, @wage_bracket = 131050, @rate = .0785
    end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMNT07] TO [public]
GO

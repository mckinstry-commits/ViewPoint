...
*			JVH 7/11/13 - TFS-38864 Fixed an issue with sm job timecards with 0 hours
*
*
...
declare @lastjcco bCompany, @lastjob bJob, @postclosedjobs bYN, @premphase bPhase, @premcosttype bJCCType,
 ...
 DECLARE @PRSMJCDistributions TABLE (PRSMJCDistributionID int IDENTITY(1,1) NOT NULL,
	...
	ActualDate bDate NOT NULL, PostedUM bUM NULL, ActualHours bHrs DEFAULT(0) NOT NULL, ActualCost bDollar NOT NULL, UM bUM NULL,
 ...
 next_EmplSeq:
	...
	AS
	(
		SELECT *, SUM(ActualCost) OVER (PARTITION BY JCCostEntryID) ActualCostTotal
		FROM @PRSMJCDistributions
	)
	UPDATE MarkupCTE
	SET ActualCost = CASE WHEN MarkupCTE.ActualCostTotal = 0 THEN 0 ELSE MarkupCTE.ActualCost / MarkupCTE.ActualCostTotal * SMWorkCompletedAllCurrent.PriceTotal END
	FROM MarkupCTE
		INNER JOIN dbo.vSMWorkCompletedJCCostEntry ON MarkupCTE.JCCostEntryID =  vSMWorkCompletedJCCostEntry.JCCostEntryID

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMET07   Script Date: 8/28/99 9:33:27 AM ******/
  CREATE  proc [dbo].[bspPRMET07]
  /********************************************************
  ...
  **********************************************************/
  ...
  select @rcode = 0, @dedn = 2850, @procname = 'bspPRMET07'
  ...
  if @status = 'S' 
  	begin
  		...
  		if @annualized_wage > 7250 select @tax_addition = 95, @wage_bracket = 7250, @rate = .045
  		if @annualized_wage > 11950 select @tax_addition = 307, @wage_bracket = 11950, @rate = .07
  		if @annualized_wage > 21450 select @tax_addition = 972, @wage_bracket = 21450, @rate = .085
  end
  
  /* married wage table and tax */
  if @status <> 'S'
  	begin
  		if @annualized_wage <= 6050 goto bspcalc
  		if @annualized_wage > 6050 select @wage_bracket = 6050, @rate = .02
  		if @annualized_wage > 15550 select @tax_addition = 190, @wage_bracket = 15550, @rate = .045
  		if @annualized_wage > 25000 select @tax_addition = 615, @wage_bracket = 25000, @rate = .07
  		if @annualized_wage > 44000 select @tax_addition = 1945, @wage_bracket = 44000, @rate = .085
  end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMET07] TO [public]
GO

...
CREATE  procedure [dbo].[bspPRProcessFed]
   ...
   *				DAN SO 09/03/2013	Bug:59956/Task:59957 - Extend bug fix for Medicare Surcharge
   *
   * USAGE:
    	...
    	if @method = 'R'
   	  ...
   	  -- issue 24545
			...
			BEGIN
				...
				-- Bug:59956/Task:59957 --  
   				EXEC @rcode = bspPRProcessGetAccums @prco, @prgroup, @prenddate, @employee, @payseq,
   													@dlcode, @dltype, 'A', @limitmth, 'Y', 
				...
				-- Bug:59956/Task:59957 -- 
				-- RETURN MEDICARE SURCHARGE AMOUNTS --
				EXEC @rcode = @procname @prco, @dlcode,	@calcbasis, 
										@accumsubj, @accumelig, @accumamt, 
										@calcamt OUTPUT, @eligamt OUTPUT, @errmsg OUTPUT

   				-- please put no code between exec and @@error check!
   				IF @@error <> 0 SET @rcode = 1
    			IF @rcode <> 0 GOTO bspexit
   
				IF @calcamt IS NULL SET @calcamt = 0.00
				SET @accumbasis = @calcbasis

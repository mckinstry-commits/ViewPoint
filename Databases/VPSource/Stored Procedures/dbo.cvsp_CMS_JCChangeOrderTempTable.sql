...
select JCCo = @toco
	...
	,CONTRACTNO    = xj.VPJob -- dbo.bfMuliPartFormat(RTRIM(d.JOBNUMBER) +  RTRIM(d.SUBJOBNUMBER),@Job)
	,Item          = case 
...
from CV_CMS_SOURCE.dbo.JCTCGH h 
	...
	left join (select JCCo, Contract, Item=min(Item)
				...
				as i on i.JCCo=@toco 
					and i.Contract = xj.VPJob --dbo.bfMuliPartFormat(RTRIM(d.JOBNUMBER) +  RTRIM(d.SUBJOBNUMBER),@Job)=i.Contract

...
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NTT13]    Script Date: 02/27/2008 13:19:16 ******/
CREATE  PROC [dbo].[bspPR_CA_NTT13]
/********************************************************
...
*				CHS 12/11/2012 TK-20160 B-11803 147553 tax update effective 1/1/2013
*
* USAGE:
...
BEGIN
   ...
   SELECT @ProcName = 'bspPR_CA_NTT13'
	...
	IF @TCP IS NULL SELECT @TCP = 13546

	-- Establish tax rate and constant [Rate, KP] used in T4 Calculation
	               SELECT @Rate = .1405, @KP = 6279
   IF @A <= 128286 SELECT @Rate = .1220, @KP = 3906
   IF @A <=  78908 SELECT @Rate = .0860, @KP = 1065
   IF @A <=  39453 SELECT @Rate = .0590, @KP = 0
...
GRANT EXECUTE ON  [dbo].[bspPR_CA_NTT13] TO [public]
GO

...
CREATE PROCEDURE[dbo].[vspHQCreateIndex]  
	...
	*			AMR			4/16/2013 - 44992			Adding Schema in for the Batch tables.
	*     
	* Usage:  
	...
	************************************************************/  
	...
	-- build out the schema and table really quick
	SELECT @hqaschematable = '[' + s.name +'].[' + o.name + ']'
	FROM sys.objects o
		JOIN sys.schemas s ON s.schema_id = o.schema_id 
	WHERE o.object_id = OBJECT_ID(@hqattable)
	...
	IF @currentState <> 'A'
     ...
     SELECT @selectstring='SELECT @returnID = UniqueAttchID FROM ' +  @hqaschematable + ' WITH ( NOLOCK ) WHERE UniqueAttchID=@uniqueID'  

...
09/25/2012	CUC		145638		Changed default value for @EndChkSort to '~~~~~~~~~~'
								...
								file.
08/09/2013	CUC		148369		Tuned procedure for performance, changes throughout, run
					TFS-58296	diff for specifics; most beneficial change was addition of
								GROUP BY clauses in earnings cursor definitions, possibly
								also WITH RECOMPILE statement; result set output unchanged,
								tested against 35,452 PRDT rows (in one pay period) in Kinsley
								database; elapsed execution time reduced from 5089 ms to 
								3837 ms (better than 6.5.0 version (pre-B-10717) at 4666 ms).
...
DECLARE bcEarn CURSOR LOCAL FAST_FORWARD FOR
	SELECT PRTH.Employee, PRTH.PaySeq, PRTH.EarnCode, SUM(PRTH.Amt), SUM(PRTH.[Hours]), PRTH.Rate, PREC.[Description]
...
OPEN bcEarn
FETCH NEXT FROM bcEarn INTO @Employee, @PaySeq, @EDLCode, @Amount, @Hours, @Rate, @Desc
...
WHILE @@fetch_status = 0
	BEGIN
		...
		ELSE
			BEGIN
				INSERT INTO dbo.#a (PRCo, PRGroup, PREndDate, PREmployee, EarnCode, TotEarnAmt, TotEarnHours, EarnDescription, AddOnYN)
				SELECT @PRCo, @PRGroup, @PREndDate, 999999999, @EDLCode, @Amount, @Hours, @Desc, 'N'
			END
...
DECLARE bcPRTA CURSOR LOCAL FAST_FORWARD FOR
	SELECT PRTA.Employee, PRTA.PaySeq, PRTA.EarnCode, SUM(PRTA.Amt), PRTA.Rate, PREC.[Description]
...
OPEN bcPRTA
FETCH NEXT FROM bcPRTA INTO @Employee, @PaySeq, @EDLCode, @Amount, @Rate, @Desc
...
WHILE @@fetch_status = 0
	BEGIN
		...
		ELSE
			BEGIN
				INSERT INTO dbo.#a (PRCo, PRGroup, PREndDate, PREmployee, EarnCode, TotEarnAmt, EarnDescription, AddOnYN)
				SELECT @PRCo, @PRGroup, @PREndDate, 999999999, @EDLCode, @Amount, @Desc, 'Y'
			END
...
DECLARE bcLiab CURSOR LOCAL FAST_FORWARD FOR
	SELECT PRDT.Employee, PRDT.PaySeq, PRDT.EDLCode, LiabilityAmount = (CASE WHEN PRDT.UseOver = 'Y' THEN PRDT.OverAmt ELSE PRDT.Amount END), PRDL.[Description]
	FROM dbo.PRDT PRDT WITH(NOLOCK)
	JOIN dbo.PREH PREH WITH(NOLOCK) ON PREH.PRCo = PRDT.PRCo AND PREH.Employee = PRDT.Employee
	JOIN dbo.PRSQ PRSQ WITH(NOLOCK) ON PRSQ.PRCo = PRDT.PRCo AND PRSQ.PRGroup = PRDT.PRGroup AND PRSQ.PREndDate = PRDT.PREndDate AND PRSQ.Employee = PRDT.Employee AND PRSQ.PaySeq = PRDT.PaySeq
...
OPEN bcLiab
FETCH NEXT FROM bcLiab INTO @Employee, @PaySeq, @EDLCode, @Amount, @Desc
...
WHILE @@fetch_status = 0
	BEGIN
		...
		ELSE
			BEGIN
				INSERT INTO dbo.#a (PRCo, PRGroup, PREndDate, PREmployee, LiabCode, TotLiabAmt, LiabDescription)
				SELECT @PRCo, @PRGroup, @PREndDate, 999999999, @EDLCode, @Amount, @Desc
			END

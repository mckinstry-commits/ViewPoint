...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[bspMSQDMatlUMVal]
   /*************************************
   ...
   *
   *
   * validates Material UM to HQMT.Material from MSQD
   *
   * Pass:
   ...
   **************************************/
   (@msco bCompany = null, @fromloc bLoc = null, @matlgroup bGroup = null,
    @material bMatl = null, @matlum bUM, @pricetemplate smallint = null, @saletype char(1) = null,
   ...
   declare @rcode int, @validcnt int, @category varchar(10), @custpriceopt tinyint,
           ...
           @retcode int, @MatlVendor bVendor, @VendorGroup bGroup
   ...
   -- validate Material UM
   select @msg=Description from bHQUM where UM=@matlum
   if @@rowcount = 0
       begin
...
GO
GRANT EXECUTE ON  [dbo].[bspMSQDMatlUMVal] TO [public]
GO

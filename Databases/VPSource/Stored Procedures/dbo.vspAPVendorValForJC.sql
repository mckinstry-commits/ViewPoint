...
/****** Object:  Stored Procedure dbo.bspAPVendorVal    Script Date: 8/28/99 9:34:06 AM ******/
    CREATE         proc [dbo].[vspAPVendorValForJC]
    (@apco bCompany, @vendgroup bGroup = null, @vendor varchar(15) = null, @activeopt char(1) = null,
        ...
        @address varchar(60) = null output, @city varchar(30) = null output,@state bState = null output,
     ...
     *				 DANF 05/16/06 - Allow an invalid vendor number to be passed back to the UI.
     * Usage:
   ...
   --  if isnumeric(@vendor) = 1 
    	select @vendorout = Vendor, @payterms=PayTerms, @msg = Name, @active = ActiveYN, @type = Type,
            ...
            @zip=Zip, @taxid=TaxId, @taxcode=TaxCode
   ...
   --  if @@rowcount = 0
       	...
       	if @@rowcount = 0
    		...
    		if @@rowcount = 0
     	  		begin
					...
					if dbo.bfIsInteger(@vendor) = 1 select @vendorout = @vendor
    			goto bspexit
    	   		end
    		end
    	end
    ...
    if exists(select * from bAPVH with (nolock) where APCo=@apco and VendorGroup=@vendgroup and Vendor=@vendorout)
    	begin
    	select @holdyn='Y'
    	end
    else
...
GO
GRANT EXECUTE ON  [dbo].[vspAPVendorValForJC] TO [public]
GO

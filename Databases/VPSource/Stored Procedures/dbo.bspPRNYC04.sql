...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRNYC04    Script Date: 8/28/99 9:33:30 AM ******/
   CREATE  proc [dbo].[bspPRNYC04]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @allowance = 1000, @procname = 'bspPRNYC04'
   ...
   if @status = 'S'
   	...
   	if @annualized_wage > 100000 select @tax_addition = 3701, @wage_bracket = 100000, @rate = .0453
   	if @annualized_wage > 150000 select @tax_addition = 5966, @wage_bracket = 150000, @rate = .0582
   	if @annualized_wage > 200000 select @tax_addition = 8876, @wage_bracket = 200000, @rate = .0453
   	if @annualized_wage > 500000 select @tax_addition = 22466, @wage_bracket = 500000, @rate = .0480
   	end
   if @status = 'M'
   	begin
   	if @annualized_wage <= 8000 select @tax_addition = 0, @wage_bracket = 0, @rate = .019
   	...
   	if @annualized_wage > 150000 select @tax_addition = 5701, @wage_bracket = 150000, @rate = .0653
   	if @annualized_wage > 200000 select @tax_addition = 8966, @wage_bracket = 200000, @rate = .0453
   	if @annualized_wage > 500000 select @tax_addition = 22556, @wage_bracket = 500000, @rate = .0480
   	end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNYC04] TO [public]
GO

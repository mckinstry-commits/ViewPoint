...
select POCo=@toco
	, PO=i.PONUMBER
	, POItem=i.POITEMT
	, ItemType=max(case when i.JOBNUMBER<>'' then 1
	...
	, Job=max(case when i.JOBNUMBER='' then null 
				else xj.VPJob --dbo.bfMuliPartFormat(RTRIM(i.JOBNUMBER) + '.' + RTRIM(i.SUBJOBNUMBER),@Job) 
			end)
...
join CV_CMS_SOURCE.dbo.cvspMcKvwCGCPurchaseOrdersForConversion PO 
	ON PO.COMPANYNUMBER   = i.COMPANYNUMBER
	and PO.DIVISIONNUMBER = i.DIVISIONNUMBER
	and PO.PONUMBER       = i.PONUMBER

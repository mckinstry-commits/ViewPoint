...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPREIC10]    Script Date: 12/13/2007 15:47:30 ******/
  CREATE  proc [dbo].[bspPREIC10]
  /********************************************************
  ...
  *				EN 12/02/04 - issue 26376  update effective 1/1/2005
  *				EN 12/08/05 - issue 119610  update effective 1/1/2006
  *				EN 12/14/06 - issue 123321  update effective 1/1/2007
  *				EN 12/13/07 - issue 126501  update effective 1/1/2008
  *				EN 12/12/08 - #131435  update effective 1/1/2009
  *				EN 2/23/09 - #132384  update effective immediately as part of federal stimulus plan
  *				EN 11/30/2010 #136828 update effective 1/1/2010
  *
  * USAGE:
  ...
  **********************************************************/
  ...
  select @rcode = 0, @rate1 = .204, @rate2 = .09588, @procname = 'bspPREIC10'
  ...
  if @EICStatus = 'S'
      select @bracket1 = 8970, @bracket2 = 16450, @maxpayamt = 1830
  if @EICStatus = 'M'
      select @bracket1 = 8970, @bracket2 = 21460, @maxpayamt = 1830
  if @EICStatus = 'B'
      select @bracket1 = 4485, @bracket2 = 10730, @maxpayamt = 915
...
GO
GRANT EXECUTE ON  [dbo].[bspPREIC10] TO [public]
GO

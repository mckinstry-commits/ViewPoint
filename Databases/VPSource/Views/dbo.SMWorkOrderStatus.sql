...
SELECT dbo.SMWorkOrder.SMCo, dbo.SMWorkOrder.WorkOrder, 
    CASE 
		WHEN SMWorkOrder.WOStatus = 1 THEN 'Closed' 
		WHEN SMWorkOrder.WOStatus = 2 THEN 'Canceled' 
		WHEN WT.TripCount IS NULL AND WC.WorkCompletedCount IS NULL THEN 'New' 
		WHEN (WT.MinStatus IS NULL OR WT.MinStatus = 7 /* Complete */ ) AND (WS.MinStatus IS NULL OR WS.MinStatus = 'Y') THEN 'Complete' 
		ELSE 'Open' 
	END AS Status
FROM dbo.SMWorkOrder 
	LEFT OUTER JOIN
		...
		FROM          dbo.SMTrip
		GROUP BY SMCo, WorkOrder) AS WT ON dbo.SMWorkOrder.SMCo = WT.SMCo AND dbo.SMWorkOrder.WorkOrder = WT.WorkOrder 
	LEFT OUTER JOIN
		(SELECT     SMCo, WorkOrder, MIN(IsComplete) AS MinStatus
		FROM          dbo.SMWorkOrderScope
		GROUP BY SMCo, WorkOrder) AS WS ON WS.SMCo = dbo.SMWorkOrder.SMCo AND WS.WorkOrder = dbo.SMWorkOrder.WorkOrder 
	LEFT OUTER JOIN

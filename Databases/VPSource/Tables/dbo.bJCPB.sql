...
ALTER TABLE [dbo].[bJCPB] ADD 
CONSTRAINT [PK_bJCPB] PRIMARY KEY CLUSTERED  ([Co], [Mth], [BatchId], [BatchSeq]) WITH (FILLFACTOR=90) ON [PRIMARY]
CREATE UNIQUE NONCLUSTERED INDEX [biJCPB] ON [dbo].[bJCPB] ([Co], [Mth], [BatchId], [Job], [PhaseGroup], [Phase], [CostType]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bJCPB] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bJCPB] ADD
CONSTRAINT [CK_bJCPB_Plugged] CHECK (([Plugged]='Y' OR [Plugged]='N'))
GO
...
EXEC sp_bindrule N'[dbo].[brYesNo]', N'[dbo].[bJCPB].[Plugged]'
GO

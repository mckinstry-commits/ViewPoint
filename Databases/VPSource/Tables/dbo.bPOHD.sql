...
[DocType] [dbo].[bDocType] NOT NULL CONSTRAINT [DF_bPOHD_DocType] DEFAULT ('PO')
) ON [PRIMARY] TEXTIMAGE_ON [PRIMARY]
CREATE UNIQUE CLUSTERED INDEX [biPOHD] ON [dbo].[bPOHD] ([POCo], [PO]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPOHD] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [IX_bPOHD_POCO_PO] ON [dbo].[bPOHD] ([PO], [POCo]) INCLUDE ([Status], [Vendor], [VendorGroup]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biPOHDUniqueAttId] ON [dbo].[bPOHD] ([UniqueAttchID]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biPOHDVendor] ON [dbo].[bPOHD] ([Vendor]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPOHD] ADD
CONSTRAINT [CK_bPOHD_Purge] CHECK (([Purge]='Y' OR [Purge]='N'))
ALTER TABLE [dbo].[bPOHD] ADD
CONSTRAINT [CK_bPOHD_Approved] CHECK (([Approved]='Y' OR [Approved]='N' OR [Approved] IS NULL))
ALTER TABLE [dbo].[bPOHD] ADD
CONSTRAINT [FK_bPOHD_bPMDT] FOREIGN KEY ([DocType]) REFERENCES [dbo].[bPMDT] ([DocType])
GO

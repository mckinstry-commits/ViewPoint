...
CREATE UNIQUE CLUSTERED INDEX [biAPTD] ON [dbo].[bAPTD] ([APCo], [Mth], [APTrans], [APLine], [APSeq]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biAPTDCMRef] ON [dbo].[bAPTD] ([CMRef]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bAPTD] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bAPTD] ADD
CONSTRAINT [CK_bAPTD_CMAcct] CHECK (([CMAcct]>(0) AND [CMAcct]<(10000) OR [CMAcct] IS NULL))
GO

...
CREATE UNIQUE CLUSTERED INDEX [biJCCM] ON [dbo].[bJCCM] ([JCCo], [Contract]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bJCCM] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bJCCM] ADD
CONSTRAINT [CK_bJCCM_CompleteYN] CHECK (([CompleteYN]='Y' OR [CompleteYN]='N'))
ALTER TABLE [dbo].[bJCCM] ADD
CONSTRAINT [CK_bJCCM_ReportRetgItemYN] CHECK (([ReportRetgItemYN]='Y' OR [ReportRetgItemYN]='N'))
ALTER TABLE [dbo].[bJCCM] ADD
CONSTRAINT [CK_bJCCM_BillOnCompletionYN] CHECK (([BillOnCompletionYN]='Y' OR [BillOnCompletionYN]='N'))
ALTER TABLE [dbo].[bJCCM] ADD
CONSTRAINT [CK_bJCCM_ClosePurgeFlag] CHECK (([ClosePurgeFlag]='Y' OR [ClosePurgeFlag]='N' OR [ClosePurgeFlag] IS NULL))
ALTER TABLE [dbo].[bJCCM] ADD
CONSTRAINT [CK_bJCCM_DefaultBillType] CHECK (([DefaultBillType]='T' OR [DefaultBillType]='P' OR [DefaultBillType]='B' OR [DefaultBillType]='N'))
ALTER TABLE [dbo].[bJCCM] ADD
CONSTRAINT [CK_bJCCM_SIMetric] CHECK (([SIMetric]='Y' OR [SIMetric]='N' OR [SIMetric] IS NULL))
ALTER TABLE [dbo].[bJCCM] ADD
CONSTRAINT [CK_bJCCM_TaxInterface] CHECK (([TaxInterface]='Y' OR [TaxInterface]='N'))
GO

...
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD
CONSTRAINT [CK_bEMEM_FuelType] CHECK (([FuelType]='D' OR [FuelType]='G' OR [FuelType]='N' OR [FuelType]='O'))
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD
CONSTRAINT [CK_bEMEM_OwnershipStatus] CHECK (([OwnershipStatus]='O' OR [OwnershipStatus]='R' OR [OwnershipStatus]='C' OR [OwnershipStatus]='L' OR [OwnershipStatus] IS NULL))
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD
CONSTRAINT [FK_bEMEM_bEMCM_Category] FOREIGN KEY ([EMCo], [Category]) REFERENCES [dbo].[bEMCM] ([EMCo], [Category])
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD
CONSTRAINT [FK_bEMEM_bEMDM_Department] FOREIGN KEY ([EMCo], [Department]) REFERENCES [dbo].[bEMDM] ([EMCo], [Department])
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_Type] CHECK (([Type]='E' OR [Type]='C'))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_Status] CHECK (([Status]='A' OR [Status]='I' OR [Status]='D'))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_AttachPostRevenue] CHECK (([AttachPostRevenue]='N' OR [AttachPostRevenue]='Y' OR [AttachPostRevenue] IS NULL))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_Capitalized] CHECK (([Capitalized]='N' OR [Capitalized]='Y' OR [Capitalized] IS NULL))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_ChangeInProgress] CHECK (([ChangeInProgress]='N' OR [ChangeInProgress]='Y' OR [ChangeInProgress] IS NULL))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_CompUpdateFuel] CHECK (([CompUpdateFuel]='N' OR [CompUpdateFuel]='Y' OR [CompUpdateFuel] IS NULL))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_CompUpdateHrs] CHECK (([CompUpdateHrs]='N' OR [CompUpdateHrs]='Y' OR [CompUpdateHrs] IS NULL))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_CompUpdateMiles] CHECK (([CompUpdateMiles]='N' OR [CompUpdateMiles]='Y' OR [CompUpdateMiles] IS NULL))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_PostCostToComp] CHECK (([PostCostToComp]='N' OR [PostCostToComp]='Y' OR [PostCostToComp] IS NULL))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [CK_bEMEM_UpdateYN] CHECK (([UpdateYN]='N' OR [UpdateYN]='Y'))
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [FK_bEMEM_bEMCO_EMCo] FOREIGN KEY ([EMCo]) REFERENCES [dbo].[bEMCO] ([EMCo])
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [FK_bEMEM_bHQGP_EMGroup] FOREIGN KEY ([EMGroup]) REFERENCES [dbo].[bHQGP] ([Grp])
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [FK_bEMEM_bEMTY_ComponentTypeCode] FOREIGN KEY ([EMGroup], [ComponentTypeCode]) REFERENCES [dbo].[bEMTY] ([EMGroup], [ComponentTypeCode])
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [FK_bEMEM_bEMRC_RevenueCode] FOREIGN KEY ([EMGroup], [RevenueCode]) REFERENCES [dbo].[bEMRC] ([EMGroup], [RevCode])
ALTER TABLE [dbo].[bEMEM] ADD
CONSTRAINT [FK_bEMEM_bHQGP_ShopGroup] FOREIGN KEY ([ShopGroup]) REFERENCES [dbo].[bHQGP] ([Grp])
GO
	...
	BEGIN
		...
		INSERT INTO dbo.budDriverHistory
			...
			(SELECT 
				...
				i.PRCo, 
				i.Operator, 
				(SELECT COUNT(d.Equipment) AS RecCount FROM dbo.udDriverHistory d INNER JOIN INSERTED i ON i.EMCo = d.Co AND i.Equipment = d.Equipment)
				FROM  INSERTED i)
		

		--UPDATE Employee table
		DECLARE @EMCOUpdate AS bCompany
		DECLARE @PRCOUpdate AS bCompany
		DECLARE @Employee AS bEmployee
		DECLARE @Equipment AS bEquip

		SET @EMCOUpdate = (SELECT EMCo FROM INSERTED i)
		SET @PRCOUpdate = (SELECT PRCo FROM INSERTED i)
		SET @Employee = (SELECT Operator FROM INSERTED i)
		SET @Equipment = (SELECT Equipment FROM INSERTED i)


		UPDATE dbo.bPREH
		SET EMCo = NULL, Equipment = NULL
		--RESET TO PRCO and Operator field on EMEM
		WHERE EMCo = @EMCOUpdate AND Equipment = @Equipment


		--Don't need this.
		UPDATE dbo.bPREH
		SET EMCo = @EMCOUpdate, Equipment = @Equipment
		WHERE PRCo = @PRCOUpdate AND Employee = @Employee
		--Don't need this.	
	END	

	IF UPDATE(HourReading) OR UPDATE (FuelUsed) OR UPDATE(OdoReading)
		BEGIN

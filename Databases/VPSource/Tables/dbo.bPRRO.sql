...
[Phase1Value] [dbo].[bHrs] NULL,
[Phase2Value] [dbo].[bHrs] NULL,
[Phase3Value] [dbo].[bHrs] NULL,
[Phase4Value] [dbo].[bHrs] NULL,
[Phase5Value] [dbo].[bHrs] NULL,
[Phase6Value] [dbo].[bHrs] NULL,
[Phase7Value] [dbo].[bHrs] NULL,
[Phase8Value] [dbo].[bHrs] NULL,
...
GO
   ...
   CREATE    trigger [dbo].[btPRROi] on [dbo].[bPRRO] for INSERT as
   ...
   return
   error:
   	select @errmsg = isnull(@errmsg,'') + ' - cannot insert PR Crew Timesheet Other Job Earnings!'
...
GO
   ...
   CREATE    trigger [dbo].[btPRROu] on [dbo].[bPRRO] for UPDATE as
   ...
   return
   error:
   	select @errmsg = isnull(@errmsg,'') + ' - cannot update PR Crew Timesheet Employee Other Job Earnings!'
   	RAISERROR(@errmsg, 11, -1);
   	rollback transaction
...
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPRRO] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPRRO] ON [dbo].[bPRRO] ([PRCo], [Crew], [PostDate], [SheetNum], [Employee], [LineSeq]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

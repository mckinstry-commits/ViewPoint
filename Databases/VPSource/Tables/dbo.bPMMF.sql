...
ALTER TABLE [dbo].[bPMMF] WITH NOCHECK ADD
CONSTRAINT [FK_bPMMF_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job])
ALTER TABLE [dbo].[bPMMF] ADD
CONSTRAINT [CK_bPMMF_ECM] CHECK (([ECM]='E' OR [ECM]='C' OR [ECM]='M' OR [ECM] IS NULL))
ALTER TABLE [dbo].[bPMMF] ADD
CONSTRAINT [CK_bPMMF_RecvYN] CHECK (([RecvYN]='Y' OR [RecvYN]='N'))
ALTER TABLE [dbo].[bPMMF] ADD
CONSTRAINT [CK_bPMMF_SendFlag] CHECK (([SendFlag]='Y' OR [SendFlag]='N'))
GO
...
CREATE TRIGGER [dbo].[vctVU8085_PONumberUpdatebPMMF] ON [dbo].[bPMMF] FOR INSERT, UPDATE
...
/**************************************************************************
      ...
      Modified By:            
      Usage:                  PMMF trigger used to get the next purchase order number formatted 
							  to today's Julian Date + Sequence Number.
            Pass:
                  Project           PM Project
		...
		Trigger fires on addition of any Material Cost Type on a Phase.  Needs to be fixed.  -ES 10/8/13
**************************************************************************/
DECLARE 
	@PMCo           bCompany, 
    @Project		bProject,
...
SELECT @PMCo = PMCo, @Project = Project, @Seq = Seq, @Vendor = Vendor from inserted

    
DECLARE
	@POPart1	VARCHAR(30),
	@MaxPMMFPO	VARCHAR(30),
	@MaxPOHDPO	VARCHAR(30),
	@LastPO		VARCHAR(30),
	@NextPO		VARCHAR(30),
...
/**Get The first part of the PO**/
select @POPart1 = dbo.JulianDate(GETDATE())


/**Get Last PO for Project, Vendor**/
SELECT @LastPO = a.PO
	FROM bPMMF a
		LEFT JOIN bPMMF b on a.PMCo =b.PMCo 
	...
	WHERE 
		...
		AND SUBSTRING(a.PO,1,5) = @POPart1
...
IF ISNULL(@LastPO,'') <> '' 
/**Populate PO from Previous**/
	BEGIN
		UPDATE b SET b.PO = a.PO
		FROM bPMMF a
			LEFT JOIN bPMMF b on a.PMCo =b.PMCo 
				AND a.Vendor=b.Vendor  AND b.PO IS NULL
		WHERE 
			a.PMCo					= @PMCo
			AND a.Project			= @Project
			AND a.Vendor			= @Vendor
...
/**Get Next PO**/
IF EXISTS
            (SELECT PO FROM POHD WITH (NOLOCK) WHERE SUBSTRING(PO,1,5) =@POPart1)
      OR EXISTS
            (SELECT * FROM dbo.POUnique WHERE SUBSTRING(PO,1,5) = @POPart1)

            BEGIN 
				SELECT @NextPO = LTRIM(RTRIM(MAX(PO)+1)) 
				FROM bPOHD 
				WHERE SUBSTRING(PO,1,5) = @POPart1
            END
            
      ELSE
            SELECT @NextPO = CONVERT(VARCHAR(30),dbo.JulianDate(GETDATE()))
                             + RIGHT('000'+ CONVERT(VARCHAR,(ROW_NUMBER() OVER (PARTITION BY @NextPO ORDER BY @NextPO))),3)

/**Update PMMF with New PO Number**/
UPDATE bPMMF SET PO = @NextPO
	WHERE 
		PMCo		=@PMCo
		AND Project = @Project
		AND Seq		= @Seq
...
WHERE 
	...
	IF @MaxPMMFItem IS NULL
		SELECT @MaxPMMFItem = 0
...
WHERE 
	...
	IF @MaxPOITItem IS NULL
		SELECT @MaxPOITItem = 0
		
/**set @NextPOItem to larger of two plus one**/
IF @MaxPMMFItem > @MaxPOITItem
	SELECT @NextPOItem = @MaxPMMFItem + 1
ELSE 
	SELECT @NextPOItem = @MaxPOITItem + 1

UPDATE bPMMF SET POItem=@NextPOItem
WHERE 
	POCo		= @PMCo 
	AND	Project	= @Project
	AND PO		= @PO
...
GO
EXEC sp_bindrule N'[dbo].[brYesNo]', N'[dbo].[bPMMF].[RecvYN]'
GO
EXEC sp_bindefault N'[dbo].[bdZero]', N'[dbo].[bPMMF].[UnitCost]'
GO
EXEC sp_bindrule N'[dbo].[brECM]', N'[dbo].[bPMMF].[ECM]'
GO
EXEC sp_bindrule N'[dbo].[brYesNo]', N'[dbo].[bPMMF].[SendFlag]'
GO

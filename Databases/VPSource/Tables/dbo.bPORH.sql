...
ALTER TABLE [dbo].[bPORH] ADD
CONSTRAINT [CK_bPORH_OldReceiptUpdate] CHECK (([OldReceiptUpdate]='Y' OR [OldReceiptUpdate]='N'))
ALTER TABLE [dbo].[bPORH] ADD
CONSTRAINT [CK_bPORH_ReceiptUpdate] CHECK (([ReceiptUpdate]='Y' OR [ReceiptUpdate]='N'))
GO
CREATE UNIQUE CLUSTERED INDEX [biPORH] ON [dbo].[bPORH] ([Co], [Mth], [BatchId]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
EXEC sp_bindrule N'[dbo].[brYesNo]', N'[dbo].[bPORH].[ReceiptUpdate]'
GO
EXEC sp_bindrule N'[dbo].[brYesNo]', N'[dbo].[bPORH].[OldReceiptUpdate]'
GO

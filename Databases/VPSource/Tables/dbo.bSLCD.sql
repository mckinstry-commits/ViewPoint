...
[ChangeCurUnitCost] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bSLCD_ChangeCurUnitCost] DEFAULT ((0)),
...
CREATE UNIQUE CLUSTERED INDEX [biSLCD] ON [dbo].[bSLCD] ([SLCo], [Mth], [SLTrans]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bSLCD] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biSLCDACO] ON [dbo].[bSLCD] ([SLCo], [SL], [AppChangeOrder], [Mth], [SLTrans]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bSLCD] ADD
CONSTRAINT [CK_bSLCD_PurgeYN] CHECK (([PurgeYN]='Y' OR [PurgeYN]='N'))
GO

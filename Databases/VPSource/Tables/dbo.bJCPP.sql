...
CREATE UNIQUE NONCLUSTERED INDEX [biJCPP] ON [dbo].[bJCPP] ([Co], [Mth], [BatchId], [Job], [PhaseGroup], [Phase], [CostType], [ActualDate], [PRCo], [Crew]) WITH (FILLFACTOR=90) ON [PRIMARY]
...
*				AW 03/21/2013 - 140173 / TFS 40565 Performance issue with updating ud fields 
*								modified to pass in params to assist SQL with cacheing query plan
*
*----------------------------------------------------------------*/
declare @batchid bBatchID, @batchseq int, @errmsg varchar(2500), @co bCompany,
		...
		@udcolumn2 nvarchar(max),@paramsin NVARCHAR(255)
...
if @udflag = 'Y'
	begin
	-- performance boost passing in params to sp_executesql as it caches the query plan prior to param substitution
	-- see proc bspBatchUserMemoUpdate as another example
	-- update view rather than table to prevent permission issues
	SET @paramsin = N'@batchseq int, @co bCompany, @mth bMonth, @batchid bBatchID, @job bJob, 
		@phasegroup bGroup,@phase bPhase,@linkcosttype bJCCType,@prco bCompany,@crew varchar(10)'

	SET @sql = 
'update p set ' + isnull(@udvalues,'') + '
from JCPP p 
	join JCPP u with (nolock)
	on p.Co = u.Co and p.Mth = u.Mth and p.BatchId = u.BatchId and u.BatchSeq = dbo.vfToString(@batchseq)
where p.Co=dbo.vfToString(@co) 
	and p.Mth=dbo.vfToString(@mth)
	and p.BatchId=dbo.vfToString(@batchid) 
	and p.Job=dbo.vfToString(@job)
	and p.PhaseGroup=dbo.vfToString(@phasegroup)
	and p.Phase=dbo.vfToString(@phase)
	and p.CostType=dbo.vfToString(@linkcosttype)
	and dbo.vfToString(p.PRCo)=dbo.vfToString(@prco)
	and dbo.vfToString(p.Crew)=dbo.vfToString(@crew)'
	exec sp_executesql @sql,@paramsin,@batchseq, @co, @mth, @batchid, @job, 
		@phasegroup,@phase,@linkcosttype,@prco,@crew
...
declare @batchid bBatchID, @batchseq int, @errmsg varchar(255), @co bCompany,
		...
		@changecount int, @costtrans bTrans, @q char(2), @rcode int,@paramsin NVARCHAR(255)
...
ALTER TABLE [dbo].[bJCPP] ADD CONSTRAINT [PK_bJCPP] PRIMARY KEY CLUSTERED  ([Co], [Mth], [BatchId], [BatchSeq]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bJCPP] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

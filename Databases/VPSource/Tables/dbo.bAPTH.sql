...
CREATE UNIQUE CLUSTERED INDEX [biAPTH] ON [dbo].[bAPTH] ([APCo], [Mth], [APTrans]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biAPTHOpen] ON [dbo].[bAPTH] ([APCo], [Mth], [OpenYN]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biAPTHPrePaidForCheckPrint] ON [dbo].[bAPTH] ([APCo], [PrePaidMth], [PrePaidChk]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bAPTH] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [IX_bAPTH_SLKeyID] ON [dbo].[bAPTH] ([SLKeyID]) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biAPTHAttach] ON [dbo].[bAPTH] ([UniqueAttchID]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biAPTHVendor] ON [dbo].[bAPTH] ([Vendor], [VendorGroup], [APRef]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_CMAcct] CHECK (([CMAcct]>(0) AND [CMAcct]<(10000) OR [CMAcct] IS NULL))
ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_ChkRev] CHECK (([ChkRev]='Y' OR [ChkRev]='N'))
ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_InPayControl] CHECK (([InPayControl]='Y' OR [InPayControl]='N'))
ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_OpenYN] CHECK (([OpenYN]='Y' OR [OpenYN]='N'))
ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_PayOverrideYN] CHECK (([PayOverrideYN]='Y' OR [PayOverrideYN]='N'))
ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_PrePaidProcYN] CHECK (([PrePaidProcYN]='Y' OR [PrePaidProcYN]='N'))
ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_PrePaidYN] CHECK (([PrePaidYN]='Y' OR [PrePaidYN]='N'))
ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_Purge] CHECK (([Purge]='Y' OR [Purge]='N'))
ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_SeparatePayYN] CHECK (([SeparatePayYN]='Y' OR [SeparatePayYN]='N'))
ALTER TABLE [dbo].[bAPTH] ADD
CONSTRAINT [CK_bAPTH_V1099YN] CHECK (([V1099YN]='Y' OR [V1099YN]='N'))
GO

...
CREATE UNIQUE CLUSTERED INDEX [biGLAS] ON [dbo].[bGLAS] ([GLCo], [GLAcct], [Mth], [Jrnl], [GLRef], [SourceCo], [Source]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [viGLAS1] ON [dbo].[bGLAS] ([GLCo], [Mth], [Jrnl], [GLRef]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [IX_bGLAS_Mth] ON [dbo].[bGLAS] ([Mth]) INCLUDE ([GLAcct], [GLCo], [NetAmt]) ON [PRIMARY]

ALTER TABLE [dbo].[bGLAS] ADD
CONSTRAINT [CK_bGLAS_Adjust] CHECK (([Adjust]='Y' OR [Adjust]='N' OR [Adjust] IS NULL))
ALTER TABLE [dbo].[bGLAS] ADD
CONSTRAINT [CK_bGLAS_Purge] CHECK (([Purge]='Y' OR [Purge]='N'))
GO

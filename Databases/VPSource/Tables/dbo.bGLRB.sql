...
CREATE UNIQUE CLUSTERED INDEX [biGLRB] ON [dbo].[bGLRB] ([Co], [Mth], [BatchId], [BatchSeq]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bGLRB] ADD CONSTRAINT [PK_bGLRB] PRIMARY KEY NONCLUSTERED  ([KeyID]) WITH (FILLFACTOR=100) ON [PRIMARY]

ALTER TABLE [dbo].[bGLRB] WITH NOCHECK ADD
CONSTRAINT [FK_bGLRB_bHQBC_CoMthBatchId] FOREIGN KEY ([Co], [Mth], [BatchId]) REFERENCES [dbo].[bHQBC] ([Co], [Mth], [BatchId])
ALTER TABLE [dbo].[bGLRB] ADD
CONSTRAINT [FK_bGLRB_bGLDT_GLCoMthGLTrans] FOREIGN KEY ([Co], [OrigMonth], [OrigGLTrans]) REFERENCES [dbo].[bGLDT] ([GLCo], [Mth], [GLTrans])
GO

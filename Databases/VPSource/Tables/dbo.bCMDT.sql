...
CREATE UNIQUE CLUSTERED INDEX [biCMDT] ON [dbo].[bCMDT] ([CMCo], [Mth], [CMTrans]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biCMDTCMRefUnique] ON [dbo].[bCMDT] ([CMCo], [CMAcct], [CMTransType], [CMRef], [CMRefSeq]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biCMDTCMRef] ON [dbo].[bCMDT] ([CMRef]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bCMDT] WITH NOCHECK ADD
CONSTRAINT [FK_bCMDT_bCMAC_CMCoCMAcct] FOREIGN KEY ([CMCo], [CMAcct]) REFERENCES [dbo].[bCMAC] ([CMCo], [CMAcct])
ALTER TABLE [dbo].[bCMDT] WITH NOCHECK ADD
CONSTRAINT [FK_bCMDT_bHQBC_SourceCoBatchID] FOREIGN KEY ([SourceCo], [Mth], [BatchId]) REFERENCES [dbo].[bHQBC] ([Co], [Mth], [BatchId])
ALTER TABLE [dbo].[bCMDT] ADD
CONSTRAINT [CK_bCMDT_CMAcct] CHECK (([CMAcct]>(0) AND [CMAcct]<(10000)))
ALTER TABLE [dbo].[bCMDT] ADD
CONSTRAINT [CK_bCMDT_Purge] CHECK (([Purge]='Y' OR [Purge]='N'))
ALTER TABLE [dbo].[bCMDT] ADD
CONSTRAINT [CK_bCMDT_Void] CHECK (([Void]='Y' OR [Void]='N'))
GO

...
BEGIN
	...
	IF EXISTS(SELECT VPUserName FROM INSERTED i WHERE i.SecurityGroup = 200 OR i.SecurityGroup = 201 OR i.SecurityGroup = 202)
		IF NOT EXISTS(SELECT DISTINCT u.UserName 
			FROM JCUO u
			INNER JOIN  inserted i ON i.VPUserName = u.UserName
			--WHERE i.SecurityGroup = 200 OR i.SecurityGroup = 201 OR i.SecurityGroup = 202
			)
		BEGIN

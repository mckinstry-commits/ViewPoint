...
CREATE UNIQUE CLUSTERED INDEX [biJCPM] ON [dbo].[bJCPM] ([PhaseGroup], [Phase]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bJCPM] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biJCPMNoGroup] ON [dbo].[bJCPM] ([Phase]) INCLUDE ([Description]) ON [PRIMARY]

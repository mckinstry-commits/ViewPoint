...
CREATE UNIQUE CLUSTERED INDEX [biGLAC] ON [dbo].[bGLAC] ([GLCo], [GLAcct]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bGLAC] ADD CONSTRAINT [PK_bGLAC] PRIMARY KEY NONCLUSTERED  ([KeyID]) WITH (FILLFACTOR=100) ON [PRIMARY]

CREATE NONCLUSTERED INDEX [biGLACAcctType] ON [dbo].[bGLAC] ([AcctType]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bGLAC] WITH NOCHECK ADD
CONSTRAINT [FK_bGLAC_bGLCO_GLCo] FOREIGN KEY ([GLCo]) REFERENCES [dbo].[bGLCO] ([GLCo])
ALTER TABLE [dbo].[bGLAC] ADD
CONSTRAINT [CK_bGLAC_Active] CHECK (([Active]='Y' OR [Active]='N'))
ALTER TABLE [dbo].[bGLAC] ADD
CONSTRAINT [CK_bGLAC_InterfaceDetail] CHECK (([InterfaceDetail]='Y' OR [InterfaceDetail]='N'))
GO

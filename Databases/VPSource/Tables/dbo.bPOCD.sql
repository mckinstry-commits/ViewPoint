...
[CurUnitCost] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPOCD_CurUnitCost] DEFAULT ((0)),
[ECM] [dbo].[bECM] NULL,
...
CREATE UNIQUE CLUSTERED INDEX [biPOCD] ON [dbo].[bPOCD] ([POCo], [PO], [POItem], [Mth], [POTrans], [Seq]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPOCD] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]

CREATE UNIQUE NONCLUSTERED INDEX [biPOCDTrans] ON [dbo].[bPOCD] ([POTrans], [Mth], [POCo]) WITH (FILLFACTOR=90) ON [PRIMARY]

ALTER TABLE [dbo].[bPOCD] ADD
CONSTRAINT [CK_bPOCD_ECM] CHECK (([ECM]='E' OR [ECM]='C' OR [ECM]='M' OR [ECM] IS NULL))
ALTER TABLE [dbo].[bPOCD] ADD
CONSTRAINT [CK_bPOCD_PurgeYN] CHECK (([PurgeYN]='Y' OR [PurgeYN]='N'))
GO

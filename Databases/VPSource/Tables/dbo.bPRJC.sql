...
GO
CREATE NONCLUSTERED INDEX [IX_bPRJC_JCCo] ON [dbo].[bPRJC] ([JCCo]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_bPRJC_MthJCCoJOBPhaseGroup] ON [dbo].[bPRJC] ([Mth], [JCCo], [Job], [PhaseGroup], [Phase]) INCLUDE ([Employee], [PaySeq], [PostSeq], [PRCo], [PREndDate], [PRGroup]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPRJC] ON [dbo].[bPRJC] ([PRCo], [PRGroup], [PREndDate], [Mth], [JCCo], [Job], [PhaseGroup], [Phase], [JCCostType], [Type], [JCFields], [Employee], [PaySeq], [PostSeq]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

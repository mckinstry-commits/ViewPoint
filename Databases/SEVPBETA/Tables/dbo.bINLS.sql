...
GO
   ...
   CREATE trigger [dbo].[btINLSd] on [dbo].[bINLS] for DELETE as
   

/*--------------------------------------------------------------
    *  Created By: GR 7/17/00
    *  Modified:
    *
    *  Delete trigger for IN Location Company Override
    *
    *--------------------------------------------------------------*/
   ...
   -- HQ Auditing
   insert bHQMA select 'bINLS','INCo:' + convert(varchar(3),d.INCo) + ' Loc:' + d.Loc + ' Co:' + convert(varchar(6),d.Co),
   ...
   error:
      select @errmsg = @errmsg + ' - cannot delete IN Location Company Override'
...
GO
   ...
   CREATE   trigger [dbo].[btINLSi] on [dbo].[bINLS] for INSERT as
   

/*--------------------------------------------------------------
    * Created By: GR 7/14/00
    * Modified by: CMW 07/09/02 - added TO Co validation (issue # 17859).
    *
    *
    *
    * Insert trigger for IN Location Company Override
   ...
   SELECT 'bINLS',' Location: ' + i.Loc + ' Company: ' + convert(varchar(6), Co), i.INCo, 'A', NULL, NULL, NULL, getdate(), SUSER_SNAME()
   FROM inserted i join bINCO c on c.INCo = i.INCo and c.AuditLoc = 'Y'
   
   return
   
   error:
       select @errmsg = @errmsg + ' - cannot insert IN Location Company Override.'
       RAISERROR(@errmsg, 11, -1);
       rollback transaction
...
GO
   ...
   CREATE   trigger [dbo].[btINLSu] on [dbo].[bINLS] for UPDATE as
...
/*--------------------------------------------------------------
   ...
   join deleted d on d.INCo = i.INCo and d.Loc=i.Loc and d.Co=i.Co
   if @validcnt <> @numrows
       begin
    	select @errmsg = 'Cannot change IN Location or Sell To Company '
    	goto error
    	end
   ...
   if exists(select * from inserted i
       	join bINCO a on i.INCo = a.INCo	where a.AuditLoc = 'Y')
       begin
   	if update(GLCo)
   		begin
       	insert dbo.bHQMA (TableName, KeyString, Co, RecType, FieldName, OldValue, NewValue, DateTime, UserName)
   		select 'bINLS', ' Location: ' + i.Loc + ' Company: ' + convert(varchar(6), i.Co), i.INCo, 'C',
   ...
   return
    ...
    error:
       select @errmsg = @errmsg + ' - cannot update IN Location Company Override'
       RAISERROR(@errmsg, 11, -1);
       rollback transaction
...
GO
CREATE UNIQUE CLUSTERED INDEX [biINLS] ON [dbo].[bINLS] ([INCo], [Loc], [Co]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bINLS] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

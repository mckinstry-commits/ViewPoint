...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_SKT08]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_SKT08]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @rate = 0, @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @LCP = 0, @LCPA = 0, @LCPB = 0, @T2 = 0
   select @calcamt = 0, @procname = 'bspPR_CA_SKT08'
   ...
   select @rate = .15, @KP = 3019
   if @A <= 111814 select @rate = .13, @KP = 783
   if @A <= 39135 select @rate = .11, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_SKT08] TO [public]
GO

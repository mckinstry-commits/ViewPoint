...
CREATE  proc [dbo].[bspPR_CA_SKT13]
/********************************************************
...
*				MV 11/30/11 TK-10427 2012 tax updates
*				CHS	B-11809 TK-20128 12/10/2012 tax update effective 01/01/13
*
* USAGE:
...
(@ppds tinyint = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_SKT13'
   ...
   if @TCP is null select @TCP = 15241

   -- establish tax rate and constant
                   select @rate = .15, @KP = 3310
   if @A <= 122589 select @rate = .13, @KP = 858
   if @A <=  42906 select @rate = .11, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_SKT13] TO [public]
GO

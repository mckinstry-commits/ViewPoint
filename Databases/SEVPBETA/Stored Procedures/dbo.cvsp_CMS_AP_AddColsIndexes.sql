...
if not exists (select o.name, c.name from CV_CMS_SOURCE.dbo.syscolumns c
			join CV_CMS_SOURCE.dbo.sysobjects o
				on o.id=c.id
			where c.name='udAPTrans' and o.name='APTOPC')
alter table CV_CMS_SOURCE.dbo.APTOPC add udAPTrans int;

if not exists (select o.name, c.name from CV_CMS_SOURCE.dbo.syscolumns c
			join CV_CMS_SOURCE.dbo.sysobjects o
				on o.id=c.id
			where c.name='udAPTrans' and o.name='APTOPD')
alter table CV_CMS_SOURCE.dbo.APTOPD add udAPTrans int;


if not exists (select name from dbo.syscolumns 
  where id=OBJECT_ID('bAPVM')and name='udSubcontractorYN')
alter table bAPVM add udSubcontractorYN bYN;

if not exists (select name from dbo.syscolumns 
  where id=OBJECT_ID('bAPVM')and name='udCGCVendor')
alter table bAPVM add udCGCVendor varchar(10);

if not exists (select name from dbo.syscolumns 
  where id=OBJECT_ID('vSMCustomer')and name='udConvertedYN')
alter table vSMCustomer add udConvertedYN bYN
...
ON vPOItemLine (POCo, PO, POItem, udCGC_ASQ02)

if not exists (select name from dbo.sysindexes 
where id=OBJECT_ID('vPOItemLine') and name='ciPOPOItem')	
CREATE NONCLUSTERED INDEX ciPOPOItem
ON vPOItemLine (POCo, PO, POItem)

...
/***********************************************************************/
CREATE procedure [dbo].[vspPMDocDistInitForSUBCO]
/************************************************************************
* Created By:	GF 03/17/2011 - TK-02604 TK-03298 COR
...
* PM Subcontract Change Order Document and load email, fax, CC addresses,
* header and query strings.
* Called from frmPMSubcontractCO form.
*
*
*
*
...
* PM Company
* Project
* Document Category	- will be 'SUBCO'
* User Name
* PMSubcontractCO_KeyID - subcontract change order record identifier
...
(@pmco bCompany, @project bProject, @DocCategory varchar(10), @user bVPUserName,
 @PMSubcontractCO_KeyId BIGINT = NULL, @template varchar(40) = null,
...
---- get document data
SELECT @description=Description, @SLCo=SLCo, @SL=SL,
		@document=CONVERT(VARCHAR(10),SubCO),
		@SubCO=SubCO
FROM dbo.PMSubcontractCO WHERE KeyID=@PMSubcontractCO_KeyId
if @@rowcount = 0
	begin
...
if not exists(select Seq from dbo.PMDistribution where SubcontractCOID = @PMSubcontractCO_KeyId
						and Send='Y' and CC='N')
	begin
...
where SubcontractCOID=@PMSubcontractCO_KeyId and Send='Y' and CC='N'
...
exec @retcode = dbo.vspPMDocDistInitCCListBuild @pmco, @project, @DocCategory, null, @document, NULL, NULL,
				----TK-11961
				@PMSubcontractCO_KeyId, @ccnames output, @ccaddr output, @bccaddr output, @msg output
...
if ltrim(rtrim(isnull(@template,''))) <> ''
	...
	select @value = @value + ' and a.SubCO = ' + CONVERT(VARCHAR(10), @SubCO)
	...
	SELECT @value = @value + ' and b.SLCo=' + CONVERT(VARCHAR(3), @SLCo)
	SELECT @value = @value + ' and b.SL=' + CHAR(39) + @SL + CHAR(39)
	---- SEND FLAG MUST BE 'Y'
	...
	if @needitemquery = 'Y'
		...
		select @value = @value + ' and a.Project = ' + CHAR(39) + @project + CHAR(39)
		select @value = @value + ' and a.SubCO = ' + CONVERT(VARCHAR(10), @SubCO)
		SELECT @value = @value + ' and a.SLCo=' + CONVERT(VARCHAR(3), @SLCo)
		...
		if charindex('a.Seq', @groupby) > 0
			...
			begin
			select @value = @value + ' group by a.SubCO, a.SLCo, a.SL, ' + @groupby
			end
...
if isnull(@usestdsubject,'Y') = 'Y'
	begin
	set @subjectline = 'SubCO: ' + isnull(@document,'') + ' - ' + isnull(@description,'')
	end
else
	begin
	...
	if isnull(@subjectline,'') = '' set @subjectline = 'SubCO: ' + isnull(@document,'') + ' - ' + isnull(@description,'')
	end
...
insert PMHI(SourceTableName, SourceKeyId, CreatedBy, VendorGroup,
			...
			bCCAddresses)
select 'PMSubcontractCO', @PMSubcontractCO_KeyId, @user, @vendorgroup, @senttofirm, @senttocontact,
		@email, @fax, @faxaddress,
		'SubCO: ' + isnull(@document,'') + ' - ' + isnull(@description,''),
		@ccaddr, @bccaddr
if @@rowcount = 0
	begin
...
GO
GRANT EXECUTE ON  [dbo].[vspPMDocDistInitForSUBCO] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPROKT09]    Script Date: 12/18/2007 10:31:21 ******/
 CREATE  proc [dbo].[bspPROKT09]
  /********************************************************
  ...
  **********************************************************/
  ...
  select @rcode = 0, @procname = 'bspPROKT09'
  ...
  if @status = 'M'
  begin
  	if @annualized_wage >= 8500 select @tax_addition = 0, @wage_bracket = 8500, @rate = .05
  	if @annualized_wage >= 10500 select @tax_addition = 10, @wage_bracket = 10500, @rate = .01
  	if @annualized_wage >= 13500 select @tax_addition = 40, @wage_bracket = 13500, @rate = .02
  	if @annualized_wage >= 16000 select @tax_addition = 90, @wage_bracket = 16000, @rate = .03
  	if @annualized_wage >= 18300 select @tax_addition = 159, @wage_bracket = 18300, @rate = .04
  	if @annualized_wage >= 20700 select @tax_addition = 255, @wage_bracket = 20700, @rate = .05
  	if @annualized_wage >= 23500 select @tax_addition = 395, @wage_bracket = 23500, @rate = .055
  end
 
  /* select calculation elements for everybody else */
  if @status = 'S' or @status = 'H'
  begin
  	if @annualized_wage >= 4250 select @tax_addition = 0, @wage_bracket = 4250, @rate = .05
  	if @annualized_wage >= 5250 select @tax_addition = 5, @wage_bracket = 5250, @rate = .01
  	if @annualized_wage >= 6750 select @tax_addition = 20, @wage_bracket = 6750, @rate = .02
  	if @annualized_wage >= 8000 select @tax_addition = 45, @wage_bracket = 8000, @rate = .03
  	if @annualized_wage >= 9150 select @tax_addition = 79.5, @wage_bracket = 9150, @rate = .04
  	if @annualized_wage >= 11450 select @tax_addition = 171.5, @wage_bracket = 11450, @rate = .05
  	if @annualized_wage >= 12950 select @tax_addition = 246.5, @wage_bracket = 12950, @rate = .055
  end
...
GO
GRANT EXECUTE ON  [dbo].[bspPROKT09] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRNYY04    Script Date: 8/28/99 9:33:31 AM ******/
    CREATE  proc [dbo].[bspPRNYY04]
    /********************************************************
    ...
    **********************************************************/
    ...
    select @rcode = 0, @allowance = 1000, @procname = 'bspPRNYY04'
    ...
    if @status = 'S'
   	begin
    	...
    	if @annualized_wage > 100000 select @tax_addition = 6532, @wage_bracket = 100000, @rate = .0867
    	if @annualized_wage > 150000 select @tax_addition = 10867, @wage_bracket = 150000, @rate = .0893
    	if @annualized_wage > 200000 select @tax_addition = 15332, @wage_bracket = 200000, @rate = .0788
    	if @annualized_wage > 500000 select @tax_addition = 38972, @wage_bracket = 500000, @rate = .0820
   	end
    if @status = 'M'
   	begin
    	if @annualized_wage <= 8000 select @tax_addition = 0, @wage_bracket = 0, @rate = .04
    	...
    	if @annualized_wage > 150000 select @tax_addition = 10604, @wage_bracket = 150000, @rate = .0945
    	if @annualized_wage > 200000 select @tax_addition = 15327, @wage_bracket = 200000, @rate = .0788
    	if @annualized_wage > 500000 select @tax_addition = 38967, @wage_bracket = 500000, @rate = .0820
   	end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNYY04] TO [public]
GO

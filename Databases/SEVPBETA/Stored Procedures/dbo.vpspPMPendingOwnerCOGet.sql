...
CREATE PROCEDURE [dbo].[vpspPMPendingOwnerCOGet]
/************************************************************
* CREATED:     11/16/06  chs
* MODIFIED:		6/7/07	CHS
...
Select p.KeyID, p.PMCo, p.Project, p.PCOType, p.PCO,
	...
	Left Join PMIM i with (nolock) on p.PMCo=i.PMCo and p.Project=i.Project and p.Issue=i.Issue
	left join PMDT dt with (nolock) on dt.DocType = p.PCOType
	----TK-09972
	...
	Where p.PMCo=@JCCo and p.Project=@Job and p.IntExt = 'E'
...
GO
GRANT EXECUTE ON  [dbo].[vpspPMPendingOwnerCOGet] TO [VCSPortal]
GO

...
BEGIN
	...
	BEGIN
		...
		IF (SELECT ISNULL(SUM(BillingAmount), 0)
			FROM dbo.SMAgreementBillingSched
			WHERE SMCo = @SMCo AND Agreement = @Agreement AND Revision = @Revision) <>
			(SELECT ISNULL(SUM(PricingPrice), 0) + ISNULL(@AgreementPrice, 0)
			FROM dbo.SMAgreementService
			WHERE SMCo = @SMCo AND Agreement = @Agreement AND Revision = @Revision AND PricingMethod = 'P' AND BilledSeparately = 'N')
		BEGIN
		...
		IF EXISTS(SELECT 1
			FROM dbo.SMAgreementService
			WHERE SMCo = @SMCo AND Agreement = @Agreement AND Revision = @Revision AND PricingMethod = 'P' AND BilledSeparately = 'Y'
				AND PricingPrice <> ISNULL((SELECT SUM(BillingAmount) FROM dbo.SMAgreementBillingSchedule WHERE SMCo = SMAgreementService.SMCo AND Agreement = SMAgreementService.Agreement AND Revision = SMAgreementService.Revision AND [Service] = SMAgreementService.[Service]), 0))
		BEGIN

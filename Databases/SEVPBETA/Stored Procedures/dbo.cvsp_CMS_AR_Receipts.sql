...
CREATE proc [dbo].[cvsp_CMS_AR_Receipts] (@fromco smallint, @toco smallint,		 
...
-- transactions are deleted in other procedures
...
select @toco
	, D.udPaidMth
	, ARTrans=isnull(H.LastTrans,1)+ROW_NUMBER() Over (partition by D.COMPANYNUMBER, D.udPaidMth
			ORDER BY D.COMPANYNUMBER, D.udPaidMth, D.udARTOPCID, D.CUSTOMERNUMBER)
	, ARLine=ROW_NUMBER ( ) OVER (partition by D.COMPANYNUMBER, D.udPaidMth
		order by D.COMPANYNUMBER, D.udPaidMth, D.ARTOPDID )
	, RecType=1
	, LineType=L.LineType
...
from CV_CMS_SOURCE.dbo.ARTOPD D
	...
	left join Viewpoint.dbo.budxrefGLAcct ON Company=@fromco and D.GENLEDGERACCT=oldGLAcct

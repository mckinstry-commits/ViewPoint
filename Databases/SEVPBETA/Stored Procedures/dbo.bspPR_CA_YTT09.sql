...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_YTT09]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_YTT09]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_YTT09'
   ...
   if @TCP is null select @TCP = 10100

   -- establish tax rate and constant
   select @rate = .1276, @KP = 4059
   if @A <= 126264 select @rate = .1144, @KP = 2392
   if @A <= 77664 select @rate = .0968, @KP = 1025
   if @A <= 38832 select @rate = .0704, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_YTT09] TO [public]
GO

...
/***********************************************************************/
CREATE procedure [dbo].[vspPMDocDistInitForPCO]
/************************************************************************
* Created By:	GF 05/15/2007 6.x
...
*				GF 11/12/2010 - issue #142083 change to use function for fax number
...
* PM PCO Document and load email, fax, CC addresses, header and query strings.
* Called from frmPMPCOS form.
*
*
*
* Input parameters:
...
if not exists(select Seq from dbo.PMCD where PMCo=@pmco and Project=@project
						and PCOType=@doctype and PCO=@document and Send='Y' and CC='N')
	begin
...
PMCD_loop:
fetch next from bcPMCD into @vendorgroup, @senttofirm, @senttocontact, @prefmethod

if @@fetch_status = -1 goto PMCD_end
if @@fetch_status <> 0 goto PMCD_loop
...
if ltrim(rtrim(isnull(@template,''))) <> ''
	...
	select @headerstring = @headerstring + ',CCList,TotalAmount'
	select @querystring = @querystring + ',PMDZ.CCList,PMOPTotals.PCORevTotal'
	...
	---- add join to PMOPtotals for PCO Revenue total
	select @value = @value + ' left join PMOPTotals with (nolock) on PMOPTotals.PMCo=a.PMCo and PMOPTotals.Project=a.Project'
	select @value = @value + ' and PMOPTotals.PCOType=a.PCOType and PMOPTotals.PCO=a.PCO'
...
if isnull(@usestdsubject,'Y') = 'Y'
	begin
	set @subjectline = 'PCO: ' + isnull(@document,'') + ' - ' + isnull(@description,'')
	end
else
	begin
	...
	if isnull(@subjectline,'') = '' set @subjectline = 'PCO: ' + isnull(@document,'') + ' - ' + isnull(@description,'')
	end
...
insert PMHI(SourceTableName, SourceKeyId, CreatedBy, VendorGroup,
			...
			bCCAddresses)
select 'PMOP', @sourcekeyid, @user, @vendorgroup, @senttofirm, @senttocontact,
		@email, @fax, @faxaddress,
		'PCO: ' + isnull(@document,'') + ' - ' + isnull(@description,''),
		@ccaddr, @bccaddr
if @@rowcount = 0
	begin
...
GO
GRANT EXECUTE ON  [dbo].[vspPMDocDistInitForPCO] TO [public]
GO

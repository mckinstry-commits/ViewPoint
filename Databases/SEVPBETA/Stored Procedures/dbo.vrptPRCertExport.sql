...
CREATE proc [dbo].[vrptPRCertExport] (@PRCo tinyint, @PRGroup tinyint, @PREndDate smalldatetime,  @TotalMasterOrFullJob char(1),
   ...
   declare @Employee int, @SaveEmp int, @FedTaxDedn smallint, @EDLCode int, @EarnCode int, @Rate numeric (10,4), @EarnFactor numeric (8,6), @Amount numeric (12,2),
  ...
  insert into #CertInformation
   ...
   (JCCo, Job, Employee, EarnCode, EarnDesc, DetOnCert, Craft, Class, Taxable, EarnRate, EarnFactor, PaySeq, PostSeq, WkDay, PRDept, JCDept, Crew, Hours, EarnAmt)
   select h.JCCo, Substring(h.Job,1,@SignifJobChar), h.Employee, h.EarnCode, e.Description, e.CertRpt, h.Craft, h.Class, 'N', h.Rate, e.Factor, PaySeq, PostSeq, datepart(dw, h.PostDate), PRDept, JCDept, Crew, h.Hours, h.Amt
   From PRTH h
   ...
   select JCCo, Job, Employee, Craft, Class, EarnCode, Taxable, TotalLine, EarnDesc, DetOnCert, EarnRate, EarnFactor, sum(EarnAmt)
   From #EarnTemp
   Where DetOnCert='Y' or DetOnCert='Z'
   Group By JCCo, Job, Employee, Craft, Class, EarnCode, Taxable, TotalLine, EarnDesc, DetOnCert, EarnRate, EarnFactor
   order by JCCo, Job, Employee, EarnCode, EarnRate, EarnFactor, TotalLine
   ...
   @JCCo, @Job, @Employee, @Craft, @Class, @EarnCode, @Taxable, @EarnTotalLine, @Desc, @DetOnCert, @Rate, @EarnFactor, @Amount
   ...
   if @RecId Is Not Null
   	...
   	set EarnCode=@EarnCode, EarnDesc=@Desc, EarnRate=@Rate, EarnFactor=@EarnFactor, EarnAmt=@Amount, Craft=@Craft, Class=@Class, EarnTotalLine=@EarnTotalLine
   	where RecId=@RecId 
   else
   	insert into #CertInformation
   	(JCCo, Job, Employee, Craft, Class, EarnCode, EarnDesc, DetOnCert, Taxable, EarnTotalLine, EarnRate, EarnFactor, EarnAmt)
   	Values (@JCCo, @Job, @Employee, @Craft, @Class, @EarnCode, @Desc, @DetOnCert, @Taxable, @EarnTotalLine, @Rate, @EarnFactor, @Amount)
  ...
  --insert liabilities to print detail on report
    ...
    select h.JCCo, Substring(h.Job,1,@SignifJobChar), h.Employee, 'N', l.LiabCode, d.Description, d.DetOnCert, max(l.Rate), sum(l.Amt)
    From PRTH h
    ...
    Group By h.JCCo, Substring(h.Job,1,@SignifJobChar), h.Employee, l.LiabCode, d.Description, d.DetOnCert
  ...
  Group by JCCo, Substring(Job,1,@SignifJobChar) 
   ...
   ci.Employee, ci.RecId, LastName, FirstName, MidName,e.Suffix, e.SortName, e.Address, e.City, e.State, e.Zip, e.Address2, SSN, Sex, Race, FileStatus, RegExempts, EmpClassDesc=ec.Description,
   ...
   ci.Taxable, ci.EarnDesc, ci.EarnRate, ci.EarnFactor, th.Hours, ci.EarnAmt, JCDept, JCDeptDesc=dm.Description, th.PRDept,PRDeptDesc=dp.Description, th.Crew, th.Craft, th.Class, DedCode, 
   ...
   Left Outer Join PRCC ec on ec.PRCo=@PRCo and ec.Craft=ci.Craft and ec.Class=ci.Class
...
GO
GRANT EXECUTE ON  [dbo].[vrptPRCertExport] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPROKT052    Script Date: 8/28/99 9:33:32 AM ******/
   CREATE proc [dbo].[bspPROKT052]
    /********************************************************
    ...
    **********************************************************/
    ...
    select @rcode = 0, @procname = 'bspPROKT052'
    ...
    if @status = 'M' or @status = 'H'
    	begin
   	select @deduction_2 = @annualized_wage - (@exempts * 3200) - 10000
       if @deduction_2 <= 14600
   		select @deduction_2 = @deduction_2 * .1
   	else
   		select @deduction_2 = ((@deduction_2 - 14600) * .15) + 1460
   	end
    else
   	begin
    	select @deduction_2 = @annualized_wage - (@exempts * 3200) - 5000
   	if @deduction_2 <= 7300
   		select @deduction_2 = @deduction_2 * .1
   	else
       	 select @deduction_2 = ((@deduction_2 - 7300) * .15) + 730
   	end
    ...
    if @status = 'M' select @amt = @amt * 1.16 --see extra value in Table Three for Dual Income calculation
   
    /* if gross wages exceed $22,560 compute withholding amount on $22,560 and add
     * 6.65% * (gross wage - $22,560) to determine total withholding */
   
    if @subjamt * @ppds > 22560 select @amt = @amt + (@subjamt * @ppds - 22560) * .0665
...
GO
GRANT EXECUTE ON  [dbo].[bspPROKT052] TO [public]
GO

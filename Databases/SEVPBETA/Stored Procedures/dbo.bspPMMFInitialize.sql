 ...
 **************************************/
 ...
 declare @rcode int, @retcode int, @validcnt int, @pono varchar(1), @mono varchar(1), @posigpartjob bYN,
		...
		@poaddchgopen varchar(1), @MatlPhaseDesc bYN

select @rcode = 0, @openmocursor = 0, @openrqcursor = 0

select @cotype = @pcotype, @pcotype = null
...
BEGIN
	...
	select @paddedstring = reverse(substring(reverse('0000000000000000000' + ltrim(str(@mseqpo))),1,@poseqlen))

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRNCT02    Script Date: 8/28/99 9:33:29 AM ******/
   CREATE     proc [dbo].[bspPRNCT02]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPRNCT02'
   ...
   if @status = 'S'
   	...
   	if @annualized_wage > 120000 select @tax_subtraction = 1177.5, @rate = .0825
   end
   ...
   if @status = 'H'
   	...
   	if @annualized_wage > 160000 select @tax_subtraction = 1570, @rate = .0825
   end
   
   /* married or qualifying widow(er) */
   if @status = 'M' or @status = 'W'
   	begin
   	select @deduction = 2750
   	...
   	if @annualized_wage > 100000 select @tax_subtraction = 981.25, @rate = .0825
   end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNCT02] TO [public]
GO

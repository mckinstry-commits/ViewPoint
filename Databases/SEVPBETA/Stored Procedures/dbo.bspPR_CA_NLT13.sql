...
CREATE  proc [dbo].[bspPR_CA_NLT13]
/********************************************************
...
*				MV 11/29/11 #TK-10422 - 2012 tax updates
*				MV 12/11/12 TK-20147 - 2013 tax updates
*
* USAGE:
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   SELECT @rcode = 0, @procname = 'bspPR_CA_NLT13'
   ...
   IF ISNULL(@TCP,0) = 0 SELECT @TCP = 8451

   -- establish tax rate and constant
   IF      @A BETWEEN     0 AND 33748		SELECT @Rate = .077, @KP = 0
   ELSE IF @A BETWEEN 33748.01 AND 67496	SELECT @Rate = .125, @KP = 1620
   ELSE IF @A > 67496						SELECT @Rate = .133, @KP = 2160
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NLT13] TO [public]
GO

...
/***********************************************************************/
CREATE procedure [dbo].[vspPMDocDistInitForTestLog]
/************************************************************************
...
* PM Test Log Document and load email, fax, CC addresses, header and query strings.
...
if not exists(select DocType from dbo.PMDT where DocType=@doctype)
	begin
	select @msg = 'Invalid Test type', @rcode = 1
	goto bspexit
	end

---- get document data
select @description=Description, @status=Status, @sourcekeyid=KeyID
from dbo.PMTL with (nolock)
where PMCo=@pmco and Project=@project and TestType=@doctype and TestCode=@document
if @@rowcount = 0
	begin
	select @msg = 'Invalid Test Document', @rcode = 1
	goto bspexit
	END
	
---- validate there is other document distribution in PMDistTestLogs
if not exists(select Seq from dbo.vPMDistribution WHERE TestLogID = @sourcekeyid
					and Send='Y' and CC='N')
					--where PMCo=@pmco and Project=@project
					--and TestType=@doctype and TestCode=@document and Send='Y' and CC='N')
	begin
...
---- create cursor on PMDistTestLogs
declare bcPMDistTestLogs cursor LOCAL FAST_FORWARD
for select VendorGroup, SentToFirm, SentToContact, PrefMethod
from dbo.vPMDistribution
WHERE TestLogID = @sourcekeyid
and Send='Y' and CC='N'
--where PMCo=@pmco and Project=@project and TestType=@doctype
--and TestCode=@document and Send='Y' and CC='N'

---- open bcPMDistTestLogs cursor
open bcPMDistTestLogs
select @opencursor = 1

---- loop through distribution list
PMDistTestLogs_loop:
fetch next from bcPMDistTestLogs into @vendorgroup, @senttofirm, @senttocontact, @prefmethod

if @@fetch_status = -1 goto PMDistTestLogs_end
if @@fetch_status <> 0 goto PMDistTestLogs_loop
...
if ltrim(rtrim(isnull(@template,''))) <> ''
	...
	exec @rcode = dbo.bspHQWDJoinClauseBuild 'TEST', 'N', 'Y', 'N', @joinstring output, @msg output
	if @rcode <> 0 goto bspexit
	...
	select @value = @value + ' and a.TestType = ' + CHAR(39) + @doctype + CHAR(39)
	select @value = @value + ' and a.TestCode = ' + CHAR(39) + @document + CHAR(39)
...
insert PMHI(SourceTableName, SourceKeyId, CreatedBy, VendorGroup,
			...
			bCCAddresses)
select 'PMTL', @sourcekeyid, @user, @vendorgroup, @senttofirm, @senttocontact,
...
PMDistTestLogs_end:
	if @opencursor = 1
		begin
		close bcPMDistTestLogs
		deallocate bcPMDistTestLogs
		set @opencursor = 0
		end
...
GO
GRANT EXECUTE ON  [dbo].[vspPMDocDistInitForTestLog] TO [public]
GO

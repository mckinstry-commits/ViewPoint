...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NBT09]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_NBT09]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NBT09'
   ...
   select @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @LCP = 0, @T2 = 0, @calcamt = 0, @TCrate = .1012
   ...
   select @rate = .1795, @KP = 4192
   if @A <= 116105 select @rate = .1680, @KP = 2857
   if @A <= 71415 select @rate = .1548, @KP = 1914
   if @A <= 35707 select @rate = .1012, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NBT09] TO [public]
GO

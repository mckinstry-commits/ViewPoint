...
SET ANSI_NULLS ON
GO
CREATE     Procedure [dbo].[vspHQRefreshIndexes]
 /*************************************************
 * Created: JonathanP 06/04/07 - Adapted from bspHQRefreshIndexes
 *
 * History:			 
 *			RM  Added Distinct into cursor to speed up processing.
 *			GG 10/03/02 - cleanup, removed join with bHQAI
 *			RM 03/26/04 - Issue# 23061 - Added IsNulls
 *			RM 09/21/05 - #29828 - added (nolock)
 *			JonathanP 06/04/07 - Changed from bspHQRefreshIndexes to vspHQRefreshIndexes.
 *		    JonathanP 03/27/08 - See issue #127602. Added "and CurrentState = 'A'" to the where 
 *								 clause to make sure the only attachments that get refreshed are attached.
 *
 * Usage:
 ...
 *************************************************/
  ...
  bspexit:
  	...
  	if @rcode <> 0 select @msg = isnull(@msg,'') + char(13) + char(10) + '[vspHQRefreshIndexes]'
  	return @rcode

GO
GRANT EXECUTE ON  [dbo].[vspHQRefreshIndexes] TO [public]
GO

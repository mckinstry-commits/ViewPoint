...
SET ANSI_NULLS ON
GO
CREATE   proc [dbo].[bspPMProjectValForIssueHist]
    /***********************************************************
     ...
     * no job passed, no project found in JCJM or no issue history
     * for job.
     *
     * INPUT PARAMETERS
   ...
   -- check to see if issues exist in bPMIH
   select @validcnt = count(*) from bPMIH with (nolock) where PMCo=@pmco and Project=@project
   if @validcnt = 0
   	begin
       select @msg = 'No issues exist in PMIH for the project!', @rcode = 1
       goto bspexit
       end
...
GO
GRANT EXECUTE ON  [dbo].[bspPMProjectValForIssueHist] TO [public]
GO

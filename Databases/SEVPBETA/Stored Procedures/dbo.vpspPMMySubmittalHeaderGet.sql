...
CREATE PROCEDURE [dbo].[vpspPMMySubmittalHeaderGet]
/************************************************************
* CREATED:     1/3/07  CHS
*
* USAGE:
...
select 
s.PMCo, s.Project, j.Description as 'ProjectDescription', s.Submittal, s.Description as 'SubmittalDescription', 
s.SubmittalType, t.Description as 'SubmittalTypeDescription',
s.Rev, s.Description, s.PhaseGroup, s.Phase, pd.Description as 'PhaseDescription', 
s.Issue, i.Description as 'IssueDescription',
s.Status, c.Description as 'StatusDescription', 
s.VendorGroup, s.ResponsibleFirm, 
rf.FirmName as 'ResFirmName', s.ResponsiblePerson,
rp.FirstName+' '+rp.LastName as 'ResPersonName',
s.SubFirm, sf.FirmName as 'SubFirmName', s.SubContact, 
sp.FirstName+' '+sp.LastName as 'SubPersonName',
s.ArchEngFirm, af.FirmName as 'ArcFirmName', s.ArchEngContact,
...
s.UniqueAttchID, s.SpecNumber, s.RecdBackArch, s.KeyID
...
from PMSM s with (nolock)
	...
	left Join JCPM pd with (nolock) on s.PhaseGroup=pd.PhaseGroup and s.Phase=pd.Phase


where 
	(@JCCo = s.PMCo) 
	and (@Job = s.Project)
	and ( ( (s.SubFirm = @FirmNumber) and (s.SubContact = @Contact) ) 
			or ( (s.ArchEngFirm = @FirmNumber) and (s.ArchEngContact = @Contact) ) )
	and s.KeyID = IsNull(@KeyID, s.KeyID) 



GO
GRANT EXECUTE ON  [dbo].[vpspPMMySubmittalHeaderGet] TO [VCSPortal]
GO

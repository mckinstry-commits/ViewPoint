...
BEGIN
	...
	DECLARE @rcode int, @ErrorOccured int, @NewMsg as varchar(255), @CostingMethod varchar(15), @ScopeAgreement varchar(15), @ScopePriceMethod char(1)
	...
	INNER JOIN SMCO ON SMCO.SMCo = SMWorkOrderScope.SMCo
	INNER JOIN HQCO ON HQCO.HQCo = SMCO.ARCo
	...
	BEGIN		
		...
		IF (@Job IS NOT NULL AND @CostingMethod='Revenue' AND @RateTemplate IS NULL)
		BEGIN
		...
		ELSE IF (@ScopeAgreement IS NOT NULL AND @ScopePriceMethod='T' AND @RateTemplate IS NULL)
		BEGIN
			INSERT INTO @MissingTable (NAMES) VALUES ('Rate Template') 
		END
		-- Check for Missing Rate Template on Customer work orders. Rate Template is always required.
		ELSE IF (@ScopeAgreement IS NULL AND @Job IS NULL AND @RateTemplate IS NULL)
		BEGIN
			INSERT INTO @MissingTable (NAMES) VALUES ('Rate Template') 
		END
	...
	BEGIN
		...
		IF (NOT @PRPostDate IS NULL)
		BEGIN
			SELECT @rcode=1
			IF @ErrorOccured=0 
				SELECT @msg = 'Related timecard record has been posted.'
		END
		ELSE IF (NOT @SMBCID IS NULL AND NOT @OldScope=@Scope)
		BEGIN
			SELECT @rcode=1
			IF @ErrorOccured=0
	...
	-- Get the accounting treatment
	SELECT @SMGLCo = GLCo, 
		@DefaultCostAcct = CostGLAcct, 
		@DefaultRevenueAcct = RevenueGLAcct, 
		@DefaultCostWIPAcct = CostWIPGLAcct,
		@DefaultRevWIPAcct = RevenueWIPGLAcct
	FROM dbo.vfSMGetAccountingTreatment (@SMCo, @WorkOrder, @Scope, @LineType, @SMCostType)

...
CREATE proc [dbo].[vspWFProcessIdGetForPMMF]
/***********************************************************
* CREATED BY:	GF 04/18/2012 TK-14088 PM PO work flow
* MODIFIED BY:	
*
*				
* USAGE:
...
* returns a work flow process id for a PMMF record .
...
FROM dbo.bPMMF src
	...
	OUTER APPLY (SELECT wflevel3.KeyID
						...
						WHERE job.JCCo = src.PMCo
							AND job.DocType = 'PO' 
							AND job.Job = src.Project
...
GO
GRANT EXECUTE ON  [dbo].[vspWFProcessIdGetForPMMF] TO [public]
GO

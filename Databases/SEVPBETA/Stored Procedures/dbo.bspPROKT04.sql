...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPROKT04    Script Date: 8/28/99 9:33:32 AM ******/
   CREATE      proc [dbo].[bspPROKT04]
    /********************************************************
    ...
    **********************************************************/
    ...
    select @rcode = 0, @procname = 'bspPROKT04'
    ...
    if @status = 'M' or @status = 'H'
    	begin
   	select @deduction_2 = @annualized_wage - (@exempts * 3100) - 9700
       if @deduction_2 <= 14300
   		select @deduction_2 = @deduction_2 * .1
   	else
   		select @deduction_2 = ((@deduction_2 - 14300) * .15) + 1430
           --select @deduction_2 = @deduction_2 - 12000 * .15 + 1200 danf
   	end
    else
   	begin
    	select @deduction_2 = @annualized_wage - (@exempts * 3100) - 4850
   	if @deduction_2 <= 7150
   		select @deduction_2 = @deduction_2 * .1
   	else
       	 select @deduction_2 = ((@deduction_2 - 7150) * .15) + 715
...
GO
GRANT EXECUTE ON  [dbo].[bspPROKT04] TO [public]
GO

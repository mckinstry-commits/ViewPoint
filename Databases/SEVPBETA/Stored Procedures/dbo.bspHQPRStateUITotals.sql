...
***********************************/
 ...
 declare @YTDOutofStateSUIWages bDollar 
 select @YTDOutofStateSUIWages = 0, @OutofStateSUIWagesYN = 'N'
...
if @state = 'ME' 
 ...
 if @state = 'MN'
 	...
 	select @totalempl = count(1), @totalgross = isnull(sum(bPRUE.GrossWages),0),
			...
			@YTDOutofStateSUIWages = sum(p2.YTDOutofStateSUIWages) -- D-03279 CHS
				
 	from bPRUE with (nolock)
 		--This code is borrowed from bspHQPRStateUnemployment.  Can be expanded as needed to pull other amounts.
 	   	Left Join (
			----get year to date   	
			select b.PRCo, b.Employee, sum(b.EligWages) 'YTDOutofStateEligWages', sum(b.SUIWages) 'YTDOutofStateSUIWages' --D-03279 
			from bPRUE b
			where b.PRCo = @prco and b.Quarter = @quarter and b.State <> @state
 	...
 	--D-03279 
 	select @OutofStateSUIWagesYN = case when (@totalsui > 0 and @YTDOutofStateSUIWages > 0) then 'Y' else 'N' end

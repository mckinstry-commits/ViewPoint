...
create proc [dbo].[cvsp_CMS_HQTC_POCD_Update] (@fromco smallint, @toco smallint, 
...
delete bHQTC where Co=@toco  and TableName='bPOCD'
 
---- add new trans
BEGIN TRAN
BEGIN TRY
Insert bHQTC(TableName, Co, Mth, LastTrans) 
select 'bPOCD',POCo, Mth, max(POTrans) 
from bPOCD 
where POCo=@toco 
group by POCo, Mth

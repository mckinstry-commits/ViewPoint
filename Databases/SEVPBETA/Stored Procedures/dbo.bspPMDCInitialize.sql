...
/****** Object:  Stored Procedure dbo.bspPMDCInitialize    Script Date: 8/28/99 9:35:09 AM ******/
   ...
   **************************************/
   ...
   declare @rcode int, @vendorgroup bGroup, @sentdate bDate, @EmailOption char(1)
   ...
   if not exists (select TOP 1 1 from bPMDC with (nolock) where PMCo=@pmco and Project=@project 
   	...
   	-- insert PMDC row
   	insert into bPMDC(PMCo, Project, LogDate, DailyLog, VendorGroup, SentToFirm, SentToContact, DateSent, CC)
   	----#141031
   	values(@pmco, @project, @logdate, @dailylog, @vendorgroup, @senttofirm, @senttocontact, dbo.vfDateOnly(), @EmailOption)
   	if @@rowcount = 0
   		begin

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPROKT03    Script Date: 8/28/99 9:33:32 AM ******/
   CREATE     proc [dbo].[bspPROKT03]
    /********************************************************
    ...
    **********************************************************/
    ...
    select @rcode = 0, @procname = 'bspPROKT03'
    ...
    if @status = 'M' or @status = 'H'
    	begin
   	select @deduction_2 = @annualized_wage - (@exempts * 3050) - 7950
   	...
   	begin
    	select @deduction_2 = @annualized_wage - (@exempts * 3050) - 4750
   	if @deduction_2 <= 6000
   		select @deduction_2 = @deduction_2 * .1
   	else
...
GO
GRANT EXECUTE ON  [dbo].[bspPROKT03] TO [public]
GO

...
SET ANSI_NULLS ON
GO
Create proc [dbo].[bspPR_CA_NUT12]
/********************************************************
...
* 	0 	    	success
*	1 		failure
*GRANT EXECUTE ON bspPR_CA_NUT11 TO public;
**********************************************************/
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NUT12'
   ...
   if @TCP is null select @TCP = 12211

   -- establish tax rate and constant
   select @rate = .115, @KP = 6161
   if @A <= 132406 select @rate = .090, @KP = 2850
   if @A <= 81442 select @rate = .070, @KP = 1222
   if @A <= 40721 select @rate = .040, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NUT12] TO [public]
GO

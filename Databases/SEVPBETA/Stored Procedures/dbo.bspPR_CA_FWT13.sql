...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_FWT13]    Script Date: 02/27/2008 13:19:16 ******/
CREATE PROC [dbo].[bspPR_CA_FWT13]
/********************************************************
* CREATED BY: 	EN	02/27/2008
* MODIFIED BY:	EN	03/07/2008	- #127081 in declare statements change bState to varchar(4)
*				EN	05/18/2009	- #133697 tax update effective 1/1/09
*				EN	05/19/2009	- #133697 tax update effective 4/1/09
...
*				CHS	11/29/2011	- #145154 tax update effective 1/1/2012
*				EN 12/12/2012 B-11794/TK-20172/#147544 tax update effective 1/1/2013
*
* USAGE:
...
**********************************************************/
	...
	SELECT @rcode = 0, @procname = 'bspPR_CA_FWT13'
	...
	IF		@A BETWEEN 0 AND 43561			SELECT @Rate = .15, @K = 0
	ELSE IF	@A BETWEEN 43561.01 AND 87123	SELECT @Rate = .22, @K = 3049
	ELSE IF @A BETWEEN 87123.01 AND 135054	SELECT @Rate = .26, @K = 6534
	ELSE IF @A > 135054						SELECT @Rate = .29, @K = 10586

	-- #142337 if form TD1 was not filed (ie. no filing status entered) use default total claim
	IF @TC IS NULL SELECT @TC = 11038
...
--	END
	...
	SELECT @maxwages = 1117.00 -- used to compute K4 amount
	...
	RETURN @rcode
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_FWT13] TO [public]
GO

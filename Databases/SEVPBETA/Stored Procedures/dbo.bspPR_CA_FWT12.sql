...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_FWT12]    Script Date: 02/27/2008 13:19:16 ******/
CREATE PROC [dbo].[bspPR_CA_FWT12]
/********************************************************
...
**********************************************************/
	...
	SELECT @rcode = 0, @procname = 'bspPR_CA_FWT12'
	...
	IF @A BETWEEN 0 AND 42707 /*41544*/
	BEGIN
		SELECT @Rate = .15, @K = 0
	END
	
	ELSE IF @A BETWEEN 42707 AND 85414
	BEGIN
		SELECT @Rate = .22, @K = 2989
	END
	ELSE IF @A BETWEEN 85414 AND 132406
	BEGIN
		SELECT @Rate = .26, @K = 6406
	END
	ELSE IF @A >= 132406
	BEGIN
		SELECT @Rate = .29, @K = 10378
	END

	-- #142337 if form TD1 was not filed (ie. no filing status entered) use default total claim
	IF @TC IS NULL SELECT @TC = 10822
...
--	END
	...
	SELECT @maxwages = 1095.00 -- used to compute K4 amount
	...
	RETURN @rcode
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_FWT12] TO [public]
GO

...
SET ANSI_NULLS ON
GO
CREATE procedure [dbo].[vspINPhyCountMatlVal]
	/************************************************************************
	* CREATED:	GP 09/10/2008
	* MODIFIED:	
	*
	* Used by the IN Physical Count Worksheet to validate that
	* the material entered does not already exist on this or any 
	* other users worksheets.
	*
    * Also validates Material stocked at the production Location in IN Materital(INMT)
    *
    * Pass:
    *	@inco           IN Company
   ...
   declare @rcode int, @active bYN, @locgroup bGroup, @validcnt int, @stocked bYN, 
   		...
   		@loctaxcode bTaxCode, @incocostmethod int, @UserName bVPUserName
   ...
   if @matlgrp is null
	...
	--Issue 129757, Check if material already exists on another users worksheet.
	IF EXISTS(SELECT TOP 1 1 FROM INCW with (nolock) WHERE INCo = @inco and Loc = @location
		and MatlGroup = @matlgrp and Material = @material) 
	BEGIN
		SELECT @UserName = UserName FROM INCW with (nolock) WHERE INCo = @inco and Loc = @location
			and MatlGroup = @matlgrp and Material = @material

		SELECT @errmsg = 'Some of the materials initialized may already exist on ' + @UserName + char(39) + 's count worksheet!'
	END
...
GO
GRANT EXECUTE ON  [dbo].[vspINPhyCountMatlVal] TO [public]
GO

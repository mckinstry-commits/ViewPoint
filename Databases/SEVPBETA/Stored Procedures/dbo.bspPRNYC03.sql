...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRNYC03    Script Date: 8/28/99 9:33:30 AM ******/
   CREATE proc [dbo].[bspPRNYC03]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @allowance = 1000, @procname = 'bspPRNYC03'
   ...
   if @status = 'S'
   	...
   	if @annualized_wage > 100000 select @tax_addition = 3701, @wage_bracket = 100000, @rate = .052
   	if @annualized_wage > 150000 select @tax_addition = 6301, @wage_bracket = 150000, @rate = .0808
   	if @annualized_wage > 200000 select @tax_addition = 10341, @wage_bracket = 200000, @rate = .052
   	if @annualized_wage > 500000 select @tax_addition = 25941, @wage_bracket = 500000, @rate = .056
   	end
   if @status = 'M'
   	begin
   	if @annualized_wage <= 8000 select @tax_addition = 0, @wage_bracket = 0, @rate = .019
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNYC03] TO [public]
GO

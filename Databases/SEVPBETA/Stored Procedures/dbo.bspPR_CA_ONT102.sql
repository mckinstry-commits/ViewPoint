...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_ONT102]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_ONT102]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_ONT102'
   ...
   select @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @V1 = 0, @S = 0, @Y = 0, @LCP = 0, @T2 = 0, @calcamt = 0, @TCrate = .0505

   -- if form TD1ON was not filed (ie. no filing status entered) use default total claim
   if @TCP is null select @TCP = 8943

   -- establish tax rate and constant
   select @rate = .1116, @KP = 3013
   if @A <= 74214 select @rate = .0915, @KP = 1521
   if @A <= 37106 select @rate = .0505, @KP = 0
   ...
   select @V1 = 0
   if @T4 > 4006 select @V1 = .2 * (@T4 - 4006)
   if @T4 > 5127 select @V1 = @V1 + .36 * (@T4 - 5127)
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_ONT102] TO [public]
GO

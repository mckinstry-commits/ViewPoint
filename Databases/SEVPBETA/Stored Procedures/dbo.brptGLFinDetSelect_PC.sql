...
SET ANSI_NULLS ON
GO
CREATE           PROC [dbo].[brptGLFinDetSelect_PC]
        (@GLCo bCompany, @BegAcct bGLAcct=' ', @EndAcct bGLAcct='zzzzzzzzzz',
        @BegMonth bMonth ='01/01/1950', @EndMonth bMonth = '12/01/2050',
        @Part2 varchar(20)= ' ', @IncludeInactive char(1)='N',  
        ...
        @FYEMO bMonth, @FYBMO bMonth, @EndAcctFull bGLAcct, @ErrorMessage varchar(60))
     	...
     	/* Issue 23660 Remove the #GLDetail table for Beg Balance and add Union statement with RecType='B' 03/04/04 NF */
           ...
           /*Issue 27073 Use the left 2 chars of the Source in restriction.  Also added @Source is null to Beg Bal Section 6/1/05 DH*/
   ...
   select GLAC.GLCo,GLAC.GLAcct,GLAC.Description,GLAC.AcctType,
    ...
    FROM GLAC  as GLAC  with (nolock)
      ...
      where GLAC.GLCo=@GLCo  and GLAC.GLAcct>=@BegAcct and GLAC.GLAcct<=@EndAcctFull
        and (@Part2 is null or CHARINDEX(',' + GLAC.Part2 +',',@Part2)>0) and @Source is null and @Journal is NULL
   ...
   Union All
      ...
      where GLAC.GLCo=@GLCo and GLAC.GLAcct>=@BegAcct and GLAC.GLAcct<=@EndAcctFull and @DetailLevel='A'
        and GLAC.Active=(Case when @IncludeInactive = 'N' then 'Y' else GLAC.Active end)
        and (@Part2 is null or CHARINDEX(',' + GLAC.Part2 + ',',@Part2)>0)
...
GO
GRANT EXECUTE ON  [dbo].[brptGLFinDetSelect_PC] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_ONT12]    Script Date: 02/27/2008 13:19:16 ******/
CREATE  proc [dbo].[bspPR_CA_ONT12]
/********************************************************
...
*				CHS	11/30/2011 tax update effective 01/01/12
*
* USAGE:
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, @addexempts tinyint = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_ONT12'
   ...
   select @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @V1 = 0, @S = 0, 
	@Y = 0, @LCP = 0, @T2 = 0, @calcamt = 0, @TCrate = .0505, @BPA = 217

   -- if form TD1ON was not filed (ie. no filing status entered) use default total claim
   if @TCP is null select @TCP = 9405

   -- establish tax rate and constant
   select @rate = .1116, @KP = 3168
   if @A <= 78043 select @rate = .0915, @KP = 1600
   if @A <= 39020 select @rate = .0505, @KP = 0
   ...
   select @V1 = 0
   if @T4 > 4213 select @V1 = .2 * (@T4 - 4213)
   if @T4 > 5392 select @V1 = @V1 + .36 * (@T4 - 5392)
   ...
   bspexit:
   	return @rcode
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_ONT12] TO [public]
GO

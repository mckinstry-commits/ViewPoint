...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_YTT092]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_YTT092]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_YTT092'
   ...
   if @TCP is null select @TCP = 10375

   -- establish tax rate and constant
   select @rate = .1276, @KP = 4205
   if @A <= 126264 select @rate = .1144, @KP = 2538
   if @A <= 82399 select @rate = .0968, @KP = 1088
   if @A <= 41200 select @rate = .0704, @KP = 0
   ...
   select @K4P = round(case when @TCrate*@A < @TCrate*1044 then @TCrate*@A else @TCrate*1044 end,2)
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_YTT092] TO [public]
GO

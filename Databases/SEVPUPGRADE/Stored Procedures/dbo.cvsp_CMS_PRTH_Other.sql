...
CREATE proc [dbo].[cvsp_CMS_PRTH_Other] (@fromco smallint, @toco smallint, 
...
=========================================================================
	Title:		Timecard Other Earnings (PRTH)
	Created:	06.01.09
	Created by:	CR
	Function:	Xref for Earnings/Deductions/Liabilities in CMS
	Revisions:	1. 6.19.09 - ADB - Ignoring following ADJTYPECODEs: UD, UB, BA
				2. 8.10.09 - CR - changed InsState and InsCode, added nolocks,
					changed PostedDate to use WEEKENDDATE instead of JOURNALDATE.
...
from Viewpoint.dbo.budCustomerDefaults a
	full outer join Viewpoint.dbo.budCustomerDefaults
 b on b.Company=@fromco and a.TableName=b.TableName and a.ColName=b.ColName
where a.Company=0 and a.ColName='JCDept' and a.TableName='bPRTH';

select @defaultCraft=isnull(b.DefaultString,a.DefaultString) 
from Viewpoint.dbo.budCustomerDefaults a
	full outer join Viewpoint.dbo.budCustomerDefaults b on 
	b.Company=@fromco and a.TableName=b.TableName and a.ColName=b.ColName
...
/* OTHER HOURS and AMOUNTS */
insert bPRTH (PRCo, PRGroup,PREndDate,Employee,PaySeq,PostSeq,Type
	,PostDate,JCCo, Job, PhaseGroup, Phase, JCDept,GLCo,TaxState,UnempState
	,InsState,PRDept,Cert,Craft,Class,Shift,EarnCode,Hours,Rate,Amt, BatchId
	,InsCode, udPaidDate, udCMCo, udCMAcct, udCMRef, udTCSource, udSchool,udSource,udConv,udCGCTable,udCGCTableID)
...
select PRCo=@toco
	...
	, PostSeq=isnull((select max(PostSeq) 
			...
			where b.PRCo=@toco
				...
				+ row_number() over(partition by PRTTCH.COMPANYNUMBER, PRTTCH.WkEndDate,
	...
	, Job=dbo.bfMuliPartFormat(right(space(@JobLen) + ltrim(rtrim(PRTTCH.JOBNUMBER)),@JobLen) + 
            left(ltrim(rtrim(PRTTCH.SUBJOBNUMBER)),@SubJobLen),@JobFormat)
	, @PhaseGroup
	, Phase=p.newPhase
...
from CV_CMS_SOURCE.dbo.PRTTCH PRTTCH
	...
	join Viewpoint.dbo.budxrefPREarn prearn on prearn.Company=@fromco and OTHHRSTYPE=prearn.CMSDedCode
...
where PRTTCH.COMPANYNUMBER=@fromco
	and OTHGROSS<>0
order by COMPANYNUMBER, WkEndDate, EMPLOYEENUMBER;

...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_YTT10]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_YTT10]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_YTT10'
   ...
   if @TCP is null select @TCP = 10382

   -- establish tax rate and constant
   select @rate = .1276, @KP = 4200
   if @A <= 127021 select @rate = .1144, @KP = 2524
   if @A <= 81941 select @rate = .0968, @KP = 1082
   if @A <= 40970 select @rate = .0704, @KP = 0
   ...
   select @K4P = round(case when @TCrate*@A < @TCrate*1051 then @TCrate*@A else @TCrate*1051 end,2)
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_YTT10] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRORT07    Script Date: 8/28/99 9:33:32 AM ******/
 CREATE  proc [dbo].[bspPRORT07]
 /********************************************************
 ...
 **********************************************************/
 ...
 select @rcode = 0, @basetax = 0, @dedn = 0, @rate = 0, @procname = 'bspPRORT07'
 ...
 if @fedmax > 5500 select @fedmax = 5500
 ...
 bspcalc: /* calculate Oregon Tax */
	select @a = @a - @stddedn
 	select @amt = ((@basetax + (@a - @dedn) * @rate) - (165 * @exempts)) / @ppds
...
GO
GRANT EXECUTE ON  [dbo].[bspPRORT07] TO [public]
GO

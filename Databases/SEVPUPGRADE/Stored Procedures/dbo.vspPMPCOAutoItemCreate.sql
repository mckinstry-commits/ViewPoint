...
CREATE  proc [dbo].[vspPMPCOAutoItemCreate]
   ...
   @DefaultPCOItem bPCOItem, @PrefillContractItem bYN, @msg varchar(255) output)
   as
   set nocount on
   
declare @rcode int, @currentKeyID varchar(10), @PCOItem bPCOItem, @PCOItemMask varchar(30), @PCOItemLen smallint,
	@Status bStatus, @dummy varchar(30), @nextitem varchar(30), @lastitem bPCOItem,	@UseDefaultPCOItem char(1), @HeaderDesc bItemDesc
...
begin
	...
	--clear params
	select @nextitem = null, @dummy = null, @lastitem = null, @PCOItem = null
	
	--use default pco item OR get next PCO Item
	if @UseDefaultPCOItem = 'Y'
	begin
		set @nextitem = @DefaultPCOItem
	end
	else
	begin
		select @nextitem = cast(isnull(max(PCOItem),0) as numeric) + 1 
		from dbo.PMOI with (nolock) 
		where PMCo = @PMCo and Project = @Project and PCOType = @PCOType and PCO = @PCO
			and substring(ltrim(PCOItem),1,1) not in ('+', '-') and isnumeric(PCOItem) = 1		
	end
	--set item if null & cast as string
	if @nextitem = null		set @nextitem = 1
	set @dummy = cast(@nextitem as varchar(10))
	--format new PCOItem
	...
	begin
		...
		begin
			...
			where PMCo = @PMCo and Project = @Project and PCOType = @PCOType and PCO = @PCO
				and substring(ltrim(PCOItem),1,1) not in ('+', '-') and isnumeric(PCOItem) = 1	
			--set item if null & cast as string
			if @nextitem is null		set @nextitem = 1
			set @dummy = cast(@nextitem as varchar(10))		
		...
		SELECT @PMCo, @Project, @PCOType, @PCO, @PCOItem, sl.PhaseGroup, sl.Phase, sl.JCCType, 0,
					ISNULL(sl.UM,'LS'), isnull(sl.CurUnitCost,0), sl.SL, sl.SLItem, hd.VendorGroup, hd.Vendor, 0,
	...
	begin	
		...
		SELECT @PMCo, @Project, @PCOType, @PCO, @PCOItem, po.PhaseGroup, po.Phase, po.JCCType, 0,
						ISNULL(po.UM,'LS'), isnull(po.CurUnitCost,0), po.PO, po.POItem, hd.VendorGroup, hd.Vendor, 0,
						'E', 'Y', ISNULL(ch.UM,'LS'), 0 , 0
		FROM dbo.POIT po

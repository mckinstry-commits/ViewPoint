...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPRDCT08]    Script Date: 01/10/2008 09:49:01 ******/
     CREATE  proc [dbo].[bspPRDCT08]
     /********************************************************
	...
	*			EN 1/10/08 - issue 126686  update effective 1/1/2008
     ...
     **********************************************************/
     ...
     select @rcode = 0, @procname = 'bspPRDCT08'
     ...
     if @status <> 'F' --Single Filers or Married Persons filing jointly
         begin
         if @a >= 4000 and @a < 10000 select @amt = ((@a - 4000) * .04)
         if @a >= 10000 and @a < 40000 select @amt = (240 + (@a - 10000) * .06)
         if @a >= 40000 select @amt = (2040 + (@a - 40000) * .085)
         end
     if @status = 'F' --Married Persons filing separate retrun or combined separate return
         begin
         if @a >= 2000 and @a < 10000 select @amt = ((@a - 2000) * .04)
         if @a >= 10000 and @a < 40000 select @amt = (320 + (@a - 10000) * .06)
         if @a >= 40000 select @amt = (2120 + (@a - 40000) * .085)
         end
     select @amt = @amt / @ppds
...
GO
GRANT EXECUTE ON  [dbo].[bspPRDCT08] TO [public]
GO

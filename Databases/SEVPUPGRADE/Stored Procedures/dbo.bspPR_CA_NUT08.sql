...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NUT08]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_NUT08]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @calcamt = 0, @procname = 'bspPR_CA_NUT08'
   ...
   select @rate = .115, @KP = 5732
   if @A <= 123184 select @rate = .090, @KP = 2652
   if @A <= 75770 select @rate = .070, @KP = 1137
   if @A <= 37885 select @rate = .040, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NUT08] TO [public]
GO

...
*       Contract		Change Order Request Contract
*		COR				Change Order Request number
...
@DateSent bDate = null, @DateSigned bDate = null, 
...
set nocount on
	...
	IF NOT EXISTS(SELECT TOP 1 1 FROM dbo.vPMDistribution WITH (NOLOCK) WHERE PMCo=@PMCo AND Project=@Project
				...
				AND Contract=@Contract AND COR=@COR)
		BEGIN
			...
			INSERT INTO vPMDistribution(PMCo, Project, Contract, COR, Seq, VendorGroup, SentToFirm, SentToContact, 
					PrefMethod, Send, CC, DateSent, CORID)
			VALUES(@PMCo, @Project, @Contract, @COR, @Seq, @VendorGroup, @SentToFirm, @SentToContact, 
					@PrefMethod, 'Y', @EmailOption, @DateSent, @CORKeyID)

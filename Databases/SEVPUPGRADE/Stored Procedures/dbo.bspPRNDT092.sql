...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPRNDT092]    Script Date: 11/19/2007 09:17:07 ******/
  CREATE  proc [dbo].[bspPRNDT092]
  /********************************************************
  ...
  **********************************************************/
  ...
  select @rcode = 0, @procname = 'bspPRNDT092'
  ...
  if @status = 'S'
  	begin
  	 ...
  	 if @taxincome > 173000 and @taxincome <= 375000 select @basetax = 5664.18, @limit = 173000, @rate = @rate4
  	 if @taxincome > 375000 select @basetax = 14592.58, @limit = 375000, @rate = @rate5
  	end
  ...
  if @status = 'M'
  	begin
  	 ...
  	 if @taxincome > 217000 and @taxincome <= 380000 select @basetax = 6621.18, @limit = 217000, @rate = @rate4
  	 if @taxincome > 380000 select @basetax = 13825.78, @limit = 380000, @rate = @rate5
  	end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNDT092] TO [public]
GO

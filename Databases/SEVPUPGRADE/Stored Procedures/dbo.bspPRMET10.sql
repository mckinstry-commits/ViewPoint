...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPRMET10]    Script Date: 10/26/2007 10:20:46 ******/
   CREATE proc [dbo].[bspPRMET10]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @dedn = 2850, @procname = 'bspPRMET10'
   ...
   if @status = 'S' 
   	begin
   		...
   		if @annualized_wage > 7800 select @tax_addition = 99, @wage_bracket = 7800, @rate = .045
   		if @annualized_wage > 12700 select @tax_addition = 320, @wage_bracket = 12700, @rate = .07
   		if @annualized_wage > 22600 select @tax_addition = 1013, @wage_bracket = 22600, @rate = .085
   end
   
   /* married wage table and tax */
   if @status <> 'S'
   	begin
   		if @annualized_wage <= 6700 goto bspcalc
   		if @annualized_wage > 6700 select @wage_bracket = 6700, @rate = .02
   		if @annualized_wage > 16650 select @tax_addition = 199, @wage_bracket = 16650, @rate = .045
   		if @annualized_wage > 26450 select @tax_addition = 640, @wage_bracket = 26450, @rate = .07
   		if @annualized_wage > 46250 select @tax_addition = 2026, @wage_bracket = 46250, @rate = .085
   end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMET10] TO [public]
GO

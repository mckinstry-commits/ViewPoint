...
BEGIN --compute for wages below the Wage Break
	...
	BEGIN
		SELECT @StandardDeduction = 1980

		SELECT @AnnualTaxable = @AnnualWages - @FedMax - @StandardDeduction

		IF @AnnualTaxable BETWEEN 0 AND 3099.99
		BEGIN
			SELECT @BaseTaxAmt = 179, @BracketBaseAmt = 0, @Rate = .05
		END
		ELSE IF @AnnualTaxable BETWEEN 3100 AND 7749.99
		BEGIN
			SELECT @BaseTaxAmt = 334, @BracketBaseAmt = 3100, @Rate = .07
		END
		ELSE If @AnnualTaxable BETWEEN 7750 AND 124999.99
		BEGIN
			SELECT @BaseTaxAmt = 660, @BracketBaseAmt = 7750, @Rate = .09
		END
	END
	--all others (includes married and single with 3 or more allowances)
	ELSE
	BEGIN
		SELECT @StandardDeduction = 3960

		SELECT @AnnualTaxable = @AnnualWages - @FedMax - @StandardDeduction

		IF @AnnualTaxable BETWEEN 0 AND 6199.99
		BEGIN
			SELECT @BaseTaxAmt = 179, @BracketBaseAmt = 0, @Rate = .05
		END
		ELSE IF @AnnualTaxable BETWEEN 6200 AND 15499.99
		BEGIN
			SELECT @BaseTaxAmt = 489, @BracketBaseAmt = 6200, @Rate = .07
		END
		ELSE If @AnnualTaxable BETWEEN 15500 AND 249999.99
		BEGIN
			SELECT @BaseTaxAmt = 1140, @BracketBaseAmt = 15500, @Rate = .09
		END
	END
END
...
BEGIN --compute tax for wages on or above the wage break
	...
	IF @status = 'S' AND @exempts < 3
	BEGIN
		IF @AnnualWages BETWEEN 50000 AND 124999.99 SELECT @PhaseOut = 5950
		ELSE IF @AnnualWages BETWEEN 125000 AND 129999.99 SELECT @PhaseOut = 4750
		ELSE IF @AnnualWages BETWEEN 130000 AND 134999.99 SELECT @PhaseOut = 3550
		ELSE IF @AnnualWages BETWEEN 135000 AND 139999.99 SELECT @PhaseOut = 2350
		ELSE IF @AnnualWages BETWEEN 140000 AND 144999.99 SELECT @PhaseOut = 1150
		ELSE IF @AnnualWages >= 145000 SELECT @PhaseOut = 0
	END
	ELSE
	BEGIN
		IF @AnnualWages BETWEEN 50000 AND 249999.99 SELECT @PhaseOut = 5950
		ELSE IF @AnnualWages BETWEEN 250000 AND 259999.99 SELECT @PhaseOut = 4750
		ELSE IF @AnnualWages BETWEEN 260000 AND 269999.99 SELECT @PhaseOut = 3550
		ELSE IF @AnnualWages BETWEEN 270000 AND 279999.99 SELECT @PhaseOut = 2350
		ELSE IF @AnnualWages BETWEEN 280000 AND 289999.99 SELECT @PhaseOut = 1150
	...
	BEGIN
		...
		IF @AnnualTaxable BETWEEN 7750 AND 124999.99
		BEGIN
			SELECT @BaseTaxAmt = 481, @BracketBaseAmt = 7750, @Rate = .09
		END
		ELSE IF @AnnualTaxable BETWEEN 125000 AND 249999.99
		BEGIN
			SELECT @BaseTaxAmt = 11033, @BracketBaseAmt = 125000, @Rate = .108
		END
		ELSE If @AnnualTaxable >= 250000
		BEGIN
			SELECT @BaseTaxAmt = 24533, @BracketBaseAmt = 250000, @Rate = .11
		END
	END
	--all others (includes married and single with 3 or more allowances)
	ELSE
	BEGIN
		...
		IF @AnnualTaxable BETWEEN 15500 AND 249999.99
		BEGIN
			SELECT @BaseTaxAmt = 961, @BracketBaseAmt = 15500, @Rate = .09
		END
		ELSE IF @AnnualTaxable BETWEEN 250000 AND 499999.99
		BEGIN
			SELECT @BaseTaxAmt = 22066, @BracketBaseAmt = 250000, @Rate = .108
		END
		ELSE If @AnnualTaxable >= 500000
		BEGIN
			SELECT @BaseTaxAmt = 49066, @BracketBaseAmt = 500000, @Rate = .11
		END
	END
END
...
GO
GRANT EXECUTE ON  [dbo].[bspPRORT11] TO [public]
GO

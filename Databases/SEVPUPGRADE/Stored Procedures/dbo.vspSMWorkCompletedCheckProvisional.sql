...
AS
	...
	DECLARE @SMWorkCompletedID bigint, @SMCo_upper bCompany, @WorkOrder int, @Scope int, @LineType tinyint, 
		...
		@Revision int,	@Coverage varchar(1), @MaterialGroup bGroup, @Material bMatl, @MaterialUM bUM,
	...
	BEGIN	
		...
		Select @Agreement = @Agreement, @Revision = @Revision, @Coverage = @Coverage
		...
		BEGIN
			...
			BEGIN
				...
				BEGIN
					...
					SELECT @PriceRate = Rate FROM dbo.vfSMRateLabor(@SMCo_upper, @WorkOrder, @Scope, @Date, @Agreement, @Revision, @Coverage, @PRCo, @PayType, @Craft, @Class, @Technician)
			...
			BEGIN
				...
				BEGIN
					...
					FROM dbo.vfSMGetStandardItemRate(@SMCo_upper, @WorkOrder, @Scope, @Date, @StandardItem, @Agreement, @Revision, @Coverage)
			...
			BEGIN
				...
				FROM dbo.vfSMRatePurchase(@SMCo_upper, @WorkOrder, @Scope, @Date, @Agreement, @Revision, @Coverage, @MaterialGroup, @Material, @MaterialUM, @Quantity, @ProjCost, @PriceUM, @PriceECM)
			END
			ELSE
			BEGIN
				-- Not Implemented.
			...
			BEGIN TRY
				...
				UPDATE vSMWorkCompletedDetail 
					SET PriceRate=@PriceRate, PriceTotal=ISNULL(@PriceTotal,0), GLCo=@SMGLCo,
					CostWIPAccount=@DefaultCostWIPAcct, RevenueWIPAccount=@DefaultRevWIPAcct,

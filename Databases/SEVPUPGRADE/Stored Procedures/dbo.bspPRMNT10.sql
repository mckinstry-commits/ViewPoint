...
/****** Object:  StoredProcedure [dbo].[bspPRMNT10]    Script Date: 11/16/2007 15:30:08 ******/
    ...
    **********************************************************/
    ...
    select @rcode = 0, @dedn = 3650, @procname = 'bspPRMNT10'
    ...
    if @status = 'S'
    	begin
    		...
    		if @annualized_wage > 24820 select @tax_addition = 1218.20, @wage_bracket = 24820, @rate = .0705
    		if @annualized_wage > 76830 select @tax_addition = 4884.91, @wage_bracket = 76830, @rate = .0785
    end
    ...
    if @status = 'M'
    	begin
    		...
    		if @annualized_wage > 41030 select @tax_addition = 1780.48, @wage_bracket = 41030, @rate = .0705
    		if @annualized_wage > 139970 select @tax_addition = 8755.75, @wage_bracket = 139970, @rate = .0785
    end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMNT10] TO [public]
GO

...
* DateSent		date sent (may be null)
*
* Returns:
* MSG if Error
*
...
(@IssueKeyID BIGINT = NULL, @SentToFirm bFirm = null,
 @SentToContact bEmployee = null, @DateSent bDate = null,
...
SELECT @PMCo=PMCo, @Project=Project, @IssueType = Type, @Issue = Issue,
		@DateSent = DateInitiated, @VendorGroup = VendorGroup
...
----for now use the system date for date sent - may change later
...
IF NOT EXISTS(SELECT TOP 1 1 FROM dbo.vPMDistribution WHERE PMCo=@PMCo AND Project=@Project
	...
	insert into vPMDistribution(PMCo, Project, IssueType, Issue, Seq, VendorGroup,
			SentToFirm, SentToContact, PrefMethod, Send, CC, DateSent, IssueID)
	values(@PMCo, @Project, @IssueType, @Issue, @Seq, @VendorGroup, @SentToFirm,
			@SentToContact, @PrefMethod, 'Y', @EmailOption, @DateSent, @IssueKeyID)
	if @@rowcount = 0
		begin

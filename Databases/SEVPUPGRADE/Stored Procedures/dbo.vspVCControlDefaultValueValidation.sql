...
-- =============================================
CREATE PROCEDURE [dbo].[vspVCControlDefaultValueValidation]
	(@defaultValue AS VARCHAR(60),
	@cacheParametersAllowed AS BIT,
...
BEGIN
	...
	IF @cacheParametersAllowed <> 1 AND SUBSTRING(@defaultValue, 1, 1) = '#' AND SUBSTRING(@defaultValue, LEN(@defaultValue), 1) = '#'
	BEGIN
	...
	SELECT @msg = LabelText
	FROM pvPortalControlDetailsFields
	WHERE PortalControlID = @portalControlID AND '#' + ColumnName + '#' = @defaultValue
...
GO
GRANT EXECUTE ON  [dbo].[vspVCControlDefaultValueValidation] TO [public]
GO

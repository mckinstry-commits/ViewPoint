...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPRMNT12]    Script Date: 11/16/2007 15:30:08 ******/
CREATE proc [dbo].[bspPRMNT12]
/********************************************************
...
**********************************************************/
    ...
    select @rcode = 0, @dedn = 3800, @procname = 'bspPRMNT12'
    ...
    if @status = 'S'
    	begin
    		if @annualized_wage <= 2150 goto bspexit
    		if @annualized_wage > 2150 select @wage_bracket = 2150, @rate = .0535
    		if @annualized_wage > 25820 select @tax_addition = 1266.35, @wage_bracket = 25820, @rate = .0705
    		if @annualized_wage > 79880 select @tax_addition = 5077.58, @wage_bracket = 79880, @rate = .0785
    end
    
    /* married wage table and tax */
    if @status = 'M'
    	begin
    		if @annualized_wage <= 6100 goto bspexit
    		if @annualized_wage > 6100 select @wage_bracket = 6100, @rate = .0535
    		if @annualized_wage > 40690 select @tax_addition = 1850.57, @wage_bracket = 40690, @rate = .0705
    		if @annualized_wage > 143530 select @tax_addition = 9100.79, @wage_bracket = 143530, @rate = .0785
    end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMNT12] TO [public]
GO

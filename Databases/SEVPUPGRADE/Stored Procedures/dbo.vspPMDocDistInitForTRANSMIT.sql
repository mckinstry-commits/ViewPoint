...
/***********************************************************************/
CREATE procedure [dbo].[vspPMDocDistInitForTRANSMIT]
/************************************************************************
...
*				GF 09/16/2008 - issue #129926 added order by for a.DocType, a.Document
...
*				GF 10/30/2009 - issue #136339 change order by when transmittal document seq is used.
...
* PM Transmittal Document and load email, fax, CC addresses, header and query strings.
* Called from frmPMTransmittal form.
*
*
*
* Input parameters:
* PM Company
* Project
* Document Category	will always be 'TRANSMIT'
* User Name
* Document
...
(@pmco bCompany, @project bProject, @doccategory varchar(10), @user bVPUserName,
 @document bDocument, @template varchar(40) = null, @filename varchar(255) = null,
...
---- get document data
select @description=Subject, @sourcekeyid=KeyID
from dbo.PMTM with (nolock)
where PMCo=@pmco and Project=@project and Transmittal=@document
if @@rowcount = 0
	begin
	select @msg = 'Invalid Pending Change Order', @rcode = 1
	goto bspexit
	end
...
if not exists(select Seq from dbo.PMTC where PMCo=@pmco and Project=@project
						and Transmittal=@document and Send='Y' and CC='N')
	begin
...
declare bcPMTC cursor LOCAL FAST_FORWARD
for select VendorGroup, SentToFirm, SentToContact, PrefMethod
from PMTC
where PMCo=@pmco and Project=@project and Transmittal=@document and Send='Y' and CC='N'
...
PMTC_loop:
fetch next from bcPMTC into @vendorgroup, @senttofirm, @senttocontact, @prefmethod

if @@fetch_status = -1 goto PMTC_end
if @@fetch_status <> 0 goto PMTC_loop
...
if ltrim(rtrim(isnull(@template,''))) <> ''
	...
	exec @rcode = dbo.bspHQWDJoinClauseBuild 'TRANSMIT', 'N', 'Y', 'N', @joinstring output, @msg output
	if @rcode <> 0 goto bspexit
	...
	select @value = @value + ' and a.Transmittal = ' + CHAR(39) + @document + CHAR(39)
	...
	if @needitemquery = 'Y'
		...
		if charindex('a.Seq', @groupby) > 0
			begin
			select @value = @value + ' group by a.Seq, a.DocType, a.Document, ' + @groupby
			end
		else
			begin
			select @value = @value + ' group by a.DocType, a.Document, ' + @groupby
			end
...
if isnull(@usestdsubject,'Y') = 'Y'
	begin
	set @subjectline = 'Transmittal: ' + isnull(@document,'') + ' - ' + isnull(@description,'')
	end
else
	begin
	...
	exec @retcode = dbo.vspPMDocCatSubjectLineCreate @pmco, @doccategory, @ovrsubject, @project,
						----TK-03298
						null, @document, null, null, null, null, @contract, @subjectline output
	if isnull(@subjectline,'') = '' set @subjectline = 'Transmittal: ' + isnull(@document,'') + ' - ' + isnull(@description,'')
	end
...
insert PMHI(SourceTableName, SourceKeyId, CreatedBy, VendorGroup,
			...
			bCCAddresses)
select 'PMTM', @sourcekeyid, @user, @vendorgroup, @senttofirm, @senttocontact,
		@email, @fax, @faxaddress,
		'Transmittal: ' + isnull(@document,'') + ' - ' + isnull(@description,''),
		@ccaddr, @bccaddr
if @@rowcount = 0
	begin
...
GO
GRANT EXECUTE ON  [dbo].[vspPMDocDistInitForTRANSMIT] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_ONT11]    Script Date: 02/27/2008 13:19:16 ******/
CREATE  proc [dbo].[bspPR_CA_ONT11]
/********************************************************
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, @addexempts tinyint = 0, 
	@PP bDollar = 0, @maxCPP bDollar = 0, @EI bDollar = 0, @maxEI bDollar = 0, 
	@K3P bDollar = 0, @capstock bDollar = 0, @calcamt bDollar = 0 output,
   ...
   select @rcode = 0, @procname = 'bspPR_CA_ONT11'
   ...
   select @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @V1 = 0, @S = 0, 
	@Y = 0, @LCP = 0, @T2 = 0, @calcamt = 0, @TCrate = .0505, @BPA = 210

   -- if form TD1ON was not filed (ie. no filing status entered) use default total claim
   if @TCP is null select @TCP = 9104 --8943

   -- establish tax rate and constant
   select @rate = .1116, @KP = 3067
   if @A <= 75550 select @rate = .0915, @KP = 1549
   if @A <= 37774 select @rate = .0505, @KP = 0
   ...
   select @V1 = 0
   if @T4 > 4078 select @V1 = .2 * (@T4 - 4078)
   if @T4 > 5219 select @V1 = @V1 + .36 * (@T4 - 5219)
   ...
   select @Y = (389 * @addexempts)
   --select @S = case when @T4+@V1 < (2*(206+@Y))-(@T4+@V1) then @T4+@V1 else (2*(206+@Y))-(@T4+@V1) end   
   ...
   select @LCP = case when .1 * @capstock < 750 then .1 * @capstock else 750 end
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_ONT11] TO [public]
GO

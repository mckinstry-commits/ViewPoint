...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMAT98    Script Date: 8/28/99 9:33:26 AM ******/
   CREATE   proc [dbo].[bspPRMAT98]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @rate = .0595, @procname = 'bspPRMAT98'
   ...
   if @total_exempts = 1
   	select @annualized_wage = @annualized_wage - 2200
   
   if @total_exempts >1
   	select @annualized_wage = @annualized_wage - (@total_exempts * 1000) - 1200
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMAT98] TO [public]
GO

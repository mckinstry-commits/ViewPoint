...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMOT03    Script Date: 8/28/99 9:33:28 AM ******/
   CREATE       proc [dbo].[bspPRMOT03]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @rate = 0, @counter = 1, @procname = 'bspPRMOT03', @allowances = 0
   ...
   if @status = 'S'
   begin
   	select @dedn = 4750, @fedlimit = 5000, @allowances = 1200 * @exempts
   ...
   if @status = 'M'
   begin
   	select @dedn = 7950, @fedlimit = 10000, @allowances = 1200 * @exempts
   ...
   if @status = 'B'
   begin
   	select @dedn = 3975, @fedlimit = 5000, @allowances = 1200 * @exempts
   ...
   if @status = 'H'
   begin
   	select @dedn = 7000, @fedlimit = 5000
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMOT03] TO [public]
GO

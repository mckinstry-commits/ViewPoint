...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NBT092]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_NBT092]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NBT092'
   ...
   select @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @LCP = 0, @T2 = 0, @calcamt = 0, @TCrate = .0918

   -- if form TD1NB was not filed (ie. no filing status entered) use default total claim
   if @TCP is null select @TCP = 8605

   -- establish tax rate and constant
   select @rate = .1605, @KP = 3736
   if @A <= 116105 select @rate = .1520, @KP = 2749
   if @A <= 71415 select @rate = .1352, @KP = 1550
   if @A <= 35707 select @rate = .0918, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NBT092] TO [public]
GO

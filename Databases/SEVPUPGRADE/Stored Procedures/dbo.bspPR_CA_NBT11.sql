...
Create  proc [dbo].[bspPR_CA_NBT11]
/********************************************************
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NBT11'
   ...
   if @TCP is null select @TCP = 8953

   -- establish tax rate and constant
   select @rate = .127, @KP = 1700
   if @A <= 120796 select @rate = .124, @KP = 1337
   if @A <= 74300 select @rate = .121, @KP = 1115
   if @A <= 37150 select @rate = .091, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NBT11] TO [public]
GO

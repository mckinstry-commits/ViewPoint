...
/****** Object:  Stored Procedure dbo.bspEMCostTypeValForCostCode    Script Date: 12/13/2001 11:07:01 AM ******/
    CREATE       proc [dbo].[bspEMCostTypeValForCostCode]
    /***********************************************************
     * CREATED BY:	JM 12/29/98
     * MODIFIED By: JM 1/16/99 - Added return of GLTransAcct.
     *              JM 10/23/99 - Added input param @forcevalidglacct to allow forms that
     *							use this procedure to make the existence of a valid GLTransAcct optional.
     *              RM 02/28/01 - Changed Cost type to varchar(10)
     *              kb 8/14/1 - issue #14322
	...
	*		TJL 01/29/09 - Issue #130083, EM CostType no longer clears on F3 default if CostCode missing.
    ...
    (@emco bCompany=null, @emgroup bGroup = null, @costtype varchar(10) = null,
     ...
     @costtypeout bEMCType output, @gltransacct bGLAcct output,@msg varchar(255) output)
    as
    set nocount on
...
GO
GRANT EXECUTE ON  [dbo].[bspEMCostTypeValForCostCode] TO [public]
GO

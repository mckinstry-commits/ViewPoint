...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NLT10]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_NLT10]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NLT10'
   ...
   select @rate = .155, @KP = 3284
   if @A <= 62556 select @rate = .128, @KP = 1595
   if @A <= 31278 select @rate = .077, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NLT10] TO [public]
GO

...
/****** Object:  Stored Procedure dbo.bspPREmplValEmplMasterName    Script Date: 8/28/99 9:33:19 AM ******/
   CREATE    proc [dbo].[vspPREmplValEmplMasterNameForJC]
	...
	*				DANF 05/16/2006 - Added for JC Cost Adjustment recode so that non valid employee numbers are returned to the UI.
    *
    * Usage:
   ...
   @empl varchar(15),
   @activeopt varchar(1),
   @emplout varchar(15)=null output,
      	...
      	if @@rowcount = 0
   		...
   		if @@rowcount = 0
               ...
               select @msg = 'Not a valid Employee', @rcode = 1
				if dbo.bfIsInteger(@empl) = 1 select @emplout = @empl
   			goto bspexit
   	   		end
   		end
   	end
...
GO
GRANT EXECUTE ON  [dbo].[vspPREmplValEmplMasterNameForJC] TO [public]
GO

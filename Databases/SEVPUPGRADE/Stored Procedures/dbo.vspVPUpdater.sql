...
SET @dd = 'vDDCB,vDDCI,vDDCS,vDDDT,vDDFH,vDDFI,vDDFL,vDDFR,'
		...
		+ 'vDDCustomActionParameters,vDDCustomActionRecordTypes,vDDCustomActions,'
		+ 'vDDCustomActionGroup,vDDCustomGroups,vDDCustomRecordTypes,vDDCustomRecordTypeGroups,'
		...
		+ 'vCalendar,'
		-- Audit tables
		+ 'vAuditFlags,vAuditTables,vAuditFlagTables,vAuditColumns,'
		-- HQ Approval Process
		+ 'vHQApprovalModule,'
		-- Assembly Dependency 
...
BEGIN TRY
	...
	SET @tsql = 'insert [' + @destdb + '].dbo.bDDUF select * from [' + @sourcedb
		+ '].dbo.bDDUF where substring(Form,1,2) <> ' + @quote + 'ud' + @quote
	EXEC(@tsql)

	SET @tsql = 'insert [' + @destdb + '].dbo.bDDUD select * from [' + @sourcedb
		+ '].dbo.bDDUD where substring(ColumnName,1,2) <> ' + @quote + 'ud'
		+ @quote
	EXEC(@tsql)
	...
	SET @tsql = 'insert [' + @destdb + '].dbo.vRPRF select * from [' + @sourcedb + '].dbo.vRPRF where ReportID < 10000'
	exec(@tsql)

	-- add any new Modules to vDDMO
	SET @tsql = 'INSERT [' + @destdb
		+ '].dbo.vDDMO ([Mod], Title, Active, LicLevel ) SELECT [Mod], Title, '
		+ @quote + 'N' + @quote + ', LicLevel from [' + @sourcedb
	...
	SELECT  @tsql = 'UPDATE destDDMO SET [HelpKeyword] = srcDDMO.[HelpKeyword] FROM ['
			+ @destdb + '].dbo.vDDMO destDDMO JOIN [' + @sourcedb
			+ '].dbo.vDDMO srcDDMO ON srcDDMO.[Mod] ' + @destCollation + ' = destDDMO.[Mod]'
	EXEC(@tsql)

	-- update Version # to vDDVS
	SET @tsql = 'update [' + @destdb + '].dbo.vDDVS set Version = (select Version ' + @destCollation + ' from ['
		+ @sourcedb + '].dbo.vDDVS)' 
	exec(@tsql)
...
-- add any new message sources to vWFMailSources
	SET @tsql = 'INSERT INTO [' + @destdb + '].dbo.vWFMailSources SELECT [Source] FROM [' + @sourcedb
	...
	SET @tsql = 'truncate table [' + @destdb + '].dbo.bWDQY' 
	exec(@tsql)

	SET @tsql = 'truncate table [' + @destdb + '].dbo.bWDQP'
	exec(@tsql)
	
	--139114 - adding the bHQAX tab
	BEGIN TRY
		SET @tsql = 'TRUNCATE TABLE [' + @destdb + '].dbo.bHQAX;' + CHAR(13) + CHAR(10)
...
begin try
	SET @tsql = 'truncate table [' + @destdb + '].dbo.bWDQF'
	exec(@tsql)
end try
begin catch
...
begin try
	SET @tsql = 'alter table [' + @destdb + '].dbo.bWDQP disable trigger all; insert [' + @destdb + '].dbo.bWDQP (QueryName, Param, Description, UniqueAttchID) select QueryName, Param, Description, UniqueAttchID from [' + @sourcedb + '].dbo.bWDQP; alter table ['

 + @destdb + '].dbo.bWDQP enable trigger all;'
	exec(@tsql)
end try
begin catch
...
begin try
	SET @tsql = 'alter table [' + @destdb + '].dbo.bWDQF disable trigger all; insert [' + @destdb + '].dbo.bWDQF (QueryName, Seq, TableColumn, EMailField, IsKeyField) select QueryName, Seq, TableColumn, EMailField, IsKeyField from [' + @sourcedb + '].dbo.bWDQF; alter table [' + @destdb + '].dbo.bWDQF enable trigger all;'
	exec(@tsql)
end try
begin catch
...
begin try
	SET @tsql = 'truncate table [' + @destdb + '].dbo.bHQAD'
	exec(@tsql)
end try
begin catch
...
begin try
	SET @tsql = 'insert [' + @destdb + '].dbo.bHQAD (RecID, ColumnName, Description, ParentColumn, Custom, Module, Form, SkipIndex) select RecID, ColumnName, Description, ParentColumn, Custom, Module, Form, SkipIndex from [' + @sourcedb + '].dbo.bHQAD d where d.Custom = ' + @quote + 'N' + @quote + ';'
	exec(@tsql)
end try
begin catch
...
BEGIN TRY
	SET @tsql = 'IF NOT EXISTS (SELECT 1 FROM [' + @destdb + '].dbo.vHQBASReasonCodes) BEGIN ' +
			...
			+ 'FROM [' + @sourcedb + '].dbo.vHQBASReasonCodes '
			+ 'END'
	EXEC(@tsql)
END TRY 
BEGIN CATCH 
...
BEGIN TRY
	...
	SET @tsql = 'SET IDENTITY_INSERT [' + @destdb + '].dbo.vVPPartFormChangedMessages ON;
				 ...
				 FROM [' + @sourcedb + '].dbo.vVPPartFormChangedMessages WHERE KeyID < 1000000;
	...
	SET @tsql = 'SET IDENTITY_INSERT [' + @destdb + '].dbo.vVPPartFormChangedParameters ON;
				 ...
				 FROM [' + @sourcedb + '].dbo.vVPPartFormChangedParameters WHERE KeyID < 1000000;

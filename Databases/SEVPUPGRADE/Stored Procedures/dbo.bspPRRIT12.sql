...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPRRIT12]    Script Date: 12/18/2007 08:04:43 ******/
CREATE Proc [dbo].[bspPRRIT12]
/********************************************************
...
IF (@subjamt * @ppds) > 202700 SELECT @allowance = 0
...
BEGIN
	IF @annualized_wage <= 57150.00 SELECT @tax_addition =    0.00, @wage_bracket =      0.00, @rate = .0375
	IF @annualized_wage >  57150.00 SELECT @tax_addition = 2143.13, @wage_bracket =  57150.00, @rate = .0475
	IF @annualized_wage > 129900.00 SELECT @tax_addition = 5598.75, @wage_bracket = 129900.00, @rate = .0599
END
...
RETURN @rcode
GO
GRANT EXECUTE ON  [dbo].[bspPRRIT12] TO [public]
GO

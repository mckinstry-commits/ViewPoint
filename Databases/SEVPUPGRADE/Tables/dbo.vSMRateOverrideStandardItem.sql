	...
	(
		...
		, KeyString		varchar(max)
							...
							SELECT
								...
								'<KeyString SMCo = "' + REPLACE(CAST(ISNULL(deleted.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(deleted.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 
								deleted.SMCo , 
								'D' , 
								'AutoAdd' , 
				...
				FROM deleted AS d
				INNER JOIN @HQMAKeys AS Keys ON Keys.KeyString = '<KeyString SMCo = "' + REPLACE(CAST(ISNULL(d.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(d.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />'
...
-- log additions to the AutoAdd column
							...
							SELECT 
								'vSMRateOverrideStandardItem' , 
								'<KeyString SMCo = "' + REPLACE(CAST(ISNULL(inserted.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(inserted.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 
...
-- log additions to the StandardItem column
 ...
 INSERT INTO HQSA ( AuditID, Datatype, Qualifier, Instance, TableName)
				...
				FROM inserted AS i
				INNER JOIN @HQMAKeys AS Keys ON Keys.KeyString = '<KeyString SMCo = "' + REPLACE(CAST(ISNULL(i.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(i.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />'
	...
	(
		...
		, KeyString		varchar(max)
							...
							INSERT dbo.bHQMA (	TableName, 
																	...
																	UserName)
										OUTPUT inserted.AuditID, inserted.KeyString INTO @HQMAKeys (AuditID, KeyString) 
								SELECT 							'vSMRateOverrideStandardItem' , 								'<KeyString SMCo = "' + REPLACE(CAST(ISNULL(inserted.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(inserted.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 								inserted.SMCo , 								'C' , 								'AutoAdd' , 								CONVERT(VARCHAR(MAX), deleted.[AutoAdd]) , 								CONVERT(VARCHAR(MAX), inserted.[AutoAdd]) , 								GETDATE() , 								SUSER_SNAME()
							FROM inserted
								INNER JOIN deleted
							...
							INSERT dbo.bHQMA (	TableName, 
																	...
																	UserName)
										OUTPUT inserted.AuditID, inserted.KeyString INTO @HQMAKeys (AuditID, KeyString) 
								SELECT 							'vSMRateOverrideStandardItem' , 								'<KeyString SMCo = "' + REPLACE(CAST(ISNULL(inserted.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(inserted.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 								inserted.SMCo , 								'C' , 								'BillableRate' , 								CONVERT(VARCHAR(MAX), deleted.[BillableRate]) , 								CONVERT(VARCHAR(MAX), inserted.[BillableRate]) , 								GETDATE() , 								SUSER_SNAME()
							FROM inserted
								INNER JOIN deleted
									ON  inserted.[SMRateOverrideStandardItemID] = deleted.[SMRateOverrideStandardItemID] 
							...
							INSERT dbo.bHQMA (	TableName, 
																	...
																	UserName)
										OUTPUT inserted.AuditID, inserted.KeyString INTO @HQMAKeys (AuditID, KeyString) 
								SELECT 							'vSMRateOverrideStandardItem' , 								'<KeyString SMCo = "' + REPLACE(CAST(ISNULL(inserted.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(inserted.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 								inserted.SMCo , 								'C' , 								'SMCo' , 								CONVERT(VARCHAR(MAX), deleted.[SMCo]) , 								CONVERT(VARCHAR(MAX), inserted.[SMCo]) , 								GETDATE() , 								SUSER_SNAME()
							FROM inserted
								INNER JOIN deleted
									ON  inserted.[SMRateOverrideStandardItemID] = deleted.[SMRateOverrideStandardItemID] 
							...
							INSERT dbo.bHQMA (	TableName, 
																	...
																	UserName)
										OUTPUT inserted.AuditID, inserted.KeyString INTO @HQMAKeys (AuditID, KeyString) 
								SELECT 							'vSMRateOverrideStandardItem' , 								'<KeyString SMCo = "' + REPLACE(CAST(ISNULL(inserted.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(inserted.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 								inserted.SMCo , 								'C' , 								'SMRateOverrideID' , 								CONVERT(VARCHAR(MAX), deleted.[SMRateOverrideID]) , 								CONVERT(VARCHAR(MAX), inserted.[SMRateOverrideID]) , 								GETDATE() , 								SUSER_SNAME()
							FROM inserted
								INNER JOIN deleted
									ON  inserted.[SMRateOverrideStandardItemID] = deleted.[SMRateOverrideStandardItemID] 
							...
							INSERT dbo.bHQMA (	TableName, 
																	...
																	UserName)
										OUTPUT inserted.AuditID, inserted.KeyString INTO @HQMAKeys (AuditID, KeyString) 
								SELECT 							'vSMRateOverrideStandardItem' , 								'<KeyString SMCo = "' + REPLACE(CAST(ISNULL(inserted.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(inserted.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 								inserted.SMCo , 								'C' , 								'SMRateOverrideStandardItemID' , 								CONVERT(VARCHAR(MAX), deleted.[SMRateOverrideStandardItemID]) , 								CONVERT(VARCHAR(MAX), inserted.[SMRateOverrideStandardItemID]) , 								GETDATE() , 								SUSER_SNAME()
							FROM inserted
								INNER JOIN deleted
									ON  inserted.[SMRateOverrideStandardItemID] = deleted.[SMRateOverrideStandardItemID] 
							...
							INSERT dbo.bHQMA (	TableName, 
																	...
																	UserName)
										OUTPUT inserted.AuditID, inserted.KeyString INTO @HQMAKeys (AuditID, KeyString) 
								SELECT 							'vSMRateOverrideStandardItem' , 								'<KeyString SMCo = "' + REPLACE(CAST(ISNULL(inserted.[SMCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SMRateOverrideID = "' + REPLACE(CAST(ISNULL(inserted.[SMRateOverrideID],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 								inserted.SMCo , 								'C' , 								'StandardItem' , 								CONVERT(VARCHAR(MAX), deleted.[StandardItem]) , 								CONVERT(VARCHAR(MAX), inserted.[StandardItem]) , 								GETDATE() , 								SUSER_SNAME()
							FROM inserted
								INNER JOIN deleted
									ON  inserted.[SMRateOverrideStandardItemID] = deleted.[SMRateOverrideStandardItemID] 
...
GO
ALTER TABLE [dbo].[vSMRateOverrideStandardItem] ADD CONSTRAINT [IX_vSMRateOverrideStandardItem_SMRateOverrideID_SMCo_StandardItem] UNIQUE NONCLUSTERED  ([SMRateOverrideID], [SMCo], [StandardItem]) ON [PRIMARY]
GO
ALTER TABLE [dbo].[vSMRateOverrideStandardItem] WITH NOCHECK ADD CONSTRAINT [FK_vSMRateOverrideStandardItem_vSMRateOverride] FOREIGN KEY ([SMRateOverrideID]) REFERENCES [dbo].[vSMRateOverride] ([SMRateOverrideID]) ON DELETE CASCADE
GO

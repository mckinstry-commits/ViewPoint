...
BEGIN
	...
	SELECT @RetentionTaken = 
			CASE WHEN d.PayCategory IS NULL
				 THEN (CASE WHEN d.PayType = a.RetPayType THEN ISNULL(SUM(d.Amount),0) - ISNULL(SUM(d.TotTaxAmount),0) ---- - ISNULL(SUM(d.GSTtaxAmt),0)
				 ELSE 0 END)                              
			ELSE (CASE WHEN d.PayType = c.RetPayType THEN ISNULL(SUM(d.Amount),0) - ISNULL(SUM(d.TotTaxAmount),0) ---- - ISNULL(SUM(d.GSTtaxAmt),0)
				 ELSE 0 END)
			END
            
			----CASE h.DefaultCountry WHEN 'US'
			----		THEN (CASE WHEN d.PayCategory Is NULL                          
			...
			----			END)
			----	ELSE
			----		CASE a.TaxBasisNetRetgYN WHEN 'N'
			----			THEN (CASE WHEN d.PayCategory IS NULL
			----				THEN (CASE WHEN d.PayType = a.RetPayType THEN ISNULL(SUM(d.Amount),0) - ISNULL(SUM(d.TotTaxAmount),0)
			----				ELSE 0 END)                              
			----			ELSE (CASE WHEN d.PayType = c.RetPayType THEN ISNULL(SUM(d.Amount),0) - ISNULL(SUM(d.TotTaxAmount),0)

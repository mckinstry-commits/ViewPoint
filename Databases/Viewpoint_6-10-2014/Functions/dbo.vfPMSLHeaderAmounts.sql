...
*				MH 09/04/2013 TFS - 60090, Task: 60532, Corrected calculation of Subcontract Amounts to not include
*														PCO's that do not have a SubCO assigned.  When the PCO Item
*														is created we do not know if it will be a new SL Item or a 
*														Sub CO.							  	
*
*
* USAGE:
...
BEGIN
	...
	WHERE d.SendFlag = 'Y'
		  ...
		  AND ((d.RecordType='O' and d.ACO is null)
		  	--Task 60532 Added "d.SubCO is not null" to where clause.  MH
		  OR   (d.RecordType='C' and (d.ACO is not null or (d.PCO is not null and d.SubCO is not null))))  

...
--				05/28/13 MB - TK-44859 - Update for checking invoice details.
--              06/05/13  Eric V - TFS-52109 - Include the NonBillable default value as an output field.
--              10/21/13 EricV TFS-64707 Added parameter for SessionID on call to vfSMWorkOrderInvoiceDetail.
...
-- =============================================
	...
	@JCCo dbo.bCompany = NULL OUTPUT, @Job dbo.bJob = NULL OUTPUT, @PhaseGroup bGroup = NULL OUTPUT, @SiteNonBillable bYN = NULL OUTPUT,
...
BEGIN
	...
	-- Check if work order has invoice details
	IF EXISTS(SELECT 1 FROM vfSMWorkOrderInvoiceDetail(NULL, @SMCo, @WorkOrder, null, null, null, null, null, null, null, null, null, null, null, null) WHERE InvoiceDetail IS NULL)
	BEGIN
		SET @isBillable = 'Y'
	END 
	...
	-- Check Site Non-Billable setting --
	SELECT @SiteNonBillable = NonBillable FROM SMServiceSite WHERE SMCo = @SMCo AND ServiceSite = @serviceSite

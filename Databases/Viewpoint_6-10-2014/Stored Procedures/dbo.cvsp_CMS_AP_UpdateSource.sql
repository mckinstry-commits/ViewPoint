...
CREATE proc [dbo].[cvsp_CMS_AP_UpdateSource] 
	( @fromco1	smallint
	, @toco		smallint
...
where JOURNALDATE<> 0
  --and COMPANYNUMBER in (@fromco1,@fromco2,@fromco3);
...
join Viewpoint.dbo.budxrefAPVendor x 
 ...
 --where d.COMPANYNUMBER in (@fromco1,@fromco2,@fromco3)
	
-- Update APTOPD with PO Number from APTOPC
update CV_CMS_SOURCE.dbo.APTOPD
set udPONumber = case when c.PONUMBER <> 0 then cast(c.COMPANYNUMBER as varchar(30)) + '-' + cast(c.PONUMBER as varchar(30)) else '0' end
from CV_CMS_SOURCE.dbo.APTOPD d
...
;with t as
(select COMPANYNUMBER, VENDORNUMBER, PAYMENTSELNO, RECORDCODE, JOURNALDATE,
			ROW_NUMBER () over (partition by case when COMPANYNUMBER = 20 then COMPANYNUMBER else 1 end,udMth
							order by case when COMPANYNUMBER = 20 then COMPANYNUMBER else 1 end, VENDORNUMBER, PAYMENTSELNO, RECORDCODE, udMth) as APTrans
...
from CV_CMS_SOURCE.dbo.APTOPC pc
  ...
  --where pc.COMPANYNUMBER in (@fromco1,@fromco2,@fromco3);
...
join CV_CMS_SOURCE.dbo.APTOPC pc
 ...
 --where pd.COMPANYNUMBER in (@fromco1,@fromco2,@fromco3);

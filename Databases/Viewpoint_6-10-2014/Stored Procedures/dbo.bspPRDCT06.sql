...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRDCT06    Script Date: 8/28/99 9:33:16 AM ******/
     CREATE  proc [dbo].[bspPRDCT06]
     /********************************************************
     ...
     **********************************************************/
     ...
     select @rcode = 0, @procname = 'bspPRDCT06'
     ...
     if @status <> 'F'
         begin
         if @a >= 2500 and @a < 10000 select @amt = ((@a - 2500) * .045)
         if @a >= 10000 and @a < 40000 select @amt = (337.5 + (@a - 10000) * .07)
         if @a >= 40000 select @amt = (2437.5 + (@a - 40000) * .087)
         end
     if @status = 'F'
         begin
         if @a >= 1250 and @a < 10000 select @amt = ((@a - 1250) * .045)
         if @a >= 10000 and @a < 40000 select @amt = (393.75 + (@a - 10000) * .07)
         if @a >= 40000 select @amt = (2493.75 + (@a - 40000) * .087)
         end
     select @amt = @amt / @ppds
...
GO
GRANT EXECUTE ON  [dbo].[bspPRDCT06] TO [public]
GO

...
CREATE proc [dbo].[cvsp_CMS_AP_OpenRetainage] 
	( @fromco1 smallint
	, @fromco2 smallint
	, @fromco3 smallint
	, @toco smallint
...
from bAPTH h 
	...
	left join HQTC t with (nolock) on h.APCo=t.Co and t.Mth=convert(nvarchar(max),datepart(mm,getdate())) + '/01/' + convert(nvarchar(max),datepart(yy,getdate())) 
	...
	left join (select a.APCo, a.Vendor, b.SL, b.SLItem, b.PO, b.POItem, b.Job, b.Phase, b.JCCType, InvRetg=sum(b.GrossAmt)
                from bAPTH a  with (nolock)
					join bAPTL b with (nolock) on a.APCo=b.APCo and a.Mth=b.Mth and a.APTrans=b.APTrans 
					left join bJCJM with (nolock) on b.JCCo=bJCJM.JCCo and b.Job=bJCJM.Job
					left join bJCCM with (nolock) on bJCJM.JCCo=bJCCM.JCCo and bJCJM.Contract=bJCCM.Contract
	...
	left join bJCJM with (nolock) on l.JCCo=bJCJM.JCCo and l.Job=bJCJM.Job
	left join bJCCM with (nolock) on bJCJM.JCCo=bJCCM.JCCo and bJCJM.Contract=bJCCM.Contract
where h.APCo=@toco

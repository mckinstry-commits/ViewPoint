...
=========================================================================
	...
	Revisions:	1. 5.19.09 - ADB - Changed Frequency Code to 'W' and DDENO in where clause to '999'.
				...
				5. 1/29/2014 AEL - Added Group by to remove duplicates.
**/
...
select ded.DCONO
	 ...
	 , ded.DBKAN
	 , RecordSeq = ROW_NUMBER() over (partition by ded.DCONO, ded.DEENO order by ded.DDEPC, ded.DDENO, ded.DAMDE)
  into #TempDirDeposit
--select *
  from CV_CMS_SOURCE.dbo.PRPDED ded
  join CV_CMS_SOURCE.dbo.PRPDTR dtr
	on dtr.BCONO	= ded.DCONO 
   and dtr.BDINO	= ded.DDENO 
   and dtr.BDICD	= 'M' 
   and dtr.BCACH	= 'Y'
 where ded.DSTAT	= 'A'
   and ded.DDEFQ	<> '0' 
   and ded.DCONO	= @fromco
 group by ded.DCONO
	 , ded.DEENO
	 , ded.DSTAT
	 , ded.DDENO

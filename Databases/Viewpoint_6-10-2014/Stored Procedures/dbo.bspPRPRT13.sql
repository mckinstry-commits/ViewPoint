...
BEGIN
	...
	BEGIN   
		...
		IF @annualized_wage BETWEEN 0.00 AND 6500.00
		BEGIN
			SELECT @tax_subtraction = 0, @rate = .0
		END
		ELSE IF @annualized_wage BETWEEN 6500.01 AND 23000.00
		BEGIN
			SELECT @tax_subtraction = 455, @rate = .07
		END
		ELSE IF @annualized_wage BETWEEN 23000.01 AND 41300.00
		BEGIN
			SELECT @tax_subtraction = 2065, @rate = .14
		END
		ELSE IF @annualized_wage BETWEEN 41300.01 AND 61300.00
		BEGIN
			SELECT @tax_subtraction = 6608, @rate = .25
		END
		ELSE IF @annualized_wage >= 61300.01
		BEGIN
			SELECT @tax_subtraction = 11512, @rate = .33
		END
...
RETURN 0
GO
GRANT EXECUTE ON  [dbo].[bspPRPRT13] TO [public]
GO

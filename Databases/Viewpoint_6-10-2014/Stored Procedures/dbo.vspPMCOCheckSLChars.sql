 ...
 *						TL 10/29/2013 - Bug 64937.  Cleaned up error messages and allow seq to expand to greater than 20 chars
 *
 *
...
if @slno = 'V'
	...
	if @slcharsvendor < 1 or @slcharsvendor > 9
		begin
		select @msg = 'Number of vendor characters for Subcontract must be between 1 and 9', @rcode = 1
		goto bspexit
		end
...
if @slno = 'P'
	...
	if @slcharsseq < 1 or (@slcharsseq > 20 and  @slcharsproject = 10)
		begin
		select @msg = 'Number of sequence characters for Subcontract must be between 1 and 20', @rcode = 1
		goto bspexit
		end

...
/****** Object:  Stored Procedure dbo.brptARAgeCustCont    Original Script Date: 8/28/99 9:32:28 AM     
       ...
       *******/    
CREATE proc [dbo].[brptARAgeCustCont]   
...
@AgeOpenCredits char(1)='N',@BegPM int=0, @EndPM int=2147483647, @BegContract bContract, @EndContract bContract)
      ...
      join #RT RT on RT.ARCo = ARTL.ARCo and RT.RecType = ARTL.RecType--changed from select statement 127001    
		...
		-- end V1-B-10810 
         ...
         WHERE    
               ...
               --AND    
           --    (JCJM.ProjectMgr is null or (JCJM.ProjectMgr>=@BegPM and JCJM.ProjectMgr<=@EndPM)) AND    
            
              --(case when @BegPM=0 then isnull(ARTL.Contract,'')  else ARTL.Contract  end = isnull(ARTL.Contract,'')     
              --and    
              --(JCJM.ProjectMgr between @BegPM and @EndPM or ARTL.Contract is null))    
        --and    
             --(case when @BegPM=0 then isnull(JCJM.ProjectMgr,@BegPM) else JCJM.ProjectMgr end >=@BegPM) and    
             --(case when @BegPM=0 then isnull(JCJM.ProjectMgr,@BegPM) else JCJM.ProjectMgr end <=@EndPM) and    
         isnull(JCJM.ProjectMgr,0) >= @BegPM and    
         isnull(JCJM.ProjectMgr,2147483647) <= @EndPM     
           
               --((isnull(ARTH1.Contract,ARTL.Contract) >= @BegContract and isnull(ARTH1.Contract,ARTL.Contract) <=@EndContract) or    
         --isnull(ARTH1.Contract,@BegContract)= case when @BegContract='' then @BegContract else null end or    
         --isnull(ARTL.Contract,@BegContract)= case when @BegContract='' then @BegContract else null end)    
        ...
        and (isnull(ARTL.Contract,' ') >= @BegContract and isnull(ARTL.Contract,' ') <= @EndContract) -- Changed from ARTH1 to ARTL CR    
               --and ARTL.LineType <> 'F' -- added 1/20/03 CR reversed 2/6/03 CR    
         --and   isnull(ARTL.Contract,'1') = case when ARTL.Contract Is Not Null then JCCM.Contract else '1' end    
           ...
           ORDER BY    
               ...
               ARTH.ARTransType 
GO
GRANT EXECUTE ON  [dbo].[brptARAgeCustCont] TO [public]
GO

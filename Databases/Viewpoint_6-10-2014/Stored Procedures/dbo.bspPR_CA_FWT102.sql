...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_FWT102]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_FWT102]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_FWT102'
   ...
   -- constants
   select @maxwages = 1051.00 -- used to compute K4 amount
   ...
   select @rate = .29, @K = 9956
   if @A <= 127021 select @rate = .26, @K = 6146
   if @A <= 81941 select @rate = .22, @K = 2868
   if @A <= 40970 select @rate = .15, @K = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_FWT102] TO [public]
GO

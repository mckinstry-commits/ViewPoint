 ...
 *				SCOTTP 03/27/2013 - TFS-45149  Add new RFQ Category. Rename old RFQ Category description
 *
 *
...
if not exists(select 1 from dbo.bPMCT where DocCat='RFQ')
	begin
	insert into bPMCT (DocCat, Description)
	values('RFQ', 'Request for Quote - 6.6')
	end
---- RFQ
if not exists(select 1 from dbo.bPMCT where DocCat='REQQUOTE')
	begin
	insert into bPMCT (DocCat, Description)
	values('REQQUOTE', 'Request for Quote')
	end	
---- ACO

...
/***************************************************/
CREATE   proc [dbo].[bspPMOLACOGenerate]
/********************************************************
 ...
 *
 * USAGE:
 *   Generates phase detail for a ACO item.
 *
 * USED IN
 *   PMACOItems
 *
 ...
 *
 **********************************************************/
(@pmco  bCompany, @project bJob, @aco bACO, @acoitem bACOItem, @contract bContract,
 @contractitem bContractItem, @msg varchar(255) output)
as
set nocount on

declare @rcode int, @validcnt int, @opencursor tinyint, @jcciunits bUnits, @jccium bUM, @jcciamt bDollar,
		@pmoium bUM, @pmoiunits bUnits, @pmoiamt bDollar, @pcotype bDocType, @pco bPCO, @pcoitem bPCOItem,
		@phasegroup bGroup, @phase bPhase, @costtype bJCCType, @jcchhours bHrs, @jcchunits bUnits,
		@jcchcost bDollar, @jcchum bUM, @pmolunits bUnits, @pmolhours bHrs, @pmolamt bDollar,
		@unithours bHrs, @hourcost bUnitCost, @unitcost bUnitCost, @pmolcount int,
...
if @jcciunits = 0 and @jcciup <> 0 select @jcciunits = 1
    ...
    -- get ACO item information from PMOI
    select @pmoium=UM, @pmoiunits=Units, @pmoiamt=ApprovedAmt, @pcotype=PCOType, @pco=PCO, @pcoitem=PCOItem
    from PMOI with (nolock) where PMCo=@pmco and Project=@project and ACO=@aco and ACOItem=@acoitem
    if @@rowcount = 0
        begin
        select @msg = 'Unable to get ACO Item data from PMOI!', @rcode = 1
        goto bspexit
        end
    else
        begin
        -- check if ACO Item UM <> Contract Item UM
        if @pmoium <> @jccium
            begin
            select @msg = 'ACO Item UM (' + isnull(@pmoium,'') + ') <> contract item UM (' + isnull(@jccium,'') + '), cannot generate!', @rcode = 1
            goto bspexit
            end
        end
    ...
    begin
        ...
        from bJCCH with (nolock) where JCCo=@pmco and Job=@project and PhaseGroup=@phasegroup and Phase=@phase
		...
		----select @pcotype, @pco, @pcoitem, @phase, convert(varchar(3),@costtype), convert(varchar(20),@pmolunits), @jcchum, convert(varchar(20),@pmolhours), convert(varchar(20),@pmolamt)
        ...
        insert into bPMOL (PMCo, Project, PCOType, PCO, PCOItem, ACO, ACOItem, PhaseGroup, Phase, CostType,
                    ...
                    ----TK-14576 146361
                    VendorGroup)
        select @pmco, @project, @pcotype, @pco, @pcoitem, @aco, @acoitem, @phasegroup, @phase, @costtype,
                    isnull(@pmolunits,0), @jcchum, isnull(@unithours,0), isnull(@pmolhours,0), 
                    isnull(@hourcost,0), isnull(@unitcost,0), 'E', isnull(@pmolamt,0), 'Y',
                     ----TK-14576 146361
                    @VendorGroup
        if @@rowcount = 0
            begin
...
GO
GRANT EXECUTE ON  [dbo].[bspPMOLACOGenerate] TO [public]
GO

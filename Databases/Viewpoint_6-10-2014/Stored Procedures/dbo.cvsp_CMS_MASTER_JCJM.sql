...
=========================================================================================
	...
	Revisions:	
		...
		4. 10/03/2013 BTC - Added JCJobs cross reference
...
insert bJCJM (JCCo, Job, Description, Contract, JobStatus, LockPhases, 
	...
	VendorGroup, TaxCode, InsTemplate, LiabTemplate, PRStateCode, RateTemplate, OurFirm,
	udSource, udConv, udCGCTable, udCGCTableID, udCGCJob,udProjWrkstrm)

select 
	 JCCo=@toco
		/* McKinstry wants to auto number the Jobs starting at 1000  CR 9/25/2013 */
	...
	,Contract=xj.VPJobExt --dbo.bfMuliPartFormat(ltrim(rtrim(j.JOBNUMBER)),@JobFormat)
	,JobStatus=case when isnull(j.CLOSEDDATE,0) = 0 then 1 else 2 end    -- modified, get from custom view
	...
	,OurFirm = '800000'    -- McKinstry Firm Number, CR 9/25/2013
	,udSource     = 'MASTER_JCJM'
	,udConv       = 'Y'
	,udCGCTable   = 'JCTDSC'
	,udCGCTableID = JCTDSCID
	,udCGCJob     = cast(j.COMPANYNUMBER as varchar(20)) + '-' + j.JOBNUMBER
	,udProjWrkstrm = w.udProjWrkstrm
--select *
...
left join budxrefProjectWorkstream w
    on j.JOBNUMBER between w.JobRangeLow and w.JobRangeHigh
   and j.COMPANYNUMBER = w.CompanyNumber

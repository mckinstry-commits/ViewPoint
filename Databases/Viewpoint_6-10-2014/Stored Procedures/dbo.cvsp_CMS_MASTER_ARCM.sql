...
select @CustGroup
                ...
                , Phone      = case
								...
								when PHONENO<>0 then convert(varchar(3),AREACODE)+'-'+
													 convert(varchar(3),(left(PHONENO,3)))+'-'+ 
													 convert(varchar(4),(right(PHONENO,4)))
								else convert(varchar(max),AREACODE) + '-' + convert(varchar(max),PHONENO) 
								end
				...
				, PayTerms  = isnull(pt.PaymentTerms,@defaultPayTerms)
                , TaxGroup  = @TaxGroup
...
left join budXRefPayTerms pt
  on pt.CGCCode = c.TERMSCODE
	
where COMPANYNUMBER=@CustGroup

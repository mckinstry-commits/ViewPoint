...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NUT09]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_NUT09]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NUT09'
   ...
   if @TCP is null select @TCP = 11644

   -- establish tax rate and constant
   select @rate = .115, @KP = 5875
   if @A <= 126264 select @rate = .090, @KP = 2718
   if @A <= 77664 select @rate = .070, @KP = 1165
   if @A <= 38832 select @rate = .040, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NUT09] TO [public]
GO

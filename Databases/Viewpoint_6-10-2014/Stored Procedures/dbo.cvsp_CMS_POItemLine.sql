...
CREATE proc [dbo].[cvsp_CMS_POItemLine] 
	( @fromco1 smallint
	, @fromco2 smallint
	, @fromco3 smallint
	, @toco smallint
...
select
	  POITKeyID = it.KeyID
	, POCo=@toco
	, PO=cast(d.COMPANYNUMBER as varchar(30)) + '-' + cast(d.PONUMBER as varchar(30))
	, POItem=d.POITEMT
	...
	, InvMiscAmt = 0
	, RemUnits=0
	, RemCost=case when d.QTYORDERED=0 and d.COSTDOLLARAMT > d.DOLLARAMTRCVD then d.COSTDOLLARAMT - d.DOLLARAMTRCVD else 0 end
	, RemTax=0
	, JCCmtdTax = 0
	, JCRemCmtdTax = 0
	, TotalUnits=case
			when it.UM='LS' then 0 else d.COSTDOLLARAMT - d.QTYORDERED end
...
join bPOIT it
	on it.PO=convert(nvarchar(30),d.COMPANYNUMBER) + '-' + convert(nvarchar(30),d.PONUMBER) 
...
left join (select oldGLAcct,newCompany,newGLAcct
		     from Viewpoint.dbo.budxrefGLAcct xg 
		    group by oldGLAcct,newCompany,newGLAcct) xg
	on xg.newCompany=@toco

...
*                   JayR 5/8/2013  Add comment from AdamR: Updating the database installe to support schemas,
*							         making the upgrader compatible with 6...  Done on 2/19/2013
...
-----Setup----------------
	...
	SET @SQL = 'CREATE TRIGGER ' + @InsertTriggerName + ' ON ' + @SchemaName + '.' + @TableName + @NewLine
		...
		SELECT	@SQL =	@SQL + '-- log additions to the ' + tac.ColumnName + ' column'	+ @NewLine
							...
							--NewValue
							+ @TabStopsP1 + QUOTENAME(tac.ColumnName) + @Seperator  + @NewLine
							--DateTime

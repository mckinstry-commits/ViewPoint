...
-- Description:	Normal Validation plus added validation to prevent changes after interface.
    ...
    *****************************************************/ 
-- =============================================
CREATE PROCEDURE [dbo].[mckHQTaxVal] 
	-- Add the parameters for the stored procedure here

   	(@taxgroup bGroup = null, @taxcode bTaxCode = null, @JCCo TINYINT, @Project bProject, @msg varchar(255) output)
   as
   ...
   IF EXISTS(SELECT 1 FROM JCJM WHERE @JCCo = JCCo AND @Project = Job AND JobStatus <> 0)
   BEGIN
	SELECT @msg = 'This project has already been interfaced.  Contact accounting to make changes.', @rcode = 1
	GOTO bspexit
   END
...
GO
GRANT EXECUTE ON  [dbo].[mckHQTaxVal] TO [public]
GO

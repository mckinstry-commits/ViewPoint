...
CREATE proc [dbo].[cvsp_CMS_MASTER_JCJP] 
	( @fromco1	smallint
	, @fromco2	smallint
	, @fromco3	smallint
	, @toco		smallint
...
=========================================================================
	...
	Revisions:	1. 02.20.09 - A. Bynum - Edited for CMS; variables entered for multiple company conversion.
				...
				6. BC 10/03/13 - Added JCJobs cross reference
...
select JCCo        = @toco
	...
	, Phase        = isnull(x.newPhase,'9999-9998-' + right('000000' + left(min(j.JCDISTRIBTUION),6),6) + '-   ')
	, Description  = max(rtrim(j.DESC20A))
	, Contract     = xj.VPJobExt --min(dbo.bfMuliPartFormat(ltrim(rtrim(j.JOBNUMBER)),@JobFormat))
	, Item         = @defaultItem
	, ActiveYN     = 'Y'
	, Notes        = isnull(max(j.DESC20A) + '  ','')+ isnull(min(j.JCDISTRIBTUION),'')
...
I(sarah) wanted to clarify that we only want CGC pay items with a budget or actual amount associated
 to be brought over.  
 --deprecated 2/27/2014 - Per Sarah and Howard, PM's will do clean up of unnecessary phases after conversion.
and 
(BUDGETAMT <> 0 OR ACTUALCSTJTD <> 0)   
*/
group by xj.VPJobExt,xj.VPJob --dbo.bfMuliPartFormat(ltrim(rtrim(j.JOBNUMBER)) + ltrim(rtrim(j.SUBJOBNUMBER)),@JobFormat)
	--,j.JCDISTRIBTUION
	,x.newPhase;

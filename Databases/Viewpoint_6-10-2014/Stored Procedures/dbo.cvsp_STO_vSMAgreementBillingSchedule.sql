...
CREATE PROCEDURE [dbo].cvsp_STO_vSMAgreementBillingSchedule (
		@ToCo			bCompany,
		@FromCo			bCompany,
		@DeleteDataYN	CHAR(1))
AS
...
Purpose:	Converts Timberline Agreement Billing Schedules into VP SM Agreement Billing Schedules
			
Change Log:

	20130508	CL	Initial Coding	
**************************************************************************************************/
BEGIN TRY
	/******************************************************************
		Declare variables
	...
	******************************************************************/
		...
		SELECT	@SQL = @SQL + @TS + ' FROM vSMAgreementBillingSchedule'
		EXEC	(@SQL)
		
	/******************************************************************
		Modify table attributes
	******************************************************************/	
		ALTER TABLE vSMAgreementBillingSchedule NOCHECK CONSTRAINT ALL
		ALTER TABLE vSMAgreementBillingSchedule DISABLE TRIGGER ALL
		SET IDENTITY_INSERT vSMAgreementBillingSchedule ON
	...
	******************************************************************/
		...
		BEGIN
			DELETE vSMAgreementBillingSchedule WHERE SMCo = @ToCo
		END
	...
	******************************************************************/
		...
		SET @MaxID = (SELECT ISNULL(MAX(SMAgreementBillingScheduleID), 0) FROM dbo.vSMAgreementBillingSchedule)
		...
		-- Create Viewpoint transactions
		INSERT	vSMAgreementBillingSchedule (
	...
	******************************************************************/	
		SET IDENTITY_INSERT vSMAgreementBillingSchedule OFF
		ALTER TABLE vSMAgreementBillingSchedule CHECK CONSTRAINT ALL;
		ALTER TABLE vSMAgreementBillingSchedule ENABLE TRIGGER ALL;

	/******************************************************************
		Review data
	******************************************************************/	
		SELECT * FROM vSMAgreementBillingSchedule WHERE SMCo = @ToCo
END TRY

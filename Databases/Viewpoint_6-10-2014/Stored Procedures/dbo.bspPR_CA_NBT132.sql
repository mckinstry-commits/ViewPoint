...
CREATE  proc [dbo].[bspPR_CA_NBT132]
/********************************************************
...
*				MV 12/11/12 - TK-20158 tax updates effective 1/1/2013
*				CHS	5/23/13	tax updates effective 7/1/2013
*
* USAGE:
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NBT132'
   ...
   select @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @LCP = 0, @T2 = 0, @calcamt = 0, @TCrate = .0968

   -- if form TD1NB was not filed (ie. no filing status entered) use default total claim
   if @TCP is null select @TCP = 9388

   -- establish tax rate and constant
   -- select @rate = .143, @KP = 3809
   IF		@A BETWEEN 0 AND 38954			SELECT @rate = .0968, @KP = 0
   ELSE IF	@A BETWEEN 38954.01 AND 77908	SELECT @rate = .1482, @KP = 2002
   ELSE IF  @A BETWEEN 77908.01 AND 126662	SELECT @rate = .1652, @KP = 3327
   ELSE IF	@A >= 126662.01					SELECT @rate = .1784, @KP = 4999 
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NBT132] TO [public]
GO

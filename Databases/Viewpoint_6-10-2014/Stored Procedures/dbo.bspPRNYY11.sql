...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRNYY11    Script Date: 8/28/99 9:33:31 AM ******/
CREATE  proc [dbo].[bspPRNYY11]
/********************************************************
* CREATED BY: 	bc 6/12/98
* MODIFIED BY:	EN 1/27/99
*				EN 1/17/00 - tax addition amount wasn't being initialized for the lowest resident tax bracket which would have causeed no tax to be calculated
*				EN 1/27/00 - nonresident tax rate changed from 50% to 25%
...
(@subjamt bDollar = 0, @ppds tinyint = 0, @status char(1) = 'S', @exempts tinyint = 0, @resident bYN = null,
   ...
   select @rcode = 0, @allowance = 1000, @procname = 'bspPRNYY11'
   ...
   select @amt = ((@tax_addition + (@annualized_wage - @wage_bracket) * @rate) / @ppds) * .175
   goto bspexit
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNYY11] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  StORed Procedure dbo.bspPRGAT13    Script Date: 8/28/99 9:33:20 AM ******/
   CREATE proc [dbo].[bspPRGAT13]
   /********************************************************
   ...
   **********************************************************/
   ...
   SELECT @stddedn = 3000, @adultexempt = 7400 -- M - Married filing jointly
   IF @status = 'S' OR @status = 'H' SELECT @stddedn = 2300, @adultexempt = 2700
   IF @status = 'F' OR @status = 'B' SELECT @stddedn = 1500, @adultexempt = 3700
...
GO
GRANT EXECUTE ON  [dbo].[bspPRGAT13] TO [public]
GO

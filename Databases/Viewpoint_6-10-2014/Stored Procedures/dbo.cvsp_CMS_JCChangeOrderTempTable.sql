...
CREATE procedure [dbo].[cvsp_CMS_JCChangeOrderTempTable] 
	( @fromco1	smallint
	, @fromco2	smallint
	, @fromco3	smallint
	, @toco		smallint
...
from CV_CMS_SOURCE.dbo.JCTCGH h 
	...
	--	ON	jobs.GCONO = h.RHCONO
	--	AND jobs.GJBNO     = h.JOBNUMBER
	--	and jobs.GSJNO  = h.SUBJOBNUMBER
		
	join Viewpoint.dbo.budxrefJCJobs xj
		on xj.COMPANYNUMBER = h.RHCONO and xj.DIVISIONNUMBER = h.RHDVNO and xj.JOBNUMBER = h.JOBNUMBER
	...
	left join Viewpoint.dbo.budxrefPhase p on p.Company=@PhaseGroup and d.JCDISTRIBTUION=p.oldPhase
	
	left join Viewpoint.dbo.budxrefCostType t on t.Company=@PhaseGroup and d.COSTTYPE=t.CMSCostType
	...
	left join bJCCH x on x.JCCo=@toco 
		and x.Job = xj.VPJob -- dbo.bfMuliPartFormat(RTRIM(d.JOBNUMBER) +  RTRIM(d.SUBJOBNUMBER),@Job)=x.Job
		and x.Phase=p.newPhase and x.CostType=t.CostType
		
where isnull(c.SubOnly,'N')<>'Y'
	and h.RHCONO in (@fromco1,@fromco2,@fromco3)
	and h.CHGORDERTYPE not in ('01','02','03')

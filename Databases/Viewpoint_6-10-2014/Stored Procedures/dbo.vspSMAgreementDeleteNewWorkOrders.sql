...
--			7/11/13 JVH - Added deleting the entity record before deleting the scope.
...
BEGIN
	...
	BEGIN TRY
		...
		DELETE vSMEntity
		FROM @WorkOrderScopesToDelete WorkOrderScopesToDelete
			INNER JOIN dbo.vSMEntity ON WorkOrderScopesToDelete.SMCo = vSMEntity.SMCo AND WorkOrderScopesToDelete.WorkOrder = vSMEntity.WorkOrder AND WorkOrderScopesToDelete.Scope = vSMEntity.WorkOrderScope

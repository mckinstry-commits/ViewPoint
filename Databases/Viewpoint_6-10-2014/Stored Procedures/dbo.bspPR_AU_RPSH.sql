...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_AU_RPSH]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_AU_RPSH]
   /********************************************************
   ...
   *	1 		failure
   **********************************************************/
   	(@prco bCompany = null, @addon bEDLCode = null, @factor bRate = null, @prgroup bGroup, @prenddate bDate, @employee bEmployee, 
	...
	select @rcode = 0, @hours = 0, @subjecthours = 0, @amt = 0, @procname = 'bspPR_AU_RPSH'
	...
	while @SubjEC is not null
		...
		next_Timecard:
			...
			--Compute allowance modified by RDO factor rate adjustment (36/40) for allowances based on regular hours
			if @factor = 1.00 select @subjecthours = @subjecthours * (36.00/40.00)
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_AU_RPSH] TO [public]
GO

...
* MODIFIED BY:	GF 05/13/2013 TFS-47315 V1099Type only applies for US (NOT CA/AU)
*		AW 08/27/2013 TFS-59883 Validate DueDate and InvoiceDate for AP
*
*
* USAGE:
...
select  @APRef=CLAIM.APRef, @InvoiceDesc=CLAIM.InvoiceDesc,
		@InvoiceDate=CLAIM.InvoiceDate, @DueDate=CLAIM.DueDate, @Claim_KeyId=CLAIM.KeyID,
		...
		----APVM
		@EFT=APVM.EFT, @V1099YN=APVM.V1099YN, 
		----TFS-47315
		@V1099Type = CASE WHEN @HQCoDefaultCountry = 'US' THEN APVM.V1099Type
						  WHEN @HQCoDefaultCountry = 'CA' THEN NULL
						  WHEN @HQCoDefaultCountry = 'AU' THEN NULL
						  END, ----@V1099Type=APVM.V1099Type,
...
-- must have invoice & due date to upload to AP
	IF @ProcessName = 'SLClaimUpdateAPTrans'
		BEGIN
			IF @InvoiceDate is null or @DueDate is null
			BEGIN
				SET @Msg = @ErrorStart + 'Claim must have an Invoice Date and Due Date to uploaded to AP.'
				SET @rcode = 1
				GOTO vspexit
			END
		END

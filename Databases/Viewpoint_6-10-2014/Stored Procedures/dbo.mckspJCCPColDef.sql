...
BEGIN
	...
	IF NOT EXISTS(SELECT 1 FROM dbo.JCUO
		WHERE JCCo = @Company AND UserName = @VPUserName)
		BEGIN
			...
			INSERT INTO @DefaultCols
				SELECT JCCo ,
					  ...
					  ColumnWidth
					FROM JCUO 
					  WHERE JCCo = 101 AND UserName = 'PML1' AND (Form = 'JCProjection' OR Form = 'JCRevProj') ;
			 ...
			 --         ColumnWidth
			 --       FROM JCUO 
			 --         WHERE JCCo = 101 AND UserName = 'PML1' AND Form = 'JCRevProj' ;
...
END
GO
GRANT EXECUTE ON  [dbo].[mckspJCCPColDef] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMET99    Script Date: 8/28/99 9:33:27 AM ******/
   CREATE   proc [dbo].[bspPRMET99]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @dedn = 2750, @procname = 'bspPRMET99'
   ...
   if @status = 'S' 
   	begin
   		if @annualized_wage <= 1500 goto bspcalc
   		if @annualized_wage > 1500 select @wage_bracket = 1500, @rate = .02
   		if @annualized_wage > 5650 select @tax_addition = 83, @wage_bracket = 5650, @rate = .045
   		if @annualized_wage > 9750 select @tax_addition = 268, @wage_bracket = 9750, @rate = .07
   		if @annualized_wage > 18000 select @tax_addition = 846, @wage_bracket = 18000, @rate = .085
   end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMET99] TO [public]
GO

...
*	1 and error message
**************************************/
CREATE PROCEDURE [dbo].[vspPCVendorValForPCQualifications]
...
BEGIN
	...
	-- Get the next vendor number if a + is entered
	...
	BEGIN
		...
		SELECT @msg = 'AP Vendor ' + ISNULL(@vendor, '') + ' is not on file.', @vendorout = @vendor
		GOTO VspExit
	END
...
GO
GRANT EXECUTE ON  [dbo].[vspPCVendorValForPCQualifications] TO [public]
GO

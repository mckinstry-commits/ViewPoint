...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMAT07    Script Date: 8/28/99 9:33:26 AM ******/
  CREATE  proc [dbo].[bspPRMAT07]
  /********************************************************
  ...
  **********************************************************/
  ...
  select @rcode = 0, @rate = .053, @procname = 'bspPRMAT07', @FICArate = .0765
  ...
  if @total_exempts = 1
  	select @annualized_wage = @annualized_wage - 4125
 
  if @total_exempts >1
  	select @annualized_wage = @annualized_wage - (@total_exempts * 1000) - 3125
 
  -- subtract extra deduction if head of household
  if @status = 'H' select @annualized_wage = @annualized_wage - 2250
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMAT07] TO [public]
GO

	...
	begin
   	if @itemtype in (1,2,4)
       	...
       	exec @slrcode = dbo.bspPMSLSubCoGet @pmco, @project, @apco, @formattedsl, @slitem, @itemtype,
					@seq, @subco output, @defaco, @defacoitem, @vendor, null,@slmsg output
...
PMSL_end:
	if @opencursor = 1
		...
		begin tran
			...
			update bSLHD 
			   set udSLContractNo	= bPMSL.udSLContractNo
			     , bSLHD.OrigDate	= bPMSL.InterfaceDate
			  from bSLHD
			  join bPMSL 
			    on bSLHD.SLCo	= bPMSL.SLCo 
			   and bSLHD.SL		= bPMSL.SL 
			   and bSLHD.Job	= bPMSL.Project 
			   and bSLHD.Vendor	= bPMSL.Vendor
			 where bSLHD.SLCo	= @pmco 

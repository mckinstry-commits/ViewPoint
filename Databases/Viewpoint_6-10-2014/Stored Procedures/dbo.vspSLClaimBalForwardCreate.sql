...
* MODIFIED BY:	GF 09/30/2013 TFS-63112 no balance forward claim if claims exist
* 
*
* USAGE:
...
---- TFS-63112 do not create balance forward claim if claims exist
---- subcontract cannot be in an open AP Transaction Entry Batch
IF EXISTS(SELECT 1 FROM dbo.vSLClaimHeader WHERE SLCo = @SLCo AND SL = @Subcontract AND ClaimNo > 0)
	BEGIN
  	SELECT @Msg = 'Subcontract exists in SL Subcontract Claims with existing claims! SLCo: ' + dbo.vfToString(@SLCo) + ' SL: ' + dbo.vfToString(@Subcontract), @rcode = 1
	GOTO vspexit
	END  

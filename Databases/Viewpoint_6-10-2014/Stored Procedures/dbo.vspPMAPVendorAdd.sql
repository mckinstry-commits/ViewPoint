 ...
 *				GF 05/14/2013 TFS-47315 no 1099 type for CA or AU
 *
 *
...
---- get AP GLCO
---- TFS-47315
select @glco=APCO.GLCo
		,@HQCountry = HQCO.DefaultCountry
from bAPCO APCO with (nolock)
INNER JOIN dbo.bHQCO HQCO ON HQCO.HQCo = APCO.APCo
where APCo=@apco
if @@rowcount = 0
	begin
...
----TFS-47315
SET @1099Type = NULL
---- get 1099 type if not CA, AU
IF @HQCountry NOT IN ('CA', 'AU')
	BEGIN
...
select @vendorgroup, @vendor, @glco, 'R', 'N', 'N', 'Y', @1099Type,
		CASE WHEN @HQCountry NOT IN ('AU','CA') THEN 7 ELSE NULL END, ----7,
		'Y', 'N', 'N', 'N', 'N', 0,

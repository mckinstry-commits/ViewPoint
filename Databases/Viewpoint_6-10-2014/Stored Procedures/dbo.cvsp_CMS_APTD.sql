...
from CV_CMS_SOURCE.dbo.APTOPD l with (nolock)
	left join CV_CMS_SOURCE.dbo.APTOPC h with (nolock) on l.COMPANYNUMBER=h.COMPANYNUMBER and l.DIVISIONNUMBER=h.DIVISIONNUMBER 
	...
	left join CV_CMS_SOURCE.dbo.APTOPC h2 with (nolock) on l.COMPANYNUMBER=h2.COMPANYNUMBER and l.DIVISIONNUMBER=h2.DIVISIONNUMBER 
	...
	left join (select distinct COMPANYNUMBER, DIVISIONNUMBER, VENDORNUMBER, PAYMENTSELNO, LineCnt=count(SEQUENCENO03), 
				...
				from CV_CMS_SOURCE.dbo.APTOPD with (nolock)
	...
	left join (select COMPANYNUMBER, DIVISIONNUMBER, VENDORNUMBER, PAYMENTSELNO, 
				...
				from CV_CMS_SOURCE.dbo.APTHCK APTHCK with (nolock)
				where COMPANYNUMBER in (@fromco1,@fromco2,@fromco3)
	...
	left join (select distinct COMPANYNUMBER, DIVISIONNUMBER, VENDORNUMBER, PAYMENTSELNO, ChkCount=count(CHECKNUMBER)
				from CV_CMS_SOURCE.dbo.APTHCK  with (nolock)
				where COMPANYNUMBER in (@fromco1,@fromco2,@fromco3)
				group by COMPANYNUMBER, DIVISIONNUMBER, VENDORNUMBER, PAYMENTSELNO)
				as t
	...
	join bHQCO HQCO with (nolock) on HQCO.HQCo=@toco
	--McKinstry: Convert active jobs only
	JOIN budxrefJCJobs j with (nolock)
	  on j.COMPANYNUMBER = l.COMPANYNUMBER
	 and j.JOBNUMBER = l.JOBNUMBER
	 and j.SUBJOBNUMBER = l.SUBJOBNUMBER
	 and j.DIVISIONNUMBER = l.DIVISIONNUMBER
	 and j.VPJob is not null
where d.CHECKNUMBER <> 0--is not null -- only pull records that have a payment issued
	and l.COMPANYNUMBER in (@fromco1,@fromco2,@fromco3)

...
CREATE PROCEDURE [dbo].[vspAPGLAcctDefaultGet]
     /***********************************************************
...
BEGIN
     ...
     declare @rcode int,@overridegl bYN, @gltransacct bGLAcct,
  ...
  if @linetype = 4 -- EM or WO
	...
	IF @linetype = 8
	BEGIN
		SELECT @smglco = GLCo, @glacct = CurrentCostGLAcct
		FROM dbo.vfSMGetAccountingTreatment (@smco, @smworkorder, @smscope, 'M', @smcosttype)
	END

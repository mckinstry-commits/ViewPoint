...
*           EricV 05/21/13 = TFS-50103 Update GLInterfaceLevel in SMDetailTransaction
*           JacobV 6/27/13 = TFS-50130 Fixed updating the GL Interface level to use the work order instead of the work completed
*           JacobV 7/11/13 = TFS-39214 Fixed posting GL for job timecards with 0 hours
* USAGE:
			...
			BEGIN TRAN
				...
				BEGIN
					...
					BEGIN
						...
						IF @@ROWCOUNT = 0
						BEGIN
							--If no GLEntryTransactions were added then get rid of the GLEntry
							DELETE dbo.vGLEntry WHERE GLEntryID = @RevenueGLEntryID
						END
						ELSE
						BEGIN
				...
				SET PRLedgerUpdateDistributionID = NULL, Posted = 1, BatchId = @BatchId, 
					GLInterfaceLevel = CASE WHEN vSMCO.UseJCInterface = 'N' THEN 0 ELSE bJCCO.GLCostLevel END
				FROM dbo.vPRLedgerUpdateDistribution
					INNER JOIN dbo.vSMDetailTransaction ON vPRLedgerUpdateDistribution.PRLedgerUpdateDistributionID = vSMDetailTransaction.PRLedgerUpdateDistributionID
					INNER JOIN dbo.vSMWorkOrder ON vSMDetailTransaction.SMWorkOrderID = vSMWorkOrder.SMWorkOrderID
					INNER JOIN dbo.vSMCO ON vSMWorkOrder.SMCo = vSMCO.SMCo
					INNER JOIN dbo.bJCCO ON vSMWorkOrder.JCCo = bJCCO.JCCo

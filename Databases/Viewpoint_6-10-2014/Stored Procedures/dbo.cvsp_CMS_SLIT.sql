...
;with m as (
select PMCo, SL, SLItem, MIN(Seq) as MinSeq 
	from bPMSL 
	group by PMCo, SL, SLItem)
...
join  m
		on m.PMCo=sl.PMCo 
		and m.SL=sl.SL 
		and m.SLItem=sl.SLItem 
		and m.MinSeq=sl.Seq
		
where sl.PMCo=@toco 
	and sl.SL is not null 
	and sl.Phase is not null


select @rowcount=@@rowcount;

;with p as (select SLCo, SL, SLItem, PMSLAmount=sum(Amount) 
			from bPMSL 
			where RecordType='O' 
				and bPMSL.SLCo=@toco

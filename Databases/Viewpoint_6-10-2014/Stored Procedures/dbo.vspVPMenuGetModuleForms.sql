...
*           DW 02/19/13 - User Story #9679 (Version 1 #B-07510) - make procedure return all Forms with NULL @mod parameter 
*
...
where m.Mod = ISNULL(@mod, m.[Mod]) and m.Active = 'Y' and h.ShowOnMenu = 'Y' 
...
form_loop:	-- check Security for each Form
	...
	if @rcode <> 0 
	begin
	   delete from @allforms where [Form]=@form
	end

...
declare @MaxID int = 0
select @MaxID = isnull(MAX(ARTrans),0)
...
select ARCo		     = A.ARCo
	, Mth		     = A.Mth
	, ARTrans        = A.ARTrans
	, ARLine         = @MaxID + ROW_NUMBER ( ) OVER (partition by A.Mth, A.ARTrans
				        order by  A.Mth
							, A.ARTrans
							, A.ARCo
							, D.SEQUENCENO05
	...
	, TaxGroup       = @TaxGroup 
	, TaxCode        = NULL
	, Amount         = CASE WHEN D.ATXDS = 'Y' then 0 else D.INVAMT + D.RETAINEDAMT end
	, TaxBasis       = 0  
	, TaxAmount      = CASE WHEN D.ATXDS = 'Y' then D.INVAMT else 0 end
    , RetgPct        = 0
	...
	, ApplyLine      = 0 
	, JCCo           = @toco 
	, Contract       = xj.VPJobExt--D.udContract
	, Item           = space(16-len(D.udItem)) + D.udItem
...
--	ON	jobs.GCONO = D.COMPANYNUMBER
--	AND jobs.GJBNO     = D.JOBNUMBER
--	and jobs.GSJNO  = D.SUBJOBNUMBER
...
JOIN Viewpoint.dbo.budxrefJCJobs xj
	on xj.COMPANYNUMBER = D.COMPANYNUMBER and xj.DIVISIONNUMBER = D.DIVISIONNUMBER and xj.JOBNUMBER = D.JOBNUMBER
		and xj.SUBJOBNUMBER = D.SUBJOBNUMBER and xj.VPJob is not null

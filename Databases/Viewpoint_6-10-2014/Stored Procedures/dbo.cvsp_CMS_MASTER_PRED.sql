...
=========================================================================
	...
	exec cvsp_CMS_MASTER_PRED 20,20,'',0
**/
...
--Fed WithHolding
--declare @toco int = 1,@fromco int = 1,@defaultEmplBased varchar(1) = 'N'
select distinct DLCode = d.DLCode
	, PRCo = @toco
...
from CV_CMS_SOURCE.dbo.PRTMST
	--join Viewpoint.dbo.budxrefPRDedLiab x on x.Company=1
	join bPRDL d on d.PRCo=@toco and 1=d.DLCode
...
select distinct DLCode = x.DLCode
	, PRCo=@toco
	, Employee=L.EMPLOYEENUMBER
	, EmplBased=@defaultEmplBased
	, Frequency=null
	, ProcessSeq=null--@defaultProcess
	, FileStatus=L.TAXSTATUS
	, RegExempts=L.EXEMPTWITHHELD
	, OverMiscAmt='N'
	, MiscAmt=0
	, GLCo=@toco
	, OverCalcs='N'
	...
	, MinNetPay=null
	, AddOnType = case
					when x.FITADDONYN = 'Y' and L.EXTRAFITA > 0 then 'A'
					when x.FITADDONYN = 'Y' and L.EXTRAFITP > 0 then 'R'
					else 'N'
				  end
	, AddOnRateAmt=case
					when x.FITADDONYN = 'Y' and L.EXTRAFITA > 0 then L.EXTRAFITA
					when x.FITADDONYN = 'Y' and L.EXTRAFITP > 0 then L.EXTRAFITP/100
					else 0
				  end
	, udSource = 'MASTER_PRED'
	, udConv='Y'
	,udCGCTable='PRTSTL'
	,udCGCTableID=PRTSTLID
from CV_CMS_SOURCE.dbo.PRTSTL L
  join budXRefStateDeductions x
    on x.PRCo = L.COMPANYNUMBER
   and x.CGCCODE = L.DISTNUMBER
   and x.CGC_ERCCD = L.RECORDCODE
	join bPRDL i on i.PRCo=1 and x.DLCode=i.DLCode
	 and i.CalcCategory = 'S'
...
----Other Deductions 
select distinct DLCode = max(x.DLCode)
	, PRCo = @toco
	, Employee = D.DEENO
	, EmplBased = 'Y'
	, Frequency = 'W'
	...
	, MiscAmt = 0
	, GLCo=@toco
	, OverCalcs=case when max(D.DAMDE)<> 0 then 'A' 
					 when MAX(D.DDEPC)<> 0 then 'R'
					 else 'R' end
	, RateAmt=case when max(D.DAMDE)<> 0 then max(D.DAMDE) 
					when max(D.DDEPC) = 0 then 0
...
where D.DDENO not in (998,999) 
	...
	group by D.DCONO, D.DEENO, x.DLCode, DED.DDTCM

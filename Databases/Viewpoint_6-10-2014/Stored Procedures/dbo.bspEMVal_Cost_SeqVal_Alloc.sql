...
SET ANSI_NULLS ON
GO
CREATE           procedure [dbo].[bspEMVal_Cost_SeqVal_Alloc]
   /***********************************************************
   ...
   *		JM 10-02-02 -  Ref Issue 18024 - For EMAloc Source, validate GLOffsetAcct SubType = null
  ...
  * 
   ...
   *****************************************************/
   ...
   /* bEMBF non-Olds decs */
   declare @actualdate bDate, @alloccode tinyint, @batchtranstype char(1), @costcode bCostCode, @description bItemDesc/*137916*/, @dollars bDollar, @emcosttype bEMCType, 
   	...
   	@matlgroup bGroup, @reversalstatus tinyint, @source varchar(10), @units bUnits
   
   /* bEMBF Olds decs */
   declare @oldactualdate bDate, @oldalloccode tinyint, @oldbatchtranstype char(1), @oldcostcode bCostCode, @olddescription bItemDesc/*137916*/, @olddollars bDollar, 
   	@oldemcosttype bEMCType, @oldemgroup bGroup,@oldemtrans bTrans, @oldemtranstype varchar(10), @oldequipment bEquip,@oldglco bCompany, 
   	@oldgloffsetacct bGLAcct, @oldgltransacct bGLAcct, @oldmatlgroup bGroup, @oldreversalstatus tinyint, @oldsource varchar(10), @oldunits bUnits
   
   /* bEMCD decs */
   declare @emcdactualdate bDate, @emcdalloccode tinyint, @emcdcostcode bCostCode, @emcddescription bItemDesc/*137916*/, @emcddollars bDollar, 
   	@emcdemcosttype bEMCType, @emcdemgroup bGroup, @emcdemtrans bTrans, @emcdemtranstype varchar(10), @emcdequipment bEquip, 
   	@emcdglco bCompany, @emcdgloffsetacct bGLAcct, @emcdgltransacct bGLAcct, @emcdinusebatchid bBatchID, @emcdmatlgroup bGroup, 
   	@emcdreversalstatus tinyint, @emcdsource varchar(10), @emcdunits bUnits
   ...
   select @actualdate = ActualDate, @alloccode = AllocCode, @batchtranstype = BatchTransType, @costcode = CostCode, @description = Description, 
   	@dollars = Dollars, @emcosttype = EMCostType, @emgroup = EMGroup, @emtrans = EMTrans, @emtranstype = EMTransType, 
   	@equipment = Equipment,@glco = GLCo, @gloffsetacct = GLOffsetAcct, @gltransacct = GLTransAcct, @matlgroup = MatlGroup, 
   	@reversalstatus = ReversalStatus, @source = rtrim(Source), @units = Units, @oldactualdate = OldActualDate, @oldalloccode = OldAllocCode, 
   	@oldbatchtranstype = OldBatchTransType, @oldcostcode = OldCostCode, @olddescription = OldDescription, @olddollars = OldDollars, 
   	...
   	@oldmatlgroup = OldMatlGroup, @oldreversalstatus = OldReversalStatus, @oldsource = rtrim(OldSource), @oldunits = OldUnits
   ...
   -- Ref Issue 18024 - For EMParts Source, validate GLOffsetAcct SubType = null
   ...
   if @batchtranstype in ('A','C')
   	...
   	if @gltransacct is not null
   		...
   		exec @rcode = dbo.bspGLACfPostable @co, @gltransacct, 'E', @errmsg output
   		if @rcode = 1
   			begin
   	...
   	/* Validate AllocCode - can be null. */
   	if @alloccode is not null
   		begin
   		exec @rcode = dbo.bspEMAHVal @co, @alloccode, @errmsg output
   		if @rcode = 1
   			begin
   			select @errtext = isnull(@errorstart,'') + 'AllocCode ' + isnull(convert(varchar(5),@alloccode),'') + '-' + isnull(@errmsg,'')
   			exec @rcode =dbo.bspHQBEInsert @co, @mth, @batchid, @errtext, @errmsg output
   			if @rcode <> 0
   				begin
   				select @errmsg = isnull(@errtext,'') + '-' + isnull(@errmsg,'')
   ...
   if @batchtranstype in ('C','D')
   	...
   	select @emcdactualdate = ActualDate, @emcdalloccode = AllocCode, @emcdcostcode = CostCode, @emcddescription = [Description], 
   		@emcddollars = Dollars, @emcdemcosttype = EMCostType, @emcdemgroup = EMGroup, @emcdemtrans = EMTrans, 
   		@emcdemtranstype = EMTransType, @emcdequipment = Equipment, @emcdglco = GLCo, @emcdgloffsetacct = GLOffsetAcct, 
   		@emcdgltransacct = GLTransAcct, @emcdinusebatchid = InUseBatchID, @emcdmatlgroup = MatlGroup,
   		@emcdreversalstatus = ReversalStatus, @emcdsource = Source, @emcdunits = Units 
   	...
   	if @oldgltransacct is not null
   		...
   		/* Use CheckSubType 'E' for WOPartsPosting per DianaR */
   		exec @rcode = dbo.bspGLACfPostable @co, @oldgltransacct, 'E', @errmsg output
   		if @rcode = 1
   			begin
   		...
   		end --if @oldgltransacct is not null
   	...
   	/* Validate OldAllocCode - can be null. */
   	/*if @oldalloccode is not null
   		begin
   		exec @rcode = bspEMAHVal @co, @oldalloccode, @errmsg output
   		if @rcode = 1
   			begin
   			select @errtext = isnull(@errorstart,'') + 'OldAllocCode ' + convert(varchar(5),@oldalloccode) + '-' + isnull(@errmsg,'')
   			exec @rcode =dbo.bspHQBEInsert @co, @mth, @batchid, @errtext, @errmsg output
   			if @rcode <> 0
   				begin
   				select @errmsg = isnull(@errtext,'') + '-' + isnull(@errmsg,'')
   	...
   	if @oldreversalstatus is null
   		...
   		if @oldreversalstatus not in (0,1,2,3,4)
   			begin
   			select @errtext = isnull(@errorstart,'') + 'OldReversalStatus ' + isnull(convert(char(2),@oldreversalstatus),'') + ' invalid. Must be 0, 1, 2, 3, or 4.'
   			exec @rcode =dbo.bspHQBEInsert @co, @mth, @batchid, @errtext, @errmsg output
   			if @rcode <> 0
   				begin
   				select @errmsg = isnull(@errtext,'') + '-' + isnull(@errmsg,'')
   ...
   bspexit:
   	if @rcode<>0 select @errmsg=isnull(@errmsg,'')	--+ char(13) + char(10) + '[bspEMVal_Cost_SeqVal_Alloc]'
   	return @rcode

GO
GRANT EXECUTE ON  [dbo].[bspEMVal_Cost_SeqVal_Alloc] TO [public]
GO

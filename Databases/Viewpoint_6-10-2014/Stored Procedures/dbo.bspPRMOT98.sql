...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMOT98    Script Date: 8/28/99 9:33:28 AM ******/
   CREATE   proc [dbo].[bspPRMOT98]
   /********************************************************
   ...
   *
   * USAGE:
   * 	Calculates 1998 Missouri Income Tax
   *
   * INPUT PARAMETERS:
   ...
   **********************************************************/
   ...
   select @rcode = 0, @rate = 0, @counter = 1, @procname = 'bspPRMOT98'
   ...
   if @status = 'S' 
   begin
   	select @dedn = 4250, @fedlimit = 5000
   	while @counter <= @exempts
   	begin
   
   		if @counter = 1 select @allowances = 1200
   
   		if @counter > 1 select @allowances = @allowances + 400
   ...
   if @status = 'M' 
   begin
   	select @dedn = 3550, @fedlimit = 10000
   	while @counter <= @exempts
   	begin
   		if @counter = 1 or @counter = 2 select @allowances = @allowances + 1200
   		
   		if @counter > 2 select @allowances = @allowances + 400
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMOT98] TO [public]
GO

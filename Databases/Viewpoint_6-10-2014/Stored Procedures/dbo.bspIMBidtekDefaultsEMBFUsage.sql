...
CREATE PROCEDURE [dbo].[bspIMBidtekDefaultsEMBFUsage]
	...
	*		GF 04/11/2013 - issue #147252 TFS-47042 added job to import parameters
	*
	*
...
select @ynactualdate ='N', @ynemgroup ='N', @yncostcode ='N', @yncosttype ='N', @ynmatlgroup ='N', @yninco ='N',
  ...
  begin
   ...
   if @defaultvalue = '[Bidtek]'  select @ynjcco ='Y'
	...
	----TFS-47042
	select @defaultvalue = IMTD.DefaultValue, @jobid = DDUD.Identifier From IMTD  with (nolock)
	inner join DDUD  with (nolock) on IMTD.Identifier = DDUD.Identifier and DDUD.Form = @Form
	Where IMTD.ImportTemplate=@ImportTemplate AND DDUD.ColumnName = 'Job'
	if @defaultvalue = '[Bidtek]'  select @ynjob ='Y'
   ...
   /*	If @Column='TaxType' select @TaxType = @Uploadval 
	...
	---- TFS-47042
	if @ynjob ='Y' and isnull(@Co,'') <> '' AND (ISNULL(@OverwriteJob, 'Y') = 'Y' OR ISNULL(@IsJobEmpty, 'Y') = 'Y')
		BEGIN
		SELECT @Job = Job
		FROM dbo.bEMEM

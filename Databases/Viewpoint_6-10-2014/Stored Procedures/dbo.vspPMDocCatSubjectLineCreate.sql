...
*				AJW 12/5/13 - TFS 65516 fix subject line for CCO & COR when contract is null
*				AJW 12/18/2013 - TFS 70040 fix for SBMTL/SBMTLPCKG
*				AJW 01/07/2014 - TFS 71351 REQQUOTE
*
*
* USAGE:
...
(@pmco bCompany = null, @doccat varchar(10) = null, @subjectlinelist nvarchar(max),
 ...
 @rfq_pco VARCHAR(30) = null, @doc_rev VARCHAR(30) = null, @doc_date bDate = null,
 @doc_log int = null, @Contract bContract = NULL, @keyid bigint = NULL, @msg nvarchar(max) = null output)
as
set nocount on
...
if @document is null and @keyid is null
	...
	if @doccat = 'SUBMIT'
		...
		if exists(select top 1 1 from dbo.PMSM with (nolock) where PMCo=@pmco and Project=@project)
			begin
			select top 1 @doctype = SubmittalType, @document = Submittal, @doc_rev = convert(varchar(30),Rev)
	...
	if @doccat = 'PURCHASE'
		begin
		if exists(select top 1 1 from dbo.POHD with (nolock) where JCCo=@pmco and Job=@project and PO=@document)
			begin
			select top 1 @document = PO
			from dbo.POHD with (nolock) where JCCo=@pmco and Job=@project and PO=@document
			order by POCo, PO
			end
		end
	...
	if @doccat = 'MTG'
		...
		if exists(select top 1 1 from dbo.PMMM with (nolock) where PMCo=@pmco and Project=@project)
			begin
			select top 1 @doctype = MeetingType, @doc_date = MeetingDate, @doc_log = Meeting, @doc_rev = convert(varchar(30),MinutesType)
	...
	if @doccat = 'CCO'
		...
		if exists(select top 1 1 from dbo.PMContractChangeOrder with (nolock) where PMCo=@pmco and Contract=@Contract)
			begin
			select top 1 @doctype = DocType, @document = CONVERT(VARCHAR(10), ID)
			from dbo.PMContractChangeOrder with (nolock) where PMCo=@pmco and Contract=@Contract
			order by PMCo, Contract, ID
			END
		end
	...
	if @doccat = 'SBMTL'
		...
		if exists(select top 1 1 from dbo.PMSubmittal with (nolock) where PMCo=@pmco and Project=@project)
			...
			order by PMCo, Project, SubmittalNumber
			END
		end
	...
	if @doccat = 'SBMTLPCKG'
		...
		if exists(select top 1 1 from dbo.PMSubmittalPackage with (nolock) where PMCo=@pmco and Project=@project)
			begin
			select top 1 @doctype = DocType, @document = CONVERT(VARCHAR(10), Package), @doc_rev = convert(varchar(30),PackageRev)
			from dbo.PMSubmittalPackage with (nolock) where PMCo=@pmco and Project=@project
			order by PMCo, Project, Package
			END
		end

	if @doccat = 'REQQUOTE'
		begin
		if exists(select top 1 1 from dbo.PMRequestForQuote with (nolock) where PMCo=@pmco)
			begin
			select top 1 @keyid = KeyID
			from dbo.PMRequestForQuote with (nolock) where PMCo=@pmco
			...
			if exists(select top 1 1 from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = @file_value and COLUMN_NAME = @column_value)
				begin
					...
					if @file_value = 'PMRQ'
						...
						set @sql = @sql + ' from dbo.PMRQ where PMRQ.KeyID = ' + dbo.vfToString(@keyid)
					...
					if @file_value = 'SLHD'
						...
						set @sql = @sql + ' from dbo.SLHD where SLHD.JCCo = ' + convert(nvarchar(10),@pmco)
					...
					if @file_value = 'PMSubmittal'
						...
						set @sql = @sql + ' from dbo.PMSubmittal where PMSubmittal.KeyID = ' + dbo.vfToString(@keyid)
					...
					if @file_value = 'PMSubmittalPackage'
						...
						set @sql = @sql + ' from dbo.PMSubmittalPackage where PMSubmittalPackage.PMCo = ' + convert(nvarchar(10),@pmco)
						set @sql = @sql + ' and PMSubmittalPackage.Project = ''' + @project + ' '''
						set @sql = @sql + ' and PMSubmittalPackage.Package = ''' + dbo.vfToString(@document) + ' '''
						set @sql = @sql + ' and PMSubmittalPackage.PackageRev = ''' + dbo.vfToString(@doc_rev) + ' '''
					...
					if @file_value = 'PMRequestForQuote' and dbo.vfToString(@keyid) <> ''
						...
						set @sql = @sql + ' from dbo.PMRequestForQuote where PMRequestForQuote.KeyID = ' + dbo.vfToString(@keyid)

...
CREATE proc  [dbo].[cvsp_CMS_APTD_Rounding] 
	( @fromco1 smallint
	, @fromco2 smallint
	, @fromco3 smallint
	, @toco smallint
...
=========================================================================
	...
	EXEC cvsp_CMS_APTD_Rounding 20,20,'',0
*/
...
from
	(Select APTD.APCo, APTH.Vendor, APTD.CMRef, PaidMth=max(APTD.PaidMth), TotalChk=sum(APTD.Amount)
		...
		where APTD.APCo=@VPtoco
		group by APTD.APCo, APTH.Vendor, APTD.CMRef)
	as APTD
	left join (select APCo=@VPtoco, VENDORNUMBER, CHECKNUMBER, Amt=sum(CHECKAMT)
			from CV_CMS_SOURCE.dbo.APTHCK
			where COMPANYNUMBER in (@fromco_1,@fromco_2,@fromco_3)
...
from CV_CMS_SOURCE.dbo.APTOPC h
	...
	left join (select d.APCo, h.Vendor, d.udYSN, PayType=max(d.PayType), CMCo=max(d.CMCo),
			...
			as d
			on d.APCo=@VPtoco and d.Vendor=h.VENDORNUMBER and d.udYSN=h.PAYMENTSELNO
...
from bAPTD 
	join (select APCo=@VPtoco, PAYMENTSELNO, 
...
where bAPTD.APCo=@VPtoco and bAPTD.CMAcct<>c.CMAcct 
	...
	join (select MinSeq, l.APCo, l.Mth, l.APTrans, l.APLine, LineAmt=sum(l.GrossAmt),
			...
			from bAPTL l
				...
				join (select APCo, Mth, APTrans, APLine, Amount=sum(Amount), SeqCount=count(APSeq), MinSeq=min(APSeq)
						from bAPTD
						where bAPTD.PayType<>2 and bAPTD.APCo=@VPtoco

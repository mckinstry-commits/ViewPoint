...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRFWT01old    Script Date: 8/28/99 9:33:20 AM ******/
      CREATE  proc dbo.bspPRFWT01old
      /********************************************************
      ...
      **********************************************************/
      ...
      select @rcode = 0, @basetax = 0, @dedn = 0, @rate = 0, @amt = 0, @procname = 'bspPRFWT01old'
      ...
      if @status = 'M'
      	...
      	if @a < 105200
      		begin
      		select @basetax = 6517.5, @dedn = 49900, @rate = .28
      		goto bspcalc
      		end
      	if @a < 171200
      		begin
      		select @basetax = 22001.5, @dedn = 105200, @rate = .31
      		goto bspcalc
      		end
      	if @a < 302050
      		begin
      		select @basetax = 42461.5, @dedn = 171200, @rate = .36
      		goto bspcalc
      		end
      	select @basetax = 89567.5, @dedn = 302050, @rate = .396
      	goto bspcalc
      	end
      /* single */
      if @status = 'S'
      	...
      	if @a < 62200
      		begin
      		select @basetax = 3907.5, @dedn = 28700, @rate = .28
      		goto bspcalc
      		end
      	if @a < 138400
      		begin
      		select @basetax =13287.5, @dedn = 62200, @rate = .31
      		goto bspcalc
      		end
      	if @a < 299000
      		begin
      		select @basetax =36909.5, @dedn = 138400, @rate = .36
      		goto bspcalc
      		end
      	select @basetax = 94725.5, @dedn = 299000, @rate = .396
      	goto bspcalc
      	end
      ...
      bspexit:
      	return @rcode
GO
GRANT EXECUTE ON  [dbo].[bspPRFWT01old] TO [public]
GO

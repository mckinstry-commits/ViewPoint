...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE [dbo].[bspIMBidtekDefaultsJCCB]
/***********************************************************
* CREATED BY: Danf
*  Modified: GG 11/27/00 - changed datatype from bAPRef to bAPReference
*			DANF 01/24/02 - Add default for Transaction Type, JC Transction Type, and Reversal Account
*			DANF 03/19/02 - Added Record Type
*			DANF 06/13/02 - Added ToJCCo
...
*			AW 2/28/2013 - TFS 40559 supply a default for JCTransType because validation will error if null also fixed incorrect joins
*
* Usage:
...
set nocount on
   ...
   declare @rcode int, @desc varchar(120), @ynphasegroup bYN, @ynglco bYN, @yngltransacct bYN, @ynum bYN,  @ynsource bYN, @ynToJCCo bYN,
			@CompanyID int, @defaultvalue varchar(30), @ReversalStatusID int, @TransTypeID int,  @JCTransTypeID int, @ToJCCoID INT,
			@Source varchar(10) -- issue #134510
   ...
   select @TransTypeID = DDUD.Identifier, @defaultvalue = IMTD.DefaultValue    
   From IMTD 
	join IMTR on IMTR.ImportTemplate = IMTD.ImportTemplate and IMTR.RecordType=IMTD.RecordType
	join DDUD on DDUD.Form=IMTR.Form and DDUD.Identifier=IMTD.Identifier
   ...
   select @ReversalStatusID = DDUD.Identifier, @defaultvalue = IMTD.DefaultValue    
   From IMTD 
	join IMTR on IMTR.ImportTemplate = IMTD.ImportTemplate and IMTR.RecordType=IMTD.RecordType
	join DDUD on DDUD.Form=IMTR.Form and DDUD.Identifier=IMTD.Identifier
   ...
   select @JCTransTypeID = DDUD.Identifier, @defaultvalue = IMTD.DefaultValue    
   From IMTD 
	join IMTR on IMTR.ImportTemplate = IMTD.ImportTemplate and IMTR.RecordType=IMTD.RecordType
	join DDUD on DDUD.Form=IMTR.Form and DDUD.Identifier=IMTD.Identifier
   ...
   if @@rowcount <> 0 and @defaultvalue = '[Bidtek]' AND (ISNULL(@OverwriteJCTransType, 'Y') = 'Y')
	...
	-- issue #134510
   select @Source = DDUD.Identifier, @defaultvalue = IMTD.DefaultValue    
   From IMTD 
	join IMTR on IMTR.ImportTemplate = IMTD.ImportTemplate and IMTR.RecordType=IMTD.RecordType
	join DDUD on DDUD.Form=IMTR.Form and DDUD.Identifier=IMTD.Identifier
   ...
   if @@rowcount <> 0 and @defaultvalue = '[Bidtek]' AND (ISNULL(@OverwriteReversalStatus, 'Y') = 'Y')
    begin
      ...
      where IMWE.ImportTemplate=@ImportTemplate and IMWE.ImportId=@ImportId and IMWE.Identifier = @Source
    end


   ----------------------------
   select @CompanyID = DDUD.Identifier, @defaultvalue = IMTD.DefaultValue    
   From IMTD 
	join IMTR on IMTR.ImportTemplate = IMTD.ImportTemplate and IMTR.RecordType=IMTD.RecordType
	join DDUD on DDUD.Form=IMTR.Form and DDUD.Identifier=IMTD.Identifier
   Where IMTD.ImportTemplate=@ImportTemplate AND DDUD.ColumnName = 'Co'
...
--			end
       ...
       If @ynToJCCo ='Y' and @Co is not null and @Co <> ''
    	     begin
   ...
   bspexit:
       select @msg = isnull(@desc,'Job Cost Detail') + char(13) + char(10) + '[bspBidtekDefaultJCCB]'
...
GO
GRANT EXECUTE ON  [dbo].[bspIMBidtekDefaultsJCCB] TO [public]
GO

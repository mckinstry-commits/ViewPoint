...
CREATE PROCEDURE [dbo].[mckspPMContractDD]
...
AS
	SELECT 
		...
		,ChangedOrderData.ACOHeaderDescription
		,ChangedOrderData.PCOHeaderDescription
	FROM 
		[dbo].[mckfnPMContractDDContract] (@Company, @BeginningContract, @EndingContract) ContractData
		INNER JOIN [dbo].[mckfnPMContractDDItem] (@Company, @BeginningContract, @EndingContract) ItemData ON (ContractData.JCCMCompany = ItemData.JCCICompany) AND (ContractData.JCCMContract = ItemData.JCCIContract)
		LEFT OUTER JOIN [dbo].[mckfnPMContractDDChangedOrder] (@Company, @BeginningContract, @EndingContract) ChangedOrderData ON (ItemData.JCCICompany = ChangedOrderData.PMOICompany) AND (ItemData.JCCIContract = ChangedOrderData.PMOIContract) AND (ItemData.JCCIItem = ChangedOrderData.PMOIACOItem)
	ORDER BY

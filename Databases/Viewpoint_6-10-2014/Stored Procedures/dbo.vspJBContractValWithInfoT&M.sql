...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.vspJBContractValWithInfoT&M   Script Date:  ******/
CREATE proc [dbo].[vspJBContractValWithInfoT&M]
/*******************************************************************************************
* CREATED BY:   TJL 102108 - Issue #128538, Created duplicate of vspJBContractValWithInfoProg before modification for Issue
* MODIFIED By :	TJL 01/05/09 - Issue #120173, Combine Progress and T&M Auto-Initialization
*  
*               
*
* USAGE:
...
(@jbco bCompany = 0, @contract bContract = null,
	...
	@billinitopt char(1) output, @msg varchar(60) output)
as
set nocount on
...
if @contractstatus > 1		--Soft or Final Closed
	begin
	---- Message displayed, Contract may be used.
	select @msg = 'Contract is closed.'
	end
...
if exists(select 1 from bJCCI with (nolock) where JCCo = @jbco and Contract = @contract and BillType = 'B')
	begin
   	select @billtype = 'B', @billinitopt = 'B'
   	end
else
   	begin
   	select @billtype = 'T', @billinitopt = 'T'
   	end
...
GO
GRANT EXECUTE ON  [dbo].[vspJBContractValWithInfoT&M] TO [public]
GO

...
--			SMAR.ARBatchId			AS ARBatchID,
			SMILDL.ARMth			AS ARPostedMonth,
			SMILDL.ARTrans			AS ARTrans,
...
LEFT JOIN	SMInvoiceListDetailLine	SMILDL
		ON	SMWC.SMCo			= SMILDL.SMCo
		AND SMWC.WorkOrder		= SMILDL.WorkOrder
		AND SMWC.WorkCompleted	= SMILDL.WorkCompleted
...
WHERE	SMI.SMCo = @Company
	...
	AND	(	@SMInvoiceNumber = SMI.InvoiceNumber OR @SMInvoiceNumber = 0)

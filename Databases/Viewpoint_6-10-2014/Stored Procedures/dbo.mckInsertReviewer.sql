...
IF (SELECT COUNT(*)
	...
	END
	BEGIN
	IF EXISTS(SELECT 1 FROM DDUP WHERE VPUserName = @VPUserName AND udReviewer IS NULL)
		BEGIN
			INSERT INTO HQRV (Reviewer, Name, RevEmail)
			SELECT @Reviewer, e.FirstName + ' ' + e.LastName, ISNULL(e.Email,(SELECT TOP 1 EMail FROM DDUP WHERE VPUserName = @VPUserName)) 
				FROM PREH e 
				WHERE e.PRCo = @Company AND e.Employee = @Employee
				--VALUES 
				--	(@Reviewer,
...
END
GO
GRANT EXECUTE ON  [dbo].[mckInsertReviewer] TO [public]
GO

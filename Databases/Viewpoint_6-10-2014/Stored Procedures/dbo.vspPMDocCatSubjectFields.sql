...
*				AJW 12/18/2013 - TFS 70040 SBMTL/SBMTLPCKG
...
--TFS 70040
if isnull(@doccategory,'') = 'SBMTL'
	begin
		insert into @columnslist (ColumnNames)
		select 'PMSubmittal.' + COLUMN_NAME from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'PMSubmittal'
		and COLUMN_NAME not in ('PMCo', 'VendorGroup', 'Notes', 'UniqueAttchID', 'KeyID')
		and isnull(DOMAIN_NAME,'') <> 'bNotes' and isnull(CHARACTER_MAXIMUM_LENGTH,0) <> -1
	end
if isnull(@doccategory,'') = 'SBMTLPCKG'
	begin
		insert into @columnslist (ColumnNames)
		select 'PMSubmittalPackage.' + COLUMN_NAME from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'PMSubmittalPackage'
		and COLUMN_NAME not in ('PMCo', 'VendorGroup', 'Notes', 'UniqueAttchID', 'KeyID')
		and isnull(DOMAIN_NAME,'') <> 'bNotes' and isnull(CHARACTER_MAXIMUM_LENGTH,0) <> -1
	end

if isnull(@doccategory,'') = 'REQQUOTE'
	begin
		insert into @columnslist (ColumnNames)
		select 'PMRequestForQuote.' + COLUMN_NAME from INFORMATION_SCHEMA.COLUMNS where TABLE_NAME = 'PMRequestForQuote'
		and COLUMN_NAME not in ('PMCo', 'VendorGroup', 'Notes', 'UniqueAttchID', 'KeyID')
		and isnull(DOMAIN_NAME,'') <> 'bNotes' and isnull(CHARACTER_MAXIMUM_LENGTH,0) <> -1
	end

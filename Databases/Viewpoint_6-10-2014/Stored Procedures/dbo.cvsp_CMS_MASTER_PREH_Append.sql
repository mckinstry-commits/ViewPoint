...
=========================================================================================
	Title:		Append PR Employees (PREH) 
	Created:	10/23/2013
	Created By:	VCS Technical Services - Bryan Clark
	Notes:		Use this procedure to add records to PR Employee Master without deleting existing
					records.
...
CREATE PROCEDURE [dbo].[cvsp_CMS_MASTER_PREH_Append] 
...
/** Not deleting transactions...appending only
...
from CV_CMS_SOURCE.dbo.PRTMST T with (nolock)
	...
	--CMSDedType = 'L'
	left join bPREH eh
		on eh.Employee = T.EMPLOYEENUMBER and eh.PRCo = @toco
	left join bPREH ssn
		on replace(ssn.SSN, '-', '') = T.SOCIALSECNO and ssn.PRCo = @toco
where eh.Employee is null and ssn.Employee is null and T.COMPANYNUMBER=@fromco

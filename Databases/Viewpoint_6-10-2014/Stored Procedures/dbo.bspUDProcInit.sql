...
SET ANSI_NULLS ON
GO
CREATE       PROCEDURE [dbo].[bspUDProcInit] 
     /******************************************************
     	Created 04/05/01 RM
     ...
     *******************************************************/
     ...
     declare @rcode int,@seq int,@andor varchar(5),@typepc char(1), @param varchar(20),@operator varchar(5),
                 @type char(1),@columnname varchar(30),@value varchar(50),@notes varchar(255),
     ...
     begin
     	...
     	begin
     		select @checkstring = '/**' +  isnull(@comment,'') + '**/' + char(13) + char(10) + 'if exists(select * from [' + @tablename + '] where ' + @checks
     	
     		select @msg = isnull(@msg,'') + @crlf + @crlf + @crlf + isnull(@checkstring,'') + ')' + @crlf +
     				'begin' + @crlf +
     				'select @msg = isnull(' + case when @desccolumn is not null then '[' + @desccolumn + ']' else 'null' end + ',@msg) from [' + @tablename + '] where ' + @checks + @crlf +
     				'end' + @crlf +
     				'else' + @crlf +
     				'begin' + @crlf +
     ...
     select @msg = @msg + @crlf + @crlf + 'spexit:' + @crlf + @crlf + 'if @rcode = 1 select @msg = @msg + ' + char(39) + ' - [' + @valproc + ']' + char(39) +
     @crlf + @crlf + 'return @rcode'

GO
GRANT EXECUTE ON  [dbo].[bspUDProcInit] TO [public]
GO

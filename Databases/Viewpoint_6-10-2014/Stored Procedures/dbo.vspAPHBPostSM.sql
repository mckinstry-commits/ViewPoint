...
*				EricV 05/14/13 TFS-50104 Receive the GL Interface Level when called and pass to the vspSMWorkCompletedPost procedure.
*               EricV 05/21/13 TFS-50951 Use value of 'N' instead of 'C' for PriceMethod
*				EricV 05/31/13 TFS-4171 Replaced Work Completed Coverage field with NonBillable and UseAgreementRates fields
*				EricV 06/27/13 TFS-50101 Update the GL Interface Level when inserting into vSMDetailTransaction
*				EricV 07/02/13 TFS-45346 Fix problem with clearing AP SM batch data before batch completed posting.
*				JVH 7/8/13 TFS-54658 Fixed updating price and tax values when the work completed is non-billable
*				DKS 12/5/13 TFS-68516 fix for multiple SM WC lines using the same total billable on post
* Usage:
*	
*
* Input params:
...
*******************************************************/
   	...
   	(@apco bCompany, @mth bMonth, @batchid int, @dateposted bDate, @GLInterfaceLevel tinyint, @errmsg varchar(100) output) 	
AS
BEGIN
	SET NOCOUNT ON
	...
	@CostRate bUnitCost,
	@PriceQuantity bUnits, @PriceRate bUnitCost, @PriceTotal bDollar,						
	--Get Agreement Information
	@Agreement Varchar(15), @Revision int, @NonBillable bYN, @UseAgreementRates bYN
	...
	Agreement, Revision,
	CASE
		WHEN SMWorkOrderScope.PriceMethod = 'T' THEN 'N'
		ELSE 'Y'
	END NonBillable,
	SMWorkOrderScope.UseAgreementRates
	FROM vAPSM
	LEFT JOIN bAPTL ON 
	...
	BEGIN TRY
			...
			BEGIN
				...
				BEGIN
					...
					BEGIN
						...
						IF @NonBillable = 'Y'
						BEGIN
							SELECT @PriceQuantity = NULL, @PriceRate = NULL, @PriceTotal = NULL, @smtaxcode = NULL, @taxtype = NULL, @taxamt = NULL
						END
						ELSE
						BEGIN
							SET @PriceTotal = NULL

							SET @PriceQuantity =  CASE @units WHEN 0 THEN NULL ELSE @units END
							...
							BEGIN
								...
								FROM dbo.vfSMRatePurchase(@smco, @smworkorder, @smscope, @apinvdate, @Agreement, @Revision, @NonBillable, @UseAgreementRates, @MaterialGroup, @Material, @MaterialUM, @Quantity, @actualamt, @MaterialUM, 'E')
							END
						
							IF @PriceTotal IS NULL
							BEGIN
								SELECT @PriceTotal = @actualamt, @PriceRate = CASE @CostRate WHEN 0 THEN NULL ELSE @CostRate END
							END
						END
						...
						FROM vfSMGetAccountingTreatment(@smco, @smworkorder, @smscope, 'M', @smcosttype)
						...
						ActualCost, CostQuantity, CostRate,
						PriceQuantity, PriceRate, PriceTotal, 
						[Description], NoCharge, APTLKeyID, TaxCode, TaxType, TaxBasis, TaxAmount, MonthToPostCost, SMCostType,
						JCCostType, PhaseGroup, CostDetailID, Agreement, Revision, NonBillable, UseAgreementRates)
						...
						@PriceQuantity, @PriceRate, @PriceTotal,
						@smdesc, 'N', @apkeyid, @smtaxcode, @taxtype, 
						CASE 
							WHEN @smtaxcode IS NULL THEN NULL
							ELSE @PriceTotal
						END
						, 
						@taxamt, @mth, @smcosttype, @jccosttype, @phasegroup, @CostDetailID, @Agreement, @Revision, @NonBillable, @UseAgreementRates)
					...
					ELSE IF @transtype = 'C'
					BEGIN
						IF EXISTS(SELECT 1 FROM dbo.SMWorkCompleted WHERE APTLKeyID = @apkeyid AND NonBillable = 'Y')
						BEGIN
							SELECT @PriceQuantity = NULL, @PriceRate = NULL, @PriceTotal = NULL, @smtaxcode = NULL, @taxtype = NULL, @taxamt = NULL
						END
						ELSE
						...
						--If the work completed is part of an invoice only update cost fields, not price fields
						IF EXISTS
						(
							SELECT 1 
							...
							WHERE vSMWorkCompleted.APTLKeyID = @apkeyid
						)
						BEGIN
					...
					BEGIN
						INSERT dbo.vSMDetailTransaction (IsReversing, Posted, GLInterfaceLevel, HQDetailID, SMWorkCompletedID, SMWorkOrderScopeID, SMWorkOrderID, LineType, TransactionType, SourceCo, Mth, BatchId, GLCo, GLAccount, Amount)
						SELECT 0 IsReversing, 1 Posted, @GLInterfaceLevel,
						SMWorkCompleted.CostDetailID, 
						SMWorkCompleted.SMWorkCompletedID, 
						vSMWorkOrderScope.SMWorkOrderScopeID, 
						vSMWorkOrder.SMWorkOrderID, 
						SMWorkCompleted.[Type], 
						'R' TransactionType, 
						@apco, 
						@mth, 
						@batchid, 
						vfSMGetWorkCompletedGL.GLCo, 
						vfSMGetWorkCompletedGL.CurrentRevenueAccount, 
						-ISNULL(SMWorkCompleted.PriceTotal, 0)
						...
						BEGIN
							EXEC @rcode = dbo.vspSMJobCostDistributionInsert @SMWorkCompletedID=@SMWorkCompletedID, @BatchCo=@apco,@BatchMth=@mth,@BatchId = @batchid, @BatchSeq = @batchseq, @Line = @apline, @JCTransType='AP',@errmsg = @errmsg OUTPUT
							IF @rcode <> 0 
							BEGIN
				...
				BEGIN
					...
					BEGIN
						INSERT dbo.vSMDetailTransaction (IsReversing, Posted, GLInterfaceLevel, HQDetailID, SMWorkCompletedID, SMWorkOrderScopeID, SMWorkOrderID, LineType, TransactionType, SourceCo, Mth, BatchId, GLCo, GLAccount, Amount)
						SELECT 1 IsReversing, 1 Posted, @GLInterfaceLevel, HQDetailID, SMWorkCompletedID, SMWorkOrderScopeID, SMWorkOrderID, LineType, TransactionType, @apco, @mth, @batchid, GLCo, GLAccount, -SUM(Amount)
						FROM dbo.vSMDetailTransaction
		...
		WHERE vAPSM.APCo = @apco AND vAPSM.Mth = @mth AND vAPSM.BatchId = @batchid AND vAPSM.SMType = 3 AND vAPSM.TransType = 'D' AND 
			vSMInvoiceDetail.SMInvoiceDetailID IS NULL
	...
	EXEC @rcode = dbo.vspSMWorkCompletedPost @BatchCo = @apco, @BatchMth = @mth, @BatchId = @batchid, @GLInterfaceLevel=@GLInterfaceLevel, @msg = @errmsg OUTPUT

...
SET ANSI_NULLS ON
GO
CREATE             proc [dbo].[brptPRCertClass] (@PRCo tinyint, @PRGroup tinyint, @PREndDate smalldatetime,  @TotalMasterOrFullJob char(1),
...
/* Issue 127622 Added Week Number */
...
for both Employee and payroll detail lines */
   ...
   @PaySeq tinyint, @CMRef varchar(10), @CompanyDesc varchar(60), @Craft varchar (10), @Class varchar (10), @EarnTotalLine tinyint
  ...
  create clustered index biJobsCert on #JobsForCert (JCCo, Job) 
   /* this is the code form the fix to brptPRCert/making same changes here
  ...
  */
   insert into #EmployeeJobs
   --select distinct JCCo, Job, Employee, 0 From   -- old code
  /*select distinct JCCo, Substring(Job,1,@SignifJobChar), Employee, 0 From
   PRTH  Where PRCo=@PRCo and PRGroup=@PRGroup and PostDate>=@FirstDayOfWeek and PostDate<=@LastDayOfWeek and
   	Cert='Y' and Amt<>0 and PREndDate = (case when @PRDateHrsYN='Y' then @PREndDate else PREndDate end) */
  ...
  Group by JCCo, Substring(Job,1,@SignifJobChar) 
   ...
   ci.Employee, ci.RecId, LastName, FirstName, MidName, Suffix,--Addition made 4/2/2002 AA.
   e.SortName, e.Address, e.City, e.State, e.Zip, e.Address2, SSN, Sex, Race, FileStatus, RegExempts, EmpCraftDesc=cm_emp.Description, EmpClassDesc=cc_emp.Description,
   CraftDesc=cm.Description, ClassDesc=cc.Description, cc_emp.EEOClass, CheckNumbers, th.PaySeq, PostSeq, WkDay, ci.EarnCode, ci.DetOnCert,
...
GO
GRANT EXECUTE ON  [dbo].[brptPRCertClass] TO [public]
GO

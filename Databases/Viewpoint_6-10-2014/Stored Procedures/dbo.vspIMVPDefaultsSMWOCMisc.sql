...
CREATE PROCEDURE [dbo].[vspIMVPDefaultsSMWOCMisc]
	...
	* Changes: 4/17/13  JVH TFS-44860 Modified for invoice detail changes
	*		   05/31/13 EricV TFS-4171 Replaced Work Completed Coverage field with UseAgreementRates field. Coverage of 'C' is not indicated by using the NonBillable flag.
    *          06/20/13 EricV TFS-52109 Set Billable fields to NULL when NonBillable flag is set to 'Y'
	*		   07/10/13 ScottAlvey TFS-55505 - Agreement Rev comes over correctly when the related Scope does not have an agreement
    *
    * Input params:
...
/* Working Variables */
    ...
    DECLARE @defaultUseAgreementRates bYN;
	DECLARE @defaultNonBillable		 bYN;
...
/* Set Overwrite flags */ 
    ...
    SELECT @OverwriteUseAgreementRates =
        dbo.vfIMTemplateOverwrite(@ImportTemplate, @Form, 'UseAgreementRates', @rectype);
...
*******/ 
    ...
    SELECT @UseAgreementRatesID = dbo.bfIMTemplateDefaults(@ImportTemplate, @Form, 'UseAgreementRates', @rectype, 'N');
...
--        IF @Column='ServiceSite' SELECT @ServiceSite=@Uploadval; - Cannot supply.
        ...
        IF @Column='UseAgreementRates' SELECT @UseAgreementRates=@Uploadval;
        ...
        IF @Column='UseAgreementRates'
            SET @IsEmptyUseAgreementRates = CASE WHEN @Uploadval IS NULL then 'Y' ELSE 'N' END ;
...
-- check to make sure the Original Type is also a three				              
 		IF @WorkCompletedID <> 0 AND ISNULL(@IsEmptyWorkCompleted, 'Y') = 'N' AND NOT ISNUMERIC(@WorkCompleted) = 1 -- updating an existing WorkCompleted
 			BEGIN;
				UPDATE IMWE
...
-- clear default values
			 SELECT @defaultCostAccount = NULL,  @defaultRevenueAccount  = NULL,  @defaultCostWIPAccount = NULL
					 ...
					 ,@defaultAgreement = NULL,  @defaultRevision = NULL, @defaultUseAgreementRates = NULL, @defaultNonBillable = NULL
...
-- validate
			IF @ScopeID <> 0 
				...
				EXEC @rc = vspSMWorkCompletedScopeVal @SMCo, @WorkOrder, @Scope, @LineType =@Type
					 ...
					 ,@UseAgreementRates=@defaultUseAgreementRates OUTPUT
					 ,@NonBillable=@defaultNonBillable OUTPUT
 ...
 --SELECT @defaultCostAccount ,  @defaultRevenueAccount  ,  @defaultCostWIPAccount 
	...
	--				 ,@defaultAgreement ,  @defaultRevision , @defaultUseAgreementRates 
	--				 ,@defaultIsAgreement , @defaultProvisional , @msg    
		IF @defaultAgreement IS NULL
		BEGIN;
			SELECT @defaultRevision = NULL, @defaultUseAgreementRates =NULL
...
/**********Revision  *************/   
        IF @RevisionID <> 0  -- always overwrite Revision
            ...
            BEGIN;
                SELECT @Revision = CASE WHEN @Agreement IS NULL THEN NULL else Coalesce(@defaultRevision, @RevisionOut) END;;
                UPDATE IMWE
					...
					ELSE 
						BEGIN;
							EXEC @rc=dbo.vspSMWorkCompletedAgreementVal @SMCo, @WorkOrder, @Agreement 
								,@Revision = @Revision
								,@AgainstCur = 'Y'
							    ,@RevisionOut = @RevisionOut OUTPUT 
								, @msg = @msg OUTPUT;	
								IF @rc<>0  
...
/**********UseAgreementRates  *************/  
        IF @UseAgreementRatesID <> 0 
            AND (ISNULL(@OverwriteUseAgreementRates, 'Y') = 'Y' 
                 OR ISNULL(@IsEmptyUseAgreementRates, 'Y') = 'Y'
                 OR @Agreement IS NULL) -- set UseAgreementRates to null if @Agreement is null)
            BEGIN;
                SELECT @UseAgreementRates = CASE WHEN @Agreement IS NULL THEN 'N' else @defaultUseAgreementRates END;
                UPDATE IMWE
...
/********** Validate UseAgreementRates  *************/ 
		IF @UseAgreementRatesID<>0 AND @UseAgreementRates IS NOT null -- null UseAgreementRates allowable
			BEGIN;
				SELECT @msg=NULL;
				IF @Agreement IS NOT NULL AND @UseAgreementRates<>'Y' AND ISNULL(@UseAgreementRates,'N')<>'N'
					SELECT @msg = '** UseAgreementRates can only be ''Y'' or ''N''.';
				IF @defaultUseAgreementRates IS NOT NULL AND @UseAgreementRates<>@defaultUseAgreementRates
					SELECT @msg = '** A different UseAgreementRates than scope is not allowed when scope has an Agreement';
				IF @Agreement IS null AND ISNULL(@UseAgreementRates,'N')<>'N'
					SELECT @msg = '** UseAgreementRates not allowed if no Agreement';
				IF @msg IS NOT null	
				UPDATE IMWE
					SET IMWE.UploadVal = @msg
					WHERE IMWE.ImportTemplate=@ImportTemplate 
...
/********** Default NonBillable ************/
			IF ISNULL(@NonBillableID,0)<>0
				...
				BEGIN
					SET @NonBillable = CASE WHEN @defaultNonBillable IS NOT NULL THEN @defaultNonBillable ELSE ISNULL(@NonBillable,'N') END
					UPDATE IMWE
						SET IMWE.UploadVal = @NonBillable
						WHERE IMWE.ImportTemplate=@ImportTemplate 
							AND IMWE.ImportId=@ImportId 
							AND IMWE.RecordType = @rectype
...
/********** Validate NonBillable  *************/
			IF @NonBillableID <> 0 AND @NonBillable IS NOT NULL  
				BEGIN;
					SELECT @msg=NULL;
					IF @NonBillable<>'Y' AND @NonBillable<>'N'
						SELECT @msg = '** NonBillable can only be ''Y'' or ''N''.';
					IF @defaultNonBillable IS NOT NULL AND @NonBillable<>@defaultNonBillable
						SELECT @msg = '** Different NonBillable than scope is not allowed';
					IF @msg IS NOT null	
					UPDATE IMWE
						SET IMWE.UploadVal = @msg
						WHERE IMWE.ImportTemplate=@ImportTemplate 
					...
					IF @IsEmptyPriceRate='Y'
						BEGIN
							...
							from [dbo].[vfSMGetStandardItemRate] (@SMCo, @WorkOrder, @Scope, @Date, @StandardItem
														, @Agreement, @Revision , @NonBillable, @UseAgreementRates)
						END			
...
/**********  PriceQuantity  *************/ 
-- per meeting IF NonBillable='Y' THEN Billable cant be entered 
		IF @PriceQuantityID<>0 
            UPDATE IMWE
            SET IMWE.UploadVal = CASE WHEN @NonBillable='Y' THEN NULL ELSE @PriceQuantity end
            WHERE IMWE.ImportTemplate=@ImportTemplate 
                AND IMWE.ImportId=@ImportId 
                AND IMWE.RecordSeq=@currrecseq
                AND IMWE.RecordType = @rectype
...
/********** PriceRate  *************/  
 		IF @PriceRateID<>0 
            UPDATE IMWE
            SET IMWE.UploadVal = CASE WHEN @NonBillable='Y' THEN NULL ELSE @PriceRate end
            WHERE IMWE.ImportTemplate=@ImportTemplate 
                AND IMWE.ImportId=@ImportId 
                AND IMWE.RecordSeq=@currrecseq
                AND IMWE.RecordType = @rectype
...
/**********PriceTotal  *************/  
 		IF @PriceTotalID<>0 
            UPDATE IMWE
            SET IMWE.UploadVal = CASE WHEN @NonBillable='Y' THEN NULL ELSE @PriceTotal END 
            WHERE IMWE.ImportTemplate=@ImportTemplate 
                AND IMWE.ImportId=@ImportId 
                AND IMWE.RecordSeq=@currrecseq
                AND IMWE.RecordType = @rectype
...
--clear default values
		...
		BEGIN;
				...
				ELSE IF ISNULL(@NonBillable,'')='Y' AND @TaxCode IS NOT NULL
					SELECT @rc=1, @msg = '** Invalid TaxCode not allowed if NonBillable';
				ELSE 
					BEGIN;								
				...
				IF @rc<>0	
					UPDATE IMWE
						...
						WHERE IMWE.ImportTemplate=@ImportTemplate 
							...
							AND IMWE.Identifier = @TaxCodeID;
		END;
IF @defaultTaxCode IS NULL OR @defaultJob IS NULL OR @NonBillable='Y' SET @IsEmptyTaxType='Y' -- trigger to reget the default
...
/********** Validate TaxType  *************/  -- 
		IF  @TaxTypeID <> 0 AND @defaultJob IS NOT NULL
			BEGIN;
				...
				Else IF ISNULL(@NonBillable,'')='Y' AND @TaxType IS NOT NULL
					SELECT @msg = '** Invalid TaxType not allowed if NonBillable',@rc=1;
				Else
					BEGIN;
...
/********** Validate TaxBasis  *************/             
         IF @TaxBasisID <> 0 AND @defaultJob IS NOT NULL AND (ISNULL(@IsEmptyTaxBasis, 'Y') = 'N')
            BEGIN;
				...
				ELSE If ISNULL(@NonBillable,'')='Y' AND @TaxBasis IS NOT NULL 
					SELECT @rc=1, @msg='**Invalid Tax Basis must be empty when NonBillable';
				IF @rc<>0 
					UPDATE IMWE
					SET IMWE.UploadVal = @msg
					WHERE IMWE.ImportTemplate=@ImportTemplate 
	...
	BEGIN;
		UPDATE IMWE
			...
			WHERE IMWE.ImportTemplate=@ImportTemplate 
				...
				AND dbo.IMWE.UploadVal NOT LIKE ('*%');
	END;

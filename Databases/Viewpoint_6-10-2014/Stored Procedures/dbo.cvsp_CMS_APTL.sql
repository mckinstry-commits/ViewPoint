...
select @PhaseGroup=PhaseGroup, @TaxGroup=TaxGroup from bHQCO where HQCo=@VPtoco;
...
--delete trans
BEGIN TRAN
delete bAPTL where APCo=@VPtoco
	--and udConv = 'Y';
COMMIT TRAN
...
left join Viewpoint.dbo.budxrefJCJobs xj
	on xj.COMPANYNUMBER = d.COMPANYNUMBER and xj.DIVISIONNUMBER = d.DIVISIONNUMBER and xj.JOBNUMBER = d.JOBNUMBER
		and xj.SUBJOBNUMBER = d.SUBJOBNUMBER and xj.VPJob is not null --McKinstry only wants to pull active jobs.
...
WHERE d.COMPANYNUMBER in (@fromco_1,@fromco_2,@fromco_3)

...
insert bAPVM (VendorGroup, Vendor, SortName, Name, Type, TempYN, Contact, Phone ,Fax ,EMail,
	...
	udSource, udConv, udCGCTable, udCGCTableID, udSubcontractorYN, udCGCVendor, udEmployeeYN,
	udEmployee, udPRCo )
...
select distinct @VendorGroup
	...
	,Address=case when E.VENDORTYPE = 'E' then null else rtrim(E.ADDRESS25A) end
	,City=case when E.VENDORTYPE = 'E' then null when E.CITY18='' then null else rtrim(E.CITY18) end
	,State=case when E.VENDORTYPE = 'E' then null when E.STATECODE='' then null else rtrim(E.STATECODE) end
	,Zip=case when E.VENDORTYPE = 'E' then null when ZIPCODE='' then null else rtrim(E.ZIPCODE) end
	,Address2= NULL
	,Purge='N'
	,@CustGroup
	,Customer=null
	...
	,PayTerms=isnull(p.PaymentTerms,'30')--case when E.TERMSCODE='' then null else + convert(varchar(2),E.TERMSCODE) end
	,GLCo=@toco
	...
	, udEmployeeYN		= case 
							when E.VENDORTYPE = 'E' then 'Y' 
							else 'N' 
						  end
	, udEmployee		= case 
							when E.VENDORTYPE = 'E' then E.CUSTREFNUMBER 
							else null 
						  end
	, udPRCo			= case 
							when E.COMPANYNUMBER IN (1,15,50) then 1
							else E.COMPANYNUMBER 
						  end
...
left join budXRefPayTerms p
  on p.CGCCode = E.TERMSCODE 
left join budxrefJCJobs job
  on job.COMPANYNUMBER = E.COMPANYNUMBER
 and job.DIVISIONNUMBER = E.DIVISIONNUMBER
 and job.SUBJOBNUMBER = E.SUBJOBNUMBER
 and job.JOBNUMBER = E.JOBNUMBER 
where E.STATUSCODE = 'A'
   or job.VPJob is not null

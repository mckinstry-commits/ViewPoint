...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_AU_AllowanceRDO]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_AU_AllowanceRDO]
   /********************************************************
   * CREATED BY: 	EN 3/10/2009 #129888
   * MODIFIED BY:  
   *
   * USAGE:
   * 	Calculates rate per hour allowance with RDO factor adjustment and posts to timecard addons table (bPRTA).
   ...
   *	2nd week (ie. 40 regular hours are posted in the first week and 32 regular plus 8 RDO hours are posted
   *	in the second week), with this scheme 36 hours are used each week to compute the allowance whereas in the
   ...
   **********************************************************/
	...
	select @rcode = 0, @hours = 0, @subjecthours = 0, @amt = 0, @procname = 'bspPR_AU_AllowanceRDO'
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_AU_AllowanceRDO] TO [public]
GO

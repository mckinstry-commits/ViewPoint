...
CREATE  procedure [dbo].[bspIMUploadDirect]
		 ...
		 *              JRE 03/12/12 - TFS# 43480 -  Do not auto increment Seq number for the PRDD Table.
         *
      ...
      -- end
       ...
       BEGIN	--LOOP A
		...
		select @coltype = ColType from DDUD with (nolock) where Form = @form and Identifier = @ident
       	...
       	if upper(@column) = 'SEQ' AND @tablename not in ('PRAE','PRDD','PMSubmittal') select @uploadval = @recseq 

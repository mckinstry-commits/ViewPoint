...
CREATE    proc [dbo].[bspMSTicMatlPriceGet]
	...
	*				GF 07/01/2013 TFS-54385 when checking MSTP for zero unit price found, need to consider rate also.
    *
    *
    *
...
if @quote is not null and @locgroup is not null and @category is not null
   	...
   	if @tojcco is not null and @phase is not null
   		...
   		if @@rowcount <> 0
   			BEGIN
			----TFS-54385              
			IF @unitprice = 0 AND @rate = 0 SET @MSMD_Found = 1  
   			GOTO cleanup
			END
        ...
        if @material is not null
            ...
            and Material=@material and UM=@um
   			if @@rowcount <> 0
				BEGIN
				----TFS-54385              
				IF @unitprice = 0 AND @rate = 0 SET @MSTP_Found = 1              
				GOTO cleanup
				END
            END ----end material

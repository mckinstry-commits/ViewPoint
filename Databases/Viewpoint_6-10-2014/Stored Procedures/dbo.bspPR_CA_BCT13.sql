...
CREATE  proc [dbo].[bspPR_CA_BCT13]
/********************************************************
...
*				CHS	11/29/2011	- TK-20165 B-11799 #147550 British Columbia tax update effective 1/1/2013
*				KK 12/17/2012   - TK-20165 B-11799 #147550 T2 calculating incorrectly
*
* USAGE:
...
BEGIN
	...
	SELECT @ProcName = 'bspPR_CA_BCT13'
	...
	IF @TCP IS NULL SELECT @TCP = 10276


	-- establish tax rate and constant
					SELECT @Rate = .1470, @KP = 7164
	IF @A <= 104754 SELECT @Rate = .1229, @KP = 4640
	IF @A <=  86268 SELECT @Rate = .1050, @KP = 3096
	IF @A <=  75138 SELECT @Rate = .0770, @KP = 992
	IF @A <=  37568 SELECT @Rate = .0506, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_BCT13] TO [public]
GO

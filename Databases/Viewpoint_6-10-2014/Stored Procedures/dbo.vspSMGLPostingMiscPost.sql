...
CREATE PROCEDURE [dbo].[vspSMGLPostingMiscPost]
	...
	*				05/14/2013 EricV - TFS-50166 Added the GL Interface Level to the call to the procedure vspSMWorkCompletedPost.
    *
...
SET NOCOUNT ON
	...
	--the cost flags.
	EXEC @rcode = dbo.vspSMWorkCompletedPost @BatchCo = @co, @BatchMth = @mth, @BatchId = @batchid, @GLInterfaceLevel = @GLLvl, @msg = @msg OUTPUT
	IF @rcode <> 0 RETURN @rcode

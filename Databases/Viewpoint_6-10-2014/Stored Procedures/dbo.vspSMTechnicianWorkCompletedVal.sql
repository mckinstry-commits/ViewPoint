...
--           EricV 5/08/11 - Changed call to vspSMTechnicianVal to use named parameters.
--           EricV 6/24/13 - TFS-52109 Change to not report an error when the Labor record has been posted.
...
BEGIN
	...
	EXEC @rcode = vspSMTechnicianVal @SMCo=@SMCo, @Technician=@Technician, @PRCo=@PRCo OUTPUT, @INCo=@INCo OUTPUT, @INLocation=@INLocation OUTPUT, @msg=@msg OUTPUT

...
CREATE PROCEDURE [dbo].[vspSMAgreementInvoiceBatchCreate] 
	...
	*			   Eric V - 01/17/2013 D-06216 Added BatchMonth to where clause when selecting Invoices
				   Dan K - 02/17/2013 TFS-40936
				   JVH - 5/30/13 TFS - 44858 Modifed to support changes for SM Invoice
	* Usage:
	*	
	*
	* Input params:
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @ARCo, @BatchMth, @BatchId,
				...
				SMDepartment.GLCo, 
				CASE 
					WHEN SMAgreement.RevenueRecognition = 'S' AND BillingType = 'S' THEN SMDepartment.AgreementRevDefGLAcct
					ELSE SMDepartment.AgreementRevGLAcct
				END,
				TaxGroup, 
				TaxCode,
		...
		BEGIN
			...
			INSERT dbo.vSMInvoiceARBH (SMCo, Invoice, IsReversing, VoidInvoice, Co, Mth, BatchId, BatchSeq)
			SELECT SMCo, Invoice, 1 IsReversing, 1 VoidInvoice, @ARCo, @BatchMth, @BatchId, @NextBatchSeq
			FROM dbo.vSMInvoice
			WHERE SMInvoiceID = @SMInvoiceID

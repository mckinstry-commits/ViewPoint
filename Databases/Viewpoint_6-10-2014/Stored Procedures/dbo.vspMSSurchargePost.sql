...
*				GF 04/30/2013 TFS-48687 surcharges haul values do not change when posted to AP
*
*
...
SET NOCOUNT ON
	...
	----TFS-48687
	SELECT @APRef = OldAPRef, @MatlAPRef = OldMatlAPRef
	FROM dbo.bMSTB WITH (NOLOCK)
		...
		BEGIN
			...
			IF @HaulerType = 'H' AND @PayCode IS NOT NULL
				BEGIN
					...
					----TFS-48687
					IF (@APRef IS NULL AND @MatlAPRef IS NULL) OR @BatchTransType = 'A'
						BEGIN                  

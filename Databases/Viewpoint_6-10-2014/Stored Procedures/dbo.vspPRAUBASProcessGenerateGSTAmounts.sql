...
GO
	...
	*				MV 11/12/13 TFS-66451/66935 include cleared transactions in non-retention AP amounts
	*
...
--------------------------------------------------------------------------------------------------------------
	...
	WHERE h.APCo=@Co 
		...
		--AND d.Status <> 4) -- don't restrict by Status, include Status 4 - cleared
		AND t.Item='G10'
		AND t.PRCo=@Co 
		AND t.TaxYear=@TaxYear
		AND t.Seq=@Seq 

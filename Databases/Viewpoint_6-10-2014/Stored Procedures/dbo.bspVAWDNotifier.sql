...
CREATE PROCEDURE [dbo].[bspVAWDNotifier]
		...
		*               JR 01/02/14 - #TFS-70815 Adding in a distinct to handle an issue where duplicate data was causing a unique constraint to fail.
		*			
        			...
        			BEGIN
						...
						ELSE --Query is Event based
							BEGIN
								...
								SELECT @CurrentRecord, @JobName
								WHERE NOT EXISTS
								   (
								   SELECT 1
								   FROM vWFSentNotifications
								   WHERE KeyHash = @CurrentRecord
								   AND JobName = @JobName
								   );
							END
					END
...
Build_Line_Query:
			...
			BEGIN			
				SELECT @sql = 'INSERT INTO vWFSentNotifications (JobName, KeyHash)
								SELECT DISTINCT ''' + @JobName + ''', KeyHash 
								FROM ' + QUOTENAME(@tempDataTableName) + ' td '
								+ ' WHERE NOT EXISTS 
								     (
								     SELECT 1
								     FROM vWFSentNotifications
								     WHERE JobName = ''' + @JobName + '''
								     AND KeyHash = td.KeyHash
								     ) ';					

...
*				AW 3/11/2013 - TFS 43398 - Verify no PM SubCOs exist to prevent FK violation
*
* USAGE:
	...
	fetch next from bcSLHB into @seq, @transtype, @sl, @vendorgroup, @vendor, @holdcode, @payterms,
   	...
   	if @@fetch_status <> 0 goto header_end
		...
		if @transtype = 'D' 
			...
			if exists(select 1 from vPMSubcontractCO where SLCo = @co and SL = @sl)
				begin
				select @errortext = @errorstart + ' - Cannot delete SL / PM SL Change Order exists!'
				+ ' PMCo:'+ dbo.vfToString(PMCo)+ ' Project:'+  dbo.vfToString(Project) + ' CO#'+dbo.vfToString(SubCO)
				from vPMSubcontractCO where SLCo = @co and SL = @sl
				exec @rcode = bspHQBEInsert @co, @mth, @batchid, @errortext, @errmsg output
				if @rcode <> 0 goto bspexit
				goto header_loop	
				end

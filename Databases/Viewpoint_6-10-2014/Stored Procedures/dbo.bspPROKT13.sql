...
(@subjamt bDollar = 0, @ppds tinyint = 0, @status char(1) = 'S', @exempts tinyint = 0,
  ...
  select @rcode = 0, @procname = 'bspPROKT13'
  ...
  if @status = 'M'
		BEGIN
		IF      @annualized_wage                 < 10150.00 SELECT @tax_addition =   0, @wage_bracket =     0, @rate = .0000
		ELSE IF @annualized_wage BETWEEN 10150 AND 12149.99 SELECT @tax_addition =   0, @wage_bracket = 10150, @rate = .0050
		ELSE IF @annualized_wage BETWEEN 12150 AND 15149.99 SELECT @tax_addition =  10, @wage_bracket = 12150, @rate = .0100
		ELSE IF @annualized_wage BETWEEN 15150 AND 17649.99 SELECT @tax_addition =  40, @wage_bracket = 15150, @rate = .0200
		ELSE IF @annualized_wage BETWEEN 17650 AND 19949.99 SELECT @tax_addition =  90, @wage_bracket = 17650, @rate = .0300
		ELSE IF @annualized_wage BETWEEN 19950 AND 22349.99 SELECT @tax_addition = 159, @wage_bracket = 19950, @rate = .0400
		ELSE IF @annualized_wage BETWEEN 22350 AND 25149.99 SELECT @tax_addition = 255, @wage_bracket = 22350, @rate = .0500
		ELSE                                                SELECT @tax_addition = 395, @wage_bracket = 25150, @rate = .0525
		END
  ...
  bspexit:
  	return @rcode
GO
GRANT EXECUTE ON  [dbo].[bspPROKT13] TO [public]
GO

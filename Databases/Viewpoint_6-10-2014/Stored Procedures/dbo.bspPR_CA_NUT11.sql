...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[bspPR_CA_NUT11]
/********************************************************
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NUT11'
   ...
   if @TCP is null select @TCP = 11878

   -- establish tax rate and constant
   select @rate = .115, @KP = 5993
   if @A <= 128800 select @rate = .090, @KP = 2773
   if @A <= 79224 select @rate = .070, @KP = 1188
   if @A <= 39612 select @rate = .040, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NUT11] TO [public]
GO

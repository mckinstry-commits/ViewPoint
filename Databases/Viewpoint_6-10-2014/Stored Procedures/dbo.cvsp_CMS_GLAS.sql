...
from bGLAC where GLCo=@toco and Active='N';

alter table bGLAC disable trigger all;
begin tran
update bGLAC set Active='Y' where GLCo=@toco and Active='N'
commit tran;
...
select @toco
	...
	, udConv='Y'
from bGLDT
join bGLAC on bGLAC.GLCo=@toco and bGLAC.GLAcct=bGLDT.GLAcct

where bGLDT.GLCo=@toco 
	
group by bGLDT.GLCo, bGLDT.GLAcct, Mth, Jrnl, GLRef, SourceCo, Source

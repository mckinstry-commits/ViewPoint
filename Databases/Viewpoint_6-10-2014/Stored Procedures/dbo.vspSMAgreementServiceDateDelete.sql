...
-- Mod:
-- 07/10/13 ScottAlvey TFS 52507 - modified delete of tasks to use new task location 
-- =============================================
...
BEGIN
	...
	/* Delete related SMEntity so it will cascade to other related areas that need to be cleaned up */
	BEGIN TRY

		DELETE SMEntity WHERE SMCo = @SMCo AND WorkOrder = @WorkOrder AND WorkOrderScope = @Scope

	END TRY
	BEGIN CATCH
		SELECT @msg = 'Unable to delete related SM Entity record. ' + ERROR_MESSAGE()
		RETURN 1
	END CATCH

...
* @TotalReportablePaid	Total Reportable paid to creditor includes tax - V1099YN = 'Y'
* @TotalReportableGST	Total Reportable GST paid to creditor - V1099YN = 'Y'
...
(@APCo bCompany = NULL
 ...
 ,@TotalReportableGST bDollar = 0 OUTPUT
...
SELECT @TotalReportablePaid = SUM(ISNULL(APTD.Amount, 0)) - SUM(ISNULL(APTD.DiscTaken, 0))
	  ,@TotalReportableGST  = SUM(ISNULL(APTD.TotTaxAmount, 0))
FROM dbo.bAPTD APTD WITH (NOLOCK)
INNER JOIN dbo.bAPTH APTH ON APTH.APCo=APTD.APCo AND APTH.Mth=APTD.Mth AND APTH.APTrans=APTD.APTrans
WHERE APTD.APCo = @APCo
	AND APTD.Status = 3

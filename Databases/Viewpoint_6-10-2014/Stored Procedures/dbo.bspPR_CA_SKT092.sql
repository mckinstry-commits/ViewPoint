...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_SKT092]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_SKT092]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_SKT092'
   ...
   if @TCP is null select @TCP = 13269

   -- establish tax rate and constant
   select @rate = .15, @KP = 3094
   if @A <= 114610 select @rate = .13, @KP = 802
   if @A <= 40113 select @rate = .11, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_SKT092] TO [public]
GO

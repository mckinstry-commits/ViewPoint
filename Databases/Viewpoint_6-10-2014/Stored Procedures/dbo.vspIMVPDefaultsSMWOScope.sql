...
CREATE PROCEDURE [dbo].[vspIMVPDefaultsSMWOScope]
	...
	*
	* Modifications:
	*   05/21/13 EricV TFS-50951 - Validate PriceMethod to use value of 'N' instead of 'C'
	*
    ************************************************************/
    (
     @Company bCompany
...
-- validation				
				...
				BEGIN
					UPDATE IMWE
					SET IMWE.UploadVal = '** Invalid Price Method should be N,T, or F'
					WHERE IMWE.ImportTemplate=@ImportTemplate 
						AND IMWE.ImportId=@ImportId 
						AND IMWE.RecordSeq=@currrecseq
						AND IMWE.Identifier = @PriceMethodID
						AND IMWE.RecordType = @rectype
						AND ISNULL(dbo.IMWE.UploadVal,'') NOT IN ('N','T','F');
				END;				

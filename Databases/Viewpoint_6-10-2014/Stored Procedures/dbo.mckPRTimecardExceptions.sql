...
SET ANSI_NULLS ON
GO
CREATE PROCEDURE dbo.mckPRTimecardExceptions
(
...
SELECT
  ...
  -- All employees, no hours
   ...
   ,CASE WHEN arh.PRGroup = 2 AND arh.RegHrs > 40 THEN 'Union, over 40 regular Hours ' ELSE '' END AS Err3
    ...
    ,CASE WHEN arh.PRGroup = 2 AND arh.udExempt = 'E' AND (arh.RegHrs + arh.OTHrs + arh.OtherHrs)  <> 40 THEN 'Exempt staff, total Hours not equal to 40 ' ELSE '' END AS Err5

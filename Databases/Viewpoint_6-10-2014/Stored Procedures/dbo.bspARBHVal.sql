...
*			EricV 06/20/13 TFS-53340 Add SMFlatPriceRevenueSplitID to SMDetailTransaction record for Flat Price billings.
*
		...
		/* Revenue/Income Account - Can also be WriteOff Account */
  		if @i=7 
			...
			BEGIN
				...
				INSERT dbo.vSMDetailTransaction (IsReversing, Posted, HQBatchDistributionID, SMAgreementID, SMAgreementBillingScheduleID, TransactionType, SourceCo, Mth, BatchId, GLCo, GLAccount, Amount, [Description])
				SELECT CASE @artranstype WHEN 'I' THEN 0 WHEN 'A' THEN 1 END, 0 Posted, @HQBatchDistributionID, vSMAgreement.SMAgreementID, vSMAgreementBillingSchedule.SMAgreementBillingScheduleID, 'R', @co, @mth, @batchid, @PostGLCo, @PostGLAcct, @PostAmount, @SMTransDesc
				FROM dbo.bARBL
				...
				INSERT dbo.vSMDetailTransaction (IsReversing, Posted, HQBatchDistributionID, SMWorkCompletedID, SMWorkOrderScopeID, SMWorkOrderID, LineType, TransactionType, SourceCo, Mth, BatchId, GLCo, GLAccount, Amount, [Description], SMFlatPriceRevenueSplitID)
				SELECT vSMInvoiceARBH.IsReversing, 0, @HQBatchDistributionID, vSMWorkCompleted.SMWorkCompletedID, vSMWorkOrderScope.SMWorkOrderScopeID, vSMWorkOrder.SMWorkOrderID, vSMWorkCompleted.[Type], 'R', @co, @mth, @batchid, @PostGLCo, @PostGLAcct, @PostAmount, @SMTransDesc, vSMFlatPriceRevenueSplit.SMFlatPriceRevenueSplitID
				FROM dbo.vSMInvoiceARBH
					...
					INNER JOIN dbo.vSMInvoiceLine AssociatedInvoiceLine ON vSMInvoiceLine.SMCo = AssociatedInvoiceLine.SMCo AND vSMInvoiceLine.InvoiceLine = AssociatedInvoiceLine.InvoiceLine AND vSMInvoiceARBH.IsReversing = AssociatedInvoiceLine.Invoiced
					LEFT JOIN dbo.vSMWorkCompleted ON vSMInvoiceDetail.SMCo = vSMWorkCompleted.SMCo AND vSMInvoiceDetail.WorkOrder = vSMWorkCompleted.WorkOrder AND vSMInvoiceDetail.WorkCompleted = vSMWorkCompleted.WorkCompleted
					LEFT JOIN dbo.vSMWorkCompletedDetail ON vSMWorkCompleted.SMWorkCompletedID = vSMWorkCompletedDetail.SMWorkCompletedID AND vSMWorkCompletedDetail.IsSession = 0
					LEFT JOIN dbo.vSMWorkOrderScope ON vSMInvoiceDetail.SMCo = vSMWorkOrderScope.SMCo AND vSMInvoiceDetail.WorkOrder = vSMWorkOrderScope.WorkOrder AND 
						(
							vSMInvoiceDetail.Scope = vSMWorkOrderScope.Scope OR
							vSMWorkCompletedDetail.Scope = vSMWorkOrderScope.Scope
						)
					LEFT JOIN dbo.vSMWorkOrder ON vSMInvoiceDetail.SMCo = vSMWorkOrder.SMCo AND vSMInvoiceDetail.WorkOrder = vSMWorkOrder.WorkOrder
					LEFT JOIN dbo.vSMEntity ON vSMEntity.SMCo = vSMWorkOrderScope.SMCo AND vSMEntity.WorkOrder = vSMWorkOrderScope.WorkOrder AND vSMEntity.WorkOrderScope = vSMWorkOrderScope.Scope
					LEFT JOIN dbo.vSMFlatPriceRevenueSplit ON vSMFlatPriceRevenueSplit.SMCo = vSMEntity.SMCo AND vSMEntity.EntitySeq = vSMFlatPriceRevenueSplit.EntitySeq AND vSMFlatPriceRevenueSplit.Seq = vSMInvoiceLine.InvoiceDetailSeq
				WHERE vSMInvoiceARBH.Co = @co AND vSMInvoiceARBH.Mth = @mth AND vSMInvoiceARBH.BatchId = @batchid AND vSMInvoiceARBH.BatchSeq = @seq AND AssociatedInvoiceLine.BatchLine = @ARLine
			END
			end

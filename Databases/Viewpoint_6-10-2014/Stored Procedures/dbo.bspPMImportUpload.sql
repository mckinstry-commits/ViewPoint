...
*					STO 03/20/2013 TFS 44514 added reset of FromImport flag (now is 'N') in existing update of bPMOL records following insert.
*
*
*
*
		...
		@vendorgroup bGroup, @taxgroup bGroup, @custgroup bGroup, @template varchar(10),
     	...
     	@ourfirm bFirm, @initiator bEmployee, @dateinit bDate, @beginstatus bStatus, @item bContractItem,
      	...
      	@vendor bVendor, @material bMatl, @ecm bECM, @phaseactive bYN, @retainpct bPct,
   		...
   		@contractdefaultsecurity int, @jobdefaultsecurity int,
...
if @@fetch_status=0
	...
	-- we also (TFS 40561) set FromImport to Y to keep the bPMOLi trigger from inserting an additional record, then
	-- in the update we reset FromImport back to N in the same update used to set ECM.
	...
	if @@rowcount = 0
		...
		update dbo.bPMOL set ECM='E',FromImport='N'

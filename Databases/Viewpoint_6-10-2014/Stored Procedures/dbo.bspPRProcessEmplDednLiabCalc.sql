...
*			 KK/EN		09/25/2013	62590/62839 Error during PR Payroll Process when employees have a fixed DL amount override
...
*				KK		01/09/2013  70011 401K/Catchup were calculating wrong amounts after fix when elig amounts in PRDT/PREA were wrong
*
* USAGE:
...
DECLARE @rcode int, @rate bUnitCost, --@calcamt bDollar, 
	...
	@liabbasis bDollar,	@accurcalc float, @exemptamt bDollar, @DednGroupLimit bDollar, 
	@limitMinusYTDAccum bDollar, @sum401KEligforCatchup bDollar
...
DECLARE @disposable bDollar, @dedncode bEDLCode, @dednamt bDollar, @numallocs tinyint, @allocgroup tinyint
	...
	BEGIN
		...
		BEGIN
			-- Assuming that the limit is Annual for Pretax Deductions and CatchUp (PREA, PRDT, PRSQ)
			EXEC @rcode = bspPRProcessGetAccums @prco, @prgroup, @prenddate, @employee, @payseq,
			...
			BEGIN
				...
				IF @limitMinusYTDAccum < 0 SELECT @limitMinusYTDAccum = 0 -- 70011, Don't let eligible amount go negative
			...
			BEGIN -- @PreTaxCatchUpYN = 'Y' (SUM of all elig amounts where PreTaxGroup in catchUp PreTaxGroup	
				SELECT @sum401KEligforCatchup = ( SELECT SUM(dt.EligibleAmt)	
													FROM dbo.bPRDT dt
													JOIN dbo.bPRDL dl
													  ON dt.PRCo = dl.PRCo 
													 AND dt.EDLCode = dl.DLCode 
												   WHERE dl.PreTaxGroup = @PreTaxGroup --70011 Defined PreTaxGroup as the one found on DLCode
												     AND dt.PRCo = @prco 
													 AND dt.Employee = @employee
													 AND dt.PRGroup = @prgroup 
													 AND dt.PREndDate = @prenddate
													 AND dl.PreTaxCatchUpYN = 'N') --70011 Do not want to combine elig amts from catchup amts
				IF @sum401KEligforCatchup > 0 SELECT @eligamt = @calcbasis - @sum401KEligforCatchup 
				-- Set the calculated amount so we can test the limits
				SELECT  @calcamt = @eligamt * @rate -- (This is assuming the method is "G" Rate of Gross)
				-- Apply the annual limit for the pretax catchup, If we are over out limit, zero out everything
				IF		@accumamt >= @limitamt			 SELECT @calcamt = 0,				 	  @eligamt = 0
				ELSE IF @calcamt + @accumamt > @limitamt SELECT @calcamt = @limitamt - @accumamt, @eligamt = @calcamt/@rate		
			END
		END

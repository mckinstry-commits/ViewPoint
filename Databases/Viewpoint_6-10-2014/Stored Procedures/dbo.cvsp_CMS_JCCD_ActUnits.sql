...
CREATE procedure  [dbo].[cvsp_CMS_JCCD_ActUnits] 
	( @fromco1	smallint
	, @fromco2	smallint
	, @fromco3	smallint
	, @toco		smallint
...
=========================================================================
	...
	Revisions:	1. JRE 08/10/09 Actual Hours
				...
				3. 10/03/13 - BTC - Added JCJobs cross reference
...
from CV_CMS_SOURCE.dbo.JCTQYD j
	...
	join Viewpoint.dbo.budxrefPhase xrefPhase on xrefPhase.Company=@PhaseGroup and xrefPhase.oldPhase=j.JCDISTRIBTUION
	join Viewpoint.dbo.budxrefCostType xrefCostType on xrefCostType.Company=@PhaseGroup 
				and xrefCostType.CMSCostType=j.COSTTYPE
	left join bJCCH h on h.JCCo=@toco and h.Job=xj.VPJob --dbo.bfMuliPartFormat(RTRIM(j.JOBNUMBER) + '.' + RTRIM(j.SUBJOBNUMBER),@Job)
...
where j.COSTQTY90<>0 
	and j.COMPANYNUMBER in (@fromco1,@fromco2,@fromco3)

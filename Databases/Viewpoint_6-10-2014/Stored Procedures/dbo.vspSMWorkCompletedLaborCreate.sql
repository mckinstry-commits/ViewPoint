...
--				EricV  05/31/13 TFS-4171 Replaced Work Completed Coverage field with NonBillable and UseAgreementRatesfields
...
BEGIN
	...
	DECLARE @TaxGroup bGroup, @TaxBasic bDollar, @GLCo bCompany, @CostAccount bGLAcct, @RevenueAccount bGLAcct, @RevenueWIPAccount bGLAcct,
			...
			@NonBillable bYN, @UseAgreementRates bYN, @Job bJob, @TaxBasis bDollar
	...
	BEGIN TRY
		...
		INSERT SMWorkCompleted (Type, SMCo, WorkOrder, WorkCompleted, Scope, Date, TaxType, TaxGroup,
			...
			PRPostDate, Provisional, SMCostType, JCCostType, PhaseGroup, Agreement, Revision, NonBillable, UseAgreementRates)
		VALUES
			(@LineType, @SMCo, @WorkOrder, @WorkCompleted, @Scope, @Date, @TaxType, @TaxGroup, @TaxCode,
			@TaxBasis, @TaxRate*@TaxBasis, @SMGLCo, @CostAccount, @RevenueAccount, @CostWIPAccount, @RevenueWIPAccount,
			@Technician, @Hours, @CostRate, @CostRate*@Hours,
			CASE WHEN @NonBillable = 'Y' THEN NULL ELSE @Hours END, 
			CASE WHEN @NonBillable = 'Y' THEN NULL ELSE @PriceRate END, 
			CASE WHEN @NonBillable = 'Y' THEN NULL ELSE @PriceRate*@Hours END
			, @PayType, @Craft, @Class, @Shift,
			@ServiceSite, 'N', @TCPRCo, @TCPRCo, @TCPRGroup, @TCPREndDate, @TCPREmployee, @TCPRPaySeq, @TCPRPostSeq, 
			@TCPRPostDate, @Provisional, @SMCostType, @SMJCCostType, @SMPhaseGroup, @Agreement, @Revision, @NonBillable, @UseAgreementRates)

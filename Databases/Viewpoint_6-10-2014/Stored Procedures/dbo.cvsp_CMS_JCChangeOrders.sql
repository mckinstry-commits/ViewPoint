...
CREATE procedure  [dbo].[cvsp_CMS_JCChangeOrders] 
	( @fromco1	smallint
	, @fromco2	smallint
	, @fromco3	smallint
	, @toco		smallint
...
=========================================================================
	...
	Revisions:	1. 09/12/09 - Changed Mth in several places to have day '1' i.e. 12/1/2009 and not the date - JRE
				...
				5. 10/05/2013 BTC - Added min item for Contract if item not available from JCCI
**/
...
select c.JCCo
	, Job
	, ACO
	, ACOSequence=row_number() over (partition by c.JCCo, Job order by c.JCCo, Job, min(ApprovalDate))
	, Contract=CONTRACTNO
...
join budxrefJCJobs j
	  on j.VPJob = c.Job
	join bJCCI i on c.JCCo=i.JCCo and j.VPJobExt=i.Contract and i.Item = c.Item
where c.JCCo=@toco
group by c.JCCo, Job, ACO,CONTRACTNO;
...
select c.JCCo
	...
	, Contract=i.Contract
	, Item=min(c.Item)/*ROW_NUMBER() over (partition by Job order by Job)Item= isnull(min(i.Item), MIN(m.MinItem))
...
from JCChangeOrders c 
	join budxrefJCJobs j
	  on j.VPJob = c.Job
	join bJCCI i on c.JCCo=i.JCCo and j.VPJobExt=i.Contract and i.Item = c.Item
	join (select JCCo, Contract, MIN(Item) as MinItem from bJCCI group by JCCo, Contract) m
		on m.JCCo = c.JCCo and m.Contract = c.CONTRACTNO
where c.JCCo=@toco	
	
group by c.JCCo, Job,i.Contract, ACO, ACOItem;
...
insert bJCOD (JCCo,Job, ACO, ACOItem, PhaseGroup, Phase, CostType, MonthAdded,
	...
	select c.JCCo
		 , c.Job
		 , c.ACO
		 , ACOItem=space(10-datalength(rtrim(c.ACOItem))) + rtrim(c.ACOItem)
		 , @PhaseGroup
		 , c.Phase
		 , c.CostType
		 , convert(nvarchar(max),datepart(mm,min(c.ApprovalDate))) + '/01/' + convert(nvarchar(max),datepart(yy,min(c.ApprovalDate)))
		 , UnitCost=0
	  ...
	  join bJCCH h 
	   ...
	   and c.Job=h.Job 
	   and c.Phase=h.Phase 
	   and c.CostType=h.CostType
	  join bJCOI i
	    on i.ACO = c.ACO
	   and i.ACOItem = c.ACOItem
	   and i.Job = c.Job
	   and i.JCCo = c.JCCo

...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_AU_PAYG092]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_AU_PAYG092]
   /********************************************************
   ...
   **********************************************************/
   ...
   declare @rcode int, 
	...
	select @rcode = 0, @x = 0, @a = 0, @b = 0, @y = 0, @whamt = 0, @FTB = 0, @wet = 0, @west = 0,
			@mlft = 0, @ac = 0, @sopm = 0, @sopd = 0, @wlaf = 0, @procname = 'bspPR_AU_PAYG092'
	...
	if @scale not in (1,2,3,4,5,6,7,11,21,31,51,61,71)
		begin
		select @msg = 'Invalid scale!', @rcode = 1
		goto bspexit
		end
	...
	if @scale = 11
		...
		if @x >= 259 select @a = .2346, @b = 18.0692
		if @x >= 355 select @a = .3350, @b = 53.7885
		if @x >= 512 select @a = .3750, @b = 53.7885
		if @x >= 607 select @a = .3800, @b = 53.7885
		if @x >= 701 select @a = .3850, @b = 53.7885
		if @x >= 755 select @a = .3900, @b = 53.7885
		if @x >= 835 select @a = .3950, @b = 53.7885
		if @x >= 908 select @a = .3750, @b = 35.6154
		if @x >= 931 select @a = .3800, @b = 35.6154
		if @x >= 996 select @a = .3850, @b = 35.6154
		if @x >= 1128 select @a = .3900, @b = 35.6154
		if @x >= 1221 select @a = .4700, @b = 133.3077
		if @x >= 1223 select @a = .4750, @b = 133.3077
		if @x >= 3144 select @a = .5450, @b = 353.4038
		end
	if @scale = 2
		begin
		select @a = 0, @b = 0
	...
	if @scale = 21
		...
		if @x >= 571 select @a = .1867, @b = 41.2502
		if @x >= 666 select @a = .3350, @b = 140.1502
		if @x >= 829 select @a = .3750, @b = 140.1502
		if @x >= 924 select @a = .3800, @b = 140.1502
		if @x >= 1018 select @a = .3850, @b = 140.1502
		if @x >= 1072 select @a = .3900, @b = 140.1502
		if @x >= 1152 select @a = .3950, @b = 140.1502
		if @x >= 1219 select @a = .3750, @b = 115.7540
		if @x >= 1248 select @a = .3800, @b = 115.7540
		if @x >= 1314 select @a = .3850, @b = 115.7540
		if @x >= 1446 select @a = .3900, @b = 115.7540
		if @x >= 1532 select @a = .4700, @b = 238.3387
		if @x >= 1541 select @a = .4750, @b = 238.3387
		if @x >= 3455 select @a = .5450, @b = 480.2156
		end
		--set medicare levy parameters
		select @wet = 339, @west = 398, @mlft = 29750, @ac = 2757, @sopm = .1000, @sopd = .0850, @wlaf = 339.0300, @ml = .0150
	if @scale = 3
	...
	if @scale = 31
		...
		select @a = .2900, @b = .2900
		if @x >= 673 select @a = .3000, @b = 6.7308
		if @x >= 829 select @a = .3400, @b = 6.7308
		if @x >= 924 select @a = .3450, @b = 6.7308
		if @x >= 1018 select @a = .3500, @b = 6.7308
		if @x >= 1072 select @a = .3550, @b = 6.7308
		if @x >= 1152 select @a = .3600, @b = 6.7308
		if @x >= 1248 select @a = .3650, @b = 6.7308
		if @x >= 1314 select @a = .3700, @b = 6.7308
		if @x >= 1446 select @a = .3750, @b = 6.7308
		if @x >= 1538 select @a = .4550, @b = 129.8077
		if @x >= 1541 select @a = .4600, @b = 129.8077
		if @x >= 3461 select @a = .5300, @b = 372.1154
		end
	if @scale = 4
		begin
	...
	if @scale = 51
		...
		if @x >= 571 select @a = .1716, @b = 41.2500
		if @x >= 666 select @a = .3200, @b = 140.2423
		if @x >= 829 select @a = .3600, @b = 140.2423
		if @x >= 924 select @a = .3650, @b = 140.2423
		if @x >= 1018 select @a = .3700, @b = 140.2423
		if @x >= 1072 select @a = .3750, @b = 140.2423
		if @x >= 1152 select @a = .3800, @b = 140.2423
		if @x >= 1219 select @a = .3600, @b = 115.8462
		if @x >= 1248 select @a = .3650, @b = 115.8462
		if @x >= 1314 select @a = .3700, @b = 115.8462
		if @x >= 1446 select @a = .3750, @b = 115.8462
		if @x >= 1532 select @a = .4550, @b = 238.4308
		if @x >= 1541 select @a = .4600, @b = 238.4308
		if @x >= 3455 select @a = .5300, @b = 480.3077
		end
	if @scale = 6
		begin
		select @a = 0, @b = 0
	...
	if @scale = 61
		...
		if @x >= 666 select @a = .3700, @b = 168.8048
		if @x >= 673 select @a = .3275, @b = 140.1966
		if @x >= 829 select @a = .3675, @b = 140.1966
		if @x >= 924 select @a = .3725, @b = 140.1966
		if @x >= 1018 select @a = .3775, @b = 140.1966
		if @x >= 1072 select @a = .3825, @b = 140.1966
		if @x >= 1152 select @a = .3875, @b = 140.1966
		if @x >= 1219 select @a = .3675, @b = 115.8004
		if @x >= 1248 select @a = .3725, @b = 115.8004
		if @x >= 1314 select @a = .3775, @b = 115.8004
		if @x >= 1446 select @a = .3825, @b = 115.8004
		if @x >= 1532 select @a = .4625, @b = 238.3850
		if @x >= 1541 select @a = .4675, @b = 238.3850
		if @x >= 3455 select @a = .5375, @b = 480.2620
		--set medicare levy parameters
		select @wet = 572, @west = 673, @mlft = 29750, @ac = 2757, @sopm = .0500, @sopd = .0425, @wlaf = 572.1200, @ml = .0075
		end
	if @scale = 7
	...
	if @scale = 71
		...
		if @x >= 576 select @a = .1850, @b = 41.2502
		if @x >= 673 select @a = .3350, @b = 142.2117
		if @x >= 829 select @a = .3750, @b = 142.2117
		if @x >= 924 select @a = .3800, @b = 142.2117
		if @x >= 1018 select @a = .3850, @b = 142.2117
		if @x >= 1072 select @a = .3900, @b = 142.2117
		if @x >= 1152 select @a = .3950, @b = 142.2117
		if @x >= 1225 select @a = .3750, @b = 117.6925
		if @x >= 1278 select @a = .3800, @b = 117.6925
		if @x >= 1314 select @a = .3850, @b = 117.6925
		if @x >= 1446 select @a = .3900, @b = 117.6925
		if @x >= 1538 select @a = .4700, @b = 240.7694
		if @x >= 1541 select @a = .4750, @b = 240.7694
		if @x >= 3461 select @a = .5450, @b = 483.0771
		--set medicare levy parameters
		select @wet = 342, @west = 402, @mlft = 30025, @ac = 2757, @sopm = .1000, @sopd = .0850, @wlaf = 342.1900, @ml = .0150
		end
	...
	if @scale in (2,6,7,21,61,71) and (@status = 'M' or @addlexempts > 0) and not (@scale = 4 and @nonresalienyn = 'Y')
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_AU_PAYG092] TO [public]
GO

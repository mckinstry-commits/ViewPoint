 ...
 *				STO 03/28/2013 - TFS 13559 - RFQ- Copy RFQ.  Added a check for the RFQ document type.
 *				SCOTTP 04/09/2013 - TFS 38977 - When parsing PCO to get next, check if return PadIndex is <= 1.
												This handles the case where the entire PCO is alpha.
 *
 *
...
---- if from RFQCOPY form then set dummy document category
if @formname = 'REQQUOTE'
	begin
	select @doccategory = 'REQQUOTE'
	goto getNextDocument
	end

---- get document category from PMDT
if @doctype in('STD_DUMMY')
	begin
...
if @doccategory = 'REQQUOTE'
	begin
	set @next_doc=str((select max(RFQ) + 1
					   from dbo.PMRequestForQuote with (nolock)
					   where PMCo = @pmco 
							AND Project=@project),10,0)
	---- if null or zero set to 1
	if @@rowcount = 0 or isnull(@next_doc,0) = 0 set @next_doc = str(1,10,0)
	set @docnumber = @next_doc

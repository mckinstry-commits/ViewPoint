...
CREATE  procedure [dbo].[vspSMServiceSiteVal]
	...
	*				ScottAlvey	2/27/12	- TFSTask - 40961 - Added output of individual address fields 
	*				Lane G		03/07/13 - TFSTask - 40965 - Fixed - ServiceSiteVal so that it validates what customer is selected on a SM Workorder Quote. 
	*               Eric V      06/05/13 - TFS-52109 - Include the NonBillable default value as an output field.
	...
	*******************************************************/
	...
	@Address1 VARCHAR(60) = NULL OUTPUT,
...
BEGIN
	...
	SELECT 
		...
		@Address1 = SMServiceSite.Address1,
		@Address2 = SMServiceSite.Address2,
		@City = SMServiceSite.City,
		@State = SMServiceSite.State,
		@Zip = SMServiceSite.Zip,
		@Country = SMServiceSite.Country,
		@NonBillable = SMServiceSite.NonBillable
	...
	IF @SiteCustomer IS NOT NULL AND NOT @SiteCustomer = @Customer
	BEGIN
		SET @msg = 'Not valid site for customer.'
		RETURN 1
    END

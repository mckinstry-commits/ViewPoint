...
CREATE proc [dbo].[cvsp_CMS_MASTER_JCCH] 
	( @fromco1	smallint
	, @fromco2	smallint
	, @fromco3	smallint
	, @toco		smallint
...
ALTER TABLE bJCCH NOCHECK CONSTRAINT CK_bJCCH_ActiveYN;
...
select JCCHInfo.JCCo
	...
	, @BuyOutYN
	, @Plugged
	, max(case when m.JobStatus = 1 then 'Y' else 'N' end)
...
from (select JCCo=@toco
	...
	from CV_CMS_SOURCE.dbo.JCTMST j
		...
		join (select CN=COMPANYNUMBER, JN=JOBNUMBER,SJ=SUBJOBNUMBER, JD=JCDISTRIBTUION, 
				...
				where j2.COMPANYNUMBER in (@fromco1,@fromco2,@fromco3)
				  and j2.JCDISTRIBTUION <> '000INCOME      '
...
ALTER TABLE bJCCH CHECK CONSTRAINT CK_bJCCH_ActiveYN;

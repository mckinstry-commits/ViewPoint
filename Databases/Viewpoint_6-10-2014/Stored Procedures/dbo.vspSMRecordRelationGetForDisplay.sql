...
*				DK 06/04/2013 - TFS-44856 - Change the function for returning Invoices.
*				JVH 6/25/13   - TFS-39389 - Optimized returing POs and modified sorting
*				LDG 08/10/13  - TFS-55334 - Adding WorkScope to a string that would make the desc null.
*
...
BEGIN
	...
	SELECT 'SMCustomer' AS RecType,
		...
		'N' AS Detail,
		'NotSure' AS DocType,
		SMInvoiceList.InvoiceNumber AS DocID,
		...
		SMInvoiceList.InvoiceNumber AS SortID,
		'Invoices' AS Title
	FROM dbo.SMCustomer
	...
	ORDER BY SMInvoiceList.InvoiceNumber
...
BEGIN
	...
	SELECT 'SMServiceSite' AS RecType,
		...
		'Invoices' AS Title
	FROM dbo.SMServiceSite
		CROSS APPLY dbo.vfSMInvoicesRelated(SMServiceSite.SMCo, NULL, NULL, NULL, SMServiceSite.ServiceSite, NULL, NULL) RelatedInvoices
		INNER JOIN dbo.SMInvoiceList ON RelatedInvoices.SMCo = SMInvoiceList.SMCo AND RelatedInvoices.Invoice = SMInvoiceList.Invoice
...
BEGIN
	...
	SELECT 'SMWorkOrder' AS RecType,
		...
		'Invoices' AS Title
	FROM SMWorkOrder
		CROSS APPLY dbo.vfSMInvoicesRelated(SMWorkOrder.SMCo, NULL, NULL, SMWorkOrder.WorkOrder, NULL, NULL, NULL) RelatedInvoice
		INNER JOIN SMInvoiceList ON RelatedInvoice.SMCo = SMInvoiceList.SMCo AND RelatedInvoice.Invoice = SMInvoiceList.Invoice
	...
	SELECT 'SMWorkOrder' RecType, @FromKeyID KeyID, @FromKeyID LinkID, @FromKeyID FormKeyID,
		'N' Detail, 'NotSure' DocType, RTRIM(SMPurchaseOrderList.PO) DocID, NULL RecDate,
		dbo.vfDateOnlyAsStringUsingStyle(SMPurchaseOrderList.OrderDate, @Co, DEFAULT) + ' - ' + CASE 
			WHEN SMPurchaseOrderList.POHDKeyID IS NULL
				THEN 'Reserved'
				ELSE SUBSTRING(DDCI.DisplayValue, 5, 10)
			END RecDesc,
		RTRIM(SMPurchaseOrderList.PO) SortID,
		'Purchase Orders' Title
	FROM dbo.SMPurchaseOrderList
		LEFT JOIN dbo.DDCI ON DDCI.ComboType= 'POEntryStatus' AND SMPurchaseOrderList.[Status] = DDCI.DatabaseValue
	WHERE SMPurchaseOrderList.SMWorkOrderID = @FromKeyID
END
...
ORDER BY a.Title,
	--Order Numerically
	ISNUMERIC(a.SortID) DESC, CASE WHEN ISNUMERIC(a.SortID) = 1 THEN CAST(CAST(a.SortID AS varchar(29)) AS numeric(29,0)) END,
	--After ordering the numeric values sort alphanumerically.
	a.SortID

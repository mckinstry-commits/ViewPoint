...
alter table bCMDT nocheck constraint FK_bCMDT_bHQBC_SourceCoBatchID;
...
--roll up by date, per McKinstry request.
select CMCo=h.CMCo
	, Mth=h.Mth
	, CMTrans=isnull(max(t.LastTrans),0)+row_number()over(Partition by h.CMCo, h.Mth Order by h.CMCo, h.Mth)
	, CMAcct=h.CMAcct
	, CMTransType=2
	, SourceCo=max(h.ARCo)
	, Source='AR Receipt'
...
where h.ARCo=@toco
	and h.ARTransType ='P' 
	and h.CreditAmt<>0 
	and h.Mth >= '01/01/2014'
group by h.CMCo,h.CMAcct,h.Mth,h.TransDate,h.CMDeposit,c.GLAcct ;
...
ALTER Table bCMDT enable trigger all;
alter table bCMDT check constraint FK_bCMDT_bHQBC_SourceCoBatchID;

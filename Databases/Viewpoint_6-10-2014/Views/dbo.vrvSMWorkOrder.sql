...
06/19/2013 ScottALvey TFS-53307 - added Flat Price information to the view
	via cteFPTotalPerScope, brought in SMEntity into the view, and changed the logic
	on how the RecordType column is calculated
...
(
	Select
		...
		, s.WorkOrder
		, s.Scope
		, sum(f.Amount) as FPAmount
	From
		SMFlatPriceRevenueSplit f
	Join
		SMEntity e on
			f.SMCo = e.SMCo
			and f.EntitySeq = e.EntitySeq
	Join
		SMWorkOrderScope s on
			e.SMCo = s.SMCo
			and e.WorkOrder = s.WorkOrder
			and e.WorkOrderScope = s.Scope
	group by
			s.SMCo
			, s.WorkOrder
			, s.Scope
...
(  
	...
	-- SMWorkCompleted  
	SELECT 
		CASE WHEN wosc.Scope IS NOT NULL or wosc.PriceMethod = 'F'
			THEN 'WorkCompleted'   
			ELSE 'WorkHeader'
		END AS RecordType  
	...
	LEFT OUTER JOIN
		SMEntity sme ON
			wosc.SMCo = sme.SMCo
			AND wosc.WorkOrder = sme.WorkOrder
			AND wosc.Scope = sme.WorkOrderScope
	LEFT OUTER JOIN
		cteFPTotalPerScope fp ON
			wosc.SMCo = fp.SMCo
			AND wosc.WorkOrder = fp.WorkOrder
			AND wosc.Scope = fp.Scope
	LEFT OUTER JOIN 
		SMWorkScope wsc ON 
			wosc.SMCo = wsc.SMCo   
...
GRANT SELECT ON  [dbo].[vrvSMWorkOrder] TO [Viewpoint]
GRANT INSERT ON  [dbo].[vrvSMWorkOrder] TO [Viewpoint]
GRANT DELETE ON  [dbo].[vrvSMWorkOrder] TO [Viewpoint]
GRANT UPDATE ON  [dbo].[vrvSMWorkOrder] TO [Viewpoint]
GO

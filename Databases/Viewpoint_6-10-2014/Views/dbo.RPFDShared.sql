...
*			CJG 04/09/13 - TFS 44707 - Added UsedForPublish flag
*
...
SELECT	ISNULL(c.Form, d.Form) AS Form,
		...
		ISNULL(c.UsedForPublish, d.UsedForPublish) AS UsedForPublish,
...
if suser_name() = 'viewpointcs'
	...
	insert dbo.vRPFD(Form, ReportID, ParameterName, ParameterDefault, UsedForPublish)
	select i.Form, i.ReportID, i.ParameterName, i.ParameterDefault, i.UsedForPublish
	from inserted i
	...
	insert dbo.vRPFDc(Form, ReportID, ParameterName, ParameterDefault, UsedForPublish)
	select i.Form, i.ReportID, i.ParameterName, i.ParameterDefault, i.UsedForPublish
	from inserted i
...
--			 ChrisG 4/19/13 - TFS 44707 - Added UsedForPublish
--
...
if suser_name() = 'viewpointcs'
	begin
		...
		update dbo.vRPFD set ParameterDefault = i.ParameterDefault, UsedForPublish = i.UsedForPublish
		from inserted i
		...
		update dbo.vRPFDc set ParameterDefault = i.ParameterDefault, UsedForPublish = i.UsedForPublish
		from dbo.vRPFDc
	...
	if update(ParameterDefault) OR update(UsedForPublish)
		begin
...
join dbo.vRPFD f (nolock) on c.Form = f.Form and c.ReportID = f.ReportID 
	and c.ParameterName = f.ParameterName and c.ParameterDefault = f.ParameterDefault and c.UsedForPublish = f.UsedForPublish;
...
GRANT SELECT ON  [dbo].[RPFDShared] TO [Viewpoint]
GRANT INSERT ON  [dbo].[RPFDShared] TO [Viewpoint]
GRANT DELETE ON  [dbo].[RPFDShared] TO [Viewpoint]
GRANT UPDATE ON  [dbo].[RPFDShared] TO [Viewpoint]
GO

...
CREATE  View [dbo].[brvPMBallInCourtNotifier] as
	   ...
	   Mod:	 TFS 51910 HH - added PMRequestForQuote, PMSubmittalRegister and PMSubmittalPackage
...
union all
 --Request for Quotes 6.6 (depending on PCOs)
SELECT     bPMRQ.PMCo, bJCJM_1.Contract, bPMRQ.Project, bJCJM_1.ProjectMgr,  bPMRQ.RFQ,  left((bPMRQ.PCOType+'           '),11), null as Item, 
          CASE WHEN bPMRQ.Status IS NULL 
                      ...
                      'Request for Quotes 6.6' AS Sort, 
...
SELECT	PMRequestForQuote.PMCo
		...
		, JCJM.ProjectMgr
		, PMRequestForQuote.RFQ
		, PMRequestForQuote.DocType
		, null									--item
		...
		, DateDiff(day, (PMRequestForQuote.DueDate ),GetDate()) as 'DaysOverDue'
		, CodeType=PMSC.[Description]
		, PMRequestForQuote.RFQ + ' ' + PMRequestForQuote.[Description]
...
SELECT	PMSubmittal.PMCo
		...
		, PMSubmittal.Project
		, JCJM.ProjectMgr
		, CAST(PMSubmittal.Seq AS varchar(20))
		, PMSubmittal.DocumentType
		, null									--item
		...
		, DateDiff(day, (PMSubmittal.DueToResponsibleFirm),GetDate()) as 'DaysOverDue'
		, CodeType=a.[Description]
		, CAST (PMSubmittal.Seq AS varchar(30)) + ' ' + PMSubmittal.[Description]
FROM PMSubmittal
...
--PM Submittal Package with latest revision
SELECT	PMSubmittalPackage.PMCo
		, JCJM.[Contract]
		, PMSubmittalPackage.Project
		, JCJM.ProjectMgr
		, CAST (PMSubmittalPackage.Package AS varchar(30))
		, PMSubmittalPackage.DocType
		, null									--item
		...
		, DateDiff(day, (PMSubmittalPackage.DueDate),GetDate()) as 'DaysOverDue'
		, CodeType=a.[Description]
		, CAST (PMSubmittalPackage.Package AS varchar(30)) + ' ' + PMSubmittalPackage.[Description]
...
WHERE ISNULL(a.CodeType,'') <> 'F' 
		AND PMSubmittalPackage.PackageRev = (SELECT max(PackageRev) 
												FROM PMSubmittalPackage p2 
												WHERE p2.PMCo = PMSubmittalPackage.PMCo 
														AND p2.Project = PMSubmittalPackage.Project
														AND p2.Package = PMSubmittalPackage.Package
											)
...
GRANT SELECT ON  [dbo].[brvPMBallInCourtNotifier] TO [Viewpoint]
GRANT INSERT ON  [dbo].[brvPMBallInCourtNotifier] TO [Viewpoint]
GRANT DELETE ON  [dbo].[brvPMBallInCourtNotifier] TO [Viewpoint]
GRANT UPDATE ON  [dbo].[brvPMBallInCourtNotifier] TO [Viewpoint]
GO

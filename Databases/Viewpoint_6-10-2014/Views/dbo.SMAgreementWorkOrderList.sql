...
-- Changes:     EricV 12/09/13 TFS-70053 Changed CTE to use a union to allow use of indexes.
-- =============================================
...
AS
	...
	(
		SELECT SMCo,
			   ...
			   [Service],
			   ServiceDate,
			   null CreatedWorkOrder,
			   NULL WorkOrder,
			   NULL Scope
			FROM SMAgreementServiceDatesGenerated
			WHERE NOT EXISTS(SELECT 1 FROM SMAgreementServiceDate WHERE SMAgreementServiceDatesGenerated.SMCo = SMAgreementServiceDate.SMCo
				AND SMAgreementServiceDatesGenerated.Agreement = SMAgreementServiceDate.Agreement
				AND SMAgreementServiceDatesGenerated.Revision = SMAgreementServiceDate.Revision
				AND SMAgreementServiceDatesGenerated.[Service] = SMAgreementServiceDate.[Service]
				AND SMAgreementServiceDatesGenerated.ServiceDate = SMAgreementServiceDate.ServiceDate
			   )

 ...
 *				GP 03/18/2013 - TFS 13553 Changed RFQ to look at PMRequestForQuote
 *
...
select a.*,
	'DocDesc' = case when a.DocCategory='TRANSMIT' then (select b.Subject from dbo.PMTM b with (nolock)
				...
				WHEN a.DocCategory='RFQ' THEN 
					(SELECT p.[Description] 
					 FROM dbo.PMRequestForQuote p
					 WHERE p.PMCo=a.PMCo AND p.Project=a.Project AND p.RFQ = a.Document)
...
GRANT SELECT ON  [dbo].[PMDHGrid] TO [Viewpoint]
GRANT INSERT ON  [dbo].[PMDHGrid] TO [Viewpoint]
GRANT DELETE ON  [dbo].[PMDHGrid] TO [Viewpoint]
GRANT UPDATE ON  [dbo].[PMDHGrid] TO [Viewpoint]
GO

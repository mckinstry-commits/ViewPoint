...
SET ANSI_NULLS ON
GO
CREATE view [dbo].[brvJCJobMatDetlDesc] as
  /* Created 7/11/06 used by the JC Job Materials report  CR  */
  select JCCD.*, 
         DetlDesc= Case when JCCD.Source = 'JC CostAdj' and JCCD.JCTransType ='JC' then 
  			...
  			(case when IsNull(JCCD.Loc,' ')<>' ' then (' /Loc '+ JCCD.Loc +' '+'Trans# '+ convert(varchar(7),JCCD.CostTrans) +' '+IsNull(INLM.Description,' ')) else '' end )
 			...
 			(Case when IsNull(JCCD.Description,' ')<> ' ' then JCCD.Description else '' end) 
  		       ...
  		       when JCCD.JCTransType='MI' then 
                         /*(Case when IsNull(JCCD.Material,' ')<>' ' then ('Mat# '+ JCCD.Material +'-'+ IsNull(HQMT.Description,' ')+' / ' +IsNull(JCCD.Description,'')) else'' end)*/
					(case when isnull(JCCD.CostTrans,0)<> 0 then 'Trans# '+ convert(varchar(7),JCCD.CostTrans)+' / ' +IsNull(JCCD.Description,'') else '' end)
...
GO
GRANT SELECT ON  [dbo].[brvJCJobMatDetlDesc] TO [public]
GRANT INSERT ON  [dbo].[brvJCJobMatDetlDesc] TO [public]
GRANT DELETE ON  [dbo].[brvJCJobMatDetlDesc] TO [public]
GRANT UPDATE ON  [dbo].[brvJCJobMatDetlDesc] TO [public]
GRANT SELECT ON  [dbo].[brvJCJobMatDetlDesc] TO [Viewpoint]
GRANT INSERT ON  [dbo].[brvJCJobMatDetlDesc] TO [Viewpoint]
GRANT DELETE ON  [dbo].[brvJCJobMatDetlDesc] TO [Viewpoint]
GRANT UPDATE ON  [dbo].[brvJCJobMatDetlDesc] TO [Viewpoint]
GO

...
GO
ALTER TABLE [dbo].[bMSHC] WITH NOCHECK ADD CONSTRAINT [CK_bMSHC_RevBased] CHECK (([RevBased]='N' OR [RevBased]='Y'))
GO
ALTER TABLE [dbo].[bMSHC] WITH NOCHECK ADD CONSTRAINT [CK_bMSHC_Taxable] CHECK (([Taxable]='Y' OR [Taxable]='N'))
GO
CREATE NONCLUSTERED INDEX [biMSHaulCodeNoCo] ON [dbo].[bMSHC] ([HaulCode]) INCLUDE ([Description]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bMSHC] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biMSHC] ON [dbo].[bMSHC] ([MSCo], [HaulCode]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

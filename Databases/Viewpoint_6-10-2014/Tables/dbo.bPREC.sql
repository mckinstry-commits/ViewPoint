...
[OTCalcs] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPREC_OTCalcs] DEFAULT ('N'),
...
[InsurableEarningsYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPREC_InsurableEarningsYN] DEFAULT ('N'),
[InsurableHoursYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPREC_InsurableHoursYN] DEFAULT ('N'),
[ROESeparationYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPREC_ROESeparationYN] DEFAULT ('N'),
[ROECategory] [varchar] (2) COLLATE Latin1_General_BIN NULL,
[OtherMonies] [char] (1) COLLATE Latin1_General_BIN NULL,
[ROESpecialYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPREC_ROESpecialYN] DEFAULT ('N'),
[ROEType] [char] (3) COLLATE Latin1_General_BIN NULL,
[ROEPeriod] [char] (1) COLLATE Latin1_General_BIN NULL
...
END
    ...
    begin
    	...
    	select 'bPREC', 'EarnCode: ' + convert(varchar(10),i.EarnCode), i.PRCo, 'C','LimitType',
		 ...
		 INSERT INTO dbo.bHQMA
    	 SELECT 'bPREC', 'EarnCode: ' + CONVERT(VARCHAR(10),i.EarnCode), i.PRCo, 'C','InsurableEarningsYN',
         		d.InsurableEarningsYN, i.InsurableEarningsYN,	GETDATE(), SUSER_SNAME() 
          FROM inserted i
          JOIN deleted d ON i.PRCo = d.PRCo AND i.EarnCode = d.EarnCode
          JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
          WHERE ISNULL(i.InsurableEarningsYN,'') <> ISNULL(d.InsurableEarningsYN,'') AND a.AuditDLs = 'Y'

		 INSERT INTO dbo.bHQMA
    	 SELECT 'bPREC', 'EarnCode: ' + CONVERT(VARCHAR(10),i.EarnCode), i.PRCo, 'C','InsurableHoursYN',
         		d.InsurableHoursYN, i.InsurableHoursYN,	GETDATE(), SUSER_SNAME() 
          FROM inserted i
          JOIN deleted d ON i.PRCo = d.PRCo AND i.EarnCode = d.EarnCode
          JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
          WHERE ISNULL(i.InsurableHoursYN,'') <> ISNULL(d.InsurableHoursYN,'') AND a.AuditDLs = 'Y'

		 INSERT INTO dbo.bHQMA
    	 SELECT 'bPREC', 'EarnCode: ' + CONVERT(VARCHAR(10),i.EarnCode), i.PRCo, 'C','ROESeparationYN',
         		d.ROESeparationYN, i.ROESeparationYN,	GETDATE(), SUSER_SNAME() 
          FROM inserted i
          JOIN deleted d ON i.PRCo = d.PRCo AND i.EarnCode = d.EarnCode
          JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
          WHERE ISNULL(i.ROESeparationYN,'') <> ISNULL(d.ROESeparationYN,'') AND a.AuditDLs = 'Y'

		 INSERT INTO dbo.bHQMA
    	 SELECT 'bPREC', 'EarnCode: ' + CONVERT(VARCHAR(10),i.EarnCode), i.PRCo, 'C','ROECategory',
         		d.ROECategory, i.ROECategory,	GETDATE(), SUSER_SNAME() 
          FROM inserted i
          JOIN deleted d ON i.PRCo = d.PRCo AND i.EarnCode = d.EarnCode
          JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
          WHERE ISNULL(i.ROECategory,'') <> ISNULL(d.ROECategory,'') AND a.AuditDLs = 'Y'

		 INSERT INTO dbo.bHQMA
    	 SELECT 'bPREC', 'EarnCode: ' + CONVERT(VARCHAR(10),i.EarnCode), i.PRCo, 'C','OtherMonies',
         		d.OtherMonies, i.OtherMonies,	GETDATE(), SUSER_SNAME() 
          FROM inserted i
          JOIN deleted d ON i.PRCo = d.PRCo AND i.EarnCode = d.EarnCode
          JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
          WHERE ISNULL(i.OtherMonies,'') <> ISNULL(d.OtherMonies,'') AND a.AuditDLs = 'Y'

		 INSERT INTO dbo.bHQMA
    	 SELECT 'bPREC', 'EarnCode: ' + CONVERT(VARCHAR(10),i.EarnCode), i.PRCo, 'C','ROESpecialYN',
         		d.ROESpecialYN, i.ROESpecialYN,	GETDATE(), SUSER_SNAME() 
          FROM inserted i
          JOIN deleted d ON i.PRCo = d.PRCo AND i.EarnCode = d.EarnCode
          JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
          WHERE ISNULL(i.ROESpecialYN,'') <> ISNULL(d.ROESpecialYN,'') AND a.AuditDLs = 'Y'

		 INSERT INTO dbo.bHQMA
    	 SELECT 'bPREC', 'EarnCode: ' + CONVERT(VARCHAR(10),i.EarnCode), i.PRCo, 'C','ROEType',
         		d.ROEType, i.ROEType,	GETDATE(), SUSER_SNAME() 
          FROM inserted i
          JOIN deleted d ON i.PRCo = d.PRCo AND i.EarnCode = d.EarnCode
          JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
          WHERE ISNULL(i.ROEType,'') <> ISNULL(d.ROEType,'') AND a.AuditDLs = 'Y'

		 INSERT INTO dbo.bHQMA
    	 SELECT 'bPREC', 'EarnCode: ' + CONVERT(VARCHAR(10),i.EarnCode), i.PRCo, 'C','ROEPeriod',
         		d.ROEPeriod, i.ROEPeriod,	GETDATE(), SUSER_SNAME() 
          FROM inserted i
          JOIN deleted d ON i.PRCo = d.PRCo AND i.EarnCode = d.EarnCode
          JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
          WHERE ISNULL(i.ROEPeriod,'') <> ISNULL(d.ROEPeriod,'') AND a.AuditDLs = 'Y'
...
GO
ALTER TABLE [dbo].[bPREC] WITH NOCHECK ADD CONSTRAINT [CK_bPREC_AutoAP] CHECK (([AutoAP]='Y' OR [AutoAP]='N'))
GO
ALTER TABLE [dbo].[bPREC] WITH NOCHECK ADD CONSTRAINT [CK_bPREC_CertRpt] CHECK (([CertRpt]='Y' OR [CertRpt]='N'))
GO
ALTER TABLE [dbo].[bPREC] WITH NOCHECK ADD CONSTRAINT [CK_bPREC_IncldLiabDist] CHECK (([IncldLiabDist]='Y' OR [IncldLiabDist]='N'))
GO
ALTER TABLE [dbo].[bPREC] WITH NOCHECK ADD CONSTRAINT [CK_bPREC_OTCalcs] CHECK (([OTCalcs]='Y' OR [OTCalcs]='N'))
GO
ALTER TABLE [dbo].[bPREC] WITH NOCHECK ADD CONSTRAINT [CK_bPREC_SubjToAddOns] CHECK (([SubjToAddOns]='Y' OR [SubjToAddOns]='N'))
GO
ALTER TABLE [dbo].[bPREC] WITH NOCHECK ADD CONSTRAINT [CK_bPREC_SubjToAutoEarns] CHECK (([SubjToAutoEarns]='Y' OR [SubjToAutoEarns]='N'))
GO
ALTER TABLE [dbo].[bPREC] WITH NOCHECK ADD CONSTRAINT [CK_bPREC_TransByEmployee] CHECK (([TransByEmployee]='Y' OR [TransByEmployee]='N' OR [TransByEmployee] IS NULL))
GO
ALTER TABLE [dbo].[bPREC] WITH NOCHECK ADD CONSTRAINT [CK_bPREC_TrueEarns] CHECK (([TrueEarns]='Y' OR [TrueEarns]='N'))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPREC] ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPREC] ON [dbo].[bPREC] ([PRCo], [EarnCode]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO

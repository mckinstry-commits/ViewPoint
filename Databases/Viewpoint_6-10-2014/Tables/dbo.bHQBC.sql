...
GO
ALTER TABLE [dbo].[bHQBC] WITH NOCHECK ADD CONSTRAINT [CK_bHQBC_Adjust] CHECK (([Adjust]='Y' OR [Adjust]='N' OR [Adjust] IS NULL))
GO
ALTER TABLE [dbo].[bHQBC] WITH NOCHECK ADD CONSTRAINT [CK_bHQBC_Rstrict] CHECK (([Rstrict]='Y' OR [Rstrict]='N'))
GO
CREATE UNIQUE CLUSTERED INDEX [biHQBC] ON [dbo].[bHQBC] ([Co], [Mth], [BatchId]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bHQBC] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biHQBCPREndDate] ON [dbo].[bHQBC] ([PREndDate]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_bHQBC_UniqueAttchID] ON [dbo].[bHQBC] ([UniqueAttchID]) WITH (FILLFACTOR=70) ON [PRIMARY]
GO

 ...
 *				SCOTTP 07/12/2013 TFS-55748 Delete template records in vPMProjectDefaultDistributions
 *
 *--------------------------------------------------------------*/
...
---- delete Template records from vPMProjectMasterTemplates
delete vPMProjectMasterTemplates from deleted d join vPMProjectMasterTemplates o on o.DefaultTemplate=d.TemplateName
if exists(SELECT 1 from vPMProjectMasterTemplates o join deleted d on o.DefaultTemplate=d.TemplateName)
	begin
	select @errmsg = 'Template exist in PMProjectMasterTemplates'
	goto error
	end
 ...
 * Created By:	GF 11/20/2001
 * Modified By:	DANF 09/14/2004 - Issue 19246 added new login
 *				SCOTTP 07/12/2013 TFS-55748 Cannot update record if records for Template exist in vPMProjectMasterTemplates
 *
 *
 *--------------------------------------------------------------*/
declare @numrows int, @errmsg varchar(255), @validcnt int,
    ...
    --   	goto error
   	--	end
		...
		---- no change if merge fields exist
   		...
   		---- no change if records for Template exist in vPMProjectMasterTemplates
   		if exists(SELECT 1 from vPMProjectMasterTemplates f join inserted i on f.DefaultTemplate=i.TemplateName)
   			begin
   			select @errmsg = 'Cannot change Template Type when Distribution Templates entries exist'
   			goto error
   			end
	end
...
GO
ALTER TABLE [dbo].[bHQWD] WITH NOCHECK ADD CONSTRAINT [CK_bHQWD_Active] CHECK (([Active]='Y' OR [Active]='N'))
GO
ALTER TABLE [dbo].[bHQWD] WITH NOCHECK ADD CONSTRAINT [CK_bHQWD_CreateFileType] CHECK (([CreateFileType]='pdf' OR [CreateFileType]='docx' OR [CreateFileType]='doc'))
GO
ALTER TABLE [dbo].[bHQWD] WITH NOCHECK ADD CONSTRAINT [CK_bHQWD_SuppressNotes] CHECK (([SuppressNotes]='Y' OR [SuppressNotes]='N'))
GO
ALTER TABLE [dbo].[bHQWD] WITH NOCHECK ADD CONSTRAINT [CK_bHQWD_SuppressZeros] CHECK (([SuppressZeros]='Y' OR [SuppressZeros]='N'))
GO
ALTER TABLE [dbo].[bHQWD] ADD CONSTRAINT [PK_bHQWD] PRIMARY KEY CLUSTERED  ([TemplateName]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bHQWD] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

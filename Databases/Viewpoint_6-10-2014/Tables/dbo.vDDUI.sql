...
GO
ALTER TABLE [dbo].[vDDUI] WITH NOCHECK ADD CONSTRAINT [CK_vDDUI_InputReq] CHECK (([InputReq]='Y' OR [InputReq]='N' OR [InputReq] IS NULL))
GO
ALTER TABLE [dbo].[vDDUI] WITH NOCHECK ADD CONSTRAINT [CK_vDDUI_InputSkip] CHECK (([InputSkip]='Y' OR [InputSkip]='N' OR [InputSkip] IS NULL))
GO
ALTER TABLE [dbo].[vDDUI] WITH NOCHECK ADD CONSTRAINT [CK_vDDUI_ShowForm] CHECK (([ShowForm]='Y' OR [ShowForm]='N' OR [ShowForm] IS NULL))
GO
ALTER TABLE [dbo].[vDDUI] WITH NOCHECK ADD CONSTRAINT [CK_vDDUI_ShowGrid] CHECK (([ShowGrid]='Y' OR [ShowGrid]='N' OR [ShowGrid] IS NULL))
GO
CREATE UNIQUE CLUSTERED INDEX [viDDUI] ON [dbo].[vDDUI] ([VPUserName], [Form], [Seq]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

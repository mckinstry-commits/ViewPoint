...
[Rate] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPRTH_Rate] DEFAULT ((0)),
...
[udArea] [varchar] (5) COLLATE Latin1_General_BIN NULL
) ON [PRIMARY]
GO
...
-- Description:	Delete trigger for SM Work Completed Entries
-- =============================================
CREATE TRIGGER [dbo].[mtr_PRTH_D] 
   ON  [dbo].[bPRTH] 
   AFTER DELETE
AS 
BEGIN
	...
	DECLARE @KeyID BIGINT, @SMWorkCompletedID BIGINT, @SMBCID BIGINT


	DECLARE D_PRTH_crsr CURSOR FOR
	SELECT KeyID FROM DELETED
	WHERE Type='S'

	OPEN D_PRTH_crsr 
	FETCH NEXT FROM D_PRTH_crsr INTO @KeyID

	WHILE @@FETCH_STATUS=0
	BEGIN
		--SET Variables
		SELECT @SMWorkCompletedID = SMWorkCompletedID, @SMBCID=SMBCID 
			FROM dbo.mckSMBC
			WHERE PRTHKeyID=@KeyID
		
		--CHECK FOR EXISTING SM RECORD AND LINK RECORD
		IF NOT EXISTS(
			SELECT TOP 1 1 
			FROM dbo.mckSMBC 
				JOIN dbo.SMWorkCompleted ON dbo.SMWorkCompleted.SMWorkCompletedID = dbo.mckSMBC.SMWorkCompletedID
			WHERE PRTHKeyID = @KeyID)
			BEGIN
				GOTO NEXTSMRecord
			END
		...
		--CHECK FOR Invoices, Set hours=0 if they exist else delete all records.
		IF EXISTS(
			SELECT TOP 1 1 
			FROM dbo.SMWorkCompleted wc
				INNER JOIN dbo.SMInvoiceDetail si ON si.SMCo = wc.SMCo AND si.WorkOrder = wc.WorkOrder AND si.Scope = wc.Scope AND si.WorkCompleted = wc.WorkCompleted
				INNER JOIN dbo.mckSMBC sb ON sb.SMWorkCompletedID = wc.SMWorkCompletedID
			WHERE sb.PRTHKeyID = @KeyID
			)
		BEGIN
			...
			UPDATE dbo.vSMWorkCompletedEquipment
			SET WorkUnits = 0, TimeUnits = 0
		...
		BEGIN
			...
			DELETE dbo.vSMWorkCompletedEquipment
			WHERE SMWorkCompletedID = @SMWorkCompletedID
...
-- Description:	Trigger to insert new SMWorkCompleted records from PR timecards with associated equipment.
-- =============================================
CREATE TRIGGER [dbo].[mtr_PRTH_I] 
   ON  [dbo].[bPRTH] 
   AFTER INSERT
AS 
BEGIN
	...
	DECLARE @KeyID INT, @PRCo bCompany, @Employee bEmployee, @EMCo bCompany, @EMGroup bGroup, @Equipment bEquip
		,@RevCode bRevCode, @EMRate bRate, @SMCo bCompany, @SMWorkOrder INT, @SMScope INT, @SMWorkCompleted INT
		, @TaxGroup bGroup, @TaxCode bTaxCode, @TaxType INT, @SMPhaseGroup bGroup, @PostDate bDate, @Hours bHrs
		, @exitnow BIT = 0

	IF NOT EXISTS(
	SELECT TOP 1 1 
	FROM INSERTED i
		INNER JOIN EMEM e ON i.PRCo=e.PRCo AND i.Employee=e.Operator
		INNER JOIN EMCO c ON e.EMCo=c.EMCo
	WHERE c.udEqOpRatioLockYN = 'Y')
	GOTO spexit

	IF NOT EXISTS(SELECT TOP 1 1 
		FROM INSERTED i
		INNER JOIN dbo.EMEM ON i.PRCo=EMEM.PRCo AND Employee=Operator
		WHERE i.Type = 'S')
	GOTO spexit

	--SELECT * FROM dbo.PRTH i
	--	INNER JOIN dbo.EMEM ON i.PRCo=EMEM.PRCo AND Employee = Operator
	--	WHERE i.Type='S'
	--SELECT Type, * FROM dbo.PRTH

    -- Insert statements for trigger here
	DECLARE time_crsr CURSOR FOR 
	SELECT KeyID, PRCo, Employee, SMCo, SMWorkOrder, SMScope, PostDate, Hours
		FROM INSERTED i
		WHERE i.Type = 'S'
	
	OPEN time_crsr
	FETCH NEXT FROM time_crsr INTO @KeyID, @PRCo, @Employee, @SMCo, @SMWorkOrder, @SMScope, @PostDate, @Hours

	WHILE @@FETCH_STATUS=0
	BEGIN
		SELECT @SMWorkCompleted=dbo.vfSMGetNextWorkCompletedSeq(@SMCo, @SMWorkOrder)

		SELECT @SMPhaseGroup=PhaseGroup 
		FROM HQCO
		WHERE HQCo=@SMCo

		--GET RATE
		SELECT @EMCo = EMCo, @EMGroup=EMGroup, @Equipment=Equipment, @RevCode=RevenueCode
		FROM dbo.EMEM
		WHERE PRCo=@PRCo AND Operator=@Employee

		SELECT @EMRate=ISNULL(Rate,0) FROM dbo.vfEMEquipmentRevCodeSetup(@EMCo, @Equipment, @EMGroup, @RevCode)

		
		----OPT OUT IF RATE = 0 OR RETURNS NULL
		--IF @EMRate = 0
		--GOTO NEXTSMItem
		
		--GET TAX SETUP
		SELECT @TaxGroup = TaxGroup, @TaxCode = TaxCode, @TaxType = TaxType 
		FROM [dbo].[vfSMGetDefaultTaxInfo] (@SMCo,@SMWorkOrder, @SMScope)

		
		DECLARE @rcode INT, @errmsg VARCHAR(MAX), @UseAgreementRates bYN--, @DefaultTaxType INT
				,@Provisional BIT, @Agreement VARCHAR(15),@Revision INT, @NonBillable bYN, @Job bJob, @msg VARCHAR(255)--, @Date bDate = GETDATE()
				, @GLCo bCompany, @EQRevAccount bGLAcct, @EQCostAccount bGLAcct, @EQWIPCostAccount bGLAcct, @EQRevWIPAccount bGLAcct, @ServiceSite VARCHAR(20)
				, @SMCostType bJCCType, @Technician VARCHAR(15), @EMPriceRate bRate

		SELECT @Technician = Technician
			FROM dbo.SMTechnician
			WHERE PRCo = @PRCo AND Employee = @Employee

		SELECT @SMCostType = SMCostType
			FROM dbo.SMCostType
			WHERE SMCo = @SMCo
				AND SMCostTypeCategory = 'E'

		--	/* Validate Workorder Scope and get Scope related defaults */
		EXEC @rcode = vspSMWorkCompletedScopeVal @SMCo=@SMCo, @WorkOrder=@SMWorkOrder, @Scope=@SMScope, @WorkCompleted=@SMWorkCompleted,
			@LineType=1, @AllowProvisional='Y', @SMCostType=@SMCostType, @DefaultCostAcct=@EQCostAccount OUTPUT, 
			@SMGLCo=@GLCo OUTPUT, @DefaultRevenueAcct=@EQRevAccount OUTPUT,
			@DefaultCostWIPAcct=@EQWIPCostAccount OUTPUT, @DefaultRevWIPAcct=@EQRevWIPAccount OUTPUT,
			@DefaultTaxType = @TaxType OUTPUT, @DefaultTaxCode=@TaxCode OUTPUT, 
		...
		--SELECT * FROM dbo.vfSMRateEquipment(101,4,1,GETDATE(),NULL,NULL,'N','N',101,2,1,(SELECT Rate FROM vfEMEquipmentRevCodeSetup(101,2,101,1)))
		--SELECT * FROM dbo.vfSMGetBillableRateSetup(@SMCo,@SMWorkOrder,@SMScope,@Date,@Agreement,@Revision, @NonBillable,@UseAgreementRates)
		--SELECT * FROM dbo.vfSMGetBillableRateSetup(101,6,1,GETDATE(),NULL,NULL,'N','N')
		SELECT @EMPriceRate= ISNULL(PriceRate,0) 
		FROM [dbo].[vfSMRateEquipment](@SMCo,@SMWorkOrder,@SMScope,@PostDate, @Agreement,@Revision,@NonBillable,@UseAgreementRates,@EMCo,@Equipment,@RevCode,@EMRate)
		--SELECT * FROM vfSMRateEquipment(101,4,1,GETDATE(),NULL,NULL,'N','N',101,2,1,7.75)
		--Opt out of processing if the price=$0
		IF @EMPriceRate=0 
		GOTO NEXTSMItem

		--If it gets this far, insert the SMWorkCompleted record.
		BEGIN TRY
			EXEC @rcode = mckSMTimecardInsert @PRCo = @PRCo, @PRTHKeyID = @KeyID/*, @Mth = @Mth, @BatchId = @BatchId, @BatchSeq = @BatchSeq*/, @Employee = @Employee, @PostDate = @PostDate
				, @SMCo = @SMCo, @WorkOrder = @SMWorkOrder, @Scope = @SMScope, @SMCostType = @SMCostType, @Hours = @Hours--, @OldPostSeq = @oldPostSeq
				, @SMJCCostType = @SMCostType, @SMPhaseGroup = @SMPhaseGroup,@BatchTransType='A'
				--, @PRTBKeyID = @PRTBKeyID
				, @errmsg = @errmsg OUT
			IF @rcode = 1
			BEGIN
		...
		BEGIN CATCH
			BEGIN
				SET @errmsg = ISNULL(@errmsg,'')+'Error creating SMWorkCompleted: ' + ERROR_MESSAGE()
				GOTO NEXTSMItem
			END
		END CATCH

		--SELECT * FROM dbo.PRTH

		NEXTSMItem:
		FETCH NEXT FROM time_crsr INTO @KeyID, @PRCo, @Employee, @SMCo, @SMWorkOrder, @SMScope, @PostDate, @Hours
	END
	CLOSE time_crsr
	DEALLOCATE time_crsr
...
-- Description:	Update trigger for SM Equipment
-- =============================================
CREATE TRIGGER [dbo].[mtr_PRTH_U] 
   ON  [dbo].[bPRTH] 
   AFTER UPDATE
AS 
BEGIN
	...
	DECLARE @KeyID INT, @PRCo bCompany, @Employee bEmployee, @Hours bHrs, @SMCo bCompany, @SMWorkOrder INT, @SMScope INT
		, @OldEmployee bEmployee, @OldSMCo bCompany, @OldSMWorkOrder INT, @OldSMScope INT, @OldHours bHrs
	--Declare other variables
	DECLARE @EMCo bCompany, @EMGroup bGroup, @Equipment bEquip, @Technician VARCHAR(15), @SMWorkCompletedID BIGINT, @SMBCID INT
		, @PostDate bDate = GETDATE(), @SMEMCostType bJCCType, @PhaseGroup bGroup, @msg VARCHAR(255), @rcode INT

	SELECT @SMEMCostType = SMCostType
	FROM dbo.SMCostType
	WHERE SMCo=@SMCo AND SMCostTypeCategory = 'E'
	

	--CURSOR ON UPDATED
	DECLARE SMEquip_crsr CURSOR FOR
	SELECT i.KeyID, i.PRCo, i.Employee, i.Hours, i.SMCo, i.SMWorkOrder, i.SMScope
		,d.Employee, d.SMCo, d.SMWorkOrder, d.SMScope, d.Hours
	FROM INSERTED i
		INNER JOIN DELETED d ON i.KeyID=d.KeyID
	WHERE i.Type = 'S'

	OPEN SMEquip_crsr
	FETCH NEXT FROM SMEquip_crsr INTO @KeyID, @PRCo, @Employee, @Hours, @SMCo, @SMWorkOrder, @SMScope, @OldEmployee, @OldSMCo, @OldSMWorkOrder, @OldSMScope,@OldHours

	WHILE @@FETCH_STATUS=0
	BEGIN
		IF (SELECT dbo.vfIsEqual(@SMCo, @OldSMCo) & dbo.vfIsEqual(@SMWorkOrder,@OldSMWorkOrder) & dbo.vfIsEqual(@Employee,@OldEmployee) 
			& dbo.vfIsEqual(@Hours,@OldHours) & dbo.vfIsEqual(@SMScope,@OldSMScope))=1
		GOTO NEXTRecord

		SELECT @SMBCID = SMBCID, @SMWorkCompletedID = SMWorkCompletedID
			FROM dbo.mckSMBC
			WHERE PRTHKeyID = @KeyID

		SELECT @PhaseGroup=PhaseGroup FROM HQCO WHERE @SMCo=HQCo

		--When a new Work Order is assigned, delete the old Work Completed and create it new. 
		IF (SELECT dbo.vfIsEqual(@SMCo,@OldSMCo) & dbo.vfIsEqual(@SMWorkOrder,@OldSMWorkOrder))<>1
		BEGIN
			DELETE FROM dbo.SMWorkCompleted
			WHERE @SMWorkCompletedID=SMWorkCompletedID
			DELETE FROM dbo.mckSMBC
			WHERE SMBCID=@SMBCID

			EXEC @rcode= dbo.mckSMTimecardInsert @PRCo = @PRCo, -- bCompany
			    @PRTHKeyID=@KeyID,
			    @Employee = @Employee, -- int
			    @PostDate = @PostDate, -- smalldatetime
			    @SMCo = @SMCo, -- bCompany
			    @WorkOrder = @SMWorkOrder, -- int
			    @Scope = @SMScope, -- int
			    @SMCostType = @SMEMCostType, -- smallint
			    @Hours = @Hours, -- bHrs
			    --@OldPostSeq = 0, -- int
			    @SMJCCostType = @SMEMCostType, -- bJCCType
			    @SMPhaseGroup = @PhaseGroup, -- bGroup
			    @BatchTransType = 'A', -- char(1)
			    --@PRTBKeyID = NULL, -- bigint
		...
		--Hours only change
		IF (SELECT dbo.vfIsEqual(@Hours,@OldHours))<>1
		BEGIN
			...
			SET WorkUnits=@Hours, TimeUnits=@Hours
			WHERE SMWorkCompletedID = @SMWorkCompletedID
		...
		--Employee Change
		IF (SELECT dbo.vfIsEqual(@Employee,@OldEmployee))<>1
		BEGIN
			SELECT @EMCo = EMCo, @EMGroup = EMGroup, @Equipment = Equipment
			FROM dbo.EMEM 
			WHERE PRCo=@PRCo AND Operator = @Employee
			...
			SELECT @Technician = Technician FROM dbo.SMTechnician
			WHERE PRCo=@PRCo AND Employee=@Employee
	...
	END
	CLOSE SMEquip_crsr
	DEALLOCATE SMEquip_crsr
	spexit:
END


--SELECT * FROM dbo.mckSMBC
GO
ALTER TABLE [dbo].[bPRTH] WITH NOCHECK ADD CONSTRAINT [CK_bPRTH_Cert] CHECK (([Cert]='Y' OR [Cert]='N'))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPRTH] ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPRTHEarnCode] ON [dbo].[bPRTH] ([PRCo], [Employee], [EarnCode]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPRTHEmpPostDate] ON [dbo].[bPRTH] ([PRCo], [Employee], [PostDate], [InUseBatchId]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPRTHEmpPREndDate] ON [dbo].[bPRTH] ([PRCo], [Employee], [PREndDate], [InUseBatchId]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPRTHJobPostDate] ON [dbo].[bPRTH] ([PRCo], [JCCo], [Job], [PostDate], [InUseBatchId]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPRTHJobEndDate] ON [dbo].[bPRTH] ([PRCo], [JCCo], [Job], [PREndDate], [InUseBatchId]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [ciPRTHJobCMRef] ON [dbo].[bPRTH] ([PRCo], [PREndDate], [Employee], [PaySeq], [Job], [udCMRef]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [ciPRTHCMRef] ON [dbo].[bPRTH] ([PRCo], [PREndDate], [Employee], [PaySeq], [udCMRef]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPRTH] ON [dbo].[bPRTH] ([PRCo], [PRGroup], [PREndDate], [Employee], [PaySeq], [PostSeq]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPRTH_PRTBKeyID] ON [dbo].[bPRTH] ([PRTBKeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPRTHUniqueAttchID] ON [dbo].[bPRTH] ([UniqueAttchID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO

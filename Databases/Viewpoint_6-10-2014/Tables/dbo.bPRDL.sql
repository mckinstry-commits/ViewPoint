...
[RateAmt1] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPRDL_RateAmt1] DEFAULT ((0)),
[RateAmt2] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPRDL_RateAmt2] DEFAULT ((0)),
...
[ROEInsurableYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPRDL_ROEInsurableYN] DEFAULT ('N'),
[SuperWeeklyMin] [dbo].[bDollar] NOT NULL CONSTRAINT [DF_bPRDL_SuperWeeklyMin] DEFAULT ((0.0))
...
*			MV  03/06/2013 - TFS 42400 ROE Insurable
*			EN 06/12/2013 User Story 49132/Task 52752 Added HQMA code for new column SuperWeeklyMin
*
...
*/----------------------------------------------------------------
   ...
   --end 25515
    ...
    if exists (select * from inserted i join dbo.bPRCO a with (nolock) on a.PRCo = i.PRCo where a.AuditDLs = 'Y')
		...
		JOIN deleted d ON i.PRCo = d.PRCo and i.DLCode = d.DLCode
		JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
		WHERE ISNULL(i.ROEInsurableYN,'') <> ISNULL(d.ROEInsurableYN,'') 
			  AND a.AuditDLs='Y'  
		...
		INSERT INTO dbo.bHQMA
 		SELECT 'bPRDL',
			   ...
			   'C',
			   'SuperWeeklyMin',
 			   CONVERT(VARCHAR(30),d.SuperWeeklyMin), 
 			   CONVERT(VARCHAR(30),i.SuperWeeklyMin),
			   GETDATE(), 
			   SUSER_SNAME()
		FROM inserted i
		JOIN deleted d ON i.PRCo = d.PRCo and i.DLCode = d.DLCode
		JOIN dbo.bPRCO a WITH (NOLOCK) ON i.PRCo = a.PRCo
		WHERE ISNULL(i.SuperWeeklyMin,'') <> ISNULL(d.SuperWeeklyMin,'') 
			  AND a.AuditDLs='Y'
...
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_AccumSubjAmts] CHECK (([AccumSubjAmts]='Y' OR [AccumSubjAmts]='N'))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_AutoAP] CHECK (([AutoAP]='Y' OR [AutoAP]='N'))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_BonusOverride] CHECK (([BonusOverride]='Y' OR [BonusOverride]='N'))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_DetOnCert] CHECK (([DetOnCert]='Y' OR [DetOnCert]='N'))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_IncldW2] CHECK (([IncldW2]='Y' OR [IncldW2]='N'))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_LimitCorrect] CHECK (([LimitCorrect]='Y' OR [LimitCorrect]='N' OR [LimitCorrect] IS NULL))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_RndToDollar] CHECK (([RndToDollar]='Y' OR [RndToDollar]='N'))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_SelectPurge] CHECK (([SelectPurge]='Y' OR [SelectPurge]='N'))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_SeqOneOnly] CHECK (([SeqOneOnly]='Y' OR [SeqOneOnly]='N'))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_TransByEmployee] CHECK (([TransByEmployee]='Y' OR [TransByEmployee]='N' OR [TransByEmployee] IS NULL))
GO
ALTER TABLE [dbo].[bPRDL] WITH NOCHECK ADD CONSTRAINT [CK_bPRDL_YTDCorrect] CHECK (([YTDCorrect]='Y' OR [YTDCorrect]='N'))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPRDL] ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPRDL] ON [dbo].[bPRDL] ([PRCo], [DLCode]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
...
GO
ALTER TABLE [dbo].[bPRDL] NOCHECK CONSTRAINT [FK_bPRDL_PRCo_PreTaxGroup]
GO

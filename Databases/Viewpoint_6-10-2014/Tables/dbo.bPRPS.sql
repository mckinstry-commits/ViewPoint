...
-- Description:	Fetch description from Standard Pay Sequence
-- =============================================
CREATE TRIGGER [dbo].[mcktPRPSGetDefaults] 
   ON  [dbo].[bPRPS] 
   AFTER INSERT
AS 
BEGIN
	...
	DECLARE @PaySeq INT, @PRCo bCompany, @PRGroup bGroup, @PREndDate bDate
	SELECT @PaySeq = PaySeq, @PRCo = PRCo, @PRGroup = PRGroup, @PREndDate = PREndDate FROM INSERTED

    -- Insert statements for trigger here
	IF EXISTS(SELECT TOP 1 1 FROM udStandardPS WHERE PaySeq = @PaySeq)
	BEGIN
		UPDATE PRPS
		SET Description = (SELECT TOP 1 u.Description 
			FROM udStandardPS u
			WHERE u.PaySeq = @PaySeq)
		WHERE PaySeq = @PaySeq AND PRCo = @PRCo AND PRGroup = @PRGroup AND PREndDate = @PREndDate
	END
END
GO
ALTER TABLE [dbo].[bPRPS] WITH NOCHECK ADD CONSTRAINT [CK_bPRPS_Bonus] CHECK (([Bonus]='Y' OR [Bonus]='N'))
GO
ALTER TABLE [dbo].[bPRPS] WITH NOCHECK ADD CONSTRAINT [CK_bPRPS_OverrideDirDep] CHECK (([OverrideDirDep]='Y' OR [OverrideDirDep]='N'))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPRPS] ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPRPS] ON [dbo].[bPRPS] ([PRCo], [PRGroup], [PREndDate], [PaySeq]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO

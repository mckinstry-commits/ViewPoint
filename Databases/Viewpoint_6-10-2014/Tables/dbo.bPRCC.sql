...
[OldCapLimit] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPRCC_OldCapLimit] DEFAULT ((0)),
[NewCapLimit] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPRCC_NewCapLimit] DEFAULT ((0)),
[Notes] [varchar] (max) COLLATE Latin1_General_BIN NULL,
[UniqueAttchID] [uniqueidentifier] NULL,
[WghtAvgOT] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPRCC_WghtAvgOT] DEFAULT ('N'),
[KeyID] [bigint] NOT NULL IDENTITY(1, 1),
[SuperWeeklyMin] [dbo].[bDollar] NOT NULL CONSTRAINT [DF_bPRCC_SuperWeeklyMin] DEFAULT ((0)),
[udShopYN] [dbo].[bYN] NULL CONSTRAINT [DF__bPRCC__udShopYN__DEFAULT] DEFAULT ('N')
...
/*-----------------------------------------------------------------
   ...
   if exists (select * from inserted i join dbo.bPRCO a with (nolock) on a.PRCo = i.PRCo where a.AuditCraftClass = 'Y')
     ...
     if update(SuperWeeklyMin)
   		insert dbo.bHQMA (TableName, KeyString, Co, RecType, FieldName, OldValue, NewValue, DateTime, UserName)
   		select 'bPRCC','Craft: ' + i.Craft + ' Class: ' + i.Class,
   			i.PRCo, 'C', 'Superannuation Weekly Minimum', convert(varchar(20),d.SuperWeeklyMin), convert(varchar(20),i.SuperWeeklyMin), getdate(), SUSER_SNAME()
       	from inserted i
   		join deleted d on i.PRCo = d.PRCo and i.Craft = d.Craft and i.Class = d.Class
           join dbo.PRCO p with (nolock) on i.PRCo = p.PRCo
       	where d.SuperWeeklyMin <> i.SuperWeeklyMin and p.AuditCraftClass = 'Y'
...
GO
ALTER TABLE [dbo].[bPRCC] WITH NOCHECK ADD CONSTRAINT [CK_bPRCC_WghtAvgOT] CHECK (([WghtAvgOT]='Y' OR [WghtAvgOT]='N'))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPRCC] ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPRCC] ON [dbo].[bPRCC] ([PRCo], [Craft], [Class]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO

...
GO
ALTER TABLE [dbo].[bPOCT] WITH NOCHECK ADD CONSTRAINT [CK_bPOCT_Complied] CHECK (([Complied]='Y' OR [Complied]='N' OR [Complied] IS NULL))
GO
ALTER TABLE [dbo].[bPOCT] WITH NOCHECK ADD CONSTRAINT [CK_bPOCT_PurgeYN] CHECK (([PurgeYN]='Y' OR [PurgeYN]='N'))
GO
ALTER TABLE [dbo].[bPOCT] WITH NOCHECK ADD CONSTRAINT [CK_bPOCT_Verify] CHECK (([Verify]='Y' OR [Verify]='N'))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPOCT] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPOCT] ON [dbo].[bPOCT] ([POCo], [PO], [CompCode], [Seq]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

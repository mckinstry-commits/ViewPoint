...
[UM] [dbo].[bUM] NOT NULL,
[UnitCost] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPMSL_UnitCost] DEFAULT ((0)),
...
*				TRL 12/31/2013 Bug 71975, add code back in to update PM Project Firms for non existing Vendor in PM Project Firms
*
*--------------------------------------------------------------*/
...
---- insert SendTo info in bPMSS if needed
	if exists(select 1 from dbo.bPMSS with (nolock) where PMCo=@pmco and Project=@project
					and SLCo=@slco and SL=@sl and (SendToFirm is null or SendToContact is null))
	begin
		exec @retcode = bspPMSSInitialize @pmco, @project, @slco, @sl, @retmsg output
	end	
...
*					GF 11/09/2012 TK-18033 SL Claim Enhancement. Changed to Use ApprovalRequired
*					SCOTTP 05/03/2014 TFS-13563 Remove code that uses PMSS. Distribution is now managed via the form or C&S Form
*					TL 01/07/2013 Bug 71974 Add code back in to create PMPF and PMSS
*
*--------------------------------------------------------------*/
...
GO
ALTER TABLE [dbo].[bPMSL] WITH NOCHECK ADD CONSTRAINT [CK_bPMSL_SendFlag] CHECK (([SendFlag]='Y' OR [SendFlag]='N'))
GO
ALTER TABLE [dbo].[bPMSL] WITH NOCHECK ADD CONSTRAINT [CK_bPMSL_SubCO] CHECK (([SubCO]>=(0)))
GO
...
GO
ALTER TABLE [dbo].[bPMSL] NOCHECK CONSTRAINT [FK_bPMSL_bJCJM]
GO
ALTER TABLE [dbo].[bPMSL] NOCHECK CONSTRAINT [FK_bPMSL_bSLAD]
GO
ALTER TABLE [dbo].[bPMSL] NOCHECK CONSTRAINT [FK_bPMSL_bHQTX]
GO
ALTER TABLE [dbo].[bPMSL] NOCHECK CONSTRAINT [FK_bPMSL_bAPVM]
GO

...
GO
ALTER TABLE [dbo].[bHRPC] WITH NOCHECK ADD CONSTRAINT [CK_bHRPC_AdYN] CHECK (([AdYN]='Y' OR [AdYN]='N'))
GO
ALTER TABLE [dbo].[bHRPC] WITH NOCHECK ADD CONSTRAINT [CK_bHRPC_BonusYN] CHECK (([BonusYN]='Y' OR [BonusYN]='N'))
GO
ALTER TABLE [dbo].[bHRPC] WITH NOCHECK ADD CONSTRAINT [CK_bHRPC_PartTimeYN] CHECK (([PartTimeYN]='Y' OR [PartTimeYN]='N'))
GO
CREATE UNIQUE CLUSTERED INDEX [biHRPC] ON [dbo].[bHRPC] ([HRCo], [PositionCode]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bHRPC] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

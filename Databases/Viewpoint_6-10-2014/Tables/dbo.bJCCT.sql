...
GO
ALTER TABLE [dbo].[bJCCT] WITH NOCHECK ADD CONSTRAINT [CK_bJCCT_TrackHours] CHECK (([TrackHours]='Y' OR [TrackHours]='N' OR [TrackHours] IS NULL))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bJCCT] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biJCCT] ON [dbo].[bJCCT] ([PhaseGroup], [CostType]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

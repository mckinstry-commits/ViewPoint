...
[Purge] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPOHD_Purge] DEFAULT ('N'),
...
-- =============================================
CREATE TRIGGER [dbo].[mcktrMCKPONumberPOHD] 
   ON  [dbo].[bPOHD] 
   AFTER INSERT,UPDATE
AS 
BEGIN
	...
	*/
	...
	SELECT @POCo = POCo, @PO = PO, @MCKPONumber = udMCKPONumber, @Approved = Approved, @ApprovedBy = ApprovedBy, @Status = Status FROM INSERTED
...
-- Description:	Messaging on PO Creation from PO module only
-- =============================================
CREATE TRIGGER dbo.mcktrPONotifier 
   ON  dbo.bPOHD 
   AFTER INSERT
AS 
BEGIN
	...
	IF EXISTS(SELECT 1 FROM INSERTED WHERE ISNULL(udPMSource,2) <> 1)
	BEGIN
		DECLARE @POCo bCompany, @PO bPO, @To VARCHAR(128)
		DECLARE insert_cr CURSOR FOR 
		SELECT POCo, PO, ISNULL(fc.EMail,'NULL EMAIL')
			FROM INSERTED i
			LEFT OUTER JOIN PMPM fc ON FirmNumber = '800000' AND i.udOrderedBy = fc.ContactCode
			WHERE ISNULL(udPMSource,2) <> 1
		
		OPEN insert_cr

		FETCH NEXT FROM insert_cr INTO @POCo, @PO, @To

		WHILE @@FETCH_STATUS = 0
		BEGIN

			SELECT @subject = N'On Demand PO ' + ISNULL(@To, 'erptest@mckinstry.com')
			...
			SET @tableHTML =
				...
				( 
					( 
						SELECT
							td = COALESCE(i.POCo,' '), ''
						,	td = COALESCE(i.PO,' '), ''
						,	td = COALESCE(i.udMCKPONumber,' '), ''
						,	td = COALESCE(i.Description,' '), ''
						,	td = COALESCE(i.Vendor,' '), ''
						,	td = COALESCE(a.Name,' '), '' AS VendorName
						--,	td = COALESCE('',' '), ''
						--,	td = COALESCE('',' '), '' AS JobName
						,	td = COALESCE(i.Status,' '), '' AS POStatus
						,	td = COALESCE(CONVERT(VARCHAR(10),i.OrderDate,101),' '), ''
						FROM 
							POHD i
							INNER JOIN APVM a ON i.VendorGroup = a.VendorGroup AND i.Vendor = a.Vendor
							LEFT OUTER JOIN JCJM j ON j.JCCo = i.POCo AND j.Job = i.Job
				...
				EXEC msdb.dbo.sp_send_dbmail 
					...
					@recipients = 'erptest@mckinstry.com'--@To --UPDATE AFTER GO LIVE
					,@subject = @subject,
					@body = @tableHTML,
					@body_format = 'HTML'
		...
		END
		CLOSE insert_cr
		DEALLOCATE insert_cr
	END
END
GO
ALTER TABLE [dbo].[bPOHD] WITH NOCHECK ADD CONSTRAINT [CK_bPOHD_Approved] CHECK (([Approved]='Y' OR [Approved]='N' OR [Approved] IS NULL))
GO
ALTER TABLE [dbo].[bPOHD] WITH NOCHECK ADD CONSTRAINT [CK_bPOHD_Purge] CHECK (([Purge]='Y' OR [Purge]='N'))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPOHD] ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_bPOHD_POCO_PO] ON [dbo].[bPOHD] ([PO], [POCo]) INCLUDE ([Status], [Vendor], [VendorGroup]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPOHD] ON [dbo].[bPOHD] ([POCo], [PO]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPOHDUniqueAttId] ON [dbo].[bPOHD] ([UniqueAttchID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPOHDVendor] ON [dbo].[bPOHD] ([Vendor]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
ALTER TABLE [dbo].[bPOHD] WITH NOCHECK ADD CONSTRAINT [FK_bPOHD_bPMDT] FOREIGN KEY ([DocType]) REFERENCES [dbo].[bPMDT] ([DocType])
GO
ALTER TABLE [dbo].[bPOHD] NOCHECK CONSTRAINT [FK_bPOHD_bPMDT]
GO

...
[MapCodeYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_udxrefPRDedLiab_MapCodeYN] DEFAULT ('N')
) ON [PRIMARY]
GO
...
CREATE TRIGGER dbo.vtbudxrefPRDedLiab_Audit_Delete ON dbo.budxrefPRDedLiab
 ...
 SET NOCOUNT ON 
 -- generated by vspHQCreateAuditTriggers on Jan 17 2014  1:18PM
...
DECLARE @HQMAKeys TABLE
 ...
 BEGIN   INSERT dbo.HQMA (TableName, KeyString, Co, RecType, FieldName, OldValue, NewValue, DateTime, UserName)
		OUTPUT inserted.AuditID, inserted.KeyString INTO @HQMAKeys (AuditID, KeyString) 
   SELECT 'budxrefPRDedLiab' , '<KeyString CMS Ded Code = "' + REPLACE(CAST(inserted.[CMSDedCode] AS VARCHAR(MAX)),'"', '&quot;') + '" CMS Ded Type = "' + REPLACE(CAST(inserted.[CMSDedType] AS VARCHAR(MAX)),'"', '&quot;') + '" CMS Union = "' + REPLACE(CAST(inserted.[CMSUnion] AS VARCHAR(MAX)),'"', '&quot;') + '" Company = "' + REPLACE(CAST(inserted.[Company] AS VARCHAR(MAX)),'"', '&quot;') + '" />' , '' , 'C' , '[MapCodeYN]' ,  CONVERT(VARCHAR(MAX), deleted.[MapCodeYN]) ,  CONVERT(VARCHAR(MAX), inserted.[MapCodeYN]) , GETDATE() , SUSER_SNAME()
		FROM inserted
			INNER JOIN deleted
         ON  inserted.[KeyID] = deleted.[KeyID] 
         AND ((inserted.[MapCodeYN] <> deleted.[MapCodeYN]) OR (inserted.[MapCodeYN] IS NULL AND deleted.[MapCodeYN] IS NOT NULL) OR (inserted.[MapCodeYN] IS NOT NULL AND deleted.[MapCodeYN] IS NULL))

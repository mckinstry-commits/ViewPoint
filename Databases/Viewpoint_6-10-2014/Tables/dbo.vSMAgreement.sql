...
[RevenueRecognition] [char] (1) COLLATE Latin1_General_BIN NOT NULL
) ON [PRIMARY]
GO
...
-- Modification: 03/05/13 Matthew B update for recognize revenue TFS#40933
-- =============================================
...
BEGIN
	...
	IF EXISTS (SELECT 1
		FROM INSERTED
			INNER JOIN dbo.SMAgreementRevenueDeferral ON INSERTED.SMCo = dbo.SMAgreementRevenueDeferral.SMCo AND INSERTED.Agreement = dbo.SMAgreementRevenueDeferral.Agreement AND INSERTED.Revision = dbo.SMAgreementRevenueDeferral.Revision
			WHERE dbo.vfIsEqual(INSERTED.RevenueRecognition, 'S') = 0)
	BEGIN
	    RAISERROR(N'All revenue deferrals must be deleted before you can switch to Billed.', 11, -1)
    	ROLLBACK TRANSACTION
		RETURN
	END
END
...
CREATE TRIGGER dbo.vtvSMAgreement_Audit_Delete ON dbo.vSMAgreement
 ...
 BEGIN TRY 
							...
							SELECT
								...
								'D' , 
								'RevenueRecognition' , 
								CONVERT(VARCHAR(MAX), deleted.[RevenueRecognition]) , 								NULL , 
								GETDATE() , 
								SUSER_SNAME()
							FROM deleted
								JOIN dbo.vAuditFlagCompany AS afc ON deleted.SMCo = afc.AuditCo 
...
-- log additions to the RevenueRecognition column
							INSERT dbo.bHQMA (	
													TableName, 
													KeyString, 
													Co, 
...
CREATE TRIGGER dbo.vtvSMAgreement_Audit_Update ON dbo.vSMAgreement
 ...
 BEGIN TRY 
							...
							INSERT dbo.bHQMA (	TableName, 
								...
								SELECT 							'vSMAgreement' , 								'<KeyString />' , 								inserted.SMCo , 								'C' , 								'RevenueRecognition' , 								CONVERT(VARCHAR(MAX), deleted.[RevenueRecognition]) , 								CONVERT(VARCHAR(MAX), inserted.[RevenueRecognition]) , 								GETDATE() , 								SUSER_SNAME()
							FROM inserted
								INNER JOIN deleted
									ON  inserted.[SMAgreementID] = deleted.[SMAgreementID] 
									AND ((inserted.[RevenueRecognition] <> deleted.[RevenueRecognition]) OR (inserted.[RevenueRecognition] IS NULL AND deleted.[RevenueRecognition] IS NOT NULL) OR (inserted.[RevenueRecognition] IS NOT NULL AND deleted.[RevenueRecognition] IS NULL))
								JOIN dbo.vAuditFlagCompany AS afc ON inserted.SMCo = afc.AuditCo 
							WHERE afc.AuditFlagID = 15
...
GO
ALTER TABLE [dbo].[vSMAgreement] WITH NOCHECK ADD CONSTRAINT [FK_vSMAgreement_vRPRTc] FOREIGN KEY ([ReportID]) REFERENCES [dbo].[vRPRTc] ([ReportID])
GO
...
GO
ALTER TABLE [dbo].[vSMAgreement] NOCHECK CONSTRAINT [FK_vSMAgreement_vRPRTc]
GO
ALTER TABLE [dbo].[vSMAgreement] NOCHECK CONSTRAINT [FK_vSMAgreement_vSMAgreement]
GO
ALTER TABLE [dbo].[vSMAgreement] NOCHECK CONSTRAINT [FK_vSMAgreement_vSMAgreementType]
GO
ALTER TABLE [dbo].[vSMAgreement] NOCHECK CONSTRAINT [FK_vSMAgreement_vSMCustomer]
GO

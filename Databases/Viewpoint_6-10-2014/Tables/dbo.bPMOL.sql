...
[EstUnits] [dbo].[bUnits] NOT NULL CONSTRAINT [DF_bPMOL_EstUnits] DEFAULT ((0)),
[UM] [dbo].[bUM] NOT NULL,
[UnitHours] [dbo].[bHrs] NOT NULL,
[EstHours] [dbo].[bHrs] NOT NULL CONSTRAINT [DF_bPMOL_EstHours] DEFAULT ((0)),
[HourCost] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPMOL_HourCost] DEFAULT ((0)),
[UnitCost] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPMOL_UnitCost] DEFAULT ((0)),
[ECM] [dbo].[bECM] NOT NULL,
[EstCost] [dbo].[bDollar] NOT NULL CONSTRAINT [DF_bPMOL_EstCost] DEFAULT ((0)),
...
[FromImport] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPMOL_FromImport] DEFAULT ('N')
...
*				TRL 02/05/2013 Task-39604, UserStory29945 Contract Item Description now defaults when not using Phase Desc
*				STO 03/12/2013 TFS 40561 - importing creates doubled entries in PM Subcontract Detail
...
declare @numrows int, @rcode int, @errmsg varchar(255), @opencursor tinyint,
		...
		@MaterialGroup bGroup, @ImpactSL bYN, @ImpactPO bYN, @ContractItemDescription bItemDesc, @FromImport bYN,
		-- TFS 43659
		@CreateSL bYN,
		@slcosttype2 bJCCType,@mtlcosttype2 bJCCType
...
---- TFS 40561 - importing creates doubled entries in PM Subcontract Detail
SELECT distinct @FromImport = FromImport FROM inserted
if @FromImport = 'Y' return
...
if @validcnt + @validcnt2 <> @numrows
	BEGIN
	select @validcnt, @validcnt2 , @numrows
...
select @apco=p.APCo, @slcosttype=p.SLCostType, @mtlcosttype=p.MtlCostType,
		@slcosttype2=p.SLCostType2, @mtlcosttype2=p.MatlCostType2,
...
IF @costtype NOT IN (ISNULL(@slcosttype,0),ISNULL(@slcosttype2,0), ISNULL(@mtlcosttype,0),ISNULL(@mtlcosttype2,0))
	BEGIN
...
if @costtype NOT IN ( isnull(@slcosttype,0),ISNULL(@slcosttype2,0) ) goto PMMF_CHECK
...
if exists (select 1 from dbo.bPMSL with (nolock) where PMCo=@pmco and Project=@project 
	...
	select @dfltwcpct = isnull(i.RetainPCT,0), @PhaseDesc=p.Description,  @ContractItemDescription=i.Description
	from dbo.bJCJP p 
	...
	if isnull(@PurchaseUnits, 0) = 0 and isnull(@PurchaseAmount, 0) <> 0 and isnull(@PurchaseUM,'LS') <> 'LS'
		...
		IF @sl IS NULL OR @POSLItem IS NULL
			...
			select @pmco, @project, @Next_Seq, 'C', @pcotype, @pco, @pcoitem, @aco, @acoitem,
					...
					case when isnull(@UsePhaseDesc, 'N') = 'Y' then @PhaseDesc else  @ContractItemDescription END
			END
		ELSE
			BEGIN
...
if @costtype NOT IN ( isnull(@mtlcosttype,0),isnull(@mtlcosttype2,0)) goto NEXTROW
...
if exists (select 1 from dbo.bPMMF with (nolock) where PMCo=@pmco and Project=@project 
	...
	---- get default retg pct from JCCI to use as a default
	select @dfltwcpct = isnull(i.RetainPCT,0), @ContractItemDescription=i.Description
	...
	if isnull(@PurchaseUnits, 0) = 0 and isnull(@PurchaseAmount, 0) <> 0 and isnull(@PurchaseUM,'LS') <> 'LS'
		...
		IF @po IS NULL OR @POSLItem IS NULL
			...
			select @pmco, @project, @Next_Seq, 'C', @pcotype, @pco, @pcoitem,
					...
					case when @UseMatlPhaseDesc = 'Y' then @PhaseDesc else  @ContractItemDescription END
			END
		ELSE
			BEGIN
...
if @validcnt + @validcnt2 <> @numrows
	BEGIN
	select @validcnt, @validcnt2, @numrows,Subcontract,CreateSL from inserted
	SELECT @errmsg = 'Vendor is Invalid for Subcontract'
	goto error
	END
...
GO
ALTER TABLE [dbo].[bPMOL] WITH NOCHECK ADD CONSTRAINT [CK_bPMOL_ECM] CHECK (([ECM]='E' OR [ECM]='C' OR [ECM]='M'))
GO
...
GO
ALTER TABLE [dbo].[bPMOL] WITH NOCHECK ADD CONSTRAINT [CK_bPMOL_SendYN] CHECK (([SendYN]='Y' OR [SendYN]='N'))
GO
...
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMOL] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [IX_bPMOL_CLUSTERED] ON [dbo].[bPMOL] ([PMCo], [Project], [PCOType], [PCO], [PCOItem], [ACO], [ACOItem], [PhaseGroup], [Phase], [CostType]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_bPMOL2_NONCLUSTERED] ON [dbo].[bPMOL] ([PMCo], [Project], [PhaseGroup], [Phase], [CostType], [PCOType], [PCO], [PCOItem], [ACO], [ACOItem]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
...
GO
ALTER TABLE [dbo].[bPMOL] NOCHECK CONSTRAINT [FK_bPMOL_bPMDT]
GO
ALTER TABLE [dbo].[bPMOL] NOCHECK CONSTRAINT [FK_bPMOL_bAPVM]
GO

...
GO
ALTER TABLE [dbo].[bGLBL] WITH NOCHECK ADD CONSTRAINT [CK_bGLBL_NetActivity] CHECK (([NetActivity]=(isnull([Debits],(0))-isnull([Credits],(0)))))
GO
ALTER TABLE [dbo].[bGLBL] ADD CONSTRAINT [PK_bGLBL] PRIMARY KEY NONCLUSTERED  ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biGLBL] ON [dbo].[bGLBL] ([GLCo], [GLAcct], [Mth]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO

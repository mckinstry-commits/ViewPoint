...
[CurCost] [dbo].[bDollar] NOT NULL CONSTRAINT [DF_vSLClaimHeader_CurCost] DEFAULT ((0))
...
CREATE TRIGGER dbo.vtvSLClaimHeader_Audit_Delete ON dbo.vSLClaimHeader
 ...
 BEGIN TRY 
							...
							SELECT
								...
								'D' , 
								'CurCost' , 
								CONVERT(VARCHAR(MAX), deleted.[CurCost]) , 								NULL , 
								GETDATE() , 
								SUSER_SNAME()
							FROM deleted
...
CREATE TRIGGER dbo.vtvSLClaimHeader_Audit_Update ON dbo.vSLClaimHeader
 ...
 BEGIN TRY 
							...
							INSERT dbo.bHQMA (	TableName, 
								...
								SELECT 							'vSLClaimHeader' , 								'<KeyString ClaimNo = "' + REPLACE(CAST(ISNULL(inserted.[ClaimNo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SL = "' + REPLACE(CAST(ISNULL(inserted.[SL],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SLCo = "' + REPLACE(CAST(ISNULL(inserted.[SLCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 								inserted.SLCo , 								'C' , 								'CurCost' , 								CONVERT(VARCHAR(MAX), deleted.[CurCost]) , 								CONVERT(VARCHAR(MAX), inserted.[CurCost]) , 								GETDATE() , 								SUSER_SNAME()
							FROM inserted
								INNER JOIN deleted
									ON  inserted.[KeyID] = deleted.[KeyID] 
							...
							INSERT dbo.bHQMA (	TableName, 
								...
								SELECT 							'vSLClaimHeader' , 								'<KeyString ClaimNo = "' + REPLACE(CAST(ISNULL(inserted.[ClaimNo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SL = "' + REPLACE(CAST(ISNULL(inserted.[SL],'') AS VARCHAR(MAX)),'"', '&quot;') + '" SLCo = "' + REPLACE(CAST(ISNULL(inserted.[SLCo],'') AS VARCHAR(MAX)),'"', '&quot;') + '" />' , 								inserted.SLCo , 								'C' , 								'DueDate' , 								CONVERT(VARCHAR(MAX), deleted.[DueDate]) , 								CONVERT(VARCHAR(MAX), inserted.[DueDate]) , 								GETDATE() , 								SUSER_SNAME()
							FROM inserted
								INNER JOIN deleted
									ON  inserted.[KeyID] = deleted.[KeyID] 
...
GO
ALTER TABLE [dbo].[vSLClaimHeader] WITH NOCHECK ADD CONSTRAINT [CK_vSLClaimHeader_ClaimStatus] CHECK (([ClaimStatus]>=(10) AND [ClaimStatus]<=(50)))
GO
...
GO
ALTER TABLE [dbo].[vSLClaimHeader] NOCHECK CONSTRAINT [FK_vSLClaimHeader_bSLHD]
GO

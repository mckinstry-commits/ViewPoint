...
error:
    ...
    rollback transaction
GO
ALTER TABLE [dbo].[vRPRT] WITH NOCHECK ADD CONSTRAINT [CK_vRPRT_ShowOnMenu] CHECK (([ShowOnMenu]='Y' OR [ShowOnMenu]='N'))
GO
CREATE UNIQUE CLUSTERED INDEX [IX_vRPRT_ReportID] ON [dbo].[vRPRT] ([ReportID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [IX_vRPRT_Title] ON [dbo].[vRPRT] ([Title]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

...
[UM] [dbo].[bUM] NOT NULL,
[Units] [dbo].[bUnits] NOT NULL CONSTRAINT [DF_bPMOI_Units] DEFAULT ((0)),
[UnitPrice] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPMOI_UnitPrice] DEFAULT ((0)),
...
[Approved] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPMOI_Approved] DEFAULT ('N'),
[ApprovedBy] [dbo].[bVPUserName] NULL,
[ForcePhaseYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPMOI_ForcePhaseYN] DEFAULT ('N'),
[FixedAmountYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPMOI_FixedAmountYN] DEFAULT ('N'),
...
-- Description:	Trigger to provide custom defaults to the PCO Item.
-- =============================================
CREATE TRIGGER dbo.mcktrPMOIi_Defaults 
   ON  dbo.bPMOI 
   AFTER INSERT
AS 
BEGIN
	...
	SET NOCOUNT ON;
	IF (SELECT TOP 1 PCO FROM INSERTED) IS NOT NULL AND (SELECT TOP 1 PCOItem FROM INSERTED) IS NOT NULL
	BEGIN
		DECLARE @PMCo bCompany, @Project bProject, @PCOType bDocType, @PCO bPCO, @PCOItem bPCOItem, @DefaultRevPct VARCHAR(5)
		SELECT @PMCo = i.PMCo, @Project = i.Project, @PCOType = i.PCOType, @PCO = i.PCO, @PCOItem = i.PCOItem, @DefaultRevPct = p.udProjRevPct
			FROM INSERTED i
			JOIN PMOP p ON p.PMCo = i.PMCo AND p.Project = i.Project AND p.PCOType = i.PCOType AND p.PCO = i.PCO
			WHERE p.PCO IS NOT NULL
		-- Insert statements for trigger here
	
		--UPDATE dbo.PMOI with Default percent from PCO header

		SELECT * FROM PMOI
		UPDATE PMOI
		SET udProjRevPct=@DefaultRevPct
		WHERE PMCo= @PMCo AND Project = @Project AND PCOType = @PCOType AND PCO = @PCO AND PCOItem = @PCOItem
	END
END
GO
ALTER TABLE [dbo].[bPMOI] WITH NOCHECK ADD CONSTRAINT [CK_bPMOI_Approved] CHECK (([Approved]='Y' OR [Approved]='N'))
GO
ALTER TABLE [dbo].[bPMOI] WITH NOCHECK ADD CONSTRAINT [CK_bPMOI_FixedAmountYN] CHECK (([FixedAmountYN]='Y' OR [FixedAmountYN]='N'))
GO
ALTER TABLE [dbo].[bPMOI] WITH NOCHECK ADD CONSTRAINT [CK_bPMOI_ForcePhaseYN] CHECK (([ForcePhaseYN]='Y' OR [ForcePhaseYN]='N'))
GO
ALTER TABLE [dbo].[bPMOI] WITH NOCHECK ADD CONSTRAINT [CK_bPMOI_PCO] CHECK (([PCO] IS NULL OR [PCOType] IS NOT NULL))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMOI] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPMOIPCOType] ON [dbo].[bPMOI] ([PCOType], [PMCo], [Project]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_bPMOI_PMCOContract] ON [dbo].[bPMOI] ([PMCo], [Contract], [Project]) INCLUDE ([ApprovedAmt]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPMOI] ON [dbo].[bPMOI] ([PMCo], [Project], [PCOType], [PCO], [PCOItem], [ACO], [ACOItem]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biPMOIStatus] ON [dbo].[bPMOI] ([Status], [PMCo], [Project]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
...
GO
ALTER TABLE [dbo].[bPMOI] NOCHECK CONSTRAINT [FK_bPMOI_bPMDT]
GO
ALTER TABLE [dbo].[bPMOI] NOCHECK CONSTRAINT [FK_bPMOI_bJCCI]
GO
ALTER TABLE [dbo].[bPMOI] NOCHECK CONSTRAINT [FK_bPMOI_bPMOH]
GO
ALTER TABLE [dbo].[bPMOI] NOCHECK CONSTRAINT [FK_bPMOI_bPMOP]
GO

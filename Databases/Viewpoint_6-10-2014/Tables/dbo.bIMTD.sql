...
*/----------------------------------------------------------------
		...
		END
GO
ALTER TABLE [dbo].[bIMTD] WITH NOCHECK ADD CONSTRAINT [CK_bIMTD_ImportPromptYN] CHECK (([ImportPromptYN]='Y' OR [ImportPromptYN]='N'))
GO
ALTER TABLE [dbo].[bIMTD] WITH NOCHECK ADD CONSTRAINT [CK_bIMTD_OverrideYN] CHECK (([OverrideYN]='Y' OR [OverrideYN]='N'))
GO
ALTER TABLE [dbo].[bIMTD] WITH NOCHECK ADD CONSTRAINT [CK_bIMTD_UpdateKeyYN] CHECK (([UpdateKeyYN]='Y' OR [UpdateKeyYN]='N'))
GO
ALTER TABLE [dbo].[bIMTD] WITH NOCHECK ADD CONSTRAINT [CK_bIMTD_UpdateValueYN] CHECK (([UpdateValueYN]='Y' OR [UpdateValueYN]='N'))
GO
CREATE UNIQUE CLUSTERED INDEX [biIMTD] ON [dbo].[bIMTD] ([ImportTemplate], [RecordType], [Identifier]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

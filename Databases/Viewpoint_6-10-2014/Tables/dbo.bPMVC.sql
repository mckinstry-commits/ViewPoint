...
GO
ALTER TABLE [dbo].[bPMVC] WITH NOCHECK ADD CONSTRAINT [CK_bPMVC_Visible] CHECK (([Visible]='Y' OR [Visible]='N'))
GO
ALTER TABLE [dbo].[bPMVC] ADD CONSTRAINT [PK_bPMVC] PRIMARY KEY CLUSTERED  ([ViewName], [Form], [ColSeq]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMVC] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
ALTER TABLE [dbo].[bPMVC] WITH NOCHECK ADD CONSTRAINT [FK_bPMVC_bPMVG] FOREIGN KEY ([ViewName], [Form]) REFERENCES [dbo].[bPMVG] ([ViewName], [Form])
GO
ALTER TABLE [dbo].[bPMVC] NOCHECK CONSTRAINT [FK_bPMVC_bPMVG]
GO

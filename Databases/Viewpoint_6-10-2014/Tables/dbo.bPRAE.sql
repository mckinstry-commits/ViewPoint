...
[Hours] [dbo].[bHrs] NULL,
[RateAmt] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPRAE_RateAmt] DEFAULT ((0)),
...
GO
ALTER TABLE [dbo].[bPRAE] WITH NOCHECK ADD CONSTRAINT [CK_bPRAE_OvrStdLimitYN] CHECK (([OvrStdLimitYN]='Y' OR [OvrStdLimitYN]='N'))
GO
ALTER TABLE [dbo].[bPRAE] WITH NOCHECK ADD CONSTRAINT [CK_bPRAE_StdHours] CHECK (([StdHours]='Y' OR [StdHours]='N'))
GO
ALTER TABLE [dbo].[bPRAE] ADD CONSTRAINT [PK_bPRAE_KeyID] PRIMARY KEY NONCLUSTERED  ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPRAE] ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPRAE] ON [dbo].[bPRAE] ([PRCo], [Employee], [EarnCode], [Seq]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO

...
[AccumulateSubjectEarningsYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bPRSI_AccumulateSubjectEarningsYN] DEFAULT ('N')
...
/*-----------------------------------------------------------------
   ...
   IF exists (select top 1 1 from inserted i join dbo.bPRCO a with (nolock) on a.PRCo = i.PRCo where a.AuditTaxes = 'Y')
    ...
    INSERT INTO dbo.bHQMA(TableName, KeyString, Co, RecType, FieldName, OldValue, NewValue, DateTime, UserName)
   	SELECT 'bPRSI',  'PRCo: ' + convert(char(10), i.PRCo) + ' State: ' + i.State, i.PRCo, 'C','AccumulateSubjectEarningsYN',
   		d.AccumulateSubjectEarningsYN,i.AccumulateSubjectEarningsYN,getdate(), SUSER_SNAME()
   	FROM inserted i
    JOIN deleted d on i.PRCo = d.PRCo and i.State = d.State
    WHERE isnull(i.AccumulateSubjectEarningsYN,'') <> isnull(d.AccumulateSubjectEarningsYN,'')
...
GO
ALTER TABLE [dbo].[bPRSI] WITH NOCHECK ADD CONSTRAINT [CK_bPRSI_TaxDiff] CHECK (([TaxDiff]='Y' OR [TaxDiff]='N'))
GO

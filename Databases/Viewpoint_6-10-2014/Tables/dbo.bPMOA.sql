...
GO
ALTER TABLE [dbo].[bPMOA] WITH NOCHECK ADD CONSTRAINT [CK_bPMOA_Include] CHECK (([Include]='Y' OR [Include]='N'))
GO
ALTER TABLE [dbo].[bPMOA] ADD CONSTRAINT [PK_bPMOA] PRIMARY KEY NONCLUSTERED  ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [IX_bPMOA_PMCoProjectPCOType] ON [dbo].[bPMOA] ([PMCo], [Project], [PCOType], [PCO], [PCOItem], [AddOn]) WITH (FILLFACTOR=80) ON [PRIMARY]
GO
ALTER TABLE [dbo].[bPMOA] WITH NOCHECK ADD CONSTRAINT [FK_bPMOA_bPMDT] FOREIGN KEY ([PCOType]) REFERENCES [dbo].[bPMDT] ([DocType])
GO
ALTER TABLE [dbo].[bPMOA] NOCHECK CONSTRAINT [FK_bPMOA_bPMDT]
GO

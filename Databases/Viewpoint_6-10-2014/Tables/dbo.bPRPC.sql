...
GO
ALTER TABLE [dbo].[bPRPC] WITH NOCHECK ADD CONSTRAINT [CK_bPRPC_APInterface] CHECK (([APInterface]='Y' OR [APInterface]='N'))
GO
ALTER TABLE [dbo].[bPRPC] WITH NOCHECK ADD CONSTRAINT [CK_bPRPC_Conv] CHECK (([Conv]='Y' OR [Conv]='N'))
GO
ALTER TABLE [dbo].[bPRPC] WITH NOCHECK ADD CONSTRAINT [CK_bPRPC_EMInterface] CHECK (([EMInterface]='Y' OR [EMInterface]='N'))
GO
ALTER TABLE [dbo].[bPRPC] WITH NOCHECK ADD CONSTRAINT [CK_bPRPC_GLInterface] CHECK (([GLInterface]='Y' OR [GLInterface]='N'))
GO
ALTER TABLE [dbo].[bPRPC] WITH NOCHECK ADD CONSTRAINT [CK_bPRPC_JCInterface] CHECK (([JCInterface]='Y' OR [JCInterface]='N'))
GO
ALTER TABLE [dbo].[bPRPC] WITH NOCHECK ADD CONSTRAINT [CK_bPRPC_LeaveProcess] CHECK (([LeaveProcess]='Y' OR [LeaveProcess]='N'))
GO
ALTER TABLE [dbo].[bPRPC] WITH NOCHECK ADD CONSTRAINT [CK_bPRPC_MultiMth] CHECK (([MultiMth]='Y' OR [MultiMth]='N'))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPRPC] ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPRPC] ON [dbo].[bPRPC] ([PRCo], [PRGroup], [PREndDate]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO

...
GO
ALTER TABLE [dbo].[bHQCP] WITH NOCHECK ADD CONSTRAINT [CK_bHQCP_AllInvoiceYN] CHECK (([AllInvoiceYN]='Y' OR [AllInvoiceYN]='N'))
GO
ALTER TABLE [dbo].[bHQCP] WITH NOCHECK ADD CONSTRAINT [CK_bHQCP_Verify] CHECK (([Verify]='Y' OR [Verify]='N'))
GO
CREATE UNIQUE CLUSTERED INDEX [biHQCP] ON [dbo].[bHQCP] ([CompCode]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bHQCP] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

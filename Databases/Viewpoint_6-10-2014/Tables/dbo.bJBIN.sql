...
-- Description:	INSERT Trigger to default Status to 'N' for BillType ='B'
-- =============================================
CREATE TRIGGER dbo.mtrJBINDefault_I 
   ON  dbo.bJBIN 
   AFTER INSERT
AS 
BEGIN
	...
	IF EXISTS(SELECT 1 FROM INSERTED WHERE BillType = 'B')
	BEGIN
		DECLARE @Inserted TABLE (JBCo bCompany, Mth bMonth, BillNumber INT)
		INSERT INTO @Inserted
		        ( JBCo, Mth, BillNumber )
		SELECT JBCo, BillMonth, BillNumber FROM INSERTED WHERE BillType = 'B'

		DECLARE @JBCo bCompany, @Mth bMonth, @BillNum INT

		DECLARE insert_cursor CURSOR FOR 
		SELECT JBCo, Mth, BillNumber FROM @Inserted

		OPEN insert_cursor
			FETCH NEXT FROM insert_cursor INTO @JBCo, @Mth, @BillNum
...
END
GO
ALTER TABLE [dbo].[bJBIN] WITH NOCHECK ADD CONSTRAINT [CK_bJBIN_AuditYN] CHECK (([AuditYN]='Y' OR [AuditYN]='N'))
GO
ALTER TABLE [dbo].[bJBIN] WITH NOCHECK ADD CONSTRAINT [CK_bJBIN_AutoInitYN] CHECK (([AutoInitYN]='Y' OR [AutoInitYN]='N'))
GO
ALTER TABLE [dbo].[bJBIN] WITH NOCHECK ADD CONSTRAINT [CK_bJBIN_BillOnCompleteYN] CHECK (([BillOnCompleteYN]='Y' OR [BillOnCompleteYN]='N'))
GO
ALTER TABLE [dbo].[bJBIN] WITH NOCHECK ADD CONSTRAINT [CK_bJBIN_OverrideGLRevAcctYN] CHECK (([OverrideGLRevAcctYN]='Y' OR [OverrideGLRevAcctYN]='N'))
GO
ALTER TABLE [dbo].[bJBIN] WITH NOCHECK ADD CONSTRAINT [CK_bJBIN_Purge] CHECK (([Purge]='Y' OR [Purge]='N'))
GO
ALTER TABLE [dbo].[bJBIN] WITH NOCHECK ADD CONSTRAINT [CK_bJBIN_RestrictBillGroupYN] CHECK (([RestrictBillGroupYN]='Y' OR [RestrictBillGroupYN]='N'))
GO
ALTER TABLE [dbo].[bJBIN] WITH NOCHECK ADD CONSTRAINT [CK_bJBIN_RevRelRetgYN] CHECK (([RevRelRetgYN]='Y' OR [RevRelRetgYN]='N'))
GO
ALTER TABLE [dbo].[bJBIN] WITH NOCHECK ADD CONSTRAINT [CK_bJBIN_TMUpdateAddonYN] CHECK (([TMUpdateAddonYN]='Y' OR [TMUpdateAddonYN]='N'))
GO
ALTER TABLE [dbo].[bJBIN] ADD CONSTRAINT [PK_bJBIN] PRIMARY KEY NONCLUSTERED  ([KeyID]) WITH (FILLFACTOR=100) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_bJBIN_Contract] ON [dbo].[bJBIN] ([Contract], [BillMonth], [BillNumber], [JBCo]) INCLUDE ([InvStatus]) WITH (FILLFACTOR=70) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biJBIN] ON [dbo].[bJBIN] ([JBCo], [BillMonth], [BillNumber]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biJBINInvoice] ON [dbo].[bJBIN] ([JBCo], [Invoice]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

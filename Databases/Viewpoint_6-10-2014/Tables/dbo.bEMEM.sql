...
[Type] [char] (1) COLLATE Latin1_General_BIN NOT NULL CONSTRAINT [DF_bEMEM_Type] DEFAULT ('E'),
...
[Status] [char] (1) COLLATE Latin1_General_BIN NOT NULL CONSTRAINT [DF_bEMEM_Status] DEFAULT ('A'),
...
[FuelType] [char] (1) COLLATE Latin1_General_BIN NOT NULL CONSTRAINT [DF_bEMEM_FuelType] DEFAULT ('D'),
...
[udFCPin] [varchar] (12) COLLATE Latin1_General_BIN NULL
...
*			GF 01/08/2013 TK-20666 change to use exists from is null to handle more than one row
*			GF 02/11/2013 TFS-40313 validate location is active there is one
*			GF 05/05/2013 TFS-49039
*	
*--------------------------------------------------------------*/

declare @numrows int, @errmsg varchar(255),
		@validcnt int, @nullcnt int, @msg varchar(255),
...
IF EXISTS(select top 1 1  from inserted where isnull(Equipment,'')!= isnull(OriginalEquipmentCode,''))
	...
	---- TFS-40313 validate location is active there is one
	IF EXISTS(SELECT 1 FROM dbo.bEMLM l WITH (NOLOCK) JOIN inserted i ON i.EMCo = l.EMCo AND i.Location = l.EMLoc
				WHERE l.Active = 'N' AND i.Location IS NOT NULL)
		BEGIN
		SET @errmsg = 'Location is inactive '
		GOTO error
		END
...
-- Author:		Eric Shafer
-- Create date: 9/23/13
-- Description:	Update EM Driver History
-- =============================================
CREATE TRIGGER [dbo].[mcktrEMHisAndPREHSync] 
   ON  [dbo].[bEMEM] 
   AFTER INSERT, UPDATE
AS 
BEGIN
	...
	DECLARE @Operator bEmployee = NULL, @ErrSeverity INT = 0
	SELECT @Operator = Operator FROM INSERTED

	DECLARE @PRCo INT, @Employee INT, @EMCo INT, @Equipment bEquip
		SELECT @PRCo = PRCo, @Employee = @Operator, @EMCo = EMCo, @Equipment = Equipment FROM INSERTED

	DECLARE @DhistoryCount INT
		SELECT @DhistoryCount = COUNT(*)
			FROM dbo.udDriverHistory d WHERE d.Co = @EMCo AND d.Equipment = @Equipment
			SET @DhistoryCount = @DhistoryCount + 1
		--UPDATE Driver/Operator History table
	IF UPDATE(Operator) AND @Operator IS NOT NULL
	BEGIN
		
		INSERT INTO dbo.budDriverHistory
				( Co ,
				  Date ,
				  Equipment ,
				  DriverCo,
	...
	END
	ELSE
	IF @Operator IS NULL AND UPDATE(Operator) AND @DhistoryCount <> 1
	BEGIN
		INSERT INTO dbo.budDriverHistory
				( Co ,
				  Date ,
			...
			(SELECT 
				...
				FROM INSERTED i)
	END
		--END UPDATE Driver/Operator History table
	
	
	--UPDATE METER HISTORY
	DECLARE @HourReading bUnits, @FuelUsed bUnits, @OdoReading bUnits
	SELECT @HourReading = HourReading, @FuelUsed = FuelUsed, @OdoReading = OdoReading FROM INSERTED

	IF (UPDATE(HourReading) OR UPDATE (FuelUsed) OR UPDATE(OdoReading)) AND (@HourReading <> 0 OR @FuelUsed <> 0 OR @OdoReading<>0)
		BEGIN
			DECLARE @SequenceCnt INT

			SET @SequenceCnt = (SELECT COUNT(m.Equipment) AS SequenceCnt FROM dbo.budMeterHis m INNER JOIN INSERTED i ON m.Co = i.EMCo AND m.Equipment = i.Equipment)


			INSERT INTO dbo.budMeterHis
			        ( Co ,
			          Date ,
			          Equipment ,
			          FuelDate ,
			...
			VALUES  ( (SELECT i.EMCo FROM INSERTED i) , -- Co - bCompany
			          CURRENT_TIMESTAMP , -- Date - bDate
			          (SELECT i.Equipment FROM INSERTED i) , -- Equipment - bEquip
			          (SELECT i.LastFuelDate FROM INSERTED i) , -- FuelDate - bDate
			          (SELECT i.FuelUsed FROM INSERTED i) , -- FuelUsed - bUnits
			          (SELECT i.HourDate FROM INSERTED i) , -- HourDate - bDate
			          (SELECT i.HourReading FROM INSERTED i) , -- HourRead - bUnits
			          (SELECT i.OdoDate FROM INSERTED i) , -- OdometerDate - bDate
			          (SELECT i.OdoReading FROM INSERTED i) , -- OdometerRead - bUnits
			          @SequenceCnt  -- Seq - int
			        )
		END
	...
	--UPDATE EMEM table.  Set unique EMEM.Operator
		

		--SET @msg = ''

	DECLARE @EMCoParamYN VARCHAR(1)
	SELECT @EMCoParamYN =  udEqOpRatioLockYN FROM EMCO c WHERE @EMCo = c.EMCo

	IF @Employee IS NOT NULL AND @EMCoParamYN = 'Y' AND EXISTS (SELECT Equipment FROM EMEM 
		WHERE PRCo = @PRCo AND Operator = @Employee AND (EMCo = @EMCo AND Equipment <> @Equipment))
	BEGIN
	--Check and remove existing operator assignments. 
		
		DECLARE @MatchingOperatedEquip VARCHAR(255)
		DECLARE @msg VARCHAR(255)
		SELECT @MatchingOperatedEquip = COALESCE(Equipment + ', ', '')
				FROM EMEM
				WHERE PRCo = @PRCo AND Operator = @Employee AND (EMCo = @EMCo AND Equipment <> @Equipment)
		SELECT @msg = @MatchingOperatedEquip + 'used the same operator and has been removed'
		UPDATE dbo.EMEM
			SET
			--PRCo = NULL,
			Operator = NULL
			WHERE PRCo = @PRCo AND Operator = @Employee AND
			(EMCo = @EMCo AND @Equipment <> Equipment)
		SET @ErrSeverity = 11
		GOTO bsperror
		
	END

	--END Check and remove existing operator assignments. 

END
bsperror: 
--select @msg
RAISERROR(@msg, @ErrSeverity, 0);

GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_AttachPostRevenue] CHECK (([AttachPostRevenue]='N' OR [AttachPostRevenue]='Y' OR [AttachPostRevenue] IS NULL))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_Capitalized] CHECK (([Capitalized]='N' OR [Capitalized]='Y' OR [Capitalized] IS NULL))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_ChangeInProgress] CHECK (([ChangeInProgress]='N' OR [ChangeInProgress]='Y' OR [ChangeInProgress] IS NULL))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_CompUpdateFuel] CHECK (([CompUpdateFuel]='N' OR [CompUpdateFuel]='Y' OR [CompUpdateFuel] IS NULL))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_CompUpdateHrs] CHECK (([CompUpdateHrs]='N' OR [CompUpdateHrs]='Y' OR [CompUpdateHrs] IS NULL))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_CompUpdateMiles] CHECK (([CompUpdateMiles]='N' OR [CompUpdateMiles]='Y' OR [CompUpdateMiles] IS NULL))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_FuelType] CHECK (([FuelType]='D' OR [FuelType]='G' OR [FuelType]='N' OR [FuelType]='O'))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_OwnershipStatus] CHECK (([OwnershipStatus]='O' OR [OwnershipStatus]='R' OR [OwnershipStatus]='C' OR [OwnershipStatus]='L' OR [OwnershipStatus] IS NULL))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_PostCostToComp] CHECK (([PostCostToComp]='N' OR [PostCostToComp]='Y' OR [PostCostToComp] IS NULL))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_Status] CHECK (([Status]='A' OR [Status]='I' OR [Status]='D'))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_Type] CHECK (([Type]='E' OR [Type]='C'))
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [CK_bEMEM_UpdateYN] CHECK (([UpdateYN]='N' OR [UpdateYN]='Y'))
GO
...
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [FK_bEMEM_bEMCO_EMCo] FOREIGN KEY ([EMCo]) REFERENCES [dbo].[bEMCO] ([EMCo])
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [FK_bEMEM_bEMCM_Category] FOREIGN KEY ([EMCo], [Category]) REFERENCES [dbo].[bEMCM] ([EMCo], [Category])
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [FK_bEMEM_bEMDM_Department] FOREIGN KEY ([EMCo], [Department]) REFERENCES [dbo].[bEMDM] ([EMCo], [Department])
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [FK_bEMEM_bHQGP_EMGroup] FOREIGN KEY ([EMGroup]) REFERENCES [dbo].[bHQGP] ([Grp])
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [FK_bEMEM_bEMTY_ComponentTypeCode] FOREIGN KEY ([EMGroup], [ComponentTypeCode]) REFERENCES [dbo].[bEMTY] ([EMGroup], [ComponentTypeCode])
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [FK_bEMEM_bEMRC_RevenueCode] FOREIGN KEY ([EMGroup], [RevenueCode]) REFERENCES [dbo].[bEMRC] ([EMGroup], [RevCode])
GO
ALTER TABLE [dbo].[bEMEM] WITH NOCHECK ADD CONSTRAINT [FK_bEMEM_bHQGP_ShopGroup] FOREIGN KEY ([ShopGroup]) REFERENCES [dbo].[bHQGP] ([Grp])
GO
ALTER TABLE [dbo].[bEMEM] NOCHECK CONSTRAINT [FK_bEMEM_bEMCO_EMCo]
GO
ALTER TABLE [dbo].[bEMEM] NOCHECK CONSTRAINT [FK_bEMEM_bEMCM_Category]
GO
ALTER TABLE [dbo].[bEMEM] NOCHECK CONSTRAINT [FK_bEMEM_bEMDM_Department]
GO
ALTER TABLE [dbo].[bEMEM] NOCHECK CONSTRAINT [FK_bEMEM_bHQGP_EMGroup]
GO
ALTER TABLE [dbo].[bEMEM] NOCHECK CONSTRAINT [FK_bEMEM_bEMTY_ComponentTypeCode]
GO
ALTER TABLE [dbo].[bEMEM] NOCHECK CONSTRAINT [FK_bEMEM_bEMRC_RevenueCode]
GO
ALTER TABLE [dbo].[bEMEM] NOCHECK CONSTRAINT [FK_bEMEM_bHQGP_ShopGroup]
GO

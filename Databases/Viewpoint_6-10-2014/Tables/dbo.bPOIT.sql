...
GO
ALTER TABLE [dbo].[bPOIT] WITH NOCHECK ADD CONSTRAINT [CK_bPOIT_CurECM] CHECK (([CurECM]='E' OR [CurECM]='C' OR [CurECM]='M' OR [CurECM] IS NULL))
GO
ALTER TABLE [dbo].[bPOIT] WITH NOCHECK ADD CONSTRAINT [CK_bPOIT_OrigECM] CHECK (([OrigECM]='E' OR [OrigECM]='C' OR [OrigECM]='M' OR [OrigECM] IS NULL))
GO
ALTER TABLE [dbo].[bPOIT] WITH NOCHECK ADD CONSTRAINT [CK_bPOIT_RecvYN] CHECK (([RecvYN]='Y' OR [RecvYN]='N'))
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPOIT] ([KeyID]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPOIT] ON [dbo].[bPOIT] ([POCo], [PO], [POItem]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
...
GO
ALTER TABLE [dbo].[bPOIT] NOCHECK CONSTRAINT [FK_bPOIT_vSMWorkOrderScope]
GO

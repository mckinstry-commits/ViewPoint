...
GO
ALTER TABLE [dbo].[bHRWI] WITH NOCHECK ADD CONSTRAINT [CK_bHRWI_OverrideMiscAmtYN] CHECK (([OverrideMiscAmtYN]='Y' OR [OverrideMiscAmtYN]='N'))
GO
ALTER TABLE [dbo].[bHRWI] ADD CONSTRAINT [PK_bHRWI_KeyID] PRIMARY KEY NONCLUSTERED  ([KeyID]) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biHRWI] ON [dbo].[bHRWI] ([HRCo], [HRRef], [DednCode]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bHRWI] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

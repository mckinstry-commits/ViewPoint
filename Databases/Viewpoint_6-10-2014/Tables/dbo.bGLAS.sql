...
GO
ALTER TABLE [dbo].[bGLAS] WITH NOCHECK ADD CONSTRAINT [CK_bGLAS_Adjust] CHECK (([Adjust]='Y' OR [Adjust]='N' OR [Adjust] IS NULL))
GO
ALTER TABLE [dbo].[bGLAS] WITH NOCHECK ADD CONSTRAINT [CK_bGLAS_Purge] CHECK (([Purge]='Y' OR [Purge]='N'))
GO
CREATE UNIQUE CLUSTERED INDEX [biGLAS] ON [dbo].[bGLAS] ([GLCo], [GLAcct], [Mth], [Jrnl], [GLRef], [SourceCo], [Source]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [viGLAS1] ON [dbo].[bGLAS] ([GLCo], [Mth], [Jrnl], [GLRef]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_bGLAS_Mth] ON [dbo].[bGLAS] ([Mth]) INCLUDE ([GLAcct], [GLCo], [NetAmt]) WITH (FILLFACTOR=85, STATISTICS_NORECOMPUTE=ON) ON [PRIMARY]
GO

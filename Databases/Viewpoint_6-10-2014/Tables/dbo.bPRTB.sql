...
[DayNum] [smallint] NOT NULL CONSTRAINT [DF_bPRTB_DayNum] DEFAULT ((0)),
...
[Rate] [dbo].[bUnitCost] NOT NULL CONSTRAINT [DF_bPRTB_Rate] DEFAULT ((0)),
    ...
    if @numrows = 0 return
      ...
      BEGIN
			...
			FROM dbo.SMWorkCompleted
				LEFT JOIN dbo.vSMInvoiceDetail ON SMWorkCompleted.SMCo = vSMInvoiceDetail.SMCo AND SMWorkCompleted.WorkOrder = vSMInvoiceDetail.WorkOrder AND SMWorkCompleted.WorkCompleted = vSMInvoiceDetail.WorkCompleted
            WHERE vSMInvoiceDetail.SMInvoiceDetailID IS NULL AND 
				SMWorkCompletedID IN (
					SELECT vSMBC.SMWorkCompletedID
					FROM dbo.vSMBC
...
IF (@PrintDebug=1) PRINT 'btPRTBd 1: Deleting SMWorkCompleted'
            ...
            SELECT SMWorkCompletedID, [Status]
			FROM dbo.SMWorkCompleted
				INNER JOIN dbo.vSMInvoiceDetail ON SMWorkCompleted.SMCo = vSMInvoiceDetail.SMCo AND SMWorkCompleted.WorkOrder = vSMInvoiceDetail.WorkOrder AND SMWorkCompleted.WorkCompleted = vSMInvoiceDetail.WorkCompleted
            WHERE 
				SMWorkCompletedID IN
				(
					SELECT vSMBC.SMWorkCompletedID
...
-- Description:	Trigger to update Type 'J' records with Equipment values
-- =============================================
CREATE TRIGGER [dbo].[mtr_PRTB_I] 
   ON  [dbo].[bPRTB] 
   AFTER INSERT
AS 
BEGIN
	...
	DECLARE @Co bCompany, @BatchId bBatchID, @Mth bMonth, @BatchSeq INT
		, @Equipment bEquip, @Employee bEmployee, @Hours bHrs, @EMCo bCompany,@JCPhaseGroup bGroup,@JCCostType bEMCType
		, @EMGroup bGroup, @RevCode bRevCode
    -- Insert statements for trigger here
	
	IF NOT EXISTS(SELECT TOP 1 1 FROM INSERTED WHERE Type='J')
	GOTO exitall
	
	DECLARE PRTB_I_crsr CURSOR FOR
	SELECT Co, BatchId, Mth,BatchSeq, Employee, Hours
	FROM INSERTED
	WHERE Type='J' 

	OPEN PRTB_I_crsr
	FETCH NEXT FROM PRTB_I_crsr INTO @Co , @BatchId , @Mth , @BatchSeq, @Employee, @Hours

	WHILE @@FETCH_STATUS=0
	BEGIN
		IF(SELECT udEqOpRatioLockYN 
		FROM EMCO ec 
		INNER JOIN PRCO pc ON pc.EMCo = ec.EMCo
		WHERE pc.PRCo=@Co)<>'Y'
		GOTO spexit

		--SELECT @EMCo=EMCo FROM dbo.PRCO
		--WHERE PRCo=@Co

		SELECT @JCPhaseGroup = PhaseGroup--, @EMGroup=EMGroup
			FROM HQCO 
			WHERE HQCo = @Co

		SELECT @JCCostType = CostType
			FROM dbo.JCCT
			WHERE PhaseGroup = @JCPhaseGroup AND JBCostTypeCategory = 'E'


		IF (SELECT COUNT(*) FROM EMEM WHERE PRCo=@Co AND Operator = @Employee)<>1
		BEGIN
			GOTO NEXT
		END

		SELECT @EMCo=EMCo, @Equipment = Equipment, @RevCode = RevenueCode, @EMGroup = EMGroup
		FROM dbo.EMEM 
		WHERE PRCo = @Co AND Operator = @Employee
		
		IF(SELECT ISNULL(Rate,0) FROM dbo.vfEMEquipmentRevCodeSetup(@EMCo,@Equipment,@EMGroup,@RevCode))=0
		GOTO NEXT		
		--SELECT * FROM dbo.PRTB
		UPDATE dbo.bPRTB
		SET EMCo= @EMCo
			, Equipment = @Equipment, EMGroup = @EMGroup
			, EquipPhase = '0100-0240-      -', EquipCType = @JCCostType, UsageUnits=@Hours, RevCode=@RevCode
...
BEGIN
	...
	DECLARE @Co bCompany, @BatchId bBatchID, @Mth bMonth, @BatchSeq INT
		, @Equipment bEquip, @Employee bEmployee, @OldEmployee bEmployee, @Hours bHrs, @EMCo bCompany,@JCPhaseGroup bGroup,@JCCostType bEMCType
		, @EMGroup bGroup, @RevCode bRevCode, @Rate bRate
    -- Insert statements for trigger here
	...
	DECLARE PRTB_I_crsr CURSOR FOR
	SELECT i.Co, i.BatchId, i.Mth,i.BatchSeq, i.Employee, i.Hours, d.Employee
	FROM INSERTED i
	LEFT OUTER JOIN DELETED d ON d.KeyID=i.KeyID
	WHERE i.Type='J' AND d.Type=i.Type 

	OPEN PRTB_I_crsr
	FETCH NEXT FROM PRTB_I_crsr INTO @Co , @BatchId , @Mth , @BatchSeq, @Employee, @Hours, @OldEmployee
	...
	BEGIN
		...
		--Validation
		SELECT @Rate=ISNULL(Rate,0) FROM dbo.vfEMEquipmentRevCodeSetup(@EMCo,@Equipment,@EMGroup,@RevCode)
		--SELECT * FROM vfEMEquipmentRevCodeSetup(101,1,101,1)
		IF @Rate=0 OR @Equipment IS NULL
		BEGIN
			UPDATE dbo.bPRTB
			SET Equipment=NULL, UsageUnits=0,EquipPhase=NULL, EquipCType = NULL
			WHERE Co=@Co AND BatchId=@BatchId AND BatchSeq=@BatchSeq AND Mth=@Mth
			GOTO NEXT		
		END

		
		UPDATE dbo.bPRTB
		SET EMCo= @EMCo, EMGroup=@EMGroup, Equipment = @Equipment, EquipPhase = '0100-0240-      -   ', EquipCType = @JCCostType, UsageUnits=@Hours, RevCode=@RevCode
		WHERE BatchId=@BatchId AND BatchSeq=@BatchSeq AND Mth=@Mth AND Co=@Co
...
END
GO
ALTER TABLE [dbo].[bPRTB] WITH NOCHECK ADD CONSTRAINT [CK_bPRTB_Cert] CHECK (([Cert]='Y' OR [Cert]='N'))
GO
ALTER TABLE [dbo].[bPRTB] WITH NOCHECK ADD CONSTRAINT [CK_bPRTB_OldCert] CHECK (([OldCert]='Y' OR [OldCert]='N' OR [OldCert] IS NULL))
GO

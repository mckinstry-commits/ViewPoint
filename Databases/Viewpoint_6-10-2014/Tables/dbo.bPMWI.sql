...
GO
ALTER TABLE [dbo].[bPMWI] ADD CONSTRAINT [PK_bPMWI] PRIMARY KEY CLUSTERED  ([PMCo], [ImportId], [Sequence]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMWI] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
ALTER TABLE [dbo].[bPMWI] WITH NOCHECK ADD CONSTRAINT [FK_bPMWI_bPMWH] FOREIGN KEY ([PMCo], [ImportId]) REFERENCES [dbo].[bPMWH] ([PMCo], [ImportId]) ON DELETE CASCADE
GO
ALTER TABLE [dbo].[bPMWI] NOCHECK CONSTRAINT [FK_bPMWI_bPMWH]
GO

...
GO
ALTER TABLE [dbo].[bAPVC] WITH NOCHECK ADD CONSTRAINT [CK_bAPVC_Complied] CHECK (([Complied]='Y' OR [Complied]='N' OR [Complied] IS NULL))
GO
ALTER TABLE [dbo].[bAPVC] WITH NOCHECK ADD CONSTRAINT [CK_bAPVC_Verify] CHECK (([Verify]='Y' OR [Verify]='N'))
GO
CREATE UNIQUE CLUSTERED INDEX [biAPVC] ON [dbo].[bAPVC] ([APCo], [VendorGroup], [Vendor], [CompCode]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bAPVC] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

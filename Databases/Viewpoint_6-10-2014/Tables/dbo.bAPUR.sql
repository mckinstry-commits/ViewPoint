...
[ApprvdYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bAPUR_ApprvdYN] DEFAULT ('N'),
...
GO
ALTER TABLE [dbo].[bAPUR] WITH NOCHECK ADD CONSTRAINT [CK_bAPUR_ApprvdYN] CHECK (([ApprvdYN]='Y' OR [ApprvdYN]='N'))
GO
ALTER TABLE [dbo].[bAPUR] WITH NOCHECK ADD CONSTRAINT [CK_bAPUR_Rejected] CHECK (([Rejected]='Y' OR [Rejected]='N' OR [Rejected] IS NULL))
GO
CREATE NONCLUSTERED INDEX [IX_bAPUR_APCoReviewerLine] ON [dbo].[bAPUR] ([APCo], [Reviewer], [Line]) INCLUDE ([KeyID], [UIMth], [UISeq]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biAPUR] ON [dbo].[bAPUR] ([APCo], [UIMth], [UISeq], [Reviewer], [Line]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bAPUR] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO

...
[StartMonth] [dbo].[bMonth] NOT NULL CONSTRAINT [DF_bJCCM_StartMonth] DEFAULT (stuff(CONVERT([varchar](8),getdate(),(1)),(4),(2),'01')),
...
[CompleteYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bJCCM_CompleteYN] DEFAULT ('N'),
[RoundOpt] [char] (1) COLLATE Latin1_General_BIN NOT NULL CONSTRAINT [DF_bJCCM_RoundOpt] DEFAULT ('N'),
[ReportRetgItemYN] [dbo].[bYN] NOT NULL CONSTRAINT [DF_bJCCM_ReportRetgItemYN] DEFAULT ('N'),
...
[udCGCJobNum] [varchar] (20) COLLATE Latin1_General_BIN NULL,
[udPOC] [dbo].[bProjectMgr] NULL,
[udBillDelMethod] [varchar] (3) COLLATE Latin1_General_BIN NULL,
[udBillEmail] [varchar] (128) COLLATE Latin1_General_BIN NULL,
[udBOClass] [varchar] (2) COLLATE Latin1_General_BIN NULL,
[udRevType] [varchar] (1) COLLATE Latin1_General_BIN NULL
...
-- Description:	Prevent duplicate job codes across companies.
-- =============================================
CREATE TRIGGER [dbo].[mckJCCMDuplicateCodes] 
   ON  [dbo].[bJCCM] 
   AFTER INSERT
AS 
BEGIN
	...
	DECLARE @IJCCo TINYINT, @IContract bContract, @ITestCo CHAR(1), @Error NVARCHAR(MAX)

	SELECT @IJCCo = JCCo, @IContract = Contract FROM INSERTED
	SELECT @ITestCo = udTESTCo FROM HQCO WHERE @IJCCo = HQCo

	IF EXISTS(
		SELECT TOP 1 1 
			FROM JCCM
				INNER JOIN HQCO ON HQCO.HQCo = JCCM.JCCo
			WHERE @IContract = JCCM.Contract AND @IJCCo <> HQCo
				AND @ITestCo = HQCO.udTESTCo
			)
		OR EXISTS(
			SELECT TOP 1 1 
			FROM JCCM
				INNER JOIN HQCO ON HQCO.HQCo = JCCM.JCCo
			WHERE LEFT(@IContract,6) = LEFT(JCCM.Contract,6) AND @IJCCo <> HQCo
				AND @ITestCo = HQCO.udTESTCo
		)
		BEGIN		
		SELECT @Error = 'This contract number: "' + LEFT(@IContract,7) + '" has already been utilized.  Please use a different number'
...
-- =============================================
CREATE TRIGGER [dbo].[mcktrInterfaceMail] 
   ON  [dbo].[bJCCM] 
   AFTER UPDATE
AS 
BEGIN
	...
	BEGIN
		DECLARE @JCCo TINYINT, @Contract VARCHAR(30), @POC INT, @POCEmail VARCHAR(60), @EmailBody VARCHAR(8000), @EmailSubject VARCHAR(255)
		...
		SET @POCEmail = 'erptest@mckinstry.com'  --COMMENT AFTER GO LIVE
...
GO
ALTER TABLE [dbo].[bJCCM] WITH NOCHECK ADD CONSTRAINT [CK_bJCCM_BillOnCompletionYN] CHECK (([BillOnCompletionYN]='Y' OR [BillOnCompletionYN]='N'))
GO
ALTER TABLE [dbo].[bJCCM] WITH NOCHECK ADD CONSTRAINT [CK_bJCCM_ClosePurgeFlag] CHECK (([ClosePurgeFlag]='Y' OR [ClosePurgeFlag]='N' OR [ClosePurgeFlag] IS NULL))
GO
ALTER TABLE [dbo].[bJCCM] WITH NOCHECK ADD CONSTRAINT [CK_bJCCM_CompleteYN] CHECK (([CompleteYN]='Y' OR [CompleteYN]='N'))
GO
ALTER TABLE [dbo].[bJCCM] WITH NOCHECK ADD CONSTRAINT [CK_bJCCM_DefaultBillType] CHECK (([DefaultBillType]='T' OR [DefaultBillType]='P' OR [DefaultBillType]='B' OR [DefaultBillType]='N'))
GO
ALTER TABLE [dbo].[bJCCM] WITH NOCHECK ADD CONSTRAINT [CK_bJCCM_ReportRetgItemYN] CHECK (([ReportRetgItemYN]='Y' OR [ReportRetgItemYN]='N'))
GO
ALTER TABLE [dbo].[bJCCM] WITH NOCHECK ADD CONSTRAINT [CK_bJCCM_SIMetric] CHECK (([SIMetric]='Y' OR [SIMetric]='N' OR [SIMetric] IS NULL))
GO
ALTER TABLE [dbo].[bJCCM] WITH NOCHECK ADD CONSTRAINT [CK_bJCCM_TaxInterface] CHECK (([TaxInterface]='Y' OR [TaxInterface]='N'))
GO

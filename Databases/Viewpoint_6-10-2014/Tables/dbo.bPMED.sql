...
GO
ALTER TABLE [dbo].[bPMED] ADD CONSTRAINT [PK_bPMED] PRIMARY KEY CLUSTERED  ([PMCo], [Project], [BudgetNo], [Seq]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMED] ([KeyID]) WITH (FILLFACTOR=90) ON [PRIMARY]
GO
ALTER TABLE [dbo].[bPMED] WITH NOCHECK ADD CONSTRAINT [FK_bPMED_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job])
GO
ALTER TABLE [dbo].[bPMED] WITH NOCHECK ADD CONSTRAINT [FK_bPMED_bPMEH] FOREIGN KEY ([PMCo], [Project], [BudgetNo]) REFERENCES [dbo].[bPMEH] ([PMCo], [Project], [BudgetNo]) ON DELETE CASCADE
GO
ALTER TABLE [dbo].[bPMED] NOCHECK CONSTRAINT [FK_bPMED_bJCJM]
GO
ALTER TABLE [dbo].[bPMED] NOCHECK CONSTRAINT [FK_bPMED_bPMEH]
GO

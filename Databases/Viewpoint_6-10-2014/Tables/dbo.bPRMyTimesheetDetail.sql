...
IF (@PrintDebug=1) PRINT 'btPRMyTimesheetDetaild 1: Collect SMWorkCompletedID'
		...
		WHERE vSMInvoiceDetail.SMInvoiceDetailID IS NULL AND vSMWorkCompleted.SMWorkCompletedID IN (
...
IF (@PrintDebug=1) PRINT 'btPRMyTimesheetDetaild 4: Delete SMMyTimesheetLink'
		...
		(
			...
			WHERE
				vSMWorkCompleted.SMWorkCompletedID IN
				(
...
IF (@PrintDebug=1) PRINT 'btPRMyTimesheetDetaild 5: Collect Invoiced SMWorkCompletedID'
			...
			FROM dbo.vSMWorkCompleted
				INNER JOIN dbo.vSMInvoiceDetail ON vSMWorkCompleted.SMCo = vSMInvoiceDetail.SMCo AND vSMWorkCompleted.WorkOrder = vSMInvoiceDetail.WorkOrder AND vSMWorkCompleted.WorkCompleted = vSMInvoiceDetail.WorkCompleted 
			WHERE vSMWorkCompleted.SMWorkCompletedID IN
			(
				SELECT SMWorkCompleted.SMWorkCompletedID
				FROM deleted 
					INNER JOIN vSMMyTimesheetLink
...
END
GO
ALTER TABLE [dbo].[bPRMyTimesheetDetail] WITH NOCHECK ADD CONSTRAINT [CK_bPRMyTimesheetDetail_RequiredJobPhase] CHECK (([dbo].[vfEqualsNull]([Phase])=(1) OR [dbo].[vfEqualsNull]([Job])=(0) AND [dbo].[vfEqualsNull]([PhaseGroup])=(0)))
GO
...
GO
ALTER TABLE [dbo].[bPRMyTimesheetDetail] NOCHECK CONSTRAINT [FK_bPRMyTimesheetDetail_vSMPayType]
GO
ALTER TABLE [dbo].[bPRMyTimesheetDetail] NOCHECK CONSTRAINT [FK_bPRMyTimesheetDetail_vSMWorkOrderScope]
GO

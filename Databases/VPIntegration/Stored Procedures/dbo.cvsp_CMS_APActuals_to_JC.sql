...
select JCCo=a.APCo
	...
	, udConv='Y'
from bAPTL a 
	join bAPTH h on a.APCo=h.APCo and a.Mth=h.Mth and a.APTrans=h.APTrans
	left join HQTC t on a.APCo=t.Co and a.Mth=t.Mth and t.TableName='bJCCD'
	left join JCCH j on a.APCo=j.JCCo and a.Job=j.Job and a.Phase=j.Phase and a.JCCType=j.CostType
	left join SLIT s on a.APCo=s.SLCo and a.SL=s.SL and a.SLItem=s.SLItem
where a.APCo=@toco

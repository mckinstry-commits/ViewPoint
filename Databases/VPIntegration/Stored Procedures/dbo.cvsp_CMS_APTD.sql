...
select APCo=@toco
	,Mth=isnull(substring(convert(nvarchar(max),h.JOURNALDATE),5,2) + '/01/' +  substring(convert(nvarchar(max),h.JOURNALDATE),1,4),
		substring(convert(nvarchar(max),l.JOURNALDATE),5,2) + '/01/' +  substring(convert(nvarchar(max),l.JOURNALDATE),1,4))
	,APTrans=convert(nvarchar(max),l.PAYMENTSELNO) + convert(nvarchar(max),l.RECORDCODE) 

...
BEGIN tran
delete from bGLYB where GLCo=@toco
COMMIT TRAN;
...
select GLCo=@toco
	...
	, BeginBal=min(g.BALFORWARDFIS)
	, udSource='GLYB'
	, udConv='Y'
	, udCGCTable='GLTACT'
	,udCGCTableID=min(g.GLTACTID)
from CV_CMS_SOURCE.dbo.GLTACT g with (nolock)
	join Viewpoint.dbo.budxrefGLAcct on g.COMPANYNUMBER = Company and (g.GENLEDGERACCT=oldGLAcct)

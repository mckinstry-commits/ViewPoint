...
*   1   fail
*****************************************************/
(@PRCo bCompany, @PRGroup bGroup, @PREndDate bDate, @Employee bEmployee, @PaySequence tinyint,
    	 @LoopBack char(1), @PaybackRecalculate bYN OUTPUT, @errmsg varchar(255) OUTPUT)
	...
	-- we don't want to proceed with arrears processing if employee is not active
	IF @NetPay < 0 AND NOT EXISTS(SELECT TOP 1 1 FROM bPREH WHERE PRCo = @PRCo AND Employee = @Employee AND ArrearsActiveYN = 'Y')
		BEGIN
		RETURN 0
		END
...
DecidePaybackArrears:		
	...
	IF (@NetPay > 0 AND  NOT EXISTS(SELECT TOP 1 1
										FROM bPRDL l
											JOIN bPRED e ON e.PRCo = l.PRCo AND e.DLCode = l.DLCode
	...
	-------------------------------------------------
	-- If NetPay >= 0 - Begin looking into paybacks -- 
	-------------------------------------------------
	IF @NetPay >= 0
		BEGIN
...
----------------------------------------------------
		...
		IF @PaybackRecalculate = 'Y'
			...
			--------------------------------------------------------------------------
			UPDATE bPRDT 
			   SET SubjectAmt = 0, EligibleAmt = 0, Amount = 0
			 WHERE PRCo = @PRCo 
			   AND PRGroup = @PRGroup 
			   AND PREndDate = @PREndDate 
			   AND Employee = @Employee 

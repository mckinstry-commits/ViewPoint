...
@stddays tinyint, @calcbasis bDollar output, @accumbasis bDollar output, @liabbasis bDollar output,
...
IF @method IN ('G', 'R')
	...
	IF @calccategory in ('S','L','I')	-- State or Local or Insurance
		...
		next_PreTaxCode:
			...
			SELECT @AccumPretax = CASE WHEN e.BasisAmt = 0 THEN @AccumPretax
					ELSE @AccumPretax + (e.DednAmt * (@PreTaxBasis / e.BasisAmt)) END

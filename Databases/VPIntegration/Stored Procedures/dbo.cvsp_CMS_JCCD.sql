...
CREATE procedure  [dbo].[cvsp_CMS_JCCD] (@fromco smallint, @toco smallint, 
...
select JCCo=@toco
	...
	, CostTrans = isnull(t.LastTrans,1) + row_number() over (partition by COMPANYNUMBER order by COMPANYNUMBER)
...
from CV_CMS_SOURCE.dbo.JCTPST j
	...
	join Viewpoint.dbo.budxrefPhase p on p.Company=@fromco and p.oldPhase=j.JCDISTRIBTUION
	join Viewpoint.dbo.budxrefCostType c on c.Company=@fromco and c.CMSCostType=j.COSTTYPE

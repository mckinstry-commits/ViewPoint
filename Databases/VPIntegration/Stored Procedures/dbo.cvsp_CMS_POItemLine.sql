...
insert vPOItemLine
	(POITKeyID, POCo, PO, POItem, POItemLine, ItemType, PostToCo, JCCo, Job, PhaseGroup, Phase, JCCType,
		...
		PostedDate, JCMonth, PurgeYN, LineDelete, udCGC_ASQ02)
select
	  POITKeyID = it.KeyID
	, POCo=@toco
	, PO=d.PONUMBER
	...
	, Job=case 
			...
			else dbo.bfMuliPartFormat(RTRIM(d.JOBNUMBER) + '.' + RTRIM(d.SUBJOBNUMBER),@Job) end
...
join bPOIT it
	on it.PO=d.PONUMBER and it.POItem=d.POITEMT and it.POCo=@toco
left join Viewpoint.dbo.budxrefPhase xp
	on xp.Company=d.COMPANYNUMBER and d.JCDISTRIBTUION=xp.oldPhase
left join Viewpoint.dbo.budxrefCostType xt 
	on xt.Company=@fromco and d.COSTTYPE=xt.CMSCostType
...
left join Viewpoint.dbo.budxrefGLAcct xg 
	on xg.Company=d.COMPANYNUMBER and xg.oldGLAcct=d.GENLEDGERACCT

...
from (select JCCo=@toco
		...
		, UM=case when BQ<>0 and max(j.UNITOFMEASURE)<>'' then MAX(isnull(j.UNITOFMEASURE, @DefaultUMUnits))
			else case when max(j.UNITOFMEASURE)<>'' then max(isnull(j.UNITOFMEASURE, @DefaultUM))else @DefaultUM end end
		, OrigHours=sum(BH)
		, OrigUnits=sum(BQ)
		, OrigCost=sum(BA)
		,udCGCTable='JCTMST'
	...
	from CV_CMS_SOURCE.dbo.JCTMST j
		join Viewpoint.dbo.budxrefPhase on xrefPhase.Company=@fromco and xrefPhase.oldPhase=JCDISTRIBTUION
		...
		join (select CN=COMPANYNUMBER, JN=JOBNUMBER,SJ=SUBJOBNUMBER, JD=JCDISTRIBTUION, 
				...
				as x 
				on CN=j.COMPANYNUMBER and JN=JOBNUMBER and SJ=SUBJOBNUMBER and JD=JCDISTRIBTUION and CT=COSTTYPE
		left join (select COMPANYNUMBER, JOBNUMBER, SUBJOBNUMBER, JOBSTATUS=max(JOBSTATUS)
					...
					as o
					on o.COMPANYNUMBER=j.COMPANYNUMBER and o.JOBNUMBER=j.JOBNUMBER 
	...
	group by j.COMPANYNUMBER
		, dbo.bfMuliPartFormat(ltrim(rtrim(j.JOBNUMBER)) +  ltrim(RTRIM(j.SUBJOBNUMBER)),@JobFormat)
		, newPhase 
		, CostType
		, UM,BH,BQ,BA ) 
	as JCCHInfo
		join bJCJM m on JCCHInfo.JCCo = m.JCCo and JCCHInfo.Job=m.Job 

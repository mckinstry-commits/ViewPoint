...
CREATE PROCEDURE [dbo].[vspIMViewpointDefaultsAPUL]
/***********************************************************
* CREATED BY:   TJL 05/15/09 - Issue #25567, Create new Import for AP Unapproved
* MODIFIED BY:	MV	03/17/10 - #136500 - taxbasis net retainage
...
*               ECV 05/24/11 - TK-05443 - Added SM fields
*			TRL  07/27/2011  TK-07143  Expand bPO parameters/varialbles to varchar(30)
*				MH 08/19/2011 - TK-07482 Replace SM MiscellaneousType with SMCostType
...
*				JG 02/10/2012 - TK-00000 - Renamed @IsSMMiscTypeEmpty to @IsSMCostTypeEmpty
*				JayR 10/16/2012 TK-16099 - Removed or renamed overlapping variables
*
* Usage:
...
declare @rcode int, @recode int, @desc varchar(120), @defaultvalue varchar(30), @errmsg varchar(120), 
	...
	@RecKey varchar(60), @HeaderRecordType varchar(10),	@HeaderReqSeq int
...
declare @CompanyID int, 
	...
	@retainageid int, @vendorid int, @paycategoryid int, @discountid int, @headeruimthid int,
	@uimthid int, @reviewergrpid int,@headerreviewergroupid int, @headercompanyid int, 
	@smcoid int, @smworkorderid int, @smscopeid int, @smcosttypeid int, @smpaytypeid int, 
	@smjccosttype_lower int, @smphasegroup int
...
declare  @Co bCompany, 
	...
	@UIMth varchar(20), @UISeq int, @HeaderReviewerGroup varchar(10), @ReviewerGroup varchar(10), @Dept bDept, @hqdfltcountry char(2),
	@apcousetaxdiscyn bYN, @VPLineType tinyint, @taxbasisnetretgyn bYN,
	@SMCo smallint, @SMWorkOrder int, @SMScope int, @SMCostType SMALLINT, @SMPayType INT, 
...
select @FormHeader = 'APUnappInv'
select @FormDetail = 'APUnappInvItems'
select @Form = 'APUnappInvItems'
...
DECLARE @IsLineTypeEmpty 			bYN
		...
		,@IsDiscountEmpty 			bYN
		,@IsReviewerGroupEmpty		bYN
		,@IsSMCoEmpty				bYN
		,@IsSMWorkOrderEmpty		bYN
		,@IsSMScopeEmpty			bYN
		,@IsSMCostTypeEmpty			bYN
		,@IsSMPayTypeEmpty			bYN
...
SELECT @OverwriteAPLine = dbo.vfIMTemplateOverwrite(@ImportTemplate, @FormDetail, 'Line', @rectype);
...
SELECT @OverwriteReviewerGroup = dbo.vfIMTemplateOverwrite(@ImportTemplate, @FormDetail, 'ReviewerGroup', @rectype);
...
SELECT @OverwriteSMPayType = dbo.vfIMTemplateOverwrite(@ImportTemplate, @FormDetail, 'SMPayType', @rectype);
...
select @aplineid=dbo.bfIMTemplateDefaults(@ImportTemplate, @FormDetail, 'Line', @rectype, 'N')		--Non-Nullable

--Co
--if ISNULL(@CompanyID, 0)<> 0 AND (ISNULL(@OverwriteCo, 'Y') = 'Y')
--	begin
--	--'Use Viewpoint Default' = Y or N and 'Overwrite Import Value' = Y  (Set ALL import records to this Company)
--	Update IMWE
...
--if ISNULL(@CompanyID, 0)<> 0 AND (ISNULL(@OverwriteCo, 'Y') = 'N')
--	begin
--	--'Use Viewpoint Default' = Y or N and 'Overwrite Import Value' = N  (Set to this Company only if no import value exists)
--	Update IMWE
--	SET IMWE.UploadVal = @Company
--	where IMWE.ImportTemplate=@ImportTemplate and
--		IMWE.ImportId=@ImportId and IMWE.Identifier = @CompanyID and IMWE.RecordType = @rectype
...
select @reviewergrpid=dbo.bfIMTemplateDefaults(@ImportTemplate, @FormDetail, 'ReviewerGroup', @rectype, 'Y')

/***** GET COLUMN IDENTIFIERS - Identifier will be returned regardless of 'Use Viewpoint Default' setting. *******/
--Header
select @headercompanyid=dbo.bfIMTemplateDefaults(@ImportTemplate, @FormHeader, 'APCo', @HeaderRecordType, 'N')
select @headeruimthid=dbo.bfIMTemplateDefaults(@ImportTemplate, @FormHeader, 'UIMth', @HeaderRecordType, 'N')
...
select @headerreviewergroupid=dbo.bfIMTemplateDefaults(@ImportTemplate, @FormHeader, 'ReviewerGroup', @HeaderRecordType, 'N')
--Detail
select @CompanyID=dbo.bfIMTemplateDefaults(@ImportTemplate, @FormDetail, 'APCo', @rectype, 'N')			--Non-Nullable
select @uimthid=dbo.bfIMTemplateDefaults(@ImportTemplate, @FormDetail, 'UIMth', @rectype, 'N')			--Non-Nullable
...
select @smscopeid=dbo.bfIMTemplateDefaults(@ImportTemplate, @FormDetail, 'Scope', @rectype, 'N')
select @smcosttypeid=dbo.bfIMTemplateDefaults(@ImportTemplate, @FormDetail, 'SMCostType', @rectype, 'N')
...
while @complete = 0
	...
	if @Recseq = @currrecseq	--Moves on to defaulting process when the first record of a DIFFERENT import RecordSeq is detected
		...
		If @Column='Line' and isnumeric(@Uploadval) =1 select @APLine = @Uploadval
		If @Column='MiscYN' select @MiscYN = @Uploadval
		If @Column='SMCo' select @SMCo = convert(smallint,@Uploadval)
		If @Column='SMWorkOrder' select @SMWorkOrder = convert(int,@Uploadval)
		If @Column='Scope' select @SMScope = convert(int,@Uploadval)
		...
		IF @Column='ReviewerGroup'
			IF @Uploadval IS NULL
				SET @IsReviewerGroupEmpty = 'Y'
			ELSE
				SET @IsReviewerGroupEmpty = 'N'
		IF @Column='SMCo'
			IF @Uploadval IS NULL
				SET @IsSMCoEmpty = 'Y'
			ELSE
		...
		select @UIMth=IMWE.UploadVal
		from IMWE with (nolock)
		where IMWE.ImportTemplate=@ImportTemplate and IMWE.ImportId=@ImportId
			and IMWE.Identifier = @headeruimthid and IMWE.RecordType = @HeaderRecordType 
			and IMWE.RecordSeq = @HeaderReqSeq
		...
		select @HeaderReviewerGroup=IMWE.UploadVal
		from IMWE with (nolock)
		where IMWE.ImportTemplate=@ImportTemplate and IMWE.ImportId=@ImportId
			and IMWE.Identifier = @headerreviewergroupid and IMWE.RecordType = @HeaderRecordType 
			and IMWE.RecordSeq = @HeaderReqSeq

		/* Get HQ Company, Country value.  Company can be different from one RecSeq to another. */
		select @hqdfltcountry = h.DefaultCountry, @apcousetaxdiscyn = UseTaxDiscountYN,
		...
		if @uimthid <> 0
			begin
			UPDATE IMWE
			SET IMWE.UploadVal = @UIMth		--Set same as Header
			where IMWE.ImportTemplate=@ImportTemplate and IMWE.ImportId=@ImportId and IMWE.RecordSeq=@currrecseq
				and IMWE.Identifier = @uimthid and IMWE.RecordType = @rectype 
  			end
		...
		if @linetypeid <> 0 AND (ISNULL(@OverwriteLineType, 'Y') = 'Y' OR ISNULL(@IsLineTypeEmpty, 'Y') = 'Y')
			...
			where IMWE.ImportTemplate=@ImportTemplate and IMWE.ImportId=@ImportId and IMWE.RecordSeq=@currrecseq
				 and IMWE.RecordType = @rectype and IMWE.Identifier = @linetypeid
			end
		...
		if @emctypeid <> 0  AND (ISNULL(@OverwriteEMCType, 'Y') = 'Y' OR ISNULL(@IsEMCTypeEmpty, 'Y') = 'Y')
			...
			-- LINE TYPE 1=Job,2=Inventory, 3=Expense, 4=Equipment, 5=Work Order, 6=Purchase Order, 7=Subcontract
			-- PO Item Type 1 = Job, 2 = Inventory, 3 = Expense, 4 = Equipment, 5 = Work Order
			if @LineType = 5 select @EMCType = OutsideRprCT from EMCO with (nolock) where EMCo = @EMCO
		...
		if @glacctid <> 0 AND (ISNULL(@OverwriteGLAcct, 'Y') = 'Y' OR ISNULL(@IsGLAcctEmpty, 'Y') = 'Y') 
 		...
 		--ReviewerGroup
		if @reviewergrpid <> 0 AND (ISNULL(@OverwriteReviewerGroup, 'Y') = 'Y' OR ISNULL(@IsReviewerGroupEmpty, 'Y') = 'Y')
 			begin
			if @LineType = 1 select @ReviewerGroup = RevGrpInv from JCJM with (nolock) where JCCo = @JCCo and Job = @Job
			if @LineType = 2 select @ReviewerGroup = ReviewerGroup from INLM with (nolock) where INCo = @INCo and Loc = @Loc
			if @LineType = 3 select @ReviewerGroup = ReviewerGroup from GLAC with (nolock) where GLCo = @GLCo and GLAcct = @GLAcct
			if @LineType = 4 select @ReviewerGroup = d.ReviewerGroup 
								from EMDM d  with (nolock)
								join EMEM e with (nolock) on e.EMCo = d.EMCo and e.Department = d.Department
								where d.EMCo = @EMCO and e.Equipment = @Equip
			select @ReviewerGroup = isnull(@ReviewerGroup, @HeaderReviewerGroup)
			
			UPDATE IMWE
			SET IMWE.UploadVal = @ReviewerGroup
			where IMWE.ImportTemplate=@ImportTemplate and IMWE.ImportId=@ImportId and IMWE.RecordSeq=@currrecseq
				and IMWE.Identifier = @reviewergrpid and IMWE.RecordType = @rectype
			end
		...
		if @umid <> 0 AND (ISNULL(@OverwriteUM, 'Y') = 'Y' OR ISNULL(@IsUMEmpty, 'Y') = 'Y')
			begin
			-- LINE TYPE 1=Job,2=Inventory, 3=Expense, 4=Equipment, 5=Work Order, 6=Purchase Order, 7=Subcontract
			-- PO Item Type 1 = Job, 2 = Inventory, 3 = Expense, 4 = Equipment, 5 = Work Order, 6=SM WorkOrder
		...
		if @retainageid <> 0 and @LineType=7 and isnull(@Co,'')<> '' AND (ISNULL(@OverwriteRetainage, 'Y') = 'Y' OR ISNULL(@IsRetainageEmpty, 'Y') = 'Y') 
  		...
  		--Discount     
  		if @discountid <> 0	AND (ISNULL(@OverwriteDiscount, 'Y') = 'Y' OR ISNULL(@IsDiscountEmpty, 'Y') = 'Y')
  			begin
    	...
    	select @Co =null, @LineType =null, @PO =null, @POItem =null, @ItemType =null, @SL =null, @SLItem =null, 
    		...
    		@GLCo =null, @UM =null, @Units =null, @VendorGroup =null, @GrossAmt =null, @MiscAmt =null, @TaxGroup =null, @TaxCode =null,
     		@TaxType =null, @TaxBasis =null, @TaxAmt =null, @Retainage =null, @Discount =null, @UIMth =null, @UISeq =null,
     		@ReviewerGroup =null, @HeaderReviewerGroup =null, @VPLineType = null, @APLine =null,
...
where IMWE.ImportTemplate=@ImportTemplate and IMWE.ImportId=@ImportId 
	...
	(IMWE.Identifier = @nmiscynid)

vspexit:
select @msg = isnull(@desc,'Line') + char(13) + char(13) + '[vspIMViewpointDefaultsAPUL]'

return @rcode
GO
GRANT EXECUTE ON  [dbo].[vspIMViewpointDefaultsAPUL] TO [public]
GO

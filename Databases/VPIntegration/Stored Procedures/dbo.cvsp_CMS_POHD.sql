...
select POCo=@toco
	...
	, OrderDate=substring(convert(nvarchar(max),h.PODATE),5,2) + '/' +  substring(convert(nvarchar(max),h.PODATE),7,2) 
				+ '/' + substring(convert(nvarchar(max),PODATE),1,4)
	, ExpDate=null
	, Status=1
	, JCCo=@toco
	, Job=case when h.JOBNUMBER='' then null 
				else dbo.bfMuliPartFormat(RTRIM(h.JOBNUMBER) + '.' + RTRIM(h.SUBJOBNUMBER),@Job) 
			end
	, INCo=@toco
	, Loc=null

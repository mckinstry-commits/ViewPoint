...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRNYC012    Script Date: 8/28/99 9:33:30 AM ******/
   CREATE   proc [dbo].[bspPRNYC012]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @allowance = 1000, @procname = 'bspPRNYC012'
   ...
   /* initialize calculation elements */
   	...
   	if @annualized_wage <= 8000 select @tax_addition = 0, @wage_bracket = 0, @rate = .017
   	if @annualized_wage > 8000 select @tax_addition = 136, @wage_bracket = 8000, @rate = .024
   	if @annualized_wage > 8700 select @tax_addition = 153, @wage_bracket = 8700, @rate = .028
   	if @annualized_wage > 15000 select @tax_addition = 329, @wage_bracket = 15000, @rate = .0335
   	if @annualized_wage > 25000 select @tax_addition = 664, @wage_bracket = 25000, @rate = .036
   	if @annualized_wage > 60000 select @tax_addition = 1924, @wage_bracket = 60000, @rate = .039
...
GO
GRANT EXECUTE ON  [dbo].[bspPRNYC012] TO [public]
GO

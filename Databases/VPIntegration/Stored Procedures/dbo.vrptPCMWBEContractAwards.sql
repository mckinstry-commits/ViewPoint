...
CREATE PROC [dbo].[vrptPCMWBEContractAwards] (@Company            bCompany, 
...
* MODIFIED: HH 06/17/11 - TK-06056/TK-06057 Renamed vrptPCSWMBESubcontractAwards
*			   to vrptPCMWBEContractAwards and Differentiation  between 
*			   CommitmentTypes (SL/PO) and validation against MWBE certificate dates 
*       
...
************************************************************************************/ 
  ...
  CREATE TABLE #Result 
    ( 
       ...
       CommitmentValue        DECIMAL(15, 2) DEFAULT(0), 
  ...
  /*** actual project MWBE goals infos **/
  ...
  CREATE TABLE #JobCalculations 
    ( 
       ...
       CommitmentVal     DECIMAL(15, 2) DEFAULT(0) 
    ) 
  ...
  SET    ContractValue = JCal.ContractValue, 
         CommitmentValue = JCal.CommitmentVal,
         CurrentPurchaseDollars = JCal.ContractValue * GoalPct, 
         CurSubPct = JCal.ContractValue * ( GoalPct / Nullif(JCal.CommitmentVal, 0) ), 
         ActSubPct = ( ActualPurchaseDollars / Nullif(JCal.CommitmentVal, 0) ), 
  ...
  /*** GoalMetYN determining from current project goals infos ***/
  UPDATE R 
  SET    GoalMetYN = P.GoalMetYN
  FROM   #Result R 
		 LEFT JOIN PCPotentialProjectCertificate P
		   ON R.JCCo = P.JCCo 
			  AND R.PotentialProject = P.PotentialProject 
			  AND R.CertificateType = P.CertificateType
  WHERE  R.GoalMetYN IS NULL
...
GO
GRANT EXECUTE ON  [dbo].[vrptPCMWBEContractAwards] TO [public]
GO

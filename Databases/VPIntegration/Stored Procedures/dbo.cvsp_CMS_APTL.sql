...
select APCo=@toco
	, Mth=substring(convert(nvarchar(max),d.JOURNALDATE),5,2) + '/01/' +  substring(convert(nvarchar(max),d.JOURNALDATE),1,4)
	, APTrans=convert(nvarchar(max),d.PAYMENTSELNO) + convert(nvarchar(max),d.RECORDCODE)

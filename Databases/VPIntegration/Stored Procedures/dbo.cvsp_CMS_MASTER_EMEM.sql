...
set @Equip = (select InputMask from vDDDTc where Datatype = 'Equip');
...
FuelUsed, EMGroup, JCCo, Job, PhaseGrp, WeightCapacity, VolumeCapacity, LicensePlateNo,-- LicensePlateExpDate,
PRCo, GrossVehicleWeight, TareWeight, NoAxles, OwnershipStatus, InServiceDate, ExpLife, ReplCost, CurrentAppraisal,
SoldDate, SalePrice, PurchasePrice, PurchDate, APCo, VendorGroup, LeasePayment, LeaseResidualValue, ARCo, CustGroup, 
...
select 	EMCo = @toco
		...
		,Type = 'E'                  --MITCL
		,Department = case when DEPARTMENTNO >0 then 4350 else 1 end 
		...
		,InServiceDate = CASE WHEN STARTDATE = 0 THEN NULL ELSE
			substring(convert(nvarchar(max),STARTDATE),5,2) + '/' + substring(convert(nvarchar(max),STARTDATE),7,2) + '/' + substring(convert(nvarchar(max),STARTDATE),1,4)
			END
		,ExpLife = 0
		,ReplCost = left(REPLACEMENTCST,9)
		,CurrentAppraisal = left(CURRMARKETVAL,9)
		,SoldDate = CASE WHEN DISPOSALDATE = 0 THEN NULL ELSE
			substring(convert(nvarchar(max),DISPOSALDATE),5,2) + '/' + substring(convert(nvarchar(max),DISPOSALDATE),7,2) + '/' + substring(convert(nvarchar(max),DISPOSALDATE),1,4)
			END
		,SalePrice = left(DISPOSALAMT,9)
		,PurchasePrice = left(ACQUISITIONCOST,9)
		,PurchDate = CASE WHEN ANTICCSTDATE = 0 THEN NULL ELSE convert(datetime,left(ANTICCSTDATE,8),101) END
		,APCo = @toco
		...
		,OriginalEquipmentCode = RTRIM(EQUIPMENTNUMBER)
		,udSource='MASTER_EMEM'
		,udConv='Y'
		,udCGCTable='EQTMST'
		
from CV_CMS_SOURCE.dbo.EQTMST t
--left join Viewpoint.dbo.budxrefEquipment e on e.Company=@fromco and e.oldEquip=t.EQUIPMENTNUMBER
--left join Viewpoint.dbo.budxrefEMCat c 
	--on t.COMPANYNUMBER = @fromco and  t.MITCL = c.EQTClass
--left join CV_CMS_SOURCE.dbo.TaxLimits l 
--	on t.MSTTC = l.StateCode and l.TaxID= 'S' --hardcoding: and l.Year = 2009

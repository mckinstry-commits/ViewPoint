...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRPRT02    Script Date: 8/28/99 9:33:32 AM ******/
      CREATE   proc [dbo].[bspPRPRT02]
      /********************************************************
      ...
      **********************************************************/
      ...
      select @rcode = 0, @procname = 'bspPRPRT02'
      ...
      if @misc_factor = 1
      begin
      	if @annualized_wage <= 1000 select @tax_subtraction = 0, @rate = .06
      	if @annualized_wage > 1000 select @tax_subtraction = 30, @rate = .09
      	if @annualized_wage > 8500 select @tax_subtraction = 455, @rate = .14
      	if @annualized_wage > 15000 select @tax_subtraction = 2405, @rate = .27
      	if @annualized_wage > 25000 select @tax_subtraction = 3905, @rate = .33
      end
   
      /* select calculation elements for everybody else */
      if @misc_factor <> 1 or @misc_factor is null
      begin
      	if @annualized_wage <= 2000 select @tax_subtraction = 0, @rate = .06
      	if @annualized_wage > 2000 select @tax_subtraction = 60, @rate = .09
      	if @annualized_wage > 17000 select @tax_subtraction = 910, @rate = .14
      	if @annualized_wage > 30000 select @tax_subtraction = 4810, @rate = .27
      	if @annualized_wage > 50000 select @tax_subtraction = 7810, @rate = .33
      end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRPRT02] TO [public]
GO

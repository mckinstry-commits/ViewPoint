...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_FWT092]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_FWT092]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_FWT092'
   ...
   -- constants
   select @maxwages = 1044.00, @TCrate = .15, --@maxQPP = 2049.30, @maxQCEI = 571.29, 
   ...
   select @rate = .29, @K = 9968
   if @A <= 126264 select @rate = .26, @K = 6180
   if @A <= 82399 select @rate = .22, @K = 2884
   if @A <= 41200 select @rate = .15, @K = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_FWT092] TO [public]
GO

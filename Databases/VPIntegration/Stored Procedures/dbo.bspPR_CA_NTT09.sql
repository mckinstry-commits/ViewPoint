...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NTT09]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_NTT09]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NTT09'
   ...
   select @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @T2 = 0, @calcamt = 0, @TCrate = .059

   -- if form TD1NT was not filed (ie. no filing status entered) use default total claim
   if @TCP is null select @TCP = 12664

   -- establish tax rate and constant
   select @rate = .1405, @KP = 5871
   if @A <= 119936 select @rate = .1220, @KP = 3652
   if @A <= 73772 select @rate = .0860, @KP = 996
   if @A <= 36885 select @rate = .0590, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NTT09] TO [public]
GO

...
POIB_loop:     -- get next Item
		...
		BEGIN
			...
			IF EXISTS(
				...
				FROM dbo.vSMWorkCompletedPurchase
					INNER JOIN dbo.vSMWorkCompletedDetail ON vSMWorkCompletedPurchase.SMWorkCompletedID = vSMWorkCompletedDetail.SMWorkCompletedID
	...
	if @itemtranstype in ('A','C')      -- validation for both 'add' and 'change' Items
		...
		BEGIN
			...
			EXEC @rcode = vspSMWorkOrderValForPO @SMCo = @smco, @SMWorkOrder = @smworkorder, @msg = @errmsg OUTPUT
			IF (@rcode <> 0)
			BEGIN
	...
	if @itemtranstype = 'D'     -- validation specific for 'delete' Items
		...
		if exists(select 1 from bPOCD WITH (NOLOCK) where POCo = @co and PO = @po and POItem = @poitem)
			BEGIN

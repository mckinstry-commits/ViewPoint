...
set nocount on
   ...
   declare @rcode int, @vendorgroup bGroup, @ourfirm bFirm, @responsiblefirm bFirm,
   		...
   		@vendor bVendor, @vendorfirm bFirm, @oldvendor bVendor
   ...
   if isnull(@vendorfirm,0) <> 0
   	...
   	from bPMPF with (nolock) where PMCo=@pmco and Project=@project and VendorGroup=@vendorgroup and FirmNumber=@sendtofirm
   	if @@rowcount = 0
   		begin
   		select @sendtocontact=min(ContactCode)
   		from bPMPM with (nolock) where VendorGroup=@vendorgroup and FirmNumber=@sendtofirm
   ...
   insert bPMSS(PMCo, Project, SLCo, SL, VendorGroup, ResponsibleFirm, ResponsiblePerson, SendToFirm, SendToContact)
   select @pmco, @project, @slco, @sl, @vendorgroup, @ourfirm, @responsibleperson, @vendorfirm, @sendtocontact

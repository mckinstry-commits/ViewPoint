...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRMNT02    Script Date: 8/28/99 9:33:28 AM ******/
    CREATE   proc [dbo].[bspPRMNT02]
    /********************************************************
    ...
    **********************************************************/
    ...
    select @rcode = 0, @dedn = 3000, @procname = 'bspPRMNT02'
    ...
    if @status = 'S'
    	begin
    		...
    		if @annualized_wage > 20410 select @tax_addition = 1001, @wage_bracket = 20410, @rate = .0705
    		if @annualized_wage > 63160 select @tax_addition = 4015, @wage_bracket = 63160, @rate = .0785
    end
    ...
    if @status = 'M'
    	begin
    		...
    		if @annualized_wage > 4850 select @wage_bracket = 4850, @rate = .0535
    		if @annualized_wage > 32200 select @tax_addition = 1463, @wage_bracket = 32200, @rate = .0705
    		if @annualized_wage > 113510 select @tax_addition = 7196, @wage_bracket = 113510, @rate = .0785
    end
...
GO
GRANT EXECUTE ON  [dbo].[bspPRMNT02] TO [public]
GO

...
select @CustGroup
                , cust.NewCustomerID  --.CUSTOMERNUMBER
                , Name=c.NAME25
                ...
                , Phone=case
					...
					else convert(varchar(max),AREACODE) + '-' + convert(varchar(max),PHONENO) end
                ...
                , BillAddress=case when SNDTOADR1='' then null else SNDTOADR1 end
                , BillCity=case when SNDTOCITY='' then null else SNDTOCITY end
                , BillState=case when SNDTOSTE='' then null else SNDTOSTE end
                , BillZip=case when SNDTOZIP='' then null else SNDTOZIP end
                , BillAddress2=case when SNDTOADR2='' then null else SNDTOADR2 end
...
from CV_CMS_SOURCE.dbo.CSTMST c with (nolock)
join Viewpoint.dbo.budxrefARCustomer cust on cust.Company=@fromco and cust.OldCustomerID=c.CUSTOMERNUMBER

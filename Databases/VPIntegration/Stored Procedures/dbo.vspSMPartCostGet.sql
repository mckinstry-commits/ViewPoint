...
-- Create date: 10/20/10
-- Modified By: GF 10/11/2011 TK-08987 EXPANDED PONumber to 30-characters
--
--
-- Description:	Determine the cost of a part.
=============================================*/
CREATE PROCEDURE [dbo].[vspSMPartCostGet]
	@Source AS tinyint,
	@INCo AS bCompany,
	@INLocation AS bLoc,
	@CreateNewPOItem bYN,
...
BEGIN
	...
	IF (@Source IS NULL OR @MaterialGroup IS NULL OR @PartUM IS NULL OR @Quantity IS NULL)
	BEGIN
	...
	IF (@Source = 0)
	BEGIN
		-- Handle Costs for parts from Inventory.
		
		IF (@Part IS NULL)
		BEGIN
			SELECT @msg = 'Part required to calculate cost for an Inventory item.'
			RETURN 1
		END
		
		IF (@INCo IS NULL OR @INLocation IS NULL)
		BEGIN
			SELECT @msg = 'IN Company and IN Location are required for costing.'
			RETURN 1
		END
		
		EXEC dbo.vspSMPartInventoryCostGet @INCo, @INLocation, @MaterialGroup, @Part, @PartUM, @CostPerUnit OUTPUT, @CostECM OUTPUT, @CostPerEach OUTPUT, @msg OUTPUT
		IF (@msg IS NOT NULL)
		BEGIN
			RETURN 1
		END
...
END
GO
GRANT EXECUTE ON  [dbo].[vspSMPartCostGet] TO [public]
GO

...
insert bAPVM (VendorGroup,Vendor,SortName,Name,Type,TempYN,Contact,Phone,Fax,EMail,
	...
	OverrideMinAmtYN,MasterVendor,APRefUnqOvr,
	UpdatePMYN,udSource,udConv,udCGCTable,udCGCTableID)
...
select @VendorGroup
	...
	,@TaxGroup
	,TaxCode=null
	,PayTerms=case when E.TERMSCODE='' then null else + convert(varchar(2),E.TERMSCODE) end
	,GLCo=@toco
...
join budxrefAPVendor x
	on /*x.Company=E.COMPANYNUMBER and*/ x.OldVendorID=E.VENDORNUMBER and x.CGCVendorType='V'
...
left join (select upper(ltrim(rtrim(ABBREVIATION10))) as ABBREVIATION10, COUNT(1) as AbbCount
			from CV_CMS_SOURCE.dbo.APTVEN
			group by upper(ltrim(rtrim(ABBREVIATION10))) having COUNT(1)>1) dup
...
set Contact = b.CONTRACTNAME,
	Phone = case 
		when bAPVM.Phone is NULL or bAPVM.Phone = '' then case 
			when b.AREACODE = 0 and b.PHONENO <> 0 then '(   ) ' 
				+ substring(convert(varchar(7),b.PHONENO),1,3) + '-' + substring(convert(varchar(7),b.PHONENO),4,4)
			when b.AREACODE = 0 and b.PHONENO = 0 then NULL 
...
from bAPVM 
	join CV_CMS_SOURCE.dbo.APTVCN b on bAPVM.Vendor = b.VENDORNUMBER and b.COMPANYNUMBER=@fromco
where b.SEQUENCENO05 = 1 and b.VENDORLOCNO=0

...
select JCCo
	...
	, Item
	, Description
	, Department=case when Department is null then @defaultDepartment else Department end
	, UM=@defaultUM
...
from 
	(select JCCo=@toco
		...
		, Item=case when c.ITEMNUMBER is null then @defaultItem
				else space(16-datalength(rtrim(c.ITEMNUMBER))) + rtrim(c.ITEMNUMBER) end 
		, Description=min(j.DESC20A) 
		, Department=min(case when x.VPDept is null then j.DEPARTMENTNO 
					else x.VPDept end)
	...
	from CV_CMS_SOURCE.dbo.JCTDSC j with (nolock)
		left join CV_CMS_SOURCE.dbo.ARTCNS c with (nolock)
				on j.COMPANYNUMBER=c.COMPANYNUMBER and j.JOBNUMBER=c.JOBNUMBER 
		...
		left join Viewpoint.dbo.budxrefJCDept x on x.Company=@fromco
				and x.CMSDept=j.DEPARTMENTNO 
	where j.COMPANYNUMBER=@fromco and ISNULL(JOBSTATUS,0) <> case when @OpenJobsYN = 'Y' then 3 else 99 end
	group by dbo.bfMuliPartFormat(rtrim(j.JOBNUMBER), @JobFormat),
		case when c.ITEMNUMBER is null then @defaultItem
			 else space(16-datalength(rtrim(c.ITEMNUMBER))) + rtrim(c.ITEMNUMBER) end) 
...
update bJCCM set Department=JCCI.Department
from bJCCM
	left join (select JCCo, Contract, Department=min(Department)
				from bJCCI
				group by JCCo, Contract)
				as JCCI
				on bJCCM.JCCo=JCCI.JCCo and bJCCM.Contract=JCCI.Contract

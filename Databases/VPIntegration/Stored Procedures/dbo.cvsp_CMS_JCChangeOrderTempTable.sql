...
select JCCo=@toco
	...
	,ACOItem='         1'
	,ApprovalDate=case when h.RHDTRV=0 then convert(varchar(5),datepart(mm,getdate() )) + '/' +
					convert(varchar(5),datepart(dd,getdate() )) + '/' + convert(varchar(5),datepart(yy,getdate() )) 
				else substring(convert(nvarchar(max),h.RHDTRV),5,2) + '/' + substring(convert(nvarchar(max),h.RHDTRV),7,2) 
						+ '/' + substring(convert(nvarchar(max),h.RHDTRV),1,4) end
	,d.REVISIONAMT
	...
	,Item=case when (c.ContOnly='Y' or c.UpdateAll='Y' or c.ContCostOnly='Y') 
			then 
	...
	,UM=case when isnull(u.VPUM,d.ESTUOM)='' then 
			case when d.CHGORDERQTY=0 then isnull(x.UM,'LS') else isnull(x.UM,'EA') end
		else isnull(u.VPUM,d.ESTUOM) end
	,udCGCTable='JCTCGH'
	,udCGCTableID=h.JCTCGHID
into JCChangeOrders
from CV_CMS_SOURCE.dbo.JCTCGH h 
	join CV_CMS_SOURCE.dbo.JCTCGO d on h.RHCONO=d.COMPANYNUMBER and h.JOBNUMBER=d.JOBNUMBER and h.SUBJOBNUMBER=d.SUBJOBNUMBER 

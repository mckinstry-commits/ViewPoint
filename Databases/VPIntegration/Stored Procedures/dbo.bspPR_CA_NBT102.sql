...
SET ANSI_NULLS ON
GO
/****** Object:  StoredProcedure [dbo].[bspPR_CA_NBT102]    Script Date: 02/27/2008 13:19:16 ******/
   CREATE  proc [dbo].[bspPR_CA_NBT102]
   /********************************************************
   ...
   **********************************************************/
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NBT102'
   ...
   select @KP = 0, @K1P = 0, @K2P = 0, @T4 = 0, @LCP = 0, @T2 = 0, @calcamt = 0, @TCrate = .093

   -- if form TD1NB was not filed (ie. no filing status entered) use default total claim
   if @TCP is null select @TCP = 8777

   -- establish tax rate and constant
   select @rate = .143, @KP = 2932
   if @A <= 118427 select @rate = .133, @KP = 1748
   if @A <= 72843 select @rate = .125, @KP = 1165
   if @A <= 36421 select @rate = .093, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NBT102] TO [public]
GO

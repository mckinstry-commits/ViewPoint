...
SET ANSI_NULLS ON
GO
CREATE proc [dbo].[vspMSCommonInfoGetForHaul]
/***********************************************************
 * Created By:  GF 08/20/2007 6.x
 ...
 *
 * USAGE:
 * Gets MS company info for use in MS Haul Entry program.
 *
...
(@msco bCompany = 0, @apco bCompany output, @arco bCompany output, @glco bCompany output,
 @taxopt tinyint output, @limitchk bYN output, @ticmatlvendor bYN output, @tictimes bYN output,
 ...
 @matlgroup tinyint output, @disctaxyn bYN output, @autoapply bYN output,
 @intercoinv bYN output, @country varchar(2) output, @errmsg varchar(255) output)
as
set nocount on
...
---- get MS company info
select @apco=APCo, @arco=ARCo, @glco=GLCo, @limitchk=LimitChk,
		@ticmatlvendor=HaulMatlVendor, @tictimes=HaulTimes, @ticloads=HaulLoads,
		@ticmiles=HaulMiles, @tichrs=HaulHrs, @ticzone=HaulZone, @ticrev=HaulRev,
		@ticpay=HaulPay, @tictax=HaulTax, @ticdisc=HaulDisc, @taxopt=TaxOpt,
		@autoapply=AutoApplyCash, @intercoinv=InterCoInv
...
GO
GRANT EXECUTE ON  [dbo].[vspMSCommonInfoGetForHaul] TO [public]
GO

...
CREATE     procedure [dbo].[vspINIBInsertExistingTrans]
    /***********************************************************
     * CREATED BY: RM 06/04/02
     * MODIFIED By : TRL 02/06/07 modified for IN MO Entry,
     *
     * USAGE:
    ...
    /* validate HQ Batch */
    exec @rcode = dbo.bspHQBatchProcessVal @co, @mth, @batchid, 'MO Entry', 'INMB', @errtext output, @status output
    if @rcode <> 0
       begin
    ...
    select @inusemth=InUseMth, @inusebatchid = InUseBatchId from dbo.INMO with(nolock) where INCo=@co and MO=@mo
    if @@rowcount = 0
    	begin
    	select @errmsg = 'The Material Order :' + isnull(@mo,'MISSING: @mo') + ' cannot be found.' , @rcode = 3
    	goto vspexit
    	end
    ...
    select @inusemth=InUseMth, @inusebatchid = InUseBatchId from dbo.INMI with(nolock) where INCo=@co and MO=@mo and MOItem = @item
    if @@rowcount = 0
    	begin
    ...
    if not @inusebatchid is null
   	...
   	Insert into dbo.INIB(Co,Mth,BatchId,BatchSeq,MOItem,BatchTransType,
	Loc,MatlGroup,Material,Description,
	JCCo,Job,PhaseGroup,Phase,JCCType,
   	GLCo,GLAcct,
	ReqDate,
	UM,OrderedUnits,UnitPrice,ECM,TotalPrice,
	TaxGroup,TaxCode,TaxAmt,
	RemainUnits,
	OldLoc,OldMatlGroup,OldMaterial,OldDesc,
	OldJCCo,OldJob,OldPhaseGroup,OldPhase,OldJCCType,
	OldGLCo,OldGLAcct,
	OldReqDate,
	OldUM,OldOrderedUnits,OldUnitPrice,OldECM,OldTotalPrice,
	OldTaxGroup,OldTaxCode,OldTaxAmt,
	OldRemainUnits)
   	Select i.INCo,@mth,@batchid,@seq,i.MOItem,'C',
	i.Loc,i.MatlGroup,i.Material,i.Description,
	i.JCCo,i.Job,i.PhaseGroup,i.Phase,i.JCCType,
	i.GLCo,i.GLAcct,
	i.ReqDate,
   	i.UM,i.OrderedUnits,i.UnitPrice,i.ECM,i.TotalPrice,
   	i.TaxGroup,i.TaxCode,i.TaxAmt,
	i.RemainUnits,
   	i.Loc,i.MatlGroup,i.Material,i.Description,
	i.JCCo,i.Job,i.PhaseGroup,i.Phase,i.JCCType,
	i.GLCo,i.GLAcct,
	i.ReqDate,
	i.UM,
	i.OrderedUnits,i.UnitPrice,i.ECM,i.TotalPrice,
   	i.TaxGroup,i.TaxCode,i.TaxAmt,
	i.RemainUnits
   	from dbo.INMI i with(nolock) where INCo=@co and MO=@mo and MOItem=@item 
    
    if @@rowcount > 0
    begin	
    /* update user memo to INIB batch table- BatchUserMemoInsertExisting */
         exec @rcode = dbo.bspBatchUserMemoInsertExisting @co, @mth, @batchid, @seq, 'MO Entry Items',
             @item, @errmsg output
             if @rcode <> 0
             begin
...
GO
GRANT EXECUTE ON  [dbo].[vspINIBInsertExistingTrans] TO [public]
GO

...
SET ANSI_NULLS ON
GO
CREATE   PROCEDURE [dbo].[bspUDDropTable] 
   /************************************************
   	Created: RM 03/12/01
   			 DANF 03/10/04 - Issue 20536 Remove Security entries from DDSL.
   ...
   ************************************************/
   ...
   if not exists(select * from sysobjects where name = 'b' + @tablename)
   goto bspexit
   ...
   --
     select @btablename = 'b' + @tablename
      ...
      select ColumnName,DataType from bUDCA
      ...
      begin
       ...
       exec @rcode = dbo.bspDDSLUserColumn @btablename, @datatype, @columnname, null, 'Deletion', @errmsg output
         if @rcode <> 0
         begin
...
GO
GRANT EXECUTE ON  [dbo].[bspUDDropTable] TO [public]
GO

...
SET ANSI_NULLS ON
GO
/****** Object:  Stored Procedure dbo.bspPRCOO983    Script Date: 8/28/99 9:33:14 AM ******/
     CREATE    proc [dbo].[bspPRCOO983]
     /********************************************************
     ...
     *					EN 6/20/03 issue 20356  return CO OP eligible amount
     *
     * USAGE:
     ...
     **********************************************************/
     (@prco bCompany, @dlcode bEDLCode, @prgroup bGroup, @prenddate bDate, @employee bEmployee,
      @subjamt bDollar = 0, @amt bDollar = 0 output, @eligamt bDollar output, @msg varchar(255) = null output)
     as
     set nocount on
     ...
     if @aearn < @limit -- if limit not yet reached ... return subj amt as elig amt
   	  begin
   	  select @eligamt = @subjamt
   	  goto bspexit
   	  end
     if @aearn - @subjamt < @limit and @aearn >= @limit --if limit reached on this pd ... return portion of subj amt that fulfills limit obligation
   	  begin
   	  select @eligamt = @limit - (@aearn - @subjamt)
   	  goto bspexit
   	  end
     if @aearn - @subjamt = @limit --if limit reached before this pd ... return elig amt of 0
   	  select @eligamt = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPRCOO983] TO [public]
GO

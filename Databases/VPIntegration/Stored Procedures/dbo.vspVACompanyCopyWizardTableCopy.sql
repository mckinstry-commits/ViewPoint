 ...
 *   1   fail    
*****************************************************/     
   (@tablename varchar(128),@SourceCo int, @SourceCombo varchar(100),@DestCo int, @zeroINMTyn bYN, @msg varchar(1000)=null output,@disabledTriggers VARCHAR (MAX)='' output)    
  AS    
	...
	IF @coltype in ('bCompany','bGroup')    
		BEGIN    
			...
			BEGIN    
				...
				BEGIN
					SELECT @selectstring = @selectstring + @commastring + ' case when ' + @colname + ' = ' + convert(varchar(3),@SourceCo)   + ' then ' + convert(varchar(3),@DestCo) + ' else ' + @colname + ' end'
				END
				ELSE
				BEGIN

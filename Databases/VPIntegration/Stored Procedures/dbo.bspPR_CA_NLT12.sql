...
CREATE  proc [dbo].[bspPR_CA_NLT12]
/********************************************************
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   SELECT @rcode = 0, @procname = 'bspPR_CA_NLT12'
   ...
   IF ISNULL(@TCP,0) = 0 SELECT @TCP = 8237

   -- establish tax rate and constant
   IF      @A BETWEEN     0 AND 32893	SELECT @Rate = .077, @KP = 0
   ELSE IF @A BETWEEN 32894 AND 65785	SELECT @Rate = .125, @KP = 1579
   ELSE IF @A > 65785					SELECT @Rate = .133, @KP = 2105
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NLT12] TO [public]
GO

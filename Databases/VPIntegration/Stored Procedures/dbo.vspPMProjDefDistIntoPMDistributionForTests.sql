...
GO
   ...
   CREATE proc [dbo].[vspPMProjDefDistIntoPMDistributionForTests]
    ...
    *
    * USAGE:
    * Pull records from project distribution defaults and add to new Test record
    *
    *
    * INPUT PARAMETERS
    *	@PMCo
   ...
   (@PMCo bCompany, @Project bJob, @Type bDocType, @Code bDocument, @DateSent bDate, @RefID bigint, @msg varchar(255) output)
	...
	BEGIN
		...
		,DateSent, TestLogID, [Send], PrefMethod, CC) 
		...
		AND dc.DocType = @Type
		AND NOT EXISTS(	SELECT TOP 1 1 FROM dbo.PMDistribution WHERE PMCo=@PMCo AND Project=@Project
						AND VendorGroup=@VendorGroup AND SentToFirm=ppdd.FirmNumber AND SentToContact=ppdd.ContactCode
						AND TestType=@Type AND TestCode=@Code)

	END
GO
GRANT EXECUTE ON  [dbo].[vspPMProjDefDistIntoPMDistributionForTests] TO [public]
GO

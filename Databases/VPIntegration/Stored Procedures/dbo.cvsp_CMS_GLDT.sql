...
select GLCo=@toco
	...
	,GLRef=convert(varchar(5),right(JOURNALCTL,5)) + '-' + 
		case when JOURNALNO < 10 THEN '000' + convert(varchar(1),rtrim(JOURNALNO))
			when JOURNALNO between 9 and 99 THEN '00' + convert(varchar(2),rtrim(JOURNALNO))
			when JOURNALNO between 100 and 1000 THEN '0' + convert(varchar(3),rtrim(JOURNALNO)) 
		else convert(varchar(4),rtrim(JOURNALNO)) END  --XJRCT
	,SourceCo=@toco
	...
	,DatePosted = case when TRANSACTIONDATE=0 then
	convert(smalldatetime,(convert(varchar(4),ENTRYYEAR)+'/'+convert(varchar(2),ENTRYMONTH)+'/'+convert(VARCHAR(2),ENTRYDAY) )) 
	  else	
	      case when substring(convert(nvarchar(max),TRANSACTIONDATE),5,2)<= 12 then 
			  convert(smalldatetime,(substring(convert(nvarchar(max),TRANSACTIONDATE),1,4)+'/'
			  +substring(convert(nvarchar(max),TRANSACTIONDATE),5,2) 			+'/'+substring(convert(nvarchar(max),TRANSACTIONDATE),7,2)))
		  else
			  convert(smalldatetime,(substring(convert(nvarchar(max),TRANSACTIONDATE),1,4)+'/'+convert(varchar(2),ENTRYMONTH) 
...
from CV_CMS_SOURCE.dbo.GLTPST
	left join Viewpoint.dbo.budxrefGLAcct x on x.Company=@fromco and GENLEDGERACCT=x.oldGLAcct
	left join Viewpoint.dbo.budxrefGLJournals j on left(JOURNALCTL,2)=CMSCode

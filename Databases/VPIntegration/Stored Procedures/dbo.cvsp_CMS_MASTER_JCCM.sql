...
from (select JCCo=@toco 
		...
		, JobStatus=case when min(j.JOBSTATUS)=0 then 1 else 3 end 
	...
	left join (select c.COMPANYNUMBER, c.JOBNUMBER, c.SUBJOBNUMBER, c.CUSTOMERNUMBER from CV_CMS_SOURCE.dbo.ARTCNS c
					join (select COMPANYNUMBER, JOBNUMBER, SUBJOBNUMBER, CONTRACTNO, MIN(CUSTOMERNUMBER) as CUSTOMERNUMBER
				from CV_CMS_SOURCE.dbo.ARTCNS where CONTRACTNO=1
						group by COMPANYNUMBER, JOBNUMBER, SUBJOBNUMBER, CONTRACTNO) a
					on a.COMPANYNUMBER=c.COMPANYNUMBER and a.JOBNUMBER=c.JOBNUMBER and a.SUBJOBNUMBER=c.SUBJOBNUMBER 
						and a.CONTRACTNO=c.CONTRACTNO and a.CUSTOMERNUMBER=c.CUSTOMERNUMBER) c
			on j.COMPANYNUMBER=c.COMPANYNUMBER and j.JOBNUMBER=c.JOBNUMBER and j.SUBJOBNUMBER=c.SUBJOBNUMBER
	left join budxrefJCDept xd
		on xd.Company=j.COMPANYNUMBER and xd.CMSDept=j.DEPARTMENTNO
	where j.COMPANYNUMBER=@fromco and
	ISNULL(JOBSTATUS,0) <> case when @OpenJobsYN = 'Y' then 3 else 99 end

...
SET ANSI_NULLS ON
GO
Create proc [dbo].[bspPR_CA_NUT13]
/********************************************************
...
*				MV	12/20/2010	- #142331 tax update effective 1/1/2011
*				MV 11/30/11 - RK-10425 - 2012 tax updates
*				CHS	12/11/2012	- TK-20154 B-11805 147554 tax update effective 1/1/2013
*
* USAGE:
...
(@ppds tinyint = 0, @A bDollar = 0, @TCP bDollar = 0, 
   ...
   select @rcode = 0, @procname = 'bspPR_CA_NUT13'
   ...
   if @TCP is null select @TCP = 12455

   -- establish tax rate and constant
	               select @rate = .115, @KP = 6284
   if @A <= 135054 select @rate = .090, @KP = 2907
   if @A <=  83071 select @rate = .070, @KP = 1246
   if @A <=  41535 select @rate = .040, @KP = 0
...
GO
GRANT EXECUTE ON  [dbo].[bspPR_CA_NUT13] TO [public]
GO

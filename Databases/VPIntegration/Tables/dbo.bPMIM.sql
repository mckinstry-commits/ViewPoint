...
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bPMIM] ([KeyID]) ON [PRIMARY]
GO
CREATE UNIQUE CLUSTERED INDEX [biPMIM] ON [dbo].[bPMIM] ([PMCo], [Project], [Issue]) ON [PRIMARY]
GO
ALTER TABLE [dbo].[bPMIM] WITH NOCHECK ADD CONSTRAINT [FK_bPMIM_bJCJM] FOREIGN KEY ([PMCo], [Project]) REFERENCES [dbo].[bJCJM] ([JCCo], [Job])
GO
ALTER TABLE [dbo].[bPMIM] WITH NOCHECK ADD CONSTRAINT [FK_bPMIM_bPMPM] FOREIGN KEY ([VendorGroup], [FirmNumber], [Initiator]) REFERENCES [dbo].[bPMPM] ([VendorGroup], [FirmNumber], [ContactCode])
GO

...
[INStkUnitCost] [dbo].[bUnitCost] NOT NULL,
                  ...
                  if @fuelmatlcode = @material
                 	...
                 	if @stdum <> @fuelcapum
                      ...
                      select @conversion = Conversion
                 	  from HQMU
                 	  where MatlGroup = @matlgroup and Material = @fuelmatlcode and UM = @fuelcapum
    
                      if @@rowcount = 0
               	        begin
               	        select @errmsg = 'No units of measure conversion is set up for material ' + @material + ' '
               	        goto error
               	        end
                 	  end
                    else
                   ...
                   -- convert the units and update the table
    	           select @convunits = @units * @conversion
             	   update EMEM
                   set FuelUsed = FuelUsed - @convunits, UpdateYN = 'N' -- avoid HQ auditing
                   where EMCo = @emco and Equipment = @equipment
                   if @@rowcount = 0
              	     begin
                     select @errmsg = 'Error updating fuel in equipment master '
...
/**************************************************************
   ...
   -- retrieve GLCo from EMCo and validate EMCo
   ...
   -- validate EM Group
   if not exists(select top 1 1 from bHQGP with (nolock) where Grp = @emgroup)
   	begin
  ...
  --if @componenttypecode is not null -135655
  if isnull(@componenttypecode,'')<>''
   	begin
   	if not exists(select top 1 1 from bEMTY with (nolock) where EMGroup=@emgroup and ComponentTypeCode=@componenttypecode)
   	 	begin
   ...
   -- Validate cost Type
   if not exists(select top 1 1 from bEMCT with (nolock) where EMGroup=@emgroup and CostType=@emcosttype)
    	begin
   ...
   if @workorder is not null
   	begin
   	if not exists(select top 1 1 from bEMWH with (nolock) where EMCo=@emco and WorkOrder=@workorder)
   	 	begin
   ...
   -- PerECM
   if @unitprice <> 0 and @perecm not in ('E','C','M')
   	begin
...
**************************************************************/
   ...
   if update(EMCo) or Update(Mth) or Update(EMTrans)
    	begin
   ...
   BEGIN
   	select @validcnt = count(*) from bEMTY r JOIN inserted i ON i.EMGroup = r.EMGroup and i.ComponentTypeCode = r.ComponentTypeCode
   	select @validcnt2 = count(*) from inserted i Where ComponentTypeCode is null
   ...
   if Update(Component)
   BEGIN
   	select @validcnt = count(*) from bEMEM r JOIN inserted i ON i.EMCo = r.EMCo and i.Component = r.Equipment and r.Status in ('A', 'D')
   	select @validcnt2 = count(*) from inserted i Where i.Component is null
   	if @validcnt + @validcnt2 <> @numrows
   	 	begin
   	 	select @errmsg = 'Component is Invalid '
   	 	goto error
   ...
   if Update(CostCode)
   BEGIN
   	select @validcnt = count(*) from bEMCC r JOIN inserted i ON i.EMGroup = r.EMGroup and i.CostCode = r.CostCode
   	if @validcnt<> @numrows
   	 	begin
   	 	select @errmsg = 'Cost Code is Invalid '
   	 	goto error
   ...
   -- Validate cost Type
   if Update(EMCostType)
   BEGIN
   	select @validcnt = count(*) from bEMCT r JOIN inserted i ON i.EMGroup = r.EMGroup and i.EMCostType = r.CostType
   	if @validcnt<> @numrows
   	 	begin
   	 	select @errmsg = 'Cost Type is Invalid '
   	 	goto error
   ...
   if Update(WorkOrder)
   BEGIN
   	select @validcnt = count(*) from bEMWH r JOIN inserted i ON i.EMCo = r.EMCo and i.WorkOrder = r.WorkOrder
   ...
   END
	...
	---------------------------------------   
	-- CURSOR BEGIN - for future cleanup --
	---------------------------------------
----	if @numrows = 1
----		begin
...
----	else
----		begin
----		declare bEMCD_update cursor LOCAL FAST_FORWARD
----		for select BatchId, EMGroup, Equipment, Component, ComponentTypeCode, Asset, WorkOrder, WOItem, CostCode,
----				EMCostType, PostedDate, ActualDate, Source, EMTransType, Description, InUseBatchID,
----   	 			GLCo, GLTransAcct, GLOffsetAcct, ReversalStatus, PRCo, PREmployee, APCo, APTrans, APLine,
----				VendorGrp, APVendor, APRef, MatlGroup, INCo, INLocation, Material, SerialNo, UM, Units,
----   	 			Dollars, UnitPrice, PerECM, TotalCost, AllocCode, TaxCode, TaxGroup, TaxBasis, TaxRate, TaxAmount,
----   	 			CurrentHourMeter, CurrentTotalHourMeter, CurrentOdometer, CurrentTotalOdometer
----  			from inserted
----
----		open bEMCD_update
----
----		fetch next from bEMCD_update into @batchid, @emgroup, @equipment, @component, @componenttypecode,
----   	 			   @asset, @workorder, @woitem, @costcode, @emcosttype, @posteddate, @actualdate, @source,
----   	 			   @emtranstype, @description, @inusebatchid, @offsetglco, @gltransacct, @gloffsetacct, @reversalstatus,
----   	 			   @prco, @premployee, @apco, @aptrans, @apline, @vendorgrp, @apvendor, @apref, @matlgroup, @inco,
----   	 			   @inlocation, @material, @serialno, @um, @units, @dollars, @unitprice, @perecm, @totalcost, @alloccode,
----   	 			   @taxcode, @taxgroup, @taxbasis, @taxrate, @taxamount, @currenthourmeter, @currenttotalhourmeter,
----   	 			   @currentodometer, @currenttotalodometer
----		
----		if @@fetch_status <> 0
----   			begin
----   			select @errmsg = 'Cursor error'
...
----	if @unitprice<>0 and @perecm not in ('E','C','M')
----		begin
----		select @errmsg = 'PerECM '+ @perecm + ' is invalid'
----		GoTo error
----		End
----		
----	-- check GL TransAcct
...
GO
CREATE UNIQUE CLUSTERED INDEX [biEMCD] ON [dbo].[bEMCD] ([EMCo], [Equipment], [Mth], [EMTrans]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biEMCDEMTrans] ON [dbo].[bEMCD] ([EMCo], [Mth], [EMTrans]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biEMCDWorkOrderDelete] ON [dbo].[bEMCD] ([EMCo], [WorkOrder], [WOItem]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biEMCDCostCodeCT] ON [dbo].[bEMCD] ([EMGroup], [CostCode], [EMCostType]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bEMCD] ([KeyID]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biEMCDAttchID] ON [dbo].[bEMCD] ([UniqueAttchID]) ON [PRIMARY]
GO
EXEC sp_bindefault N'[dbo].[bdZero]', N'[dbo].[bEMCD].[INStkUnitCost]'
GO
EXEC sp_bindrule N'[dbo].[brECM]', N'[dbo].[bEMCD].[INStkECM]'
GO
EXEC sp_bindefault N'[dbo].[bdZero]', N'[dbo].[bEMCD].[UnitPrice]'
GO
EXEC sp_bindrule N'[dbo].[brECM]', N'[dbo].[bEMCD].[PerECM]'
GO

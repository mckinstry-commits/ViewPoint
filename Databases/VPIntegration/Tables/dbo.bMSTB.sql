	...
	if exists(select 1 from inserted where BatchTransType = 'A' and Equipment is not null and RevCode is not null
		...
		begin
			...
			attachment_check:
			   ...
			   if exists(select 1 from bEMEM with (nolock) where EMCo = @emco and AttachToEquip = @equipment and Status = 'A')
				   begin
						...
						while @attachment is not null
						   begin
							   select @attachpostrev = AttachPostRevenue, @emcategory = Category, @mstrucktype = MSTruckType --,@equipctype = UsageCostType,
							   ...
							   if @attachpostrev = 'Y'
								   begin
									   ...
									   if @rcode <> 0 select @revrate = 0
				   
										-- #20702 check for revenue based haul code
										set @revbased = 'N'
										select @revbased = RevBased from bMSHC with (nolock) where MSCo = @co and HaulCode = @haulcode
										-- #22227 - check for haul based revenue code
										set @haulbased = 'N'
										...
										select @co, @mth, @batchid, @seq, 'A', SaleDate, FromLoc, Ticket, VendorGroup,
											...
											(case @revbased when 'Y' then @revrate else HaulRate end),		
											(case @revbased when 'Y' then (RevBasis * @revrate) else 0 end),	
											null, 0, 0, 0, RevCode,
											-- #22227 - if rev code is haul based, init rev charge using haul code
											(case @haulbased when 'Y' then 0 else RevBasis end), --RevBasis,
											(case @haulbased when 'Y' then HaulRate else @revrate end), --@revrate,
											(case @haulbased when 'Y' then 0 else (RevBasis * @revrate) end), --(RevBasis * @revrate),
		...
		BEGIN
			...
			IF NOT EXISTS(SELECT 1 FROM bMSTB WITH (NOLOCK) WHERE Co = @co AND Mth = @mth 
				...
				BEGIN			
					...
					IF (@SurchargeKeyID IS NULL) AND (@Invoice IS NULL) --AND (@Changed = 'Y')
						BEGIN
...
GO
CREATE UNIQUE CLUSTERED INDEX [biMSTB] ON [dbo].[bMSTB] ([Co], [Mth], [BatchId], [BatchSeq]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bMSTB] ([KeyID]) ON [PRIMARY]
GO

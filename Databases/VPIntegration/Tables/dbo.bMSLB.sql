...
/*--------------------------------------------------------------
    ...
    if exists(select * from inserted i
        ...
        attachment_check:
            ...
            if exists(select * from bEMEM where EMCo = @emco and AttachToEquip = @equipment and Status = 'A')
                begin
                ...
                while @attachment is not null
                    ...
                    if @attachpostrev = 'Y'
    					...
    					set @revbased = 'N'
    					select @revbased = RevBased from bMSHC where MSCo = @co and HaulCode = @haulcode
    
   			-- -- -- issue #24722 for attachments get next for bMSLB not inserted
                       select @line = isnull(max(HaulLine),0) + 1 -- next available Haul Line
                        -- -- from inserted where Co = @co and Mth = @mth and BatchId = @batchid and BatchSeq = @seq
    			 from bMSLB where Co = @co and Mth = @mth and BatchId = @batchid and BatchSeq = @seq
                        ...
                        select @co, @mth, @batchid, @seq, @line, 'A', FromLoc, VendorGroup,
    						...
    						null, 0, 0, 0, @revcode, RevBasis, @revrate,
    						(RevBasis * @revrate), TaxGroup, TaxCode, TaxType, 0, 0, 0, DiscRate, 0, 0
                        from inserted
...
GO
CREATE UNIQUE CLUSTERED INDEX [biMSLB] ON [dbo].[bMSLB] ([Co], [Mth], [BatchId], [BatchSeq], [HaulLine]) ON [PRIMARY]
GO

...
[PreviousOdoReading] [dbo].[bHrs] NOT NULL,
[HourReading] [dbo].[bHrs] NOT NULL,
[PreviousHourReading] [dbo].[bHrs] NOT NULL,
[UM] [dbo].[bUM] NULL,
...
-- Validate TransType 
select @validcnt = count(*) from inserted i where i.TransType in ('J','E','X','W','C','I')
if @validcnt <> @numrows
	begin
	select @errmsg = 'Transaction Type must be one of: J, E, X, W, C, or I'
	goto error
	end

-- Validate Source
select @validcnt = count(*) from inserted i where i.Source in ('EMRev','PR', 'MS', 'SM')
if @validcnt <> @numrows
	begin
...
-- Validate Category based upon Used/Usage Equipment
select @validcnt = count(*) from inserted i
JOIN bEMCM c with (nolock) ON c.EMCo = i.EMCo and c.Category = i.Category
join bEMEM e with (nolock) on e.EMCo = i.EMCo and e.Equipment = i.Equipment and e.Category = c.Category
if @validcnt <> @numrows
	begin
...
-- Validate component type (Cost/Repaired component type)
select @validcnt = count(*) from bEMTY r with (nolock) 
JOIN inserted i ON i.UsedOnEquipGroup = r.EMGroup and i.UsedOnComponentType = r.ComponentTypeCode
select @validcnt2 = count(*) from inserted i Where i.UsedOnComponentType is null
if @validcnt + @validcnt2 <> @numrows
	begin
	select @errmsg = 'Component Type is Invalid '
	goto error
	end
...
-- Validate cost code base upon Cost/Repaired EMCo/EMGroup
select @validcnt = count(*) from bEMCC r with (nolock) 
JOIN inserted i ON i.UsedOnEquipGroup = r.EMGroup and i.EMCostCode = r.CostCode
select @validcnt2 = count(*) from inserted i Where i.EMCostCode is null
if @validcnt + @validcnt2 <> @numrows
	begin
	select @errmsg = 'Cost Code is Invalid '
	goto error
	end

-- Validate work order
select @validcnt = count(*) from bEMWH r with (nolock) 
JOIN inserted i ON i.UsedOnEquipCo = r.EMCo and i.WorkOrder = r.WorkOrder
select @validcnt2 = count(*) from inserted i Where i.WorkOrder is null
if @validcnt + @validcnt2 <> @numrows
	begin
	select @errmsg = 'Work Order is Invalid '
	goto error
	end
...
/**************************************************************
unrem the following code when IN and MS are ready
-- Validate MSCo
select @validcnt = count(*) from bMSCO r JOIN inserted i ON i.MSCo = r.MSCo
select @validcnt2 = count(*) from inserted i Where i.MSCo is null
if @validcnt + @validcnt2 <> @numrows
	begin
	select @errmsg = 'Material Sales Company is Invalid '
	goto error
	end

-- Validate INCo
select @validcnt = count(*) from bINCO r JOIN inserted i ON i.INCo = r.INCo
select @validcnt2 = count(*) from inserted i Where i.INCo is null
if @validcnt + @validcnt2 <> @numrows
	begin
	select @errmsg = 'Inventory Company is Invalid '
	goto error
	end

-- Validate From Location
select @validcnt = count(*) from bINLM r JOIN inserted i ON i.INCo = r.INCo and i.FromLoc = r.Loc
select @validcnt2 = count(*) from inserted i Where i.FromLoc is null
if @validcnt + @validcnt2 <> @numrows
	begin
	select @errmsg = 'Inventory From location is Invalid '
	goto error
	end

-- Validate To Location
select @validcnt = count(*) from bINLM r JOIN inserted i ON i.INCo = r.INCo and i.ToLoc = r.Loc
select @validcnt2 = count(*) from inserted i Where i.ToLoc is null
if @validcnt + @validcnt2 <> @numrows
	begin
	select @errmsg = 'Inventory To location is Invalid '
	goto error
	end
****************************************************************/
...
-- TV TaxDay 2004 23255 - Move the update out of the trigger.
-- update job information in bEMEM
-- do not update if interfaced from PR, update occurs from bPRTB triggers
/*if @transtype = 'J' and @source <> 'PR'	
and exists(select 1 from bEMEM with (nolock) where EMCo = @emco and Equipment = @equip and
			((JobDate is null) or (JobDate is not null and JobDate <= @actualdate)))
and not exists(select top 1 1 from bEMLH with (nolock) where EMCo = @emco and Equipment = @equip and DateIn >= @actualdate)
begin

 select @oldjob = (Select Job from bEMEM where EMCo = @emco and Equipment = @equip)

update bEMEM
 set JCCo = @jcco, 
     Job = case when @joblocflag <> 'N' then @job else Job end, 
     JobDate =  case when @job <> @oldjob then @actualdate else JobDate end, 
     Location = case when @joblocflag = 'U' and @job <> @oldjob then null else Location end, 
     UpdateYN = 'N' -- avoid HQ auditing
where EMCo = @emco and Equipment = @equip
if @@rowcount = 0
	begin
	select @errmsg = 'Error updating Equipment table'
	goto error
	end
...
declare @numrows int, @oldnumrows int, @errmsg varchar(255), @bemsg varchar(15),
     @errno tinyint, @audit bYN, @validcnt int, @validcnt2 int, @nullcnt int
...
if update(EMCo) or update(Mth) or update(Trans)
	...
	if update (Source)
		begin
		select @validcnt = count(*) from inserted i where i.Source in ('EMRev', 'MS')
		if @validcnt <> @numrows
			begin
			select @errmsg = 'Invalid Source '
			goto error
     ...
     if update(Category)
		 begin
		 select @validcnt = count(*) from inserted i
		 JOIN bEMCM c ON c.EMCo = i.EMCo and c.Category = i.Category
		 join bEMEM e on e.EMCo = i.EMCo and e.Equipment = i.Equipment and e.Category = c.Category
		  if @validcnt <> @numrows
			begin
			select @errmsg = 'Category is Invalid '
			goto error
     ...
     /* Validate component type (Cost/Repaired component type) */
     if update(UsedOnComponentType)
		 begin
		 select @validcnt = count(*) from bEMTY r JOIN inserted i ON i.UsedOnEquipGroup = r.EMGroup and i.UsedOnComponentType = r.ComponentTypeCode
		 select @validcnt2 = count(*) from inserted i Where i.UsedOnComponentType is null
		 if @validcnt + @validcnt2 <> @numrows
			begin
			select @errmsg = 'Component Type is Invalid '
     ...
     /* Validate cost code base upon Cost/Repaired EMCo/EMGroup */
     if update(EMCostCode)
		 Begin
		 select @validcnt = count(*) from bEMCC r JOIN inserted i ON i.UsedOnEquipGroup = r.EMGroup and i.EMCostCode = r.CostCode
		 select @validcnt2 = count(*) from inserted i Where i.EMCostCode is null
		 if @validcnt + @validcnt2 <> @numrows
			begin
			select @errmsg = 'Cost Code is Invalid '
     ...
     if update(WorkOrder)
		 begin
		 select @validcnt = count(*) from bEMWH r JOIN inserted i ON i.UsedOnEquipCo = r.EMCo and i.WorkOrder = r.WorkOrder
		 select @validcnt2 = count(*) from inserted i Where i.WorkOrder is null
		 if @validcnt + @validcnt2 <> @numrows
			begin
			select @errmsg = 'Work Order is Invalid '
...
--  End SPECIAL Cost/Repaired item validation
			...
			*/
			...
			if @oldtranstype = 'E' and @transtype <> 'E'
   			...
   			--TV 04/14/04 23255 Changed
			/* update job information in EMEM 
			if @transtype = 'J' and
				@source <> 'PR'	-- last job updated from bPRTB triggers
					and exists(select * from bEMEM where EMCo = @emco and Equipment = @equip and
						((JobDate is null) or (JobDate is not null and JobDate <= @actualdate)))
					and not exists(select * from bEMLH where EMCo = @emco and Equipment = @equip and DateIn >= @actualdate)
...
GO
CREATE UNIQUE CLUSTERED INDEX [biEMRD] ON [dbo].[bEMRD] ([EMCo], [Equipment], [Mth], [Trans]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biEMRDTrans] ON [dbo].[bEMRD] ([EMCo], [Mth], [Trans]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bEMRD] ([KeyID]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [IX_bEMRD_MthTransEquipmentUniqueAttach] ON [dbo].[bEMRD] ([Mth], [EMCo], [Trans], [Equipment], [UniqueAttchID]) ON [PRIMARY]
GO
CREATE NONCLUSTERED INDEX [biEMRDAttchID] ON [dbo].[bEMRD] ([UniqueAttchID]) ON [PRIMARY]
GO
...
GO
EXEC sp_bindefault N'[dbo].[bdZero]', N'[dbo].[bEMRD].[OdoReading]'
GO
EXEC sp_bindefault N'[dbo].[bdZero]', N'[dbo].[bEMRD].[PreviousOdoReading]'
GO
EXEC sp_bindefault N'[dbo].[bdZero]', N'[dbo].[bEMRD].[HourReading]'
GO
EXEC sp_bindefault N'[dbo].[bdZero]', N'[dbo].[bEMRD].[PreviousHourReading]'
GO

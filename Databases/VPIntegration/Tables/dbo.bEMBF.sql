...
*				GF 03/11/2013 TFS-43067 remmed out code to delete EMBC and EMGL
*
*
...
----TK-20889 clear EMBC records for batch sequence if exists
--DELETE dbo.bEMBC FROM dbo.bEMBC b INNER JOIN deleted d ON b.EMCo=d.Co AND b.Mth=d.Mth AND b.BatchId=d.BatchId AND b.BatchSeq=d.BatchSeq
--DELETE dbo.bEMGL FROM dbo.bEMGL g INNER JOIN deleted d ON g.EMCo=d.Co AND g.Mth=d.Mth AND g.BatchId=d.BatchId AND g.BatchSeq=d.BatchSeq
...
if @embfsource = 'EMMeter' 
	...
	EMMR if next meter reading isn't in current batch as new or changed record, update will occur in batch post process or (EMMR Trigger)*/
	exec @rcode = dbo.vspEMBFUpdateNextBatchSeq @co, @mth, @batchid,@batchseq, @batchtranstype, @emtrans,
				'N', @equip, @actualdate, @currhourmeter, @currodometer, @errtext output
    IF @rcode <> 0
		BEGIN
     	SELECT @errmsg = @errtext, @rcode = 1
     	GOTO error
...
GO
CREATE UNIQUE CLUSTERED INDEX [biEMBF] ON [dbo].[bEMBF] ([Co], [Mth], [BatchId], [BatchSeq]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bEMBF] ([KeyID]) ON [PRIMARY]
GO

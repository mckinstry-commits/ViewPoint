...
GO
   ...
   /****** Object:  Trigger dbo.btEMLMi    Script Date: 8/28/99 9:37:18 AM ******/
   CREATE  trigger [dbo].[btEMLMi] on [dbo].[bEMLM] for insert as
   

/*--------------------------------------------------------------
    *
    *  Insert trigger for EMLM
    *  Created By:  bc  06/15/99
    *  Modified by:  TV 02/11/04 - 23061 added isnulls
    *
    *
   ...
   error:
      select @errmsg = isnull(@errmsg,'') + ' - cannot insert into EMLM'
      RAISERROR(@errmsg, 11, -1);
      rollback transaction
...
GO
CREATE UNIQUE CLUSTERED INDEX [biEMLM] ON [dbo].[bEMLM] ([EMCo], [EMLoc]) ON [PRIMARY]
GO
CREATE UNIQUE NONCLUSTERED INDEX [biKeyID] ON [dbo].[bEMLM] ([KeyID]) ON [PRIMARY]
GO

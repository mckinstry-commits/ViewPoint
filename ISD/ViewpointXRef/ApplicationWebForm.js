...
function detailRolloverPopup(aTitle, aCloseBtnText, aContent, aPersist, aWidth, aHeight, aIsScrollable) {
    ...
    */
    ...
    // only need to correct offset in initial popup (i.e., correction not needed in nested popups)
    if ((gEventClientY > 135) && (gEventClientX > 155)) {
        gEventClientY -= 135;
        gEventClientX -= 155;
    }
...
function QPopupCreateHeader() {
    ...
    if (lQSContainers.length > 0) {
        var lHeaders = lQSContainers[0].getElementsByClassName("ms-rteThemeBackColor-3-0 dh");
        if (lHeaders.length > 0) {
            if (lHeaders[0].className == "ms-rteThemeBackColor-3-0 dh") {
                // position header of first panel as header of QuickPopup
                lHeaders[0].className = "ms-rteThemeBackColor-3-0 dh2";
            }
        }
    }
}

    ...
    public class BaseApplicationPage : BaseClasses.Web.UI.BasePage
    {
        ...
        private void Page_PreInit(object sender, System.EventArgs e)
        {
           ...
           if (BaseClasses.Configuration.ApplicationSettings.Current.CurrentSharePointFunctions == null) {
              BaseClasses.Configuration.ApplicationSettings.Current.CurrentSharePointFunctions = new SharePointUtils();
           }
       ...
       private const string SCRIPT_DOFOCUS = "" + "<script language=\"javascript\" type=\"text/javascript\">" + 
        ...
        public string GetResourceValue(string keyVal, string appName, bool checkCulture)
        {
            if (checkCulture && System.Web.HttpContext.Current != null && System.Web.HttpContext.Current.Session != null)
            {
                string selectedLanguage = (string)System.Web.HttpContext.Current.Session["AppCultureUI"];
                if (!string.IsNullOrEmpty(selectedLanguage))
                {
                    if (!string.Equals(selectedLanguage, System.Threading.Thread.CurrentThread.CurrentUICulture.Name, StringComparison.InvariantCultureIgnoreCase))
                    {
                        System.Globalization.CultureInfo culInfo = new System.Globalization.CultureInfo(selectedLanguage);
                        if ((culInfo != null))
                        {
                            System.Threading.Thread.CurrentThread.CurrentUICulture = culInfo;
                        }
                    }
                }
            }

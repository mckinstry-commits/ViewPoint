...
namespace VPLookup.UI.Controls.Show_MvwISDVendorXref_Table_Mobile
{
...
public class BaseMvwISDVendorXrefTableControl : VPLookup.UI.BaseApplicationTableControl
{
      ...
      cols.Add(MvwISDVendorXrefView.Zip);
        ...
        public virtual string[] GetAutoCompletionList_SearchText(String prefixText,int count)
        {
            ...
            foreach (MvwISDVendorXrefRecord rec in recordList ){
                ...
                if (resultItem != null) {
                    ...
                    if (!string.IsNullOrEmpty(prText) && resultItem.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture).Contains(prText.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture))) {
                        bool isAdded = FormatSuggestions(prText, resultItem, 20, "InMiddleOfMatchedString", "WordsStartingWithSearchString", "[^a-zA-Z0-9]", resultList, MvwISDVendorXrefView.VPVendor.IsFullTextSearchable);
                        if (isAdded) {
                            continue;
                        }
                    }
                ...
                if (resultItem != null) {
                    ...
                    if (!string.IsNullOrEmpty(prText) && resultItem.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture).Contains(prText.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture))) {
                        bool isAdded = FormatSuggestions(prText, resultItem, 20, "InMiddleOfMatchedString", "WordsStartingWithSearchString", "[^a-zA-Z0-9]", resultList, MvwISDVendorXrefView.CGCVendor.IsFullTextSearchable);
                        if (isAdded) {
                            continue;
                        }
                    }
                ...
                if (resultItem != null) {
                    ...
                    if (!string.IsNullOrEmpty(prText) && resultItem.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture).Contains(prText.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture))) {
                        bool isAdded = FormatSuggestions(prText, resultItem, 20, "InMiddleOfMatchedString", "WordsStartingWithSearchString", "[^a-zA-Z0-9]", resultList, MvwISDVendorXrefView.City.IsFullTextSearchable);
                        if (isAdded) {
                            continue;
                        }
                    }
                ...
                resultItem = rec.Format(MvwISDVendorXrefView.Zip);
  
                if (resultItem != null) {
                    string prText = prefixText;
                    if(MvwISDVendorXrefView.Zip.IsFullTextSearchable) {
                        FullTextExpression ft = new FullTextExpression();
                        prText = ft.GetFirstNonExcludedTerm(prText);
                    }
                    if (!string.IsNullOrEmpty(prText) && resultItem.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture).Contains(prText.ToUpper(System.Threading.Thread.CurrentThread.CurrentCulture))) {
                        bool isAdded = FormatSuggestions(prText, resultItem, 20, "InMiddleOfMatchedString", "WordsStartingWithSearchString", "[^a-zA-Z0-9]", resultList, MvwISDVendorXrefView.Zip.IsFullTextSearchable);
                        if (isAdded) {
                            continue;
                        }
                    }
...
#region "Helper Functions"
        ...
        public virtual MvwISDVendorXrefTableControlRow[] GetSelectedRecordControls()
        {
            ...
            return (MvwISDVendorXrefTableControlRow[])(selectedList.ToArray(Type.GetType("VPLookup.UI.Controls.Show_MvwISDVendorXref_Table_Mobile.MvwISDVendorXrefTableControlRow")));
        ...
        public MvwISDVendorXrefTableControlRow[] GetRecordControls()
        {
            ...
            return (MvwISDVendorXrefTableControlRow[])recordList.ToArray(Type.GetType("VPLookup.UI.Controls.Show_MvwISDVendorXref_Table_Mobile.MvwISDVendorXrefTableControlRow"));
        }

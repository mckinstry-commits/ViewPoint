...
UPDATE @rettable 
SET 
	ContractDesc=jccm.Description
,	ContractPOC=jcmp.Name
FROM
	JCCM jccm 
LEFT OUTER JOIN JCMP jcmp ON
	jccm.JCCo=jcmp.JCCo
AND jccm.udPOC=jcmp.ProjectMgr
WHERE 
	jccm.JCCo=[@rettable].JCCo
AND jccm.Contract=[@rettable].Contract

UPDATE @rettable 
SET 
	ContractItemDesc=jcci.Description
,	GLDepartment=glpi.Instance
,	GLDepartmentName=glpi.Description
FROM
	JCCI jcci JOIN
	JCDM jcdm ON jcci.Department=jcdm.Department AND jcci.JCCo=jcdm.JCCo LEFT OUTER JOIN
	GLPI glpi ON jcdm.GLCo=glpi.GLCo AND glpi.PartNo=3 AND glpi.Instance=SUBSTRING(jcdm.OpenRevAcct,10,4)
WHERE 
	jcci.JCCo=[@rettable].JCCo
AND jcci.Contract=[@rettable].Contract
AND jcci.Item=[@rettable].ContractItem

UPDATE @rettable
SET 
	SMServiceSite=smsite.Description
,	SMWOType=smsite.Type
,	SMWOTypeDesc=
	CASE 
		WHEN [@rettable].SMWorkOrder IS NOT NULL AND smsite.Type='Job' THEN 'PM/SPG'
		WHEN [@rettable].SMWorkOrder IS NOT NULL AND smsite.Type<>'Job' THEN 'BreakFix'
		ELSE NULL
    END
,	Job=COALESCE([@rettable].Job,smsite.Job)
FROM
	SMWorkOrder smwo JOIN
	dbo.SMServiceSite smsite ON
		smwo.SMCo=smsite.SMCo
	AND smwo.ServiceSite=smsite.ServiceSite
WHERE
	smwo.SMCo=[@rettable].SMCo
AND smwo.WorkOrder=[@rettable].SMWorkOrder
...
SELECT * FROM mers.mfnARDetail(null,'3/1/2015','3/1/2015',NULL,NULL) 
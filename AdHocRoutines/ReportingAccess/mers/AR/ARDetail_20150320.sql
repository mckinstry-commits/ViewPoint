...
if @ThroughDate is null
	select @ThroughDate=getdate();

if @AgeDate is null
	select @AgeDate=getdate();
...
(
	...
	WHERE 
		( arth.ARCo < 100 )
	AND	( arth.ARCo=@ARCo OR @ARCo IS NULL)
	...
	and ( arth.TransDate <= @ThroughDate or @ThroughDate is null )
	and ( arth.DueDate is not null )
)
INSERT @retTable 
(
	ARCo			--bCompany	NOT NULL
...
,	CASE ar_txn.AmountDue	
	...
	END AS AgeAmount
,	CASE 
		WHEN DATEDIFF(day,isnull(arth.DueDate,arth.TransDate), @AgeDate) < 0  /* AND ar_txn.AmountDue <> 0 */ THEN SUM(artl.Amount)-SUM(artl.DiscOffered)-SUM(artl.Retainage) 
		ELSE 0
	END AS DueFuture
,	CASE 
		WHEN DATEDIFF(day,isnull(arth.DueDate,arth.TransDate), @AgeDate) >= 0 AND DATEDIFF(day,isnull(arth.DueDate,arth.TransDate), @AgeDate) < 30 /* AND ar_txn.AmountDue <> 0 */ THEN SUM(artl.Amount)-SUM(artl.DiscOffered)-SUM(artl.Retainage) 
		ELSE 0
	END AS DueCurrent
,	CASE
		WHEN DATEDIFF(day,isnull(arth.DueDate,arth.TransDate), @AgeDate) >=30 and DATEDIFF(day,isnull(arth.DueDate,arth.TransDate), @AgeDate) < 60 /* AND ar_txn.AmountDue <> 0 */ THEN SUM(artl.Amount)-SUM(artl.DiscOffered)-SUM(artl.Retainage)
		ELSE 0
	END AS Due30to60
,	CASE 
		WHEN DATEDIFF(day,isnull(arth.DueDate,arth.TransDate), @AgeDate) >= 60 and DATEDIFF(day,isnull(arth.DueDate,arth.TransDate), @AgeDate) < 90 /* AND ar_txn.AmountDue <> 0 */  THEN SUM(artl.Amount)-SUM(artl.DiscOffered)-SUM(artl.Retainage)
		ELSE 0
	END AS Due60to90
,	CASE 
		WHEN DATEDIFF(day,isnull(arth.DueDate,arth.TransDate), @AgeDate) >= 90 and DATEDIFF(day,isnull(arth.DueDate,arth.TransDate), @AgeDate) < 120 /* AND ar_txn.AmountDue <> 0 */  THEN SUM(artl.Amount)-SUM(artl.DiscOffered)-SUM(artl.Retainage) 
		ELSE 0
	END AS Due90to120
--,	0 AS Due90to120
,	CASE 
...
GRANT SELECT ON  mers.mfnARDetail_temp TO PUBLIC
go
...
SELECT * FROM  mers.mfnARDetail_temp(@ARCo,@Customer,@Contract,@Invoice,@ThroughDate,@AgeDate)
...
--SELECT * FROM  mers.mfnARDetail_temp(@ARCo,@Customer,@Contract,@Invoice,@AgeDate) ORDER BY Invoice
	...
	AND ( arth.Customer=@Customer OR @Customer IS NULL)
	AND ( artl.Contract=@Contract OR @Contract IS NULL)
	AND ( arth.Invoice IS NOT NULL AND (arth.Invoice=@Invoice OR @Invoice IS NULL) )
	*/

...
--sp_rename bspAPLBVal,bspAPLBVal_Original
--go
...
*****************************************************/
  ...
  set nocount on
	...
	SELECT @APHBCheckRevYN = ChkRev, @APHBUIMth = UIMth, @APHBUISeq=UISeq	FROM dbo.APHB WHERE Co = @apco AND Mth = @mth AND BatchId = @batchid AND BatchSeq = @batchseq
...
APLB_loop:      -- loop through each line
	...
	' Line: ' + isnull(convert(varchar(6),@apline), '') + ' '  + @APHBUIMsg --#23061
...
GRANT EXEC ON [bspAPLBVal] TO PUBLIC
go

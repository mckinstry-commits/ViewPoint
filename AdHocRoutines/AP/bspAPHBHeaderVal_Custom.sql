...
--sp_rename bspAPHBHeaderVal,bspAPHBHeaderVal_Original
--go


ALTER   procedure [dbo].[bspAPHBHeaderVal]
/***********************************************************
...
SELECT @APHBUIMth = UIMth, @APHBUISeq=UISeq	FROM dbo.APHB WHERE Co = @co AND Mth = @mth AND BatchId = @batchid AND BatchSeq = @batchid
...
BEGIN
    ...
    BEGIN
		...
		SELECT @errortext=@errortext+' '+coalesce(@APHBUIMsg,'')
...
GRANT EXEC ON bspAPHBHeaderVal TO PUBLIC
GO

--SELECT OBJECT_NAME(id),* FROM syscomments WHERE text LIKE '%type is only%'
--SELECT OBJECT_NAME(id),* FROM syscomments WHERE text LIKE '%bspAPHBHeaderVal%'
--SELECT OBJECT_NAME(id),* FROM syscomments WHERE text LIKE '%bspAPHBVal%'

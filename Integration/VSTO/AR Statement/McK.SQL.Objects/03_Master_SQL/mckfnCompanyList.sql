...
AS
 /* 
	...
	2019.04.02 LG - add FedTaxId
	2019.03.11 LG - init
*/
RETURN
(
	SELECT CAST (HQCo as Varchar) + '-' + Name AS CompanyName, HQCo, FedTaxId FROM dbo.HQCO WITH (NOLOCK) WHERE udTESTCo = 'N' AND (HQCo = 1 OR HQCo = 20)
)
...
--  SELECT CompanyName, HQCo, FedTaxId FROM dbo.mckfnCompanyList()
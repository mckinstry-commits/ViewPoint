...
AS
 /* 
	...
	07.10.2019 LG - get McKinstry's email and phone ONLY if invoice is "Work Order", agrmts are handled separately 
...
BEGIN TRY
	...
	Declare @BillToCustomer bCustomer = 214934	
	Declare @InvoiceNumber	VARCHAR(10)	= '  10075087'
	Declare @PrintStatus   CHAR(1) = NULL -- 'N'			/* <-- NOT DELIVERED */
*/	
	...
	DECLARE @invoiceType CHAR(1) = (SELECT TOP 1 I.InvoiceType From dbo.SMInvoiceList I 
											WHERE I.Customer = @BillToCustomer
...
--ELSE	-- agreement TFS 4797
 /* moving this to different functionality; this can be multidivision which means multiple rows; UI needs to handle via a drop down combobox*/
END
...
BEGIN
	...
	FETCH NEXT FROM delivery_cursor Into @Bill, @division, @InvoiceNumber, @sendFrom, @DeliveredDate, @DeliveryMethod, @DeliveryTo, @Customer, @CustomerName, @BillTo, @BillToName, @EMail, @CCBillEmail, @Address1, @Address2, @City, @State, @Zip, 
										 @InvoiceSummaryLevel, @udReqWOwithBilling, @udReqInspectionRptithBilling, @udSignOffReq, @udLienRelease, @udCertifiedPayroll, @SMCo, @serviceSite, @workOrders, @BillingPhone, @CustGroup
	 WHILE (@@FETCH_STATUS <> -1)
		...
		FETCH NEXT FROM delivery_cursor INTO @Bill, @division, @InvoiceNumber, @sendFrom, @DeliveredDate, @DeliveryMethod, @DeliveryTo, @Customer, @CustomerName, @BillTo, @BillTo, @EMail, @CCBillEmail, @Address1, @Address2, @City, @State, @Zip, 
											 @InvoiceSummaryLevel, @udReqWOwithBilling, @udReqInspectionRptithBilling, @udSignOffReq, @udLienRelease, @udCertifiedPayroll, @SMCo, @serviceSite, @workOrders, @BillingPhone, @CustGroup
		END
...
exec dbo.MCKspSMRecipients 200000,'  10053200'

exec dbo.MCKspSMRecipients 200000,'  10053196'

exec dbo.MCKspSMRecipients 207669,'  10053194'

exec dbo.MCKspSMRecipients 216685,'  10053250'

exec dbo.MCKspSMRecipients 247706,'  10053260'

exec dbo.MCKspSMRecipients 213409,'  2209517'

exec dbo.MCKspSMRecipients 239483,'  2038917'

exec dbo.MCKspSMRecipients 219564,'  10053442'

exec dbo.MCKspSMRecipients 211507,'  10057408'
...
exec dbo.MCKspSMRecipients 214393,'  10080839'

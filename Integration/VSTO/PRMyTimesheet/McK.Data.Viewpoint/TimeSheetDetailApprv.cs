...
namespace McK.Data.Viewpoint
{
    public static partial class TimesheetDetail
    {
        ...
        /// <returns></returns>
        public static List<dynamic> GetMyTimesheetsApprv(byte? prco, byte? prgroup, dynamic prenddate, bool? posted = null, dynamic payseqincl = null, dynamic payseqexcl = null)
        {
            ...
            SqlParameter _posted = new SqlParameter("@Posted", SqlDbType.Bit)
            {
                Value = posted ?? (object)DBNull.Value
            };
            ...
            try
            {
                using (SqlConnection _conn = new SqlConnection(HelperData._conn_string))
                {
                    ...
                    using (var _cmd = new SqlCommand("Select * from mckfnPRMyTimesheetApprv(@PRCo, @PRGroup, @PREndDate, @Posted, @PaySeqIncl, @PaySeqExcl)", _conn))
                    {
                        _cmd.CommandTimeout = 600;
                        _cmd.Parameters.Add(_prco);
                        _cmd.Parameters.Add(_prgroup);
                        _cmd.Parameters.Add(_prenddate);
                        _cmd.Parameters.Add(_posted);

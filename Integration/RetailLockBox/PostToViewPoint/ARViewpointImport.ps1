...
[System.Collections.Hashtable]$ProcParameterDataMappings = @{"@Company"="Company";"@Customer"="Customer";"@CheckAmount"="CollectedCheckAmount";"@CustomerGroup"="CustGroup";"@InvoiceNumber"="InvoiceNumber";"@CheckNumber"="CollectedCheckNumber";"@CheckDate"="CollectedCheckDate";"@ImageFileName"="CollectedImage"} 
...
$ProcOutputValueMessageMappings.Add(1,"Batch, Header and Attachment records created successfully.  Attachment exists.")
$ProcOutputValueMessageMappings.Add(2,"Header and Attachment records created successfully.")
$ProcOutputValueMessageMappings.Add(3,"Header and Attachment records created successfully.  Attachment exists.")
...
function FetchLastModifiedImageDate
{
    [CmdletBinding()]
    param (
     ...
     begin
     {
        ...
        [string]$CurrentDate = Get-Date -Format d
     }
     process
     {
        try
        {
            if ((Test-Path $FilePath) -eq $true) {
                $PDFFile = Get-ChildItem -Path $FilePath -Recurse | Where { $_.Extension -eq ".pdf"  } | Sort LastWriteTime -Descending | Select-Object -first 1
                if ($PDFFile -ne $null) {
                    return $PDFFile.LastWriteTime.ToShortDateString()
                }
            }
        }
        catch [Exception]
    ...
    param (
     ...
     process
     {
        try
        {
            ...
            $SqlCommand.Parameters.Add("@TransactionDate",  [System.Data.SqlDbType]::SmallDateTime) | Out-Null
            $SqlCommand.Parameters["@TransactionDate"].Direction = [System.Data.ParameterDirection]::Input;
        ...
        foreach ($ZIPFile in $ZIPFiles) {
            try
            {
                ...
                if ($ZipUnpackaged -eq $true) {
                    # Fetch greatest modified image file date for transatcion date
                    [string]$TransactionDate = FetchLastModifiedImageDate($UnpackedFilePath)
                    # Fetch all CSV files from unpacked files
                    $DataFiles = Get-ChildItem -Path $UnpackedFilePath | Where { $_.Extension -eq ".csv"  }
                    foreach ($DataFile in $DataFiles) {
                        $File = Import-Csv -LiteralPath $DataFile.FullName | Sort-Object {[int] $_.Company}
                        if ($File -ne $null) {
                            ...
                            foreach ($DataRow in $File) {
                                if ((ValidateDataRow $DataRow) -eq $true) {
                                    $OutputValue = UploadDataRowAR $DataRow $TransactionDate
                                    ...
                                    if ($ShouldCopyImage -eq $true) {
                                        ...
                                        $FileCopied = CopyImageFile $ImageFilePath $DataRow.Company $Setting.Module $Setting.Form $TransactionDate
                                    }
                                }
                                else {

...
-- Common variables
DECLARE @rcode int, @UISeq int, @HeaderParamsCheck bit, @HeaderCheck bit, @UIMonth [dbo].[bMonth]

SELECT @rcode = -1, @RetVal = -1, @HeaderParamsCheck=1, @HeaderCheck=1

SELECT @UIMonth = [dbo].[vfFirstDayOfMonth] (GETDATE())
...
BEGIN TRY
	...
	EXECUTE @rcode = [dbo].[mckspAPUIAddNew] @Company, @UIMonth, @VendorGroup, @Vendor, @APRef,
		@Description, @InvDate, @InvTotal, @udFreightCost, @HeaderKeyID OUTPUT, @UISeq OUTPUT

...
DECLARE @JCCo [dbo].[bCompany], @Job [dbo].[bJob], @PhaseGroup [dbo].[bGroup], @Phase [dbo].[bPhase], 
	...
	@Discount [dbo].[bDollar], @SMCo [dbo].[bCompany], @SMWorkOrder int, @SMScope int, @SMJCCostType [dbo].[bJCCType], @SMPhase [dbo].[bPhase], @SMPhaseGroup bGroup
	, @SMCostType bJCCType
...
BEGIN
	...
	BEGIN
		...
		PO.Description, PO.UM, ISNULL(PO.RemUnits,0), ISNULL(PO.CurUnitCost,0), PO.CurECM, PO.TaxGroup, PO.TaxCode, PO.TaxType, PO.SMCo, PO.SMWorkOrder, PO.SMScope, PO.SMJCCostType
			--ADDING MISSING SM FIELDS
			, PO.SMPhaseGroup, PO.SMPhase, sct.SMCostType
		FROM   POIT PO
			--ADDED LOGIC FOR MISSING SM FIELDS
			--Changed the logic as SMWorkOrders are getting umatched because of cost type mismatch
			--LEFT JOIN dbo.SMCostType sct ON sct.SMCo = PO.SMCo AND sct.PhaseGroup = PO.PhaseGroup AND sct.JCCostType = PO.SMJCCostType
			LEFT JOIN dbo.SMWorkCompleted sct on sct.SMCo = PO.SMCo	and sct.PO = PO.PO and sct.POItem = PO.POItem
		WHERE PO.POCo = @Company 
			AND PO.PO = (
				SELECT PO FROM POHD 
				WHERE POCo = @Company 
					AND (udMCKPONumber = @Number OR PO = @Number)
				)
		...
		OPEN @GetPO
			...
			FROM @GetPO INTO @PO, @POItem, @PayType, @ItemType, @JCCo, @Job, @PhaseGroup, @Phase, @JCCType, @GLCo, @GLAcct, @Description, @UM, @Units, @UnitCost, @ECM, @TaxGroup, @TaxCode, @TaxType, @SMCo, @SMWorkOrder, @SMScope, @SMJCCostType
				--ADDING MISSING SM FIELDS
				, @SMPhaseGroup, @SMPhase, @SMCostType
			WHILE @@FETCH_STATUS = 0
			BEGIN
				SET @Line = @Line + 1
				INSERT INTO [dbo].[bAPUL] (

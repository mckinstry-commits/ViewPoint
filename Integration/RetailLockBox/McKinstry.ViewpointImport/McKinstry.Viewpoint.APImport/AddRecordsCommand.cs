...
namespace McKinstry.Viewpoint.APImport
{
    internal class AddRecordsCommand : ICommand
    {
        ...
        public string Description 
        {
            get
            {
                return "Adds unprocessed AP records to the viewpoint database. Adds AP Upload record to the integration database.  Updates detail records";
            }
        }
        ...
        public void RunWith(ILog log, string fileName, RLBImportBatch batch, APImportFile file)
        {
            ...
            foreach (var record in records)
            {
                ...
                try
                {
                    ...
                    int recordID = APImportDb.CreateAPImportRecord(detailRecordID, results, headerRecord, attachmentRecord, fileCopied, processNoteID);
                    log.InfoFormat("Adding AP Import Record to Integration.  Success: {0}. ID: {1}.", recordID > 0, recordID);

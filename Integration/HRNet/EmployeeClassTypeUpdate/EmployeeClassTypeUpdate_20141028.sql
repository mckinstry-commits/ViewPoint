...
,	p.PEOPLE_ID
FROM 
	[DEV-HRISSQL02].HRNET.dbo.PEOPLE p JOIN
	[DEV-HRISSQL02].HRNET.dbo.JOBDETAIL jd ON
		p.PEOPLE_ID=jd.PEOPLE_ID LEFT OUTER JOIN
	[DEV-HRISSQL02].HRNET.dbo.COMPANY c ON
		jd.COMPANY=c.COMPANY_ID LEFT OUTER JOIN
	[DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTLANGUAGES pll ON
		pll.CULTURECODE='en-US'
	AND pll.SHORTDESCRIPTION='Union Affiliation' LEFT OUTER JOIN
	[DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTVALUES pl ON
...
AND (p.REFERENCENUMBER=@Employee OR @Employee IS NULL)
ORDER BY
	1,2
FOR READ ONLY
...
SELECT @unionlid=PICKLISTID FROM [DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTLANGUAGES pll WHERE pll.CULTURECODE='en-US' AND pll.SHORTDESCRIPTION='Union Affiliation'
SELECT @hrclasstypeplid=PICKLISTID FROM [DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTLANGUAGES pll WHERE pll.CULTURECODE='en-US' AND pll.SHORTDESCRIPTION='Standard Cost Code'
	
--Update existing Picklist values as CGC Legacy values
UPDATE [DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTVALUES 
...
BEGIN
	...
	--	IF NOT EXISTS (SELECT 1 FROM [DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTVALUES plv WHERE plv.PICKLISTID=@hrclasstypeplid AND plv.STOREVALUE=@EmployeeClass + '.' + @EmployeeType )
	--	BEGIN
	--		INSERT [DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTVALUES ( PICKLISTID, CULTURECODE, STOREVALUE, DISPLAYVALUE )
	--		SELECT 
	--			PICKLISTID
	--		,	CULTURECODE
	...
	--		FROM
	--			[DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTLANGUAGES pll
	--		WHERE
	...
	--	UPDATE [DEV-HRISSQL02].HRNET.dbo.JOBDETAIL SET COSTCODE=@EmployeeClass + '.' + @EmployeeType WHERE JOBDETAIL_ID=@jobdetailid AND COSTCODE <> @EmployeeClass + '.' + @EmployeeType
	--END
	--ELSE
	--BEGIN 
	...
	--BEGIN 
	--	IF NOT EXISTS (SELECT 1 FROM [DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTVALUES plv WHERE plv.PICKLISTID=@unionlid AND plv.STOREVALUE=@UNIONNUMBER )
	--	BEGIN
	--		INSERT [DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTVALUES ( PICKLISTID, CULTURECODE, STOREVALUE, DISPLAYVALUE )
	--		SELECT 
	--			PICKLISTID
	...
	--	UPDATE [DEV-HRISSQL02].HRNET.dbo.PEOPLE SET UNIONTYPE=@UNIONNUMBER WHERE PEOPLE_ID=@peopleid AND UNIONTYPE <> @UNIONNUMBER
	--END
	--ELSE
	--BEGIN
	...
	SELECT @VPCraft=Craft, @VPClass=Class, @VPCraftClassDesc=Description from budxrefUnion WHERE Company=@CompanyNumber AND CMSClass=@EmployeeClass AND CMSType=@EmployeeType AND CAST(CMSUnion AS INT)=cast(COALESCE(@UNIONNUMBER,'0001') AS INT)
	...
	--SELECT * FROM budxrefUnion WHERE Company=@CompanyNumber AND CMSClass=@EmployeeClass AND CMSType=@EmployeeType and cast(CMSUnion as int)=cast(COALESCE(@UNIONNUMBER,'0001') AS INT)


--Co        Emp#      EmpName                                                     Union     ClassType                Class     Type      UnionId   UnionName                     Union#    
--20        321       Axel Walker                                                 N         501SU                    501SU     [?]       0001      z0001 - Staff - Non Union     0001      
--20        321       Axel Walker                                                 N         501SU                    501SU     SU        0001      z0001 - Staff - Non Union     0001      
--           Viewpoint Craft/Class: Not FOUND
           
	--SELECT * FROM dbo.budxrefUnion
	...
	PRINT 
		CAST(COALESCE(@VPCraft,'[?]') AS CHAR(10))		--int
	+	CAST(COALESCE(@VPCraftDesc,'[?]') AS CHAR(30))	--int
	+	CAST(COALESCE(@VPClass,'[?]') AS CHAR(10))			--VARCHAR(100)
	+	CAST(COALESCE(@VPClass,'[?]') AS CHAR(30))			--VARCHAR(100)
	
	IF ( (@VPCraft IS NOT NULL OR LTRIM(RTRIM(@VPCraft)) <> '' ) AND (@VPClass IS NOT NULL OR LTRIM(RTRIM(@VPClass)) <> '' ) )
	begin

		IF NOT EXISTS (SELECT 1 FROM [DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTVALUES plv WHERE plv.PICKLISTID=@unionlid AND plv.CULTURECODE='en-US' AND plv.STOREVALUE=ltrim(rtrim(@VPCraft)) )
		BEGIN
		...
		IF NOT EXISTS (SELECT 1 FROM [DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTVALUES plv WHERE plv.PICKLISTID=@hrclasstypeplid AND plv.CULTURECODE='en-US' AND plv.STOREVALUE=LTRIM(RTRIM(@VPClass)) )
		BEGIN
			...
			FROM
				[DEV-HRISSQL02].HRNET.dbo.OC_PICKLISTLANGUAGES 
			WHERE
				PICKLISTID=@hrclasstypeplid	
		END 
		else
		...
		UPDATE [DEV-HRISSQL02].HRNET.dbo.PEOPLE SET UNIONTYPE=LTRIM(RTRIM(@VPCraft)) WHERE PEOPLE_ID=@peopleid --AND UNIONTYPE<>LTRIM(RTRIM(@VPCraft))
		UPDATE [DEV-HRISSQL02].HRNET.dbo.JOBDETAIL SET COSTCODE=LTRIM(RTRIM(@VPClass)) WHERE JOBDETAIL_ID=@jobdetailid --AND COSTCODE<>LTRIM(RTRIM(@VPClass))
...
select	
	@CompanyNumber=null		--int
,	@EmployeeNumber=null		--int
,	@FullName=null			--VARCHAR(100)
,	@UnionYN=null			--CHAR(1)
,	@COSTCODE=null			--VARCHAR(20)
,	@EmployeeClass=null		--VARCHAR(10)
,	@EmployeeType=null		--VARCHAR(10)
,	@UNIONTYPE=null			--VARCHAR(50)
,	@UNIONTYPENAME=null		--VARCHAR(50)
,	@UNIONNUMBER=null		--VARCHAR(20)
,	@CGCClass=null	--VARCHAR(10)
,	@CGCType=null	--VARCHAR(10)
,	@CGCUnion=null	--VARCHAR(10)
,	@CGCClassTypeDesc=null		--VARCHAR(100)
,	@CGCUnionDesc=null		--VARCHAR(100)
,	@VPCraft=null		--bCraft
,	@VPClass=null		--bClass
,	@VPCraftDesc=null		--VARCHAR(100)
,	@VPCraftClassDesc=null		--VARCHAR(100)
,	@jobdetailid=null --UNIQUEIDENTIFIER
,	@peopleid=null --UNIQUEIDENTIFIER
,	@hrclasstypeplid=null --UNIQUEIDENTIFIER
,	@unionlid=null --UNIQUEIDENTIFIER
...
mspUpdateHRNetCraftClass null -- Staff 501/SU : 0001 - Staff Default
go
mspUpdateHRNetCraftClass 891 -- Union 2501.000LF : 0032.01 - Local 32 - Sea Building Trades
go

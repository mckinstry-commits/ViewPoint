...
/****** Object:  Job [Time Off Transfer From Viewpoint]    Script Date: 9/11/2015 7:26:02 AM ******/
BEGIN TRANSACTION
DECLARE @ReturnCode INT
SELECT @ReturnCode = 0
/****** Object:  JobCategory [[Uncategorized (Local)]]    Script Date: 9/11/2015 7:26:02 AM ******/
...
EXEC @ReturnCode =  msdb.dbo.sp_add_job @job_name=N'Time Off Transfer From Viewpoint', 
		...
		@notify_level_page=0, 
		@delete_level=0, 
		@description=N'Viewpoint to HRDB on DEV-HRISSQL02  (2015.09.11 - LWO - copied from Production for VP 6.10 testing)', 
		@category_name=N'[Uncategorized (Local)]', 
		@owner_login_name=N'sa', 
		@notify_email_operator_name=N'Brendan Mason', 
		@job_id = @jobId OUTPUT
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Populate PRPTCH table with CGC Information]    Script Date: 9/11/2015 7:26:02 AM ******/
...
FROM 
	[VIEWPOINTSTAGING].Viewpoint.dbo.mvwPRTH prth JOIN
	[VIEWPOINTSTAGING].Viewpoint.dbo.bPREH preh ON
		prth.PRCo=preh.PRCo
	AND prth.PRGroup=preh.PRGroup
	AND prth.Employee=preh.Employee JOIN
	[VIEWPOINTSTAGING].Viewpoint.dbo.PREC prec ON
		prth.PRCo=prec.PRCo
	AND prth.EarnCode=prec.EarnCode JOIN
	[VIEWPOINTSTAGING].Viewpoint.dbo.HQET hqet ON
	...
	JOIN	
	[VIEWPOINTSTAGING].Viewpoint.dbo.HQCO hqco ON
		prth.PRCo=hqco.HQCo
WHERE
--	LTRIM(RTRIM(ecm.ShortCode)) <> '''' 
--and	
--prth.udArea IS NOT NULL OR  ( 	prec.Description IN (SELECT  *  FROM [VIEWPOINTSTAGING].Viewpoint.dbo.PREC WHERE )
...
', 
		...
		@flags=0
IF (@@ERROR <> 0 OR @ReturnCode <> 0) GOTO QuitWithRollback
/****** Object:  Step [Run Transfer Sproc]    Script Date: 9/11/2015 7:26:02 AM ******/

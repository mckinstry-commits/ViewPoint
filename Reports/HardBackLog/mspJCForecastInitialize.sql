alter  proc [dbo].[mspJCForecastInitialize]
/***********************************************************
...
*				LWO 09/18/2014 - McKinstry version cloned from [vspJCForecastInitialize]
*								 Used in JCCP trigger to populate Forecasts based on Cost Projections
*				
* USAGE:
...
( 
	@JCCo bCompany = NULL
,	@InitializeBy char(1) = NULL
,	@ReInit bYN = NULL
,	@FutureMonths tinyint = NULL
,	@InitPending char(1) = NULL
,	@InitOpen char(2) = NULL
,	@InitSoft char(1) = NULL
,	@InitHard char(1) = NULL
,	@BeginContract bContract = NULL
,	@EndContract bContract = NULL
,	@StartDate bMonth = NULL
,	@EndDate bMonth = NULL
,	@ProjectMgr bProjectMgr = NULL
...
begin
	...
	begin
		...
		select JCCo, Contract, @StartDate /* StartMonth */, isnull( @EndDate /*ProjCloseDate*/, (isnull(@EndMonth, @StartDate /* StartMonth */))), ContractStatus
	...
	begin
		insert into @ContractInit(JCCo, Contract, StartDate, EndDate, Status)
		select m.JCCo, m.Contract, m.StartMonth, isnull(@EndDate /*ProjCloseDate*/, (isnull(@EndMonth, @StartDate /* StartMonth */))), 
			m.ContractStatus
...
begin
	...
	begin
		insert into @ContractInit(JCCo, Contract, StartDate, EndDate, Status)
		select distinct c.JCCo, c.Contract, c.StartMonth, isnull(@EndDate /*ProjCloseDate*/, (isnull(@EndMonth, @StartDate /* StartMonth */))), c.ContractStatus
...
begin	
	...
	if @RevInterval > @TotalMonths 
	select @RevMethod = 'L', @CostMethod = 'L'
	else 
	select @RevMethod = @OrigRevMethod, @CostMethod = @OrigCostMethod
